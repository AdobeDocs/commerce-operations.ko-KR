...
* Nginx 및 [PHP-FPM](https://www.php.net/manual/en/install.fpm.php)
...
[시스템 요구 사항](../installation/system-requirements.md)에 나열된 대로 설치 중인 Adobe Commerce 릴리스에서 지원하는 PHP 버전을 사용하십시오. 요청 처리 시 최대 속도와 효율성을 얻기 위해 PHP를 구성할 때 고려해야 할 몇 가지 요소가 있습니다.
...
[!DNL Commerce]에서 최대 속도를 얻으려면 OpCache 모듈을 활성화하고 적절하게 구성해야 합니다. 모듈에는 다음 설정이 권장됩니다.
...
| 웹 서버 | 속성 이름 | 위치 | 관련 정보 |
|--- | --- | --- | ---|
| Ngix | `worker_connections` | `/etc/nginx/nginx.conf`(데비안) | [성능을 위해 NGINX 조정](https://www.f5.com/company/blog/nginx/tuning-nginx) |
...
최근 [!DNL MySQL] 릴리스에는 많은 성능 개선이 포함되며 [!DNL MySQL]은(는) 일반적으로 양호한 기본 설정으로 배포됩니다. 가장 중요한 설정은 다음과 같습니다.
