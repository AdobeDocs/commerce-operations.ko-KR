---
title: 고급 JavaScript 번들
description: Adobe Commerce의 고급 JavaScript 번들에 대해 알아봅니다. 구현 지침 및 최적화 전략을 살펴보십시오.
...
성능 향상을 위해 JavaScript 모듈을 번들로 제공한다는 것은 두 가지 사항을 줄이는 것입니다.

1. 서버 요청 수입니다.
1. 해당 서버 요청의 크기입니다.

모듈식 응용 프로그램에서는 서버 요청 수가 수백 개에 이를 수 있습니다. 예를 들어 다음 스크린샷은 깔끔한 설치의 홈 페이지에 로드된 JavaScript 모듈 목록의 시작만 보여줍니다.

![번들 없음](../assets/performance/images/noBundling.png)

## 병합 및 번들링

Commerce은 서버 요청 수를 줄이기 위해 번들을 지원합니다. 번들링은 기본적으로 꺼져 있습니다. **[!UICONTROL Stores]** > **설정** > **[!UICONTROL Configuration]** > **[!UICONTROL Advanced]** > **[!UICONTROL Developer]** > **[!UICONTROL JavaScript Settings]** 또는 명령줄에서 켤 수 있습니다.

타사 도구, HTTP/2 및 더 이상 사용되지 않는 JS 및 CSS 병합에 대한 지침은 *구성 모범 사례*&#x200B;의 [번들 팁](configuration.md#bundling-tips)을 참조하십시오.
...
시스템에 있는 모든 자산을 결합하고 크기가 동일한 번들 (bundle_0.js, bundle_1.js ... bundle_x.js) 간에 배포하는 기본 Commerce 메커니즘입니다.

![Commerce 번들](../assets/performance/images/magentoBundling.png)

더 좋지만 브라우저는 필요한 번들뿐만 아니라 모든 JavaScript 번들을 로드합니다.

Commerce 번들링을 사용하면 페이지당 연결 수가 줄어들지만, 요청된 페이지가 번들 중 하나 또는 두 개 내의 파일에만 종속될 수 있는 경우에도 각 페이지 요청에 대해 모든 번들이 로드됩니다. 브라우저가 번들을 캐시한 후 성능이 향상됩니다. 그러나 브라우저가 이러한 번들을 동기식으로 로드하기 때문에 Commerce 스토어를 처음 방문하는 사용자에게 렌더링으로 인한 사용자 경험에 부정적인 영향을 줄 수 있습니다.

### 기본 병합(권장되지 않음)

>[!NOTE]
>
>**[!UICONTROL Merge JavaScript Files]**&#x200B;을(를) 사용하지 않는 것이 좋습니다. 이 설정은 페이지의 HEAD 섹션에서 동기적으로 로드된 JavaScript에 대해서만 디자인되었으며 번들링 및 [!DNL RequireJS] 논리가 제대로 작동하지 않을 수 있습니다. 이전 버전과의 호환성을 위해서만 유지되며 HTTP/2가 활성화된 경우 성능 이점을 제공하지 않습니다.
...
이 명령은 모든 동기 JavaScript 파일을 하나의 파일로 병합합니다. Commerce에서 [!DNL RequireJS]을(를) 사용하므로 번들도 활성화하지 않고 병합을 활성화하면 유용하지 않습니다. 번들을 사용하지 않으면 Commerce은 [!DNL RequireJS]과(와) 해당 구성만 병합합니다. 번들링과 병합을 모두 활성화하면 Commerce에서 단일 JavaScript 파일을 만듭니다.
...
느린 3G 연결에서는 깔끔한 Commerce 설치의 홈 페이지에 대한 모든 번들을 로드하는 데 약 44초가 소요됩니다.
...
JavaScript 번들에 대한 보다 고급 접근 방식을 통해 이러한 로드 시간을 개선할 수 있습니다.

## 고급 번들

JavaScript 번들링의 목표는 브라우저에 로드되는 각 페이지에 대해 요청된 에셋의 수와 크기를 줄이는 것입니다. 이를 위해 스토어의 각 페이지가 액세스하는 각 페이지에 대한 공통 번들과 페이지별 번들만 다운로드하면 되도록 번들을 빌드하려고 합니다.

이를 수행하는 한 가지 방법은 페이지 유형별로 번들을 정의하는 것입니다. Commerce 페이지를 카테고리, 제품, CMS, 고객, 장바구니 및 체크아웃을 포함한 여러 페이지 유형으로 분류할 수 있습니다. 이러한 페이지 유형 중 하나로 분류된 각 페이지에는 서로 다른 [!DNL RequireJS] 모듈 종속성 집합이 있습니다. 페이지 유형별로 [!DNL RequireJS] 모듈을 번들로 채우면 스토어에 있는 페이지의 종속성을 다루는 번들이 몇 개만 제공됩니다.
...
깔끔한 Commerce 설치를 통해 페이지 유형별로 번들을 분할하여 충분히 우수한 성능에 도달할 수 있지만, 일부 사용자 지정 작업에서는 보다 심층적인 분석과 다른 자산 배포가 필요할 수 있습니다.
...
Commerce 루트 디렉터리에 `build.js` 파일을 만듭니다. 이 파일에는 번들에 대한 전체 빌드 구성이 포함됩니다.
...
#### 2\. [!DNL RequireJS] 종속성, shim, 경로 및 맵 추가

빌드 파일에 다음 [!DNL RequireJS] 빌드 구성 노드 `deps`, `shim`, `paths` 및 `map`을(를) 추가합니다.
...
![[!DNL RequireJS] 구성](../assets/performance/images/RequireJSConfig.png)
...
#### 5\. [!DNL RequireJS] 종속성 검색

다음을 사용하여 스토어의 페이지 유형에서 모든 [!DNL RequireJS] 모듈 종속성을 검색할 수 있습니다.

1. 명령줄에서 [!DNL PhantomJS]을(를) 가져왔습니다([!DNL PhantomJS]이(가) 설치되어 있다고 가정).
1. 브라우저의 콘솔에서 [!DNL RequireJS] 명령을 사용합니다.

#### [!DNL PhantomJS]을(를) 사용하려면

Commerce 루트 디렉터리에서 `deps.js`(이)라는 새 파일을 만들고 아래 코드를 복사합니다. 이 코드는 [!DNL [!DNL PhantomJS]]을(를) 사용하여 페이지를 열고 브라우저가 모든 페이지 자산을 로드할 때까지 기다립니다. 그런 다음 주어진 페이지에 대한 모든 [!DNL RequireJS] 종속성을 출력합니다.
...
Commerce 루트 디렉터리 내에서 터미널을 열고 특정 페이지 유형을 나타내는 저장소의 각 페이지에 대해 스크립트를 실행합니다.
...
목표는 모든 페이지에 필요한 JavaScript 파일의 공통 번들을 만드는 것입니다. 이러한 방식으로 브라우저는 하나 이상의 특정 페이지 유형과 함께 공통 번들을 로드하기만 하면 됩니다.

Commerce 루트 디렉터리에서 터미널을 열고 다음 명령을 사용하여 별도의 번들로 분할할 수 있는 종속성이 있는지 확인합니다.
...
종속성이 필요한 페이지 유형을 확인하려면 Commerce 루트 디렉터리(`deps-map.sh`)에 새 파일을 만들고 아래 코드를 복사합니다.
...
Commerce 루트 디렉터리에서 터미널을 열고 파일을 실행합니다.
...
- `include`— 페이지에 대한 종속성으로 포함된 에셋(문자열)의 배열입니다. [!DNL RequireJS]은(는) 모든 종속성을 추적하고 제외되지 않는 한 번들에 포함합니다.
...
아래 단계에서는 보다 효율적인 Commerce 번들을 생성하기 위한 기본 프로세스를 설명합니다. 원하는 대로 이 프로세스를 자동화할 수 있지만 실제로 번들을 생성하려면 `nodejs` 및 `r.js`을(를) 사용해야 합니다. 또한 테마에 JavaScript 관련 사용자 지정 내용이 있어 동일한 `build.js` 파일을 다시 사용할 수 없는 경우 테마당 여러 개의 `build.js` 구성을 만들어야 할 수 있습니다.
...
#### &#x200B;2. 정적 저장소 콘텐츠를 임시 디렉터리로 이동

먼저 [!DNL RequireJS]이(가) 대상 디렉터리 내의 모든 콘텐츠를 대체하므로 정적 콘텐츠를 대상 디렉터리에서 일부 임시 디렉터리로 이동해야 합니다.
...
그런 다음 Commerce의 루트 디렉터리에서 `build.js` 파일에서 r.js 최적화 프로그램을 실행합니다. 모든 디렉터리 및 파일의 경로는 작업 디렉터리를 기준으로 합니다.
...
#### &#x200B;4. 번들을 사용하도록 [!DNL RequireJS] 구성

번들을 사용하도록 [!DNL RequireJS]을(를) 가져오려면 `build.js` 파일의 `modules` 노드 뒤에 `onModuleBundleComplete` 콜백을 추가하십시오.
...
`pub/static/frontend/Magento/luma/en_US` 디렉터리에서 `requirejs-config.js`을(를) 열어 [!DNL RequireJS]이(가) 번들 구성 호출을 사용하여 파일을 추가했는지 확인합니다.
...
이제 빈 홈 페이지의 페이지 로드 시간이 기본 Commerce 번들링을 사용하는 것보다 두 배 더 빠릅니다. 하지만 우리는 훨씬 더 잘할 수 있어요.

#### &#x200B;7. 번들 최적화

압축되더라도 JavaScript 파일은 여전히 큽니다. JavaScript을 축소하는 데 무시기를 사용하는 [!DNL RequireJS]을(를) 사용하여 축소하십시오.
...
이제 기본 Commerce 번들링보다 로드 시간이 3배 더 빨라졌습니다.
