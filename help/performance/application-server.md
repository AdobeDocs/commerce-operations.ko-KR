...
* Swool PHP 확장 v5+(아래 프로젝트별 요구 사항 참조)
...
온-프레미스 프로젝트에 대한 Swool PHP 확장을 수동으로 [설치 및 구성](#install-and-configure-swoole)해야 합니다.
...
1. `.magento/services.yaml` 파일에서 `files` 섹션의 주석 처리를 제거합니다.
...
1. `.magento.app.yaml` 파일에서 탑재 구성의 `TEMPORARY SHARED MOUNTS` 부분에 대한 주석 처리를 제거합니다.
...
1. SSH를 사용하여 원격 클라우드 환경(`application-server` 앱 _not_)에 로그인합니다.
...
1. `.magento.app.yaml` 파일에서 탑재 구성의 `DEFAULT MOUNTS` 및 `TEMPORARY SHARED MOUNTS` 부분을 주석 처리합니다.
...
1. `.magento.app.yaml` 파일에서 탑재 구성의 `OLD LOCAL MOUNTS` 및 `SHARED MOUNTS` 부분에 대한 주석 처리를 제거합니다.
...
1. `.magento.app.yaml` 파일에서 탑재 구성의 `OLD LOCAL MOUNTS` 부분을 주석 처리합니다.
...
* **목록을 추가, 건너뛰기 또는 필터링할 수 없음**. 이 오류가 발생하면 상태를 변경할 수 있는 서비스 클래스에 팩토리를 사용하도록 클래스를 리팩터링하십시오.
...
기본적으로 GraphQL Application Server는 `app/etc/di.xml` 파일에 정의된 `x-forwarded-for` 헤더에 대한 표준 구성을 지원하므로 일반적인 프록시 및 CDN 환경에서 클라이언트 IP 주소를 정확하게 검색할 수 있습니다.

추가 또는 사용자 지정 헤더(예: `x-client-ip`, `fastly-client-ip` 또는 `x-real-ip`)를 지원해야 하는 경우 `app/etc/di.xml` 파일에서 `alternativeHeaders` 인수를 확장하거나 재정의할 수 있습니다. 사용자 환경에서 `x-forwarded-for` 이외의 헤더를 사용하여 클라이언트 IP 주소를 전달하는 경우에만 필요합니다.
