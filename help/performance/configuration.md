...
Commerce의 모든 비동기 작업은 Linux `cron` 명령을 사용하여 수행됩니다. 올바르게 구성하려면 [cron 구성 및 실행](../configuration/cli/configure-cron-jobs.md)을 참조하십시오.
...
**[!UICONTROL Asynchronous email notifications]** 설정을 사용하면 체크아웃 및 주문 처리 전자 메일 알림을 처리하는 프로세스가 백그라운드로 이동합니다. 이 기능을 사용하려면 **[!UICONTROL Stores]> [!UICONTROL Settings] > [!UICONTROL Configuration] > [!UICONTROL Sales] > [!UICONTROL Sales Emails] > [!UICONTROL General Settings] >[!UICONTROL Asynchronous Sending]**(으)로 이동하십시오. 자세한 내용은 [관리 사용 안내서](https://experienceleague.adobe.com/ko/docs/commerce-admin/config/sales/sales-emails)의 _판매 전자 메일_&#x200B;을 참조하십시오.

## 비동기 주문 데이터 처리

Commerce에서 집중 주문 처리를 수행하는 동시에 점포에서 집중 판매가 발생하는 경우가 있을 수 있습니다. 해당 테이블에서 읽기 및 쓰기 작업 간의 충돌을 방지하기 위해 데이터베이스 수준에서 이 두 트래픽 패턴을 구분하도록 Commerce을 구성할 수 있습니다. 주문 데이터를 비동기식으로 저장하고 인덱싱할 수 있습니다. 주문이 임시 저장소에 보관되고 충돌 없이 Order Management 그리드로 대량으로 이동됩니다. 이 옵션은 **[!UICONTROL Stores]> [!UICONTROL Settings] > [!UICONTROL Configuration] > [!UICONTROL Advanced] > [!UICONTROL Developer] > [!UICONTROL Grid Settings] >[!UICONTROL Asynchronous indexing]**&#x200B;에서 활성화할 수 있습니다. 자세한 내용은 관리자 가이드의 [예약된 격자 업데이트](https://experienceleague.adobe.com/ko/docs/commerce-admin/stores-sales/order-management/orders/order-scheduled-operations#enable-scheduled-grid-updates-and-reindexing)를 참조하십시오.
...
>[!INFO]
>
>이 옵션은 [Inventory management](high-throughput-order-processing.md#asynchronous-order-placement)과(와) 함께 [비동기 주문 배치](https://experienceleague.adobe.com/docs/commerce-admin/inventory/guide-overview.html?lang=ko)에서도 작동합니다.
...
* 축소 및 번들링(예: [r.js](https://requirejs.org/))에는 서드파티 도구를 사용하는 것이 좋습니다. Commerce 기본 제공 메커니즘은 최적이 아니며 대체 수단으로 제공됩니다.
...
그리드의 총 제품 수를 제한하는 경우 제품 그리드 질량 작용에 영향을 주지 않습니다. 제품 격자 프레젠테이션 레이어에만 영향을 줍니다. 예를 들어, 그리드에 제한된 `20000` 제품 수가 있고 사용자가 **[!UICONTROL Select All]**&#x200B;을(를) 클릭하고 **[!UICONTROL Update attributes]** 대량 작업을 선택한 다음 일부 특성을 업데이트합니다. 따라서 `20000` 레코드의 제한된 컬렉션이 아니라 모든 제품이 업데이트됩니다.
