...
[모듈 참조 안내서](https://developer.adobe.com/commerce/php/module-reference/module-async-order/)에서 _AsyncOrder_&#x200B;을(를) 참조하십시오.
...
[모듈 참조 안내서](https://developer.adobe.com/commerce/php/module-reference/module-deferred-total-calculating/)에서 _DeferredTotalCalculating_&#x200B;을(를) 참조하십시오.
...
**장바구니 로드 시 인벤토리 검사 사용**&#x200B;은(는) 기본적으로 사용(예로 설정)됩니다. 장바구니를 로드할 때 인벤토리 검사를 비활성화하려면 관리 UI **[!UICONTROL Enable Inventory Check On Cart Load]**&#x200B;스토어`No` > **구성** > **카탈로그** > **인벤토리** > **재고 옵션** 섹션에서 **을(를)**(으)로 설정하십시오. [사용 안내서](https://experienceleague.adobe.com/ko/docs/commerce-admin/inventory/configuration/global-options)에서 [전역 옵션 구성](https://experienceleague.adobe.com/ko/docs/commerce-admin/inventory/guide-overview) 및 _카탈로그 인벤토리_&#x200B;를 참조하십시오.

## 로드 밸런싱

MySQL 데이터베이스 및 Redis 인스턴스에 대한 보조 연결을 활성화하면 서로 다른 노드에 대한 로드 밸런스를 조정할 수 있습니다.

Adobe Commerce은 여러 데이터베이스 또는 Redis 인스턴스를 비동기식으로 읽을 수 있습니다. 클라우드 인프라에서 Commerce을 사용하는 경우 [&#x200B; 파일의 &#x200B;](https://experienceleague.adobe.com/ko/docs/commerce-cloud-service/user-guide/configure/env/stage/variables-deploy#mysql_use_slave_connection)MYSQL_USE_SLAVE_CONNECTION[&#x200B; 및 &#x200B;](https://experienceleague.adobe.com/ko/docs/commerce-cloud-service/user-guide/configure/env/stage/variables-deploy#redis_use_slave_connection)REDIS_USE_SLAVE_CONNECTION`.magento.env.yaml` 값을 편집하여 보조 연결을 구성할 수 있습니다. 읽기-쓰기 트래픽을 처리하는 노드는 하나뿐이므로 변수를 `true`(으)로 설정하면 읽기 전용 트래픽에 대한 보조 연결이 만들어집니다. `false` 파일에서 기존 읽기 전용 연결 배열을 제거하려면 값을 `env.php`(으)로 설정하십시오.
