...
- [협상 가능 견적 비동기 주문](#negotiable-quote-asyn-order)—협상 가능 견적 저장 주문 항목을 비동기식으로 처리합니다.
- [지연된 합계 계산](#deferred-total-calculation)- 체크아웃이 시작될 때까지 주문 합계에 대한 계산을 정의합니다.

모든 기능은 독립적으로 작동합니다. 모든 기능을 동시에 사용하거나 모든 조합에서 기능을 활성화 및 비활성화할 수 있습니다.
...
DelayedTotalCalculation이 **비활성화됨** 기본적으로 제공됩니다.
...
장바구니 로드 시 인벤토리 활성화: **활성화됨** 기본적으로 제공됩니다.

장바구니를 로드할 때 재고 확인을 비활성화하려면 **[!UICONTROL Enable Inventory Check On Cart Load]** to `No` 관리자 UI에서 클릭합니다. 자세한 내용은 [글로벌 옵션 구성][global] 및 [카탈로그 인벤토리][inventory] 에서 _사용 안내서_.
