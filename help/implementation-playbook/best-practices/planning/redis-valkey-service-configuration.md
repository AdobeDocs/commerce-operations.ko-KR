...
description: Adobe Commerce용 Valkey 및 Redis 서비스를 구성하는 방법에 대해 알아봅니다. L2 캐시, 복제본 연결, 오래된 캐시 및 세션을 통해 캐시 성능을 향상시킵니다.
solution: Commerce
...
이러한 권장 사항을 사용하여 클라우드 인프라에서 Adobe Commerce에 대한 Valkey 또는 Redis 캐싱 및 세션을 구성합니다. 온-프레미스 캐시 구성에 대해서는 [캐시 백 엔드 옵션 및 저장소 참조](../../../configuration/cache/cache-options.md)를 참조하세요.

- [!DNL Symfony]개의 L2 캐시를 포함하여 L2 캐시 구성
- 읽기 전용 복제본 연결 사용
- 미리 로드 키
- 부실 캐시 활성화
- 별도의 캐시 및 세션
...
2.4.8-p4, 2.4.7-p9, 2.4.6-p14 및 2.4.5-p16 이상 버전의 Adobe Commerce의 경우 Valkey로 L2 캐시를 구성합니다. 이 페이지의 Redis 구성 예제는 Redis를 사용하는 지원되는 Adobe Commerce 버전에만 적용됩니다. 릴리스별로 지원되는 캐시 서비스에 대해서는 [시스템 요구 사항](../../../installation/system-requirements.md)을 참조하십시오.
...
>[!NOTE]
>
>Adobe Commerce 2.4.9용 패치 ACP2E-5132는 Symfony L2 캐시 성능과 안정성을 개선합니다. 즉, 최적화된 태그 저장소, 오래된 캐시 재생성 잠금, 다시 태그 지정 후 오래된 태그 멤버십, 변경되지 않은 저장에 대한 중복 원격 쓰기, L1 크기 기반 제거(`cleanup_percentage`) 등에 대한 수정 사항이 있습니다. 이를 통해 디스크 I/O 및 백엔드 로드를 줄이는 동시에 캐시 일관성을 향상시킬 수 있습니다. _Adobe Commerce 구성 가이드_&#x200B;에서 [향상된 Symfony L2 캐시 성능 및 안정성](../../../configuration/cache/level-two-cache.md#enhanced-symfony-l2-cache-performance-and-reliability)을(를) 참조하십시오.
>
...
`.magento.env.yaml` 파일에서 읽기 전용 복제본 연결을 사용하도록 설정하여 Adobe Commerce에서 쓰기에 기본 끝점을 계속 사용하는 동안 읽기에 추가 캐시 연결을 사용하도록 합니다. 이 구성을 통해 기본 캐시 서비스의 읽기 로드를 줄이고 읽기 트래픽을 보다 효과적으로 분산할 수 있습니다.
...
환경 변수 구성에 대한 자세한 내용은 _Commerce on Cloud Infrastructure Guide_&#x200B;의 [VALKEY_USE_SLAVE_CONNECTION](https://experienceleague.adobe.com/docs/commerce-cloud-service/user-guide/configure/env/stage/variables-deploy.html#valkey_use_slave_connection)을(를) 참조하십시오.
...
환경 변수 구성에 대한 자세한 내용은 _Commerce on Cloud Infrastructure Guide_&#x200B;의 [REDIS_USE_SLAVE_CONNECTION](https://experienceleague.adobe.com/docs/commerce-cloud-service/user-guide/configure/env/stage/variables-deploy.html#redis_use_slave_connection)을(를) 참조하십시오.
...
6GB 이상의 Redis 또는 Valkey `maxmemory`을(를) 사용하는 경우 캐시 압축을 사용하여 키에 사용되는 공간을 줄일 수 있습니다. 이 설정은 메모리 절감을 위해 클라이언트측 성능을 처리합니다. 여유 CPU 용량이 있는 경우 활성화해 보십시오. _구성 가이드_&#x200B;에서 [세션 저장소에 Redis 사용](../../../configuration/cache/redis-session.md) 또는 [세션 저장소에 Valkey 사용](../../../configuration/cache/valkey-session.md)을 참조하십시오.
