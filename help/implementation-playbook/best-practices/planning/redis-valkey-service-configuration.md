...
- Symfony L2 캐시를 포함한 L2 캐시 구성
- Cloud Pro 환경에서 읽기 전용 복제본(슬레이브) 연결을 활성화합니다. 이 기능은 초보자 환경에서는 사용할 수 없습니다.
- 미리 로드 키
- 부실 캐시 활성화
- 별도의 캐시 및 세션
- 캐시 압축
...
>[!IMPORTANT]
>
>이 페이지의 Redis 구성 예제는 Redis를 사용하는 지원되는 Adobe Commerce 버전에만 적용됩니다. 릴리스별로 지원되는 캐시 서비스에 대해서는 [시스템 요구 사항](../../../installation/system-requirements.md)을 참조하십시오. Redis 캐시는 Adobe Commerce 2.4.9 또는 2.4.5-p16, 2.4.6-p14, 2.4.7-p9 및 2.4.8-p4 이상의 패치 릴리스에서는 지원되지 않습니다. Redis가 지원되지 않는 캐시 구성에 Valkey를 사용합니다. 릴리스별로 지원되는 캐시 서비스에 대해서는 [시스템 요구 사항](../../../installation/system-requirements.md)을 참조하십시오.

`VALKEY_BACKEND` 또는 `REDIS_BACKEND`을(를) 설정하면 L2 캐시 백엔드가 구성되지만 Adobe Commerce에서 Redis 또는 Valkey를 원격 캐시 서비스로 사용하는지 여부는 결정되지 않습니다. 할당한 클래스 값(예: `\Magento\Framework\Cache\Backend\Redis` 또는 `symfony_l2`)에서도 서비스를 선택하지 않습니다. Adobe Commerce은 사용자 환경에서 사용할 수 있는 모든 서비스(Redis 또는 Valkey)를 사용하며 둘 다 사용할 수 있는 경우 Redis가 우선합니다. 예를 들어 `VALKEY_BACKEND: '\Magento\Framework\Cache\Backend\Redis'`은(는) 사용 가능한 경우 Redis를 사용하고 Redis를 사용할 수 없는 경우에만 Valkey로 돌아갑니다.

>[!BEGINTABS]

>[!TAB VALKEY_BACKEND 사용]

`RemoteSynchronizedCache` 구현이 있는 Valkey의 경우 다음을 사용합니다.
...
Symfony L2 캐시 구현이 있는 Valkey에 대해서는 [Symfony L2 캐시 구성](#configure-symfony-l2-cache)을 참조하십시오.

>[!TAB REDIS_BACKEND 사용]
...
- 클라우드 프로젝트에서 [ECE 도구 패키지 v2002.2.12](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/dev-tools/ece-tools/update-package) 이상을 사용하고 있는지 확인하십시오.
...
`VALKEY_BACKEND`을(를) `symfony_l2`(으)로 설정하면 캐시 가능 형식 `layout`, `block_html`, `full_page` 및 `translate`이(가) 오래된 사용 프런트 엔드에 매핑되어 `default` 및 `stale_cache_enabled` 프런트 엔드를 포함한 Valkey 서비스 세부 정보에서 전체 L2 캐시 구성이 빌드됩니다. `CACHE_CONFIGURATION` 정의는 선택 사항이며 특정 백엔드 옵션을 사용자 지정하려는 경우에만 필요합니다.
...
Redis는 시스템 요구 사항이 Valkey를 지정하는 Adobe Commerce 2.4.9 및 패치 릴리스의 캐시 구성에 대해 지원되지 않습니다. [캐시 백엔드 옵션 및 저장소 참조](/help/configuration/cache/cache-options.md) 및 [시스템 요구 사항](/help/installation/system-requirements.md)에서 항상 정확한 Commerce 버전, 패치 수준 및 서비스 버전을 확인하십시오.
...
## 읽기 전용 복제본 연결 사용 {#enable-slave-connection}

`.magento.env.yaml` 파일에서 읽기 전용 복제본 연결을 사용하도록 설정합니다. 이렇게 하면 Adobe Commerce에서 쓰기에 기본 끝점을 계속 사용하는 동안 읽기에 추가 캐시 연결을 사용할 수 있습니다. 이 구성을 통해 기본 캐시 서비스의 읽기 로드를 줄이고 읽기 트래픽을 보다 효과적으로 분산할 수 있습니다.

>[!IMPORTANT]
>
>`VALKEY_BACKEND` 및 `REDIS_BACKEND`과(와) 달리 `VALKEY_USE_SLAVE_CONNECTION` 및 `REDIS_USE_SLAVE_CONNECTION` 변수는 특정 서비스에 연결되어 있습니다. 환경에서 사용할 수 있는 캐시 서비스와 일치하는 변수를 설정합니다. L2 캐시를 구성하는 데 사용한 `*_BACKEND` 변수에 포함된 동일한 서비스를 사용할 필요는 없습니다.
...
>[!TAB VALKEY_USE_SLAVE_CONNECTION 사용]
...
환경 변수 구성에 대한 자세한 내용은 _Commerce on Cloud Infrastructure Guide_&#x200B;의 [VALKEY_USE_SLAVE_CONNECTION](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/configure/env/stage/variables-deploy#valkey_use_slave_connection)을(를) 참조하십시오.

>[!TAB REDIS_USE_SLAVE_CONNECTION 사용]
...
환경 변수 구성에 대한 자세한 내용은 _Commerce on Cloud Infrastructure Guide_&#x200B;의 [REDIS_USE_SLAVE_CONNECTION](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/configure/env/stage/variables-deploy#redis_use_slave_connection)을(를) 참조하십시오.

>[!ENDTABS]

## 미리 로드 키

Adobe Commerce은 일반적으로 Redis 또는 Valkey에서 캐시 항목을 한 번에 하나씩 로드합니다. 미리 로드 기능을 사용하면 Adobe Commerce이 요청 중에 처음 액세스할 때 단일 파이프라인에서 가져오는 자주 사용하는 키 목록을 제공할 수 있습니다. 그런 다음 Adobe Commerce은 가져온 값을 나머지 해당 요청에 대해 PHP 메모리에 유지하므로 Redis 또는 Valkey로의 반복된 라운드 트립이 줄어들고 해당 키에 대한 요청 부트스트랩 성능을 향상시킬 수 있습니다.

Redis 또는 Valkey에서 활성 명령을 모니터링하여 자주 사용되는 키를 식별할 수 있습니다.

>[!BEGINTABS]

>[!TAB VALKEY_BACKEND로 키 미리 로드]
...
부실 캐시는 `RemoteSynchronizedCache` 및 `symfony_l2` 구현 모두에서 지원되는 L2 캐시 기능입니다. 활성화하면 모든 동시 요청을 대기시키는 대신 다른 요청이 이미 동일한 항목을 다시 생성하는 동안 Adobe Commerce은 `/dev/shm`에서 기존 로컬 캐시 값을 제공할 수 있습니다. 이렇게 하면 값비싼 캐시 항목을 재생성하는 동안 캐시 스템피드 및 잠금 경합이 줄어듭니다.

### 작동 방식

L2 캐시는 각 캐시 항목의 복사본 두 개를 유지 관리합니다. `/dev/shm`의 로컬 복사본과 Redis 또는 Valkey의 원격 복사본입니다. 원격 복사본을 사용할 수 없고 해당 키에 대한 재생성 잠금이 이미 있는 경우, 동시 요청은 새 값이 기록될 때까지 기다리지 않고 이전 로컬 값을 수신할 수 있습니다.

부실 캐시를 활성화하려면 `.magento.env.yaml` 파일에서 구성하십시오.

>[!BEGINTABS]

>[!TAB VALKEY_BACKEND를 사용하여 부실 캐시 구성]
...
>[!TAB REDIS_BACKEND를 사용하여 부실 캐시 구성]
...
>[!IMPORTANT]
>
>프로덕션 및 스테이징 환경에서 전용 세션 인스턴스를 프로비전하려면 아래 3단계에 설명된 대로 업데이트된 `.magento/services.yaml` 및 `.magento.app.yaml` 파일과 함께 [Adobe Commerce 지원 티켓](https://experienceleague.adobe.com/en/docs/commerce-learn/tutorials/help-and-support/create-a-support-ticket)을 제출해야 합니다.
...
1. 프로덕션 및 스테이징 환경에서 세션 전용 새 Valkey 인스턴스를 요청합니다.
   ...
   [Adobe Commerce 지원 티켓](https://experienceleague.adobe.com/en/docs/commerce-learn/tutorials/help-and-support/create-a-support-ticket)을 제출하세요. 업데이트된 `.magento/services.yaml` 및 `.magento.app.yaml` 구성 파일을 포함합니다.
...
1. Valkey 캐시 인스턴스의 기본 데이터베이스(`db 0`)에서 세션을 제거합니다.
...
1. Redis 캐시 인스턴스의 기본 데이터베이스(`db 0`)에서 세션을 제거합니다.
...
Adobe Commerce 클라우드 인프라에서 `lazyfree`을(를) 사용하려면 다음 Redis 또는 Valkey 구성을 환경에 적용할 것을 요청하는 [Adobe Commerce 지원 티켓을 제출](https://experienceleague.adobe.com/en/docs/commerce-learn/tutorials/help-and-support/create-a-support-ticket)합니다.
...
- `RemoteSynchronizedCache`이(가) 로컬 저장소에 `Cm_Cache_Backend_File`을(를) 사용합니다.
- `symfony_l2`은(는) PSR-6을 준수하며 Valkey만 지원합니다.

| Commerce 버전 | Valkey가 있는 RemoteSynchronizedCache | 권장 구성 |
| -------------- | ----------------------------------- | ------------------------- |
| 2.4.8 및 이전 <br>(Valkey가 지원되는 경우) | 지원됨 | `VALKEY_BACKEND: '\Magento\Framework\Cache\Backend\RemoteSynchronizedCache'` |
...
미리 로드 기능을 사용하면 Adobe Commerce이 요청 중에 처음 액세스할 때 단일 파이프라인에서 가져오는 자주 사용하는 키 목록을 제공할 수 있습니다. 그런 다음 Adobe Commerce은 가져온 값을 나머지 해당 요청에 대해 PHP 메모리에 유지하므로 Redis 또는 Valkey로의 반복된 라운드 트립이 줄어들고 해당 키에 대한 요청 부트스트랩 성능을 향상시킬 수 있습니다.
...
다음 구성은 `RemoteSynchronizedCache`을(를) 사용하고 수동 부실 캐시 및 프론트엔드 구성이 필요한 Adobe Commerce 2.4.8 이하에 적용됩니다. 여기에도 동일한 전역 중복 권장 사항이 적용됩니다.

#### RemoteSynchronizedCache 백 엔드의 작동 방식

`RemoteSynchronizedCache`을(를) 사용하면 Adobe Commerce은 각 캐시 항목의 복사본 두 개를 유지 관리합니다. 하나는 `/dev/shm`에 로컬 복사본이고 다른 하나는 Redis 또는 Valkey에 원격 복사본입니다. 원격 복사본을 사용할 수 없고 해당 키에 대한 재생성 잠금이 이미 있는 경우, 동시 요청은 새 값이 기록될 때까지 기다리지 않고 이전 로컬 값을 수신할 수 있습니다.
...
>[!WARNING]
>
>위의 구성은 `default` 캐시 프런트 엔드에서 오래된 캐시를 활성화하며, 이로 인해 해당 프런트 엔드를 사용하는 모든 캐시 항목에 오래된 캐시 동작이 적용됩니다. Adobe Commerce 코어 캐시 유형은 이 설정에서 예상대로 작동합니다. 그러나 프로젝트에 전용 캐시 프론트엔드 없이 일반 `\Magento\Framework\App\Cache` API(예: `$this->cache->save()`)를 통해 캐시에 쓰는 사용자 지정 코드 또는 확장이 포함되어 있는 경우 이러한 항목은 재생성 중에도 부실 값을 제공할 수 있습니다.
>
>
>이렇게 하면 사용자 지정에 예기치 않은 동작이 발생하는 경우 `default` 프런트 엔드에 오래된 캐시를 사용하지 않도록 설정한 후 아래 표시된 것처럼 선택한 캐시 유형에 대해서만 사용하도록 설정하십시오.

#### 캐시 유형당 부실 캐시를 개별적으로 활성화(RemoteSynchronizedCache)

`.magento.env.yaml`에서 전용 캐시 프런트 엔드를 정의하고 선택한 캐시 유형을 매핑하여 선택한 캐시 유형에 대해서만 오래된 캐시를 활성화할 수 있습니다. 이 수동 접근 방식은 `RemoteSynchronizedCache` 백엔드에 적용됩니다. `symfony_l2`은(는) 위에서 설명한 대로 이 매핑을 자동으로 수행합니다.
...
- [`ece-tools` 패키지의 최신 버전](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/dev-tools/ece-tools/update-package)&#x200B;(으)로 업데이트합니다. ECE Tools 패키지 v2002.2.13 이상을 사용해야 합니다.
...
>[!NOTE]
>
>Adobe Commerce 2.4.9용 패치 ACP2E-5132는 [!DNL Symfony] L2 캐시 성능과 안정성을 개선합니다. 태그 저장소를 최적화하고, 오래된 캐시 재생성 잠금을 추가하고, 오래된 태그 멤버십, 중복 원격 쓰기 및 L1 크기 기반 제거(`cleanup_percentage`)와 관련된 문제를 수정합니다. 이를 통해 디스크 I/O 및 백엔드 로드를 줄이는 동시에 캐시 일관성을 향상시킬 수 있습니다. _Adobe Commerce 구성 가이드_&#x200B;에서 [향상된 Symfony L2 캐시 성능 및 안정성](/help/configuration/cache/level-two-cache.md#enhanced-symfony-l2-cache-performance-and-reliability)을(를) 참조하십시오.
>
