...
- **트래픽 처리를 위한 자동 크기 조정**:
   ...
   - Adobe Commerce Cloud는 실시간 트래픽 요구에 따라 서버 리소스(예: 웹 노드)를 동적으로 조정하기 위한 자동 크기 조절을 지원하여 스토어에서 수동 개입 없이 높은 방문자 볼륨을 처리할 수 있도록 합니다. _클라우드 가이드_&#x200B;에서 [자동 크기 조정](https://experienceleague.adobe.com/en/docs/commerce-cloud-service/user-guide/architecture/autoscaling)을 참조하십시오.
