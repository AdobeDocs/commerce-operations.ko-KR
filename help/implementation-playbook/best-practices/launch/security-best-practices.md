...
모든 보안 위험을 제거할 수는 없지만 이러한 모범 사례를 적용하면 Commerce 설치의 보안 자세가 강화됩니다. 안전한 사이트 및 인프라는 악의적인 공격의 위험을 줄이고 민감한 정보를 보호하며 비용이 많이 드는 보안 사고를 최소화합니다.
...
![검사 목록](/help/assets/icons/Smock_CheckmarkCircleOutline_18_N.svg) **보안 파트너 또는 솔루션 통합자(SI) 선택** - 보안 개발 실적이 있는 조직을 선택하여 보안 통합 및 사용자 지정 코드를 보장합니다.
...
![검사 목록](/help/assets/icons/Smock_CheckmarkCircleOutline_18_N.svg) **감사 코드** - 서버 및 소스 코드 리포지토리에서 개발 중 남은 부분을 검토하십시오. 액세스 가능한 로그 파일, .git 디렉터리, SQL 터널, 데이터베이스 덤프 또는 php 정보 파일이 없는지 확인합니다.

## 최신 릴리스로 업그레이드

Adobe은 보안을 개선하고 가능한 손상으로부터 고객을 더 잘 보호하기 위해 업데이트된 솔루션 구성 요소를 지속적으로 릴리스합니다. 최신 버전의 Adobe Commerce 애플리케이션, 설치된 서비스 및 확장으로 업그레이드하고 현재 패치를 적용하는 것이 보안 위협으로부터 보호하는 주요 방법입니다.
...
- [보안 및 규정 준수 알림](../../../release/security-enforcement-policy.md)(Cloud 버전 2.4.4부터 2.4.9까지의 Adobe Commerce)
- [업그레이드 안내서](../../../upgrade/overview.md)
- [패치 적용 방법](../../../upgrade/patches/overview.md)

>[!TIP]
>
>[Adobe 보안 알림 서비스](https://www.adobe.com/subscription/adobesecuritynotifications.html)를 구독하여 최신 보안 정보를 얻고 알려진 보안 문제를 완화하세요.
...
이 섹션에서는 Adobe Commerce 설치를 위한 사이트 및 인프라 보안 유지 관리에 대한 모범 사례를 요약합니다. 이러한 우수 사례 중 대부분은 일반적으로 컴퓨터 인프라 보안에 중점을 두므로, 일부 권장 사항은 이미 구현되었습니다.

![검사 목록](/help/assets/icons/Smock_CheckmarkCircleOutline_18_N.svg) **무단 액세스 차단**—Commerce 사이트 및 고객 데이터에 대한 무단 액세스를 차단하려면 호스팅 파트너와 함께 VPN 터널을 설정하십시오. Commerce 애플리케이션에 대한 무단 액세스를 차단하려면 SSH 터널을 설정하십시오.
...
전자 상거래 사이트를 타깃으로 한 맬웨어 공격은 빈번하며 위협 행위자는 거래에서 신용 카드와 개인 정보를 수집하는 새로운 방법을 지속적으로 개발합니다.

그러나 Adobe은 대부분의 사이트 타협이 혁신적인 공격자 때문이 아님을 밝혀냈습니다. 오히려 위협 행위자가 파일 시스템의 기존 취약성, 부실한 암호, 취약한 소유권 및 권한 설정을 활용하는 경우가 많습니다.
...
양식 제출이나 필드 수정과 같은 작업 후 스키머는 데이터를 정리하여 타사 엔드포인트로 전송합니다. 이러한 끝점은 일반적으로 데이터를 최종 대상으로 전송하는 릴레이 역할을 하는 다른 손상된 웹 사이트입니다.


>[!TIP]
>
>맬웨어 공격이 Commerce 사이트에 영향을 미치는 경우 [보안 문제에 응답](../maintenance/respond-to-security-incident.md)하기 위한 Adobe Commerce 모범 사례를 따르십시오.
...
- **봇넷** - 고객의 Commerce 서버는 스팸 이메일을 보내는 봇넷의 일부가 됩니다. 일반적으로 사용자 데이터가 손상되지는 않지만 스팸 은 고객의 도메인 이름을 필터링하여 도메인에서 이메일이 전달되지 않도록 합니다. 또는 고객의 사이트가 봇넷의 일부가 되어 다른 사이트에 대한 분산 서비스 거부(DDoS) 공격을 야기합니다. 봇넷은 Commerce 서버에 대한 인바운드 IP 트래픽을 차단하여 고객이 쇼핑을 할 수 없도록 합니다.
...
- 외부에서 Commerce 설치에 액세스할 수 있는 모든 지점을 식별하고 보호합니다.
  ...
  Commerce 프로젝트를 구성할 때 Adobe의 [우선 순위 권장 사항](#priority-recommendations)을 따라 보호가 가장 필요한 관리자에 대한 액세스를 보호할 수 있습니다.
