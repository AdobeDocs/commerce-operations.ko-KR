...
>[!NOTE]
>
>클라우드 프로젝트의 Adobe Commerce에 대한 캐시 구성 지침은 [Redis 및 Valkey 서비스 구성에 대한 모범 사례](/help/implementation-playbook/best-practices/planning/redis-valkey-service-configuration.md)를 참조하십시오.

## Redis 캐시 지원 {#redis-cache-support}

Redis 캐시는 Adobe Commerce 2.4.9 또는 2.4.5-p16, 2.4.6-p14, 2.4.7-p9 및 2.4.8-p5 이상의 패치 릴리스에서는 지원되지 않습니다. Redis를 지원하지 않는 버전으로 업그레이드하는 경우 Valkey를 설정하고 이를 사용하려면 캐시 구성을 업데이트해야 합니다. 온-프레미스 배포에 대해서는 [Valkey 설정](/help/configuration/cache/config-valkey.md)을(를) 참조하십시오.
...
>
>지원되는 버전의 전체 목록은 [제품 가용성 매트릭스](/help/release/product-availability.md)를 참조하십시오.
...
>[!IMPORTANT]
>
>데이터베이스 분할 기능은 Adobe Commerce 버전 2.4.2에서 더 이상 사용되지 않습니다. [분할 데이터베이스에서 단일 데이터베이스로 되돌리기](/help/configuration/storage/revert-split-database.md)를 참조하십시오.
...
* **Apache ActiveMQ Artemis STOMP 프로토콜에 대한 지원이 추가되었습니다**
  ...
  STOMP(Simple Text Oriented Messaging Protocol)를 통해 ActiveMQ Artemis 오픈 소스 메시지 브로커에 대한 지원을 추가했습니다. 안정적이고 확장 가능한 메시징 시스템을 제공하여 STOMP 기반 통합을 위한 유연성을 제공합니다. *Commerce 구성 가이드*&#x200B;에서 [Apache ActiveMQ Artemis](/help/configuration/queues/message-queue-framework.md#apache-activemq-artemis-stomp)을(를) 참조하십시오.
...
>[!NOTE]
>
>Adobe Commerce 2.4.9부터 Valkey는 CLI 툴링에서 Redis를 공식적으로 교체했습니다. **버전 2.4.8 및 이전**&#x200B;의 경우 동일한 [Redis CLI 명령](/help/configuration/cache/config-redis.md#set-up-redis-configuration)을 사용하십시오.
