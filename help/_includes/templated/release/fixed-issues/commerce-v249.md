...
#### 속성 집합 정보가 캐시되지만(EAV 캐시), 속성 집합이 관리 영역에서 업데이트된 후에는 정리되지 않습니다

EAV 캐시를 사용할 수 있을 때 새 속성을 속성 집합에 추가한 후 누락된 제품 속성 값을 해결했습니다(캐시 무효화/API 가져오기 동작).

_AC-12321 - [GitHub 문제](https://github.com/magento/magento2/issues/38827) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/38864)_
...
#### GraphQl 미디어 갤러리가 비활성화된 항목을 반환합니다.

Graphql 요청 응답의 제품에 대해 비활성화되지 않은 미디어 갤러리 이미지만 가져오는 문제를 해결했습니다.

_AC-1608 - [GitHub 문제](https://github.com/magento/magento2/issues/34342) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/34897)_
...
이전에는 잘못된 형식의 요청 본문이나 매개 변수를 다양한 REST API 엔드포인트로 보내기(예: /V1/carts/search, /V1/orders, /V1/products 등) 에서 일반적인 &quot;내부 서버 오류&quot;(500)가 발생하여 입력 문제를 진단하기 어려웠습니다.
...
REST API V1/guest-carts/&lt;cartId>/items/ 및 V1/carts/mine/items/ 이제 &quot;product_options.extension_attributes.custom_options.*.option_id&quot;를 확인하여 장바구니 항목 SKU에 대한 유효한 option_id를 참조하는지 확인합니다. 이전에는 이 매개 변수가 유효성 검사 없이 처리되어 데이터베이스에 저장되었습니다.
...
다시 연결된 채널에서 잘못된 승인/거부 시도를 차단하여 AMQP 소비자 복원력을 개선했습니다.

_ACP2E-4621 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c12a7510)_
...
이제 &quot;제품을 조회함&quot; 고객 세그먼트 조건이 더 이상 사용되지 않습니다.
이전에는 이 조건을 사용하면 많은 MySQL 쿼리로 인해 사이트 중단이 발생할 수 있었습니다. 이제 조건이 더 이상 사용되지 않는 것으로 표시되어 지원되지 않습니다.
...
#### 허용되는 문자가 없을 경우 고객 주소 전화 유효성 검사가 작동하지 않음

시스템이 전화기가 유효하지 않다는 오류 메시지를 반환합니다.

_AC-16227 - [GitHub 문제](https://github.com/magento/magento2/issues/40361) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40416)_

#### 웹 사이트 및 스토어 오브젝트는 ID를 문자열로 반환합니다.

웹 사이트 및 스토어 ID가 정수가 아닌 문자열로 반환되던 문제를 수정했습니다. 이렇게 하면 엄격 모드에서 유형 오류가 해결되어 EmailNotification::newAccount() 및 유사한 메서드가 올바르게 작동할 수 있습니다.

_AC-16486 - [GitHub 문제](https://github.com/magento/magento2/issues/40469)_

#### 허용되지 않는 문자에 대해 고객 도시 유효성 검사가 작동하지 않음

잘못된 문자(예: /, @, #, !)로 고객 주소를 저장할 수 있는 문제가 해결되었습니다. 도시 들판에서. 이 수정 사항이 적용되면 시스템에서는 허용되지 않는 문자가 포함된 주소를 올바르게 확인하고 거부합니다.

_AC-16504 - [GitHub 문제](https://github.com/magento/magento2/issues/40521) - [GitHub 코드 기여](https://github.com/magento/magento2/commit/57bad413)_

#### [문제] 수정: admin_user_session에서 IPv6 지원을 위한 IP 열 길이를 늘립니다.

현재 admin_user_session의 ip 열은 15자로 제한되어 IPv6 주소(최대 45자)에는 부족합니다. 이 PR은 IPv6 호환성을 완벽하게 보장하고 IPv6를 사용할 때 데이터가 잘리거나 오류가 발생하지 않도록 길이를 45로 늘립니다.

_AC-16659 - [GitHub 문제](https://github.com/magento/magento2/issues/40570) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40563)_
...
&quot;고객이 초대 이메일에 사용자 정의 메시지를 추가할 수 있도록 허용&quot; 설정이 비활성화된 경우 초대 보내기 페이지에서 이메일 필드를 추가할 때 &quot;최대 X개 이메일 주소 허용됨&quot; 경고 메시지가 표시되지 않던 문제를 수정했습니다.
이전에는 사용자 지정 메시지가 활성화되어 일관되지 않은 사용자 경험이 만들어질 때만 경고가 표시되었습니다. 이제 사용자 지정 메시지 구성 설정에 관계없이 최대 이메일 제한 경고가 일관되게 표시됩니다.

_ACP2E-4374_

#### 구/군/시 이름 유효성 검사에서 허용되지 않는 문자(!&quot;#?)를 수락합니다.

이제 고객 주소에 도시를 입력할 때 유효성 검사에서 슬래시를 허용하고 !, &quot;, # 및 ? 등의 잘못된 문자를 올바르게 거부합니다.

_ACP2E-4527 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/f2fd9628)_

#### 2.4.8-p3로 업그레이드한 후 기존 고객을 손상시키는 주소 유효성 검사기

이전에는 이 수정 전에 문자가 있는 최신 전화 번호였습니다. 또는 의 유효성이 제대로 확인되지 않았습니다. 이 수정 사항이 적용되면 문자가 있는 최신 전화 번호입니다. 또는 는 이제 유효성이 확인되었습니다.

_ACP2E-4554 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/aa2e16d1)_
...
#### 암호 재설정 요청 후 [Cloud] PHPSESSID가 변경되었으며 현재 장바구니가 지워졌습니다.

수정하기 전에 암호 찾기 양식을 제출하면 세션이 파기/재생성되었으며(PHPSESSID가 변경됨) 게스트 카트가 지워졌습니다. 수정 후 암호 찾기 양식을 제출하면 세션이 변경되지 않고 게스트 카트가 보존됩니다.

_ACP2E-4524 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/aa2e16d1)_
...
#### [문제] 수정 사항(관리자): 모달-슬라이드 내에서 날짜 선택기 위치 수정

이 끌어오기 요청은 Magento 날짜 선택기가 관리 모달 대화 상자 내에 올바르게 표시되지 않는 문제를 해결합니다.
예를 들어 관리 패널에서 제품을 편집하거나 만들 때 고급 가격 책정 섹션에 영향을 주었습니다.

_AC-16378 - [GitHub 문제](https://github.com/magento/magento2/issues/40468) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40225)_

#### 장바구니 페이지와 제품 페이지의 FPT 값이 동일한 구성에 대해 다릅니다.

반올림 불일치로 인해 제품 페이지와 장바구니 페이지에 고정 제품 세금(FPT) 값이 다르게 표시되는 문제를 해결했습니다. 이 수정 사항이 적용되면 카탈로그, 제품 및 장바구니 페이지 간에 FPT 가격이 일관되어 불일치를 피할 수 있습니다.

_AC-16550 - [GitHub 코드 기여](https://github.com/magento/magento2/commit/8391dbcc)_
...
#### 제품 이미지 변경 사항이 작업 로그에 기록되지 않음

제품 이미지 업로드 및 삭제가 관리 작업 로그에서 추적되지 않는 문제를 해결했습니다. 이전에는 관리자가 제품에 새 이미지를 추가하거나 제품의 미디어 갤러리에서 기존 이미지를 삭제할 때 이러한 변경 사항이 로깅 시스템에 기록되지 않았습니다. 이미지 역할 변경(예: 이미지를 기본 제품 이미지, 썸네일 또는 작은 이미지로 할당)만 기록되었습니다. 이제 이미지 추가 및 삭제를 포함한 모든 미디어 갤러리 변경 사항이 관리 작업 로그에 올바르게 기록되어 제품 이미지 관리 활동에 대한 완전한 감사 추적 가시성을 제공합니다.
...
_ACP2E-4337 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/31258bf6)_
...
#### 관리 패널에서 저장소 구성을 업데이트할 수 없음

이전에는 Redis를 사용할 수 없을 때 로컬 캐시, 오래된 캐시 및 Redis가 동기화되지 않을 수 있었습니다. Redis가 복구된 후 Redis 중단 중에 수행된 구성 변경 사항이 제대로 반영되지 않아 오래된 값이 지속되었습니다. 이 수정 사항으로 모든 캐시에서 올바른 동기화가 수행되므로 Redis가 다운되었을 때에도 저장된 최신 값을 유지하고 볼 수 있습니다.

_ACP2E-4447 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/e2b23caa)_

#### 비동기 그리드 인덱싱이 활성화된 경우 보관 그리드가 최신 주문 데이터와 동기화되지 않음

이제 영업 주문 아카이브 그리드에 비동기 인덱싱과 함께 올바른 주문 상태가 표시됩니다.

_ACP2E-4493 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/f2fd9628)_

#### 제품 속성 대량 갱신이 제품 유형을 변경합니다. 가상->단순

&quot;가중치가 있습니까?&quot;를 방지합니다. 대량 업데이트 속성 패널을 삽입하기 전에 활성화되지 않도록 제어하여 가상 제품에 대한 의도하지 않은 가중치 변경을 방지할 수 있습니다.

_ACP2E-4500 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/e2b23caa)_

#### 제품 이름과 사용자 정의 옵션이 긴 경우 여러 페이지의 송장 PDF에서 텍스트가 겹침

수정 전 다중 페이지 송장, 선적, 대변 메모 및 반환 PDF에서 사용자 지정 옵션이 있는 긴 제품 이름이 페이지 나누기 후 인접한 열/항목과 겹쳐 줄 항목 섹션을 읽을 수 없습니다.
수정 후 라인 항목이 페이지 간에 올바르게 흐르고 긴 이름/옵션이 송장, 선적, 대변 메모 및 반환 PDF에서 겹치지 않고 렌더링됩니다.

_ACP2E-4630 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/d8a248cd)_

#### [클라우드] HugeRTE 편집기가 업그레이드 후 이모지를 차단합니다.

수정되기 전에 WYSIWYG/PageBuilder 유효성 검사는 실제 저장소 경로가 utf8mb4-safe인 설치에서도 너무 광범위하게 적용된 UI 규칙으로 인해 최신 4바이트 이모지 콘텐츠를 무조건 차단했습니다. 수정 후 대상 콘텐츠 열 데이터 정렬과 함께 활성 DB 연결 charset/collation을 확인하여 실제 지속성 경로를 기반으로 유효성 검사가 결정되므로 utf8/utf8mb3 경로는 보호되는 반면 utf8mb4-safe 콘텐츠는 허용됩니다.

_ACP2E-4675 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c58a8ad0) - [GitHub 코드 기여도](https://github.com/magento/magento2-page-builder/commit/5fd20dbb)_

#### [클라우드] 이미지 경로는 Pagebuilder에서 두 번째 편집에서 절대 경로입니다.

Page Builder 텍스트 인라인 편집에서 이식 가능한 `{{media url=...}}` 지시문을 유지하는 대신 이미지를 다시 편집한 후 절대 미디어 이미지 URL을 저장할 수 있는 문제를 해결했습니다.

_ACP2E-4698 - [GitHub 코드 기여도](https://github.com/magento/magento2-page-builder/commit/5fd20dbb)_
...
#### 미디어 갤러리의 gettree 호출 성능 저하

전체 트리를 사전에 빌드하는 대신 디렉터리 트리 로드를 온디맨드 노드 확장으로 변경하여 미디어 갤러리 성능을 개선하여 대형 미디어 디렉터리 구조에서 초기 gettree 응답 시간을 크게 줄였습니다.

_ACP2E-4605 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c12a7510)_
...
#### 고객 추가 및 편집 페이지의 &quot;회사에 연결&quot; 필드에 대한 UI 양식 재설정 단추가 예상대로 작동하지 않음

재설정 단추가 고객 추가/편집 양식에서 &quot;회사에 연결&quot; 필드를 올바르게 재설정하지 않은 문제가 수정되었습니다. 이제 양식이 페이지를 다시 로드하지 않고 모든 필드를 제대로 재설정하여 예상 동작을 복원합니다.

_AC-16364_

### 관리자 UI, 가져오기/내보내기

#### 내보내기가 완료되지 않은 동안에는 CSV 파일 내보내기를 다운로드할 수 있습니다

수정하기 전에 대기 중인 제품 내보내기 파일이 var/export의 최종 위치에 직접 기록되므로, 관리자 내보내기 그리드는 생성이 완료되기 전에 이를 표시하고 다운로드를 허용할 수 있습니다. 따라서 사용자는 비어 있거나 부분적으로 생성된 CSV 파일을 다운로드할 수 있으며, var/export에 있는 관련 없는 파일도 내보내기 목록에 표시되어 다운로드 가능할 수 있습니다.
수정 후 대기 중인 내보내기는 임시 파일에 기록되고 생성이 완료된 후에만 최종 내보내기 위치에 게시됩니다. 이제 관리 내보내기 그리드 및 다운로드 작업에 구성된 내보내기 파일 형식 옵션과 형식이 일치하는 완료된 내보내기 파일만 표시되므로 부분 다운로드를 방지하고 관련 없는 파일을 UI에서 제외할 수 있습니다.

_ACP2E-4724 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c58a8ad0)_
...
#### 부분 환불을 처리할 수 없음 - 환불 계산 불일치

이제 온라인 주문 환불이 정상적으로 작동합니다. 이전에는 &quot;PayPal 게이트웨이가 요청을 거부합니다. 내부 오류&quot;.

_ACP2E-4615 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c58a8ad0)_

### 관리자 UI, 성능

#### 제품을 저장하는 동안 스테이징 관리 패널의 속도가 심각하게 느려짐

관리 제품 양식 저장/편집은 기존 유효성 검사 동작을 유지하면서 값비싼 클라이언트측 유효성 검사 통과를 줄여 큰 속성 집합에 대해 더 빠릅니다. 이렇게 하면 관리 UI에서 &quot;페이지가 응답하지 않음&quot;이 중지될 가능성이 줄어듭니다.

_ACP2E-4488 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/f2fd9628)_

### 관리자 UI, 제품

#### [Cloud] RequireJS mixins.js로 인해 모든 deps 모듈에 대해 중복 스크립트가 로드됩니다.

수정하기 전에 일부 storefront JavaScript 파일을 두 번 요청 및 실행하여 간헐적인 중복 로드와 불안정한 동작을 야기할 수 있습니다. 수정 후 JavaScript 종속성이 한 번만 로드되므로 기존 mixin 기능을 그대로 유지하면서 중복 요청을 제거할 수 있습니다.

_ACP2E-4626 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c12a7510)_
...
#### 기프트 카드 계정 만료일 필드의 입력 처리 문제

관리자에서 기프트 카드 계정을 만들거나 편집할 때 이제 만료 날짜 필드의 유효성이 적절하게 확인됩니다. 이전에는 수동으로 입력한 값 중 유효한 날짜 형식과 일치하지 않는 값은 계정을 저장할 때 잘못 처리될 수 있었습니다.
...
#### analytics_collect_data cron 작업 장기 실행에 대한 [문제] 성능 수정

LIMIT 0 대신 LIMIT 1을 사용하도록 보고서 검증 쿼리를 업데이트하여 대형 데이터베이스에 대한 analytics_collect_data cron 작업 성능을 최적화했습니다. 이렇게 하면 유효성 검사 중에 불필요한 전체 테이블 데이터 로드를 방지하고 실행 시간을 크게 줄일 수 있습니다. 기능에 영향을 미치지 않으며 이전 버전과 완벽하게 호환되는 개선 사항이 있습니다.

_AC-16639 - [GitHub 문제](https://github.com/magento/magento2/issues/40590) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40463)_

#### 관리자 작업 로그 사용자 보고서에는 필터를 적용할 때 사용된 필터에 대한 세부 정보가 표시되지 않습니다

수정 전에 필터링 매개 변수가 관리자 활동 보고서에 기록되지 않았습니다. 이제 수정 후 모든 요청 데이터가 기록됩니다.
...
#### aggregate_sales_report_order_data Cron 작업으로 인한 서비스 노드의 최대 디스크 I/O 스파이크

일별 집계 주문 수 보고서에서 대량의 주문을 생성하는 데 시간이 너무 오래 걸려 디스크 I/O 스파이크 및 성능 저하가 발생하는 문제를 해결했습니다. 수정 후 보고서가 적시에 생성됩니다.

_ACP2E-4363 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/e2b23caa)_

#### analytics_collect_data cron 실행으로 인해 [CLOUD] 성능 저하

분석 cron 작업과 관련된 성능을 개선합니다.

_ACP2E-4496 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/f2fd9628)_

### B2B

#### PayFlow Pro 신용 카드 결제 방법으로 협상가능 견적을 통해 체크아웃 진행

이제 Adobe Commerce은 Payflow Pro 신용 카드 결제 방법을 사용하여 협상 가능한 견적을 체크아웃할 때 주문을 성공적으로 처리합니다. 이전에는 B2B 기능이 활성화되어 있고 구매자가 협상 가능한 견적에서 체크아웃을 진행할 때 Payflow Pro를 선택하고 주문 진행을 클릭하면 페이지가 오류 메시지 없이 무기한 로드되었으며 주문이 생성되지 않았습니다. AC-11973

_AC-11973_

#### 견적 이름 변경 후 성공 메시지가 간헐적으로 사라짐

이제 Adobe Commerce은 상점 첫 화면에서 협상 가능한 견적 또는 견적 템플릿의 이름이 변경된 후에도 성공 메시지를 일관되게 표시합니다. 이전에는 구매자가 협상 가능한 견적의 이름을 변경하면 성공 메시지가 간헐적으로 나타나지 않고(종종 거의 즉시 지워짐), 이름 변경 작업이 성공하더라도 이 메시지를 기다리는 자동화된 테스트가 실패하는 원인이 되었습니다. AC-13447
...
#### 제한된 관리자는 공유 카탈로그에 회사를 할당할 수 없습니다.

공유 카탈로그에 회사를 할당할 때 제한된 관리자 사용자에게 예외가 발생하는 문제가 해결되었습니다. 업데이트는 할당이 오류 없이 올바르게 작동하도록 합니다.

_AC-15662_

#### 범주 권한이 활성화된 경우 그룹화된 제품을 구매요청 목록에 추가할 때 예외 발생

제품 옵션이 배열로 안전하게 처리되도록 하여 모든 제품 유형을 예외 없이 추가할 수 있도록 범주 권한이 활성화된 구매요청 목록에 그룹화된 제품을 추가할 때 발생하는 TypeError를 수정했습니다.

_AC-15862_

#### 로그인 중 고객을 회사에 할당한 후 장바구니에 표시되지 않는 견적을 체크아웃 및 요청으로 진행

고객이 상점 앞에 로그인한 상태에서 회사에 할당되면 &quot;체크아웃 진행&quot; 및 &quot;견적 요청&quot; 버튼이 장바구니에 표시되지 않는 문제가 해결되었습니다. 이 수정 후에는 고객이 로그아웃했다가 다시 로그인하지 않아도 두 버튼이 즉시 나타납니다.
...
#### [B2B] 일부 견적에 삭제된 제품이 포함된 경우 내 견적 페이지에 견적이 표시되지 않습니다.

일부 견적에 삭제된 제품이 포함되어 있을 때 내 견적에 해당 견적이 올바르게 표시됩니다.

_ACP2E-4608 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c12a7510)_
...
#### 여러 쿠폰이 Google Pay의 예상대로 작동하지 않음

여러 쿠폰 코드가 Google Pay Express Pay Sheet 내에서 예상대로 작동하지 않던 문제를 수정했습니다. 이전에는, 두 번째 쿠폰 코드를 적용하면 첫 번째 쿠폰의 할인이 잘못 제거되고 부정확한 합계가 표시됩니다. 쿠폰을 제거해도 지급 시트가 올바르게 업데이트되지 않았습니다.
고객은 이제 Google 페이 익스프레스 페이시트에 있는 여러 쿠폰 코드를 적용할 수 있으며, 모든 할인이 정확하게 반영된다.

_BUNDLE-3507_

#### Braintree SDK은 나중에 결제 메시지가 비활성화되더라도 모든 페이지에서 클라이언트 토큰을 가져올 수 있도록 호출을 받습니다

Braintree SDK은 나중에 지불 메시지가 비활성화된 경우에도 페이지를 로드할 때마다 클라이언트 토큰을 검색하기 위해 호출을 했습니다. 이로 인해 불필요한 요청과 잠재적인 성능 문제가 발생했습니다.
이제 카트에 대한 PayPal 결제 방법과 나중에 결제 메시지가 모두 활성화된 경우에만 Braintree 클라이언트 토큰을 요청합니다. 이렇게 하면 불필요한 토큰 요청이 방지되고 페이지 로드 성능이 향상됩니다.

_BUNDLE-3508_

#### 고객 계정 영역의 &#39;저장된 결제 방법&#39; 섹션에서 Braintree 결제 방법의 보관 템플릿 및 js 파일 렌더링 중지

Braintree 결제 방법 및 보관 구성이 비활성화된 경우에도 고객 계정 영역의 &#39;저장된 결제 방법&#39; 섹션에서 Braintree 결제 방법에 대한 JavaScript 및 템플릿 파일이 렌더링되고 있었습니다. 이로 인해 브라우저 콘솔에서 불필요한 오류가 발생했습니다.
이제 시스템은 관련 파일 및 양식을 렌더링하기 전에 Braintree 결제 방법 및 해당 보관 구성이 활성화되었는지 확인합니다. 둘 다 활성화된 경우에만 보관 템플리트와 스크립트가 &#39;저장된 결제 방법&#39; 섹션에 표시됩니다.

_BUNDLE-3509_

#### Apple Pay 및 Google Pay 서드파티 호환성 문제

고객이 서드파티 체크아웃 확장(예: Hyvä Checkout) 또는 사용자 정의를 사용하여 Apple Pay 또는 Google Pay를 통해 결제하려고 할 때 오류가 발생했습니다. 특히 Apple Pay는 판매자 이름이 누락되어 TypeError를 발생시키며, 사용자 지정 코드 상호 작용에 따라 Google Pay도 실패할 수 있습니다.
Apple Pay 및 Google Pay와의 호환성 문제가 해결되었습니다. 이제 시스템에서 타사 체크아웃 확장 및 사용자 정의를 적절하게 처리하여 필요한 구성 값이 존재하며 올바르게 반환되도록 합니다.

_BUNDLE-3510_

#### Braintree을 통한 Google Pay를 사용하면 체크아웃 중에 무한 로딩을 할 수 있습니다

Google Pay가 Braintree을 통해 활성화되면 결제 방법으로 Google Pay를 선택한 후 체크아웃 프로세스가 무한 로드 상태로 전환됩니다. 이 경우 오류 메시지나 피드백이 제공되지 않은 상태에서 고객이 주문을 완료할 수 없습니다.
예상대로 결제 흐름이 진행될 수 있도록 Braintree을 통한 Google Pay 통합이 수정되었습니다. 이제 고객은 체크아웃 중에 Google 페이를 선택하고, 결제 프로세스를 완료하고, 성공적으로 주문할 수 있습니다.
...
#### 여러 주소로 체크아웃하고 구성 가능한 제품의 다른 옵션을 추가한 후 소계에 0이 표시됩니다.

다중 주소 체크아웃 후에 구성 가능한 제품의 다른 옵션을 추가한 후 장바구니 소계가 0으로 잘못 표시되던 문제를 수정했습니다. 이제 소계가 실제 장바구니 값을 올바르게 반영합니다.

_AC-14460 - [GitHub 코드 기여](https://github.com/magento/magento2/commit/57bad413)_
...
#### 로그인 후 내 계정 페이지를 리디렉션하는 체크아웃으로 진행

세션 만료 후 사용자가 체크아웃 로그인 대신 내 계정 로그인 페이지로 리디렉션되어 로그인 양식으로 올바르게 체크아웃되도록 하는 문제를 해결했습니다.

_AC-15962_

#### [문제] 장바구니에서 ItemRepository 대신 RemoveItem을 호출합니다.

CartItemRepository의 deleteById 호출을 장바구니 인스턴스의 removeItem으로 바꿉니다.
이렇게 하면 예상 관찰자가 트리거되고 항목이 데이터베이스에서만 업데이트하지 않고 장바구니 개체에서도 실제로 업데이트됩니다.

_AC-15963 - [GitHub 문제](https://github.com/magento/magento2/issues/40255) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40248)_
...
#### 장바구니 가격 규칙 할인이 제외 sku 조건과 작동하지 않음

제외된 SKU가 소계 계산에 있는 경우에도 장바구니 가격 규칙이 잘못 적용되는 문제가 수정되었습니다. 이제 할인 논리에서 규칙 조건을 평가할 때 제외된 항목을 올바르게 무시합니다.

_AC-16417 - [GitHub 문제](https://github.com/magento/magento2/issues/40503) - [GitHub 코드 기여](https://github.com/magento/magento2/commit/8391dbcc)_
...
#### 번들 제품을 추가하는 동안 관리자 고객 장바구니 섹션에서 0으로 표시되는 수량  
...
_ACP2E-4245 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/cbca0396)_
...
#### Recaptcha v2가 포함된 기프트 카드를 적용할 때 [CLOUD] 문제(&quot;로봇이 아님&quot;)

이제 reCAPTCHA 유효성 검사가 성공적으로 수행된 후 reCAPTCHA v2(&quot;I am a robot&quot;)가 활성화되면 기프트 카드가 올바르게 적용됩니다.

_ACP2E-4410_

#### 관리자의 고객으로 로그인을 사용하는 경우 null 견적이 생성됩니다.

&quot;고객으로 로그인&quot; 흐름 중에 null 견적이 생성되던 문제를 해결했습니다.

_ACP2E-4472 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/e2b23caa)_

#### 정액 배송 및 무료 배송을 사용하는 번들 제품 동작의 불일치

수정 이전에는 장바구니 할인 규칙을 통해 제공되는 경우 번들 제품에 대해 무료 배송이 제대로 계산되지 않았습니다. 수정 후 번들 제품에 대해 무료 배송이 올바르게 계산됩니다.

_ACP2E-4536 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c12a7510)_

#### 숫자 SKU가 있는 제품에 대해서는 주문을 할 수 없습니다.

숫자 SKU를 사용하는 제품을 주문할 때 시스템에서 더 이상 오류가 발생하지 않습니다.

_ACP2E-4669 - [GitHub 코드 기여도](https://github.com/magento/inventory/commit/83d11138)_

#### EAV(고객 주소 속성) 유효성 검사 규칙이 하드코딩된 유효성 검사 규칙과 충돌합니다.

최대 255자의 값을 지원하도록 고객 주소 전화 확인 기능이 향상되어 서버측 확인을 데이터베이스 제한에 맞게 조정했습니다. 또한 긴 전화 번호가 명확한 길이별 메시지를 반환하도록 오류 처리가 개선되었습니다.

_ACP2E-4735 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c58a8ad0)_
...
#### [쿠폰] quote_coupons 테이블에 중복 쿠폰 입력 오류가 발생했습니다.

여러 장바구니 병합 또는 견적 저장 요청이 동시에 실행될 때 quote_coupons 테이블에서 간헐적인 중복 키 오류를 해결했습니다.
...
_ACP2E-4266 - [GitHub 코드 기여도](https://github.com/magento/inventory/commit/ae1f272f)_
...
#### Magento\Store\Model\System\Store 클래스의 [TOYOTA] 성능 문제

여러 스토어 및 웹 사이트와 관련된 성능 개선.

_ACP2E-4491 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/e2b23caa)_

### 장바구니 및 체크아웃, 가격, 세금

#### 번들 제품 가격은 다중 통화 설정에서 PDP와 장바구니 간에 잘못된 세금 및 통화 값을 표시합니다.

이제 PDP와 체크아웃 페이지 간에 번들 제품 가격이 올바르게 표시됩니다.

_ACP2E-4673 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c58a8ad0)_
...
#### [\Magento\ConfigurableProduct\Model\Product\Type\Configurable] PHP 오류를 알 수 없습니다.
...
#### Magento 카탈로그 제품 목록 위젯이 잘못된 개수를 반환합니다.

이제 Adobe Commerce 카탈로그 제품 목록 위젯에서 올바른 총 개수 및 페이지 매김(예: 구성된 대로 9개의 페이지 항목)을 반환합니다.

_AC-14541 - [GitHub 문제](https://github.com/magento/magento2/issues/39863) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40421)_
...
#### Meta 태그 자동 생성: 변수를 여러 번 사용할 수 없음

이제 제품 메타 키워드 자동 생성이 첫 번째 발생뿐만 아니라 구성된 마스크(예: `{{name}}, order {{name}}`)의 모든 자리 표시자를 대체합니다.

_AC-14626 - [GitHub 문제](https://github.com/magento/magento2/issues/35670) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40412)_
...
#### 회사 관리자가 변경되면 회사의 고객 그룹 ID가 회사 관리자의 고객 그룹 ID에 할당되지 않습니다

B2B에서 회사의 관리자를 변경할 때 새 회사 관리자의 고객 그룹 ID가 회사의 할당된 공유 카탈로그와 일치하도록 업데이트되지 않았습니다. 이로 인해 새 관리자의 계층 가격 및 범주 권한은 이전 회사의 고객 그룹을 기반으로 하므로 잘못된 가격 및 권한이 발생합니다.
이제 관리자가 변경되면 시스템이 회사 관리자의 고객 그룹 ID를 올바르게 업데이트합니다. 이렇게 하면 새 관리자가 회사의 공유 카탈로그와 연결된 올바른 계층 가격 및 범주 권한을 받게 됩니다.

_AC-14897 - [GitHub 문제](https://github.com/magento/magento2/issues/39964)_
...
#### 제품 스톡 경고의 웹 사이트 및 다중 웹 사이트에 대한 스톡 불일치 문제

이 수정 사항에는 collection SELECT 절의 product_alert_stock 및 product_alert_price 테이블의 website_id 및 store_id가 포함되어 있으므로 관리 그리드에 각 경고가 구독한 올바른 웹 사이트가 표시되는지 확인합니다.

_AC-15044 - [GitHub 문제](https://github.com/magento/magento2/issues/40027) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40431)_
...
#### [문제] 항목이 없는 비교 페이지를 방문하는 동안 오류가 발생했습니다.

비교 작업이 예외를 발생시키는 대신 빈 항목 컬렉션을 안전하게 처리되도록 하여 비교 항목이 없는 카탈로그 제품 비교 페이지를 방문하면 치명적인 오류가 발생하는 버그를 수정했습니다.

_AC-15746 - [GitHub 문제](https://github.com/magento/magento2/issues/40205) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40203)_
...
#### 매장 내 픽업 이메일이 여전히 이전 방법을 사용 중입니다.

사용 가능한 설명이 없습니다.

_AC-16380 - [GitHub 문제](https://github.com/magento/inventory/issues/3384) - [GitHub 코드 기여](https://github.com/magento/inventory/pull/3385) - [GitHub 코드 기여](https://github.com/magento/inventory/commit/b4aeda50)_

#### 제품 페이지에서 속성을 만드는 동안 드롭다운 옵션 삭제가 작동하지 않음
...
제품 속성을 만들 때 삭제된 드롭다운 옵션이 계속 유지되던 문제를 수정했습니다. 이제 선택한 옵션이 올바르게 제거되고 저장 후 정확한 카운트가 반영됩니다.

_AC-16437 - [GitHub 코드 기여](https://github.com/magento/magento2/commit/57bad413)_
...
수정 후 `--skip_hidden_images` 플래그와 함께 사용된 `catalog:images:resize` 명령은 이미지가 없는 웹 사이트에 대한 이미지 캐시를 생성하지 않습니다.
...
#### [클라우드] 서로 다른 스토어에 대한 제품 수 비교 불일치 문제

이제 다른 저장소로 전환하면 제품 목록 비교가 제대로 작동합니다.

_ACP2E-4249 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/98f028ab)_

#### 구성 가능한 제품을 만드는 동안 제한된 관리자용 &quot;새 속성 추가&quot; 버튼이 표시됨

이제 구성 가능한 제품을 만드는 동안 일반 관리 사용자에 대해서만 &quot;새 속성 추가&quot; 버튼이 표시됩니다.
이전에는 제한된 관리자에 대해 &quot;새 속성 추가&quot; 버튼이 표시되었습니다
...
이제 대상 규칙과 일치하는 관련 제품이 색인 재지정 프로세스 전체에서 올바르게 연결되어 있습니다

_ACP2E-4430_

#### [클라우드] 순환 범주 종속성

수정하기 전에 순환 의존성이 있는 범주를 저장할 수 있습니다. 수정 후에는 유효성 검사 규칙을 통해 이 동작을 방지할 수 있습니다.

_ACP2E-4505 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/f2fd9628)_

#### 하위 페이지 URL 키를 변경해도 CMS 계층은 계층 구조 URL을 업데이트하지 않습니다(2.4.8-p3).

CMS 페이지의 URL 키를 변경해도 페이지의 계층적 URL이 업데이트되지 않는 문제가 해결되었습니다(예: my-account-2로 이름을 바꾼 후 /help/my-account가 동일하게 유지). 이제 계층 업데이트 논리에 페이지의 자체 노드가 올바르게 포함되므로 해당 request_url이 업데이트되고 새 경로(예: /help/my-account-2)가 예상대로 작동합니다.

_ACP2E-4531_

#### 이미지 자리 표시자가 다른 저장소에 대해 예상대로 표시되지 않음

수정 자리 표시자 이미지가 URL에 삽입되었을 때 이전 버전의 이미지가 스토어 코드에 로드되지 않았습니다. 수정 후 이미지 자리 표시자는 URL에 제공된 스토어 코드를 사용하거나 사용하지 않고 범위에 따라 다릅니다.

_ACP2E-4533 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/aa2e16d1)_

#### magento_catalogpermissions_index_product 테이블의 [CLOUD] 교착 상태. 연속

수정하기 전에 카탈로그 권한 제품 다시 색인을 실행하면 기존 권한 색인 행이 변경되지 않을 수 있으므로 업데이트된 범주 권한 부여가 제품에 안정적으로 반영되지 않았습니다.
수정 후 reindex는 기존 권한 인덱스 행을 올바르게 업데이트하므로 권한 변경 사항은 상점 액세스, 가격 가시성 및 체크아웃 사용 가능 여부에 일관되게 적용됩니다.

_ACP2E-4603_

#### Target 규칙 인덱서 cron은 스토어 보기 범위에서만 활성화된 제품을 선택하지 않습니다.

수정 전, 관리 범위에서 활성화되지 않은 제품이 대상 규칙 인덱서에서 선택되지 않았습니다. 이제, 수정 후 가 하나 이상의 범위에서 활성화된 경우 제품이 선택됩니다.
...
#### [Cloud] - attribute_type의 catalog_product_entity_int 값이 graphql 제품에서 0 또는 Null을 선택한 경우 오류가 표시됩니다.

집계가 있는 GraphQL 제품 쿼리가 잘못된 속성 값 또는 옵션과 함께 오류를 반환하는 문제를 해결했습니다. 이제 쿼리가 잘못된 데이터를 올바르게 처리하고 올바른 결과를 반환합니다.

_ACP2E-4529 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/f2fd9628)_
...
#### 번들 제품 행에 대한 [Cloud] 주문 반환 페이지가 자동으로 선택 해제됩니다.

이전에는 관리 패널 그리드 보기에서 번들 제품 함께 배송 반환에 대해 번들 제품 함께 배송 옵션에 대해 혼동을 일으킨 &quot;항목 선택&quot; 옵션이 있었습니다. 이 수정 사항이 적용되면 번들 제품 배송의 경우 더 이상 &quot;품목 선택&quot; 옵션이 없습니다.
...
#### 필드 값에 `\`이(가) 포함된 경우 관리 패널 `listing component`의 [작은 버그] 필터를 누를 수 없습니다.
...
#### 카테고리 이미지 이름이 &quot;-&quot;로 시작하면 Media Gallery 폴더 구조가 사라짐(대시)

사용 가능한 설명이 없습니다.

_AC-16544 - [GitHub 문제](https://github.com/magento/magento2/issues/40536)_

#### &quot;카탈로그 이벤트 목록&quot; CMS 블록을 저장할 때 &quot;CMS 블록에 잘못된 콘텐츠가 포함됨&quot; 오류가 표시됩니다.

기본 &quot;카탈로그 이벤트 목록&quot; CMS 블록을 저장하면 &quot;CMS 블록에 잘못된 콘텐츠가 포함됨&quot; 오류가 표시되는 문제를 해결했습니다. 이 수정 사항이 적용되면 관리 패널에서 오류 없이 블록을 저장할 수 있습니다.

_AC-16686 - [GitHub 코드 기여](https://github.com/magento/magento2/commit/0c9a749b)_
...
#### 웹 사이트 수준 권한이 있는 관리자가 동적 블록을 편집할 수 없음

이제 웹 사이트 범위 권한이 있는 관리자는 액세스 가능한 저장된 보기에서 배너의 콘텐츠를 편집할 수 있습니다.

_ACP2E-4468_

#### [ADOBE STOCK] 통합을 사용하지 않도록 설정한 경우에도 &quot;Adobe Stock 검색 파일 결과 검색 실패&quot;에 대한 중요 로그가 있습니다.

Adobe Stock 통합이 비활성화될 때 시스템이 더 이상 &quot;Adobe Stock 검색 파일 검색 결과 실패&quot; 오류를 기록하지 않습니다.

_ACP2E-4634_

#### 새 페이지를 만들고 페이지 빌더를 사용할 때 오류가 발생합니다.

페이지 빌더를 사용할 때 CMS 페이지를 저장하지 못하는 문제가 수정되었습니다.
...
#### 고객 관리자의 `[Admin][Customer]` 암호 재설정이 실패했습니다.

관리자 패널에서 고객 암호를 재설정하면 시스템 오류가 발생하고 페이지가 충돌하는 문제를 해결했습니다. 이제 암호 재설정이 제대로 작동하며 스토어 변경과 관련된 극단적인 경우에도 오류 없이 재설정 링크를 전송합니다.

_AC-14940 - [GitHub 코드 기여](https://github.com/magento/magento2/commit/3e41a43b)_
...
이 PR을 사용하면 다중 값 양식 요소가 웹 사이트/그룹/스토어 양식에 데이터를 제출할 수 있습니다.
...
이제 배송 라벨 금액은 배송 및 취급 가격과 일치합니다.
이전에는 배송 라벨을 다운로드한 후 표시된 금액이 배송 및 취급 가격과 일치하지 않았습니다.
AC-14560
...
_AC-14883 - [GitHub 문제](https://github.com/magento/magento2/issues/39947) - [GitHub 코드 기여](https://github.com/magento/magento2/commit/3717e6cb) - [GitHub 코드 기여](https://github.com/magento/magento2/commit/d3ea191d)_
...
_AC-15446 - [GitHub 문제](https://github.com/magento/magento2/issues/40170) - [GitHub 코드 기여](https://github.com/magento/magento2/commit/e9412b24)_
...
#### NewRelic 모듈 - 필요 없는 종속성 제거

Magento/module-new-relic-reporting 모듈에서 불필요한 magento/magento-composer-installer 종속성을 제거하여 종속성에서 사용되지 않는/긴밀한 연결을 정리합니다.

_AC-15945 - [GitHub 문제](https://github.com/magento/magento2/issues/40243)_

#### [문제] `comment=""`을(를) 사용한 `db_schema.xml` 선언이 가동 중지 시간이 없는 배포를 중단하지 않도록 합니다.
...
#### [문제] `Magento_Downloadable`에서 금지된 `@author` 태그를 제거하십시오.
...
#### [문제] `Magento_Backup` 및 `Magento_Bundle`에서 금지된 `@author` 태그를 제거합니다.
...
수정 전에 헤드 섹션을 통해 사용자 지정 javascript를 로드하면 문제가 발생할 수 있습니다. 새 설정이 도입된 후에는 이러한 스크립트를 자동으로 연기할 수 있으므로 Magento 2 프레임워크와의 호환성을 높일 수 있습니다.
...
#### REST API를 통해 제품 변경 사항을 저장할 때 [CLOUD] DateTimeZone 오류가 발생했습니다.

수정 전에 코드 &quot;default&quot;를 사용하는 저장소가 없는 경우 제품 업데이트 REST API 요청이 오류를 생성합니다. 이제, 수정 후 &quot;기본&quot; 저장소가 있는지 여부에 관계없이 제품 업데이트 요청이 제대로 실행됩니다.
...
#### Magento 2.4.8 업데이트 후 Cron captcha_delete_expired_images가 작동하지 않음

수정 전에 만료된 captcha 이미지가 더 이상 자동으로 삭제되지 않았습니다. 수정 후 만료된 captcha 이미지는 해당 cron 작업에 의해 삭제됩니다.

_ACP2E-4512 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/e2b23caa)_

### 프레임워크, 성능

#### [CLOUD] 판매 그리드 비동기 cron - 전체 테이블 스캔, 20+ s, ~14M 행을 검사하여 MySQL 조정 쿼리가 느려집니다.

누락 ID 검색을 커서 기반 진행으로 경계 entity_id 창으로 전환하여 대용량 저장소에서 반복되는 대용량 테이블 검색을 줄여 영업 그리드 비동기 조정 성능을 개선했습니다.

_ACP2E-4585 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/aa2e16d1)_
...
다른 형식으로 오류를 반환하는 이전 변경 사항을 되돌렸습니다. 이제 잠재적인 오류가 GraphQL 스키마를 손상시키지 않고 일관된 방식으로 반환됩니다.
...
#### 제품이 삭제될 때 customerOrders graphql이 오류를 반환함
...
이 문제를 해결하기 전에는 오류 노드 위치가 2.4.7 및 2.4.9 버전과의 완벽한 호환성을 제공하지 않았습니다. 이제 수정 후 두 버전을 모두 수용하도록 오류 노드가 제대로 배치됩니다.
...
#### AC-2.4.9 - EDS 보일러판에서 주문을 검색할 수 없음

guestOrder GraphQL 쿼리 입력 유형이 OrderInformationInput 대신 GuestOrderInformationInput으로 업데이트되었습니다.

_LYNX-872_

#### [AC-2.4.9] 장바구니 쿼리가 MSI(Multi-Source Inventory)가 있는 제품에 대해 잘못된 가용성 상태를 반환하며 기본 재고는 0입니다.

여러 Source 인벤토리(MSI) 소스 간에 충분한 인벤토리가 있는 제품에 대해 장바구니 API에서 is_available: false가 잘못 반환되는 문제가 해결되었습니다.

_LYNX-918_

#### [AC-2.4.9]은(는) setNegotiableQuoteShippingAddress/setNegotiableQuoteBillingAddress GQL로 고객의 배송/청구 주소를 설정할 수 없습니다.

고객 주소 GraphQL 쿼리에 uid 필드를 노출하여 운송 및 청구 주소를 협상 가능한 견적으로 설정할 수 있도록 했습니다.

_LYNX-924_

#### [AC-2.4.9] 순서 목록 | 내부 서버 오류

누락된 구매 제품에 대해 null을 반환하여 주문 내역 및 최근 주문의 예외를 방지하여 주문 흐름의 안정성을 보장했습니다.

_LYNX-937_

#### [AC-2.4.9] BillingAddressInput과 NegotiableQuoteBillingAddressInput 정렬

&#39;
고객 주소 ID를 사용하지 않는 동안 고객 주소 uid를 지원하기 위해 NegotiateQuoteBillingAddressInput, BillingAddressInput, ShippingAddressInput 및 GiftRegistryShippingAddressInput을 정렬했습니다.
고객 주소 uid를 지원하기 위해 deleteCustomerAddressV2 및 updateCustomerAddressV2 돌연변이를 도입했으며 기존 deleteCustomerAddress 및 updateCustomerAddress 돌연변이를 더 이상 사용하지 않습니다.
...
#### [클라우드] SalesGraphQl customerOrders 쿼리가 주문 항목 필드에 대해 잘못된 값을 반환합니다

이미 세금이 포함된 가격에 세금이 두 번째로 적용되었기 때문에 카탈로그 가격이 &quot;세금 포함&quot;으로 설정되었을 때 SalesGraphQl customerOrders 쿼리가 부풀려진 original_price_including_tax 및 original_row_total_including_tax 값을 반환하는 문제를 해결했습니다.

_ACP2E-4762 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c58a8ad0)_
...
#### Advanced Pricing CSV, 제품 CSV가 내보내지지 않습니다. 4G 메모리 제한이 있는 OOM

수정하기 전에 대형 카탈로그에서 메모리 사용량/OOM이 많아 전체 제품 및 Advanced Pricing 내보내기가 실패할 수 있으며, 처리하는 동안 동적 페이지 크기를 변경하면 제품 내보내기 페이지 매김에 중복 행이 발생할 수 있습니다. 수정 후 페이지 단위 일괄 처리를 사용하여 안정적인 메모리 비헤이비어로 내보내기를 완료하고, 페이지 매김 겹침의 중복 행을 제거하고, 내보낸 행 콘텐츠를 기준 출력과 정렬된 상태로 유지합니다.

_ACP2E-4587 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c12a7510)_
...
이 문제 이전에는 하위 제품이 사용자 정의 소스에 따라 재고를 보유할 때 번들 가격이 제대로 색인화되지 않았습니다. 이제 수정 후 번들 가격은 하위 제품 재고 할당에 관계없이 올바르게 색인화됩니다.
...
#### 스테이징 업데이트 중 SKU 변경 후 재고 상태가 수량으로 잘못 재설정됩니다.

이제 활성 예약 업데이트가 있는 제품에 대해 SKU 변경이 금지됩니다. 명확한 오류로 인해 저장이 실패하고 활성 업데이트 중에 관리 SKU 필드가 비활성화됩니다. 이렇게 하면 스테이징 롤백 중에 SKU 변경으로 인해 MSI 인벤토리 불일치가 발생하지 않습니다.

_ACP2E-4389_

#### 주문이 취소되면 번들 판매 가능성 상태가 업데이트되지 않음

이제 주문 취소 후 번들 제품 판매성이 올바르게 계산됩니다.

_ACP2E-4481 - [GitHub 코드 기여도](https://github.com/magento/inventory/commit/96b77663)_

#### Magento에서는 여러 고객이 동시에 주문을 할 때 초과 판매를 허용합니다

MSI를 이용하면 여러 고객이 같은 상품에 동시 주문해 가용재고를 넘어설 수 있어 과잉판매가 발생할 수 있다. 수정 후에는 체크아웃이 동시에 실행될 때 제품당 하나의 주문만 성공할 수 있으므로 더 이상 초과 판매가 발생하지 않습니다.

_ACP2E-4509 - [GitHub 코드 기여도](https://github.com/magento/inventory/commit/a33c60f2)_

#### 클라이언트에 140개 이상의 문서가 있는 경우 [클라우드] 시간 초과

인벤토리 데이터베이스 쿼리는 체크아웃 중에 인벤토리 데이터를 열심히 로드하도록 최적화되었습니다. 이렇게 하면 각 SKU에 대해 개별 쿼리가 필요 없으므로 대형 장바구니로 체크아웃할 때 상당한 성능이 향상됩니다.

_ACP2E-4519 - [GitHub 코드 기여도](https://github.com/magento/inventory/commit/40858072)_

### 충성도

#### [CLOUD] getCustomerRewardPoints는 고객에게 점수가 없을 때 예외를 throw합니다

관리자를 통해 고객 보상 포인트를 만든 경우 고객 보상 포인트가 정상적으로 시작됩니다.

_ACP2E-4403 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c12a7510)_
...
#### Invoice getComments를 처음 호출할 때 Invoice addComment에서 오류가 발생할 수 있음

이 PR은 $invoice->addComment() 전에 $invoice->getComments()를 호출하면 치명적인 오류 &quot;배열의 멤버 함수 addItem() 호출&quot;이 발생하는 중요한 버그를 해결합니다. 이 수정 사항은 송장 모델의 설명 방법에서 배열 및 수집 유형 간의 적절한 유형 확인 및 변환을 구현하므로 이전 버전과의 호환성을 유지하면서 두 방법 모두 오류 없이 임의의 순서로 호출할 수 있습니다.

_AC-14848 - [GitHub 문제](https://github.com/magento/magento2/issues/39884) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40428)_
...
#### 새 웹 사이트 및 소스에 할당된 제품에 대한 구매요청 목록을 생성할 수 없음

&quot;URL에 스토어 코드 추가&quot;가 활성화된 경우 새 웹 사이트 및 소스에 할당된 제품에 대해 구매요청 목록을 생성할 수 없는 문제가 수정되었습니다. 저장소 코드가 API 요청에서 제거되어 승인되지 않은 오류가 발생했기 때문에 문제가 발생했습니다. 수정 후에는 올바른 스토어 컨텍스트가 유지되고 구매요청 목록이 성공적으로 생성됩니다.

_AC-16226_

#### Sales->Orders 그리드 &quot;shipping address&quot; 열에 국가가 없습니다.

관리 주문 그리드의 배송 주소 열에 국제 주문에 대한 국가가 표시되지 않던 문제를 수정했습니다. 이제 주소에는 국가 정보가 포함되어 있어 주문 가시성과 유용성이 향상되었습니다.

_AC-16256 - [GitHub 문제](https://github.com/magento/magento2/issues/40403) - [GitHub 코드 기여](https://github.com/magento/magento2/commit/3e41a43b)_

#### 비동기 주문 소비자가 지연된 재고 업데이트가 있는 주문을 거부합니다. CartManagementPlugin::getActive() (비활성 견적)의 NoSuchEntityException

게스트 주문에 대해 지연된 재고 업데이트와 함께 비동기 체크아웃이 실패하는 중요한 문제를 해결했습니다. 이전에는 비활성 견적으로 인해 소비자가 NoSuchEntityException을 throw하여 주문이 보류 중 대신 거부됨으로 표시되었습니다. 이 수정 후 소비자는 비활성 견적을 올바르게 처리하고, 주문을 접수에서 대기중으로 이동하고, 예상대로 판매 가능 수량을 조정합니다. 이제 MFTF 테스트 AsyncOrderWithAsyncStockReservationTest가 전달됩니다.

_AC-16680 - [GitHub 코드 기여](https://github.com/magento/inventory/commit/d3b2ef0b)_
...
수정 이전에는 비동기 이메일 전송을 사용할 때 배송 이메일이 스토어 주문과 일치하지 않았습니다. 이제, 수정 후, 적절한 매장 선적 이메일 주문이 전달됩니다.
...
#### 특정 발신자별 저장소는 기프트 카드 이메일에 사용되지 않음

이전에는 다른 스토어에서 송장이 생성된 후 기프트 카드용 이메일 템플릿을 보낼 때 고객이 이메일을 수신하면 관리자 구성 설정의 소유자 이름이 이메일 헤더에 반영되지 않았습니다. 이 수정 사항이 적용되면 이제 이메일 헤더에 적절한 스토어 소유자의 이메일 정보가 포함됩니다.
...
#### GraphQL cancelOrder는 기프트 카드를 사용하여 완전히 지불된 주문을 취소하지 않음

이 같은 수정에 앞서 기프트 카드를 통해 전액 지급된 주문이 GraphQL 변이를 통해 호출될 때 자동으로 &#39;폐쇄&#39; 상태로 전환되지 않았다. 이제 수정 후 GraphQL을 통해 취소되면 주문이 종료됨 상태로 전환됩니다.

_ACP2E-4455 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/e2b23caa)_

#### 쿠폰 사용은 주문 취소/거절 시 되돌릴 수 없습니다.

이제 주문을 취소하면 주문 상태 구성이 수정되었더라도 적용된 쿠폰 사용이 올바르게 재설정됩니다. 이로 인해 사용자 지정 주문 상태 설정이 있을 때 주문이 취소된 후 쿠폰이 사용된 것으로 표시되는 문제가 발생할 수 있습니다.
...
#### &quot;상한 보상 포인트 잔액&quot; 값을 비워둘 수 없음- 저장됨

이제 Adobe Commerce을 통해 판매자는 보상 포인트 잔액 환매 임계값에 대한 값을 설정하는 동안 상한 보상 포인트 잔액을 필드에 비워둘 수 있습니다. 이전에는 저장소 > 구성 > 고객 > 보상 포인트에서 보상 포인트를 구성할 때 보상 포인트 잔액 상환 임계값에 양수를 입력하고 상한 보상 포인트 잔액을 공백으로 두면 검증 오류 &quot;\&quot;상한 보상 포인트 잔액\&quot;이 유효하지 않습니다. 잔고는 양수이거나 비워둘 필요가 있다. 확인 후 다시 시도하십시오.&quot;, 상인이 캡 없이 구성을 저장할 수 없습니다. ACP2E-3977

_ACP2E-3977_

#### 등록된 사용자의 고객 세그먼트 지연에 따른 장바구니 가격 규칙

고객 세그먼트를 기반으로 한 장바구니 가격 규칙이 등록된 사용자에 대해 즉시 적용되지 않던 문제를 수정했습니다. 이제 토큰을 새로 고치지 않고도 자격을 갖춘 제품을 장바구니에 추가하면 할인이 올바르게 적용됩니다.

_LYNX-814_

#### GQL - ProductInterface의 수량 필드에서 판매 가능 수량을 반환하지 않음

ProductInterface의 수량 필드가 판매 가능 수량을 반환하지 않는 GraphQL 문제를 해결했습니다.

_LYNX-816_

#### Null을 허용하지 않는 필드 &quot;SelectedShippingMethod.carrier_code&quot;에 대해 null을 반환할 수 없습니다.

미국 주소를 사용하여 체크아웃하는 동안 두 번째 배송 방법을 선택하면 Null을 허용하지 않는 필드 &quot;SelectedShippingMethod.carrier_code&quot;에 대해 Null을 반환할 수 없음 오류가 발생하는 문제가 해결되었습니다. 이 수정 사항을 통해 내부 서버 오류를 트리거하지 않고도 체크아웃을 원활하게 탐색할 수 있습니다.

_LYNX-817_

#### [AC-2.4.9] addProductsToCartMutation이 오류 시 200을 반환합니다.

장바구니에 추가 중 구성 가능한 제품 옵션이 선택되지 않았을 때 사용자 오류를 처리하기 위해 새 오류 코드인 REQUIRED_PARAMETER_MISSING이 도입되었습니다.

_LYNX-884_

#### [AC-2.4.9] 장바구니 인벤토리 검사를 사용하지 않도록 설정할 때 CartItem.not_available_message 또는 is_available 필드를 요청할 때 오류 발생

장바구니 재고 확인이 비활성화되었을 때 장바구니에서 not_available_message 또는 is_available에 액세스할 때 발생하는 오류를 수정했습니다.
...
#### OOM으로 인해 크론 작업이 중단됨 - 크론에 대한 innodb_buffer_pool_size 및 PHP memory_limit 검토 요청

결제 거래 처리가 개선되었습니다.

_ACP2E-4601 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c12a7510)_
...
이제 서드파티 컨트롤러가 고객 세그먼트를 사용하여 올바르게 캐시됨
...
#### [Cloud] php-fpm이 max_execution_time을 따르지 않음

이제 배포 구성이 단일 요청에서 한 번 로드됩니다.
...
이 문제를 해결하기 전에는 원격 저장소 메타데이터에 사용되는 캐시 키가 만료되지 않았습니다. 이제 수정 후 종속성 삽입을 통해 이러한 캐시 키에 대한 TTL을 설정할 수 있습니다.

_ACP2E-4345 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/0a3b7032)_

#### `[CLOUD][B2B]` `[Mainline]` sales_clean_quotes getSize() 병목

sales_clean_quotes cron 작업의 성능을 향상시킵니다.

_ACP2E-4610 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c58a8ad0)_

#### 마지막 수준 하위 범주는 관리자 범주 트리에 제품 수를 0으로 표시합니다.

이 문제를 해결하기 전에는 리프 카테고리가 관리자 영역 카테고리 트리에 표시될 때 제품 수를 통합하지 않았습니다. 수정 후에는 각 범주 잎에 대한 제품 수가 올바르게 표시됩니다.

_ACP2E-4689 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/d8a248cd)_

#### 데이터가 커서 magento_reward_history 테이블의 쿼리 성능이 느려집니다.

큰 magento_reward_history 테이블에서 보상 포인트 만료 cron 작업이 느리게 수행되던 문제를 해결했습니다. 이전에는 예약된 포인트 만료에서 사용하는 쿼리에 복합 데이터베이스 인덱스가 없으므로 수백만 개의 레코드가 있는 테이블에서 전체 테이블이 검색되었습니다.
...
#### 품절 번들 제품에 대해 작동하지 않는 가격으로 주문 또는 필터링

&quot;재고 부족 제품 표시&quot; 옵션이 활성화된 경우 이제 제품 목록 페이지에서 재고 부족 번들 제품을 가격별로 올바르게 필터링하고 정렬합니다.

_ACP2E-4475 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/f2fd9628) - [GitHub 코드 기여도](https://github.com/magento/inventory/commit/a33c60f2)_
...
이제 구매요청 목록 페이지의 인쇄 옵션이 올바르게 작동합니다.
이전에는 &quot;인쇄&quot;를 클릭하면 &quot;응용 프로그램을 실행하는 동안 오류가 발생했습니다. 자세한 내용은 예외 로그를 참조하십시오.&quot;
AC-14711
...
#### ProductInterface에서 필연적으로 이스케이프 처리된 문자

이제 시스템이 GraphQL 응답의 이름에 이스케이프 처리되지 않은 `>`을(를) 반환합니다.

_AC-15494 - [GitHub 문제](https://github.com/magento/magento2/issues/40178) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40423)_
...
#### 고객 세그먼트 정렬 순서 불일치로 인해 일부 페이지에서 X-Magento-Vary 쿠키가 항상 다시 생성됩니다

이제 X-Magento-Vary 쿠키는 제품 페이지에서 한 번 설정되며, 이전에는 고객 세그먼트의 일부 구성에서 쿠키가 PDP 로드 중에 여러 번 설정합니다
...
#### ACP2E-2926에 대한 수정 후 고객 세그먼트가 각 체크아웃 요청에서 일치되어 불필요한 처리가 발생합니다

이제 고객 세그먼트 기능에는 성능을 개선하기 위한 캐싱 메커니즘이 포함되어 있습니다.
...
#### Target 규칙 증분 인덱서가 비활성화된 제품을 선택하지 않습니다.

이제 Target 규칙 증분 색인화는 제품이 활성화되거나 비활성화될 때 응답하므로 전체 색인 재지정 없이 관련/업셀/크로스셀 권장 사항이 업데이트되도록 합니다.

_ACP2E-4402_

#### 고객 세분화가 Rest API에서 제대로 작동하지 않음

이제 주문이 REST API를 통해 이루어지면 주문 카운트에 따른 고객 세그먼트가 올바르게 업데이트됩니다. 이전에는 관찰자가 API 요청에서 사용할 수 없는 고객 세션에 의존했기 때문에 세그먼트 멤버십이 API 주문에 대해 업데이트되지 않았습니다.

_ACP2E-4590_

#### REST API를 통해 &#39;범주(하위 항목만)&#39; 작업 조건으로 장바구니 가격 규칙을 만들 수 없음

REST API를 통해 규칙을 검색하거나 업데이트할 때 카테고리(상위 전용) 및 카테고리(하위 전용)와 같은 장바구니 가격 규칙 조건 특성 범위가 노출되지 않은 문제를 수정했습니다.

_ACP2E-4653 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/d8a248cd)_

#### 무료 배송에 대한 장바구니 가격 규칙이 번들 제품의 예상대로 작동하지 않습니다.

무료 배송에 대한 처리 개선

_ACP2E-4720 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/c58a8ad0)_

### 반환

#### 번들 제품을 반환할 때 간단한 옵션 선택 문제

이전에는, 이 수정 사항이 구현되기 전에 반품을 위해 항목을 별도로 배송하는 옵션이 있는 번들 제품의 경우, &quot;항목 선택&quot; 기능이 처음으로 팝업 블록만 열었습니다. 팝업 블록을 선택하거나 닫은 후 &quot;항목 선택&quot; 기능이 호출될 때마다 팝업 블록이 열리지 않았습니다. 그러나 이 수정 사항을 적용한 후 문제가 해결되었으며 이제 기능이 의도한 대로 작동합니다.
...
_AC-16296 - [GitHub 문제](https://github.com/magento/magento2/issues/40411) - [GitHub 코드 기여](https://github.com/magento/magento2/commit/8391dbcc)_

#### 음역 누락 문자

이제 시스템에서 누락된 문자를 올바르게 음역합니다.

_AC-16626 - [GitHub 문제](https://github.com/magento/magento2/issues/40580) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40581)_
...
이제 주문 상태 할당이 예상대로 작동합니다.
이전에는 사용자 지정 주문 상태를 할당할 때 상태 할당을 취소한 후 &quot;처리 중&quot; 상태가 드롭다운에서 사라져 재할당이 불가능했습니다.
AC-15010
...
#### AC-15210이 완전하지 않은 것 같습니다. USPS REST API는 1개월 이내에 필요합니다.

Adobe Commerce의 USPS REST API 통합(AC-15210)이 완료되지 않았습니다. 주요 문제에는 추적 정보 누락, API 유형 전환 시 허용된 메서드 설정 손실, 할당량 한도를 요구하는 과도한 요청 및 &quot;429&quot; 할당량 한도 오류에 대한 보호 부재 등이 있습니다. 할당량을 초과하여 고객에게 배송비가 표시되지 않았습니다.
USPS REST API 통합이 다음과 같이 개선되었습니다.
- 발송된 소포에 대한 적절한 추적 정보를 제공하십시오.
- USPS WEBtools 와 REST API 간을 전환할 때 허용되는 메서드 설정을 유지합니다.
- USPS API에 대한 불필요한 요청을 방지하여 할당량 제한에 도달할 위험을 줄입니다.
- &quot;429&quot; 할당량 제한 오류를 정상적으로 처리하여 고객이 배송 비율을 유지할 수 있도록 합니다.

_AC-16577 - [GitHub 문제](https://github.com/magento/magento2/issues/40394) - [GitHub 코드 기여](https://github.com/magento/magento2/commit/15a9362d)_
...
#### USPS Rest API 배송 방법이 일부 제품의 체크아웃 시 표시되지 않습니다.

이제 체크아웃 중인 제품에 USPS REST API 배송 방법을 사용할 수 있습니다.

_ACP2E-4556 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/aa2e16d1)_
...
스테이징 업데이트를 통한 카탈로그 가격 규칙 변경 사항이 장바구니에 일관되게 반영되지 않던 문제를 수정했습니다. 이 수정 후 예약된 가격 규칙 업데이트가 적용되면 모든 장바구니 섹션(중앙 블록 및 요약 블록)의 제품 가격이 올바르게 업데이트됩니다.

_AC-16530 - [GitHub 코드 기여](https://github.com/magento/magento2/commit/57bad413)_
...
#### 적용된 쿠폰이 적용된 후 바로 사라지는 프로모션 미리 보기 모드의 문제.

수정 전에는 스테이징 미리 보기 모드에서 바우처 코드를 제대로 사용할 수 없었습니다. 이제 수정 후 바우처 코드가 체크아웃 페이지에 제대로 적용됩니다.
...
수정 이전에는 스테이징 업데이트를 저장할 때마다 날짜의 특별 가격이 제대로 저장되지 않았습니다. 이제 수정 후 해당 특별 가격 시작/종료 날짜가 스테이징 업데이트 시작 및 종료 날짜와 상관 관계가 있습니다.

_ACP2E-4433 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/f2fd9628)_
...
이제 setGiftOptionsOnCart GraphQL 돌연변이를 통해 선물 포장을 제거하면 Magento에서 장바구니 세금 합계를 올바르게 업데이트합니다. 이전에는 선물 포장 옵션을 선택한 다음 돌연변이 입력의 &quot;giftWrappingId&quot;를 전달하여 설정이 해제된 경우 견적의 세액이 업데이트되지 않았고 Magento은 선물 포장이 적용되지 않았음에도 선물 포장 세금을 장바구니 합계에 계속 포함했습니다.
...
견적 항목에 대한 세금 계산을 제외한 번들 제품 동적 가격의 전환이 올바르게 작동합니다.

_ACP2E-4411 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/aa2e16d1)_
...
#### [문제] SwiftOtter-SOP-348 테스트 실행에 대해 컴파일된 구성 캐시를 사용하도록 설정합니다.

테스트 실행 중에 compiled_config 캐시를 활성화하여 통합 테스트 성능을 향상시켜 CPU 사용 및 실행 시간을 줄입니다.

_AC-16018 - [GitHub 문제](https://github.com/magento/magento2/issues/40253) - [GitHub 코드 기여](https://github.com/magento/magento2/pull/40239)_

#### [Cloud Native Service] CNS 빌드 실패 - 2.4.9-beta1- 통합
...
#### PR 및 작성기 빌드 간 PHPUnit 구성 파일 불일치
...
#### [단위 테스트] Magento\GiftCardImportExport\Test\Unit\Model\Import\Product\Type\GiftCardTest::testIsRowValid

무작위 실패 단위 테스트에 대한 수정 제공
...
#### [문제] `Magento_Backend`에서 금지된 `@author` 태그를 제거하십시오.
...
#### GUID가 보안 형식으로 저장되지 않습니다.

사용 가능한 설명이 없습니다.

_AC-15809_

#### 잘못된 중요 문제가 있는 호환성 도구 업그레이드
