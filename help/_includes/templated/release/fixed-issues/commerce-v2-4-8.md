...
#### 다른 웹 사이트의 주문을 표시하는 REST API입니다. 
...
#### 고객의 보상 포인트 업데이트를 구독하는 데 사용 가능한 graphQl 없음
...
#### Luma(Rest API) 및 Graphql은 모두 우편번호만 제공되는 경우 세금을 계산하지 않습니다.
...
#### 내 프로필 드롭다운의 프런트 엔드 UI 홈 페이지 버튼이 없습니다.(간헐적으로)
...
#### 비공개 판매가 활성화된 경우 [Cloud]에서 API를 통해 고객을 만들 수 없습니다.
...
#### 공유 선물 레지스트리에 이벤트 날짜가 1일 앞으로 표시됩니다.
...
#### 최대 암호 재설정 요청 수 가 0보다 크게 설정된 경우 예: 3 - 제한을 다시 시작하기 전에, 즉 두 번째 시도부터 &quot;제한 초과 오류 메시지가 전송됩니다.

사용 가능한 설명이 없습니다.

_AC-13767_

#### 최대 암호 재설정 요청 수 가 0( 비활성화됨)으로 설정되어 있어도 &quot;제한 초과 오류 메시지가 두 번째로 전송됩니다.&quot;
...
#### Admin When We try to Reorder submit order 단추를 클릭할 수 없습니다. (간헐적으로)
...
#### 제한된 관리자로 허용된 웹 사이트의 범주 제품에 대한 위치를 변경할 수 없습니다.
...
#### GTM에 사용자 지정 옵션으로 구성 가능한 제품에 대한 dataLayer의 addToCart 이벤트가 누락되었습니다.
...
#### 2.4.8-beta102 패키지 Enterprise Edition이 애플리케이션 예외로 인해 실패했습니다.
...
#### [Cloud] 모바일 및 데스크톱 버전의 가격 표시가 &quot;내 견적&quot;에서 동일하지 않음
...
#### NoDDL 및 범주 권한을 사용할 때 재인덱싱하는 동안 표시되는 제품/범주
...
#### [Cloud] graphql 호출을 통해 만드는 동안 custom_attributes를 설정할 수 없습니다.
...
#### Admin Express 체크아웃 버튼이 비활성화되었습니다.
...
#### Virtual as Child Product로 구성 가능
...
#### 일부 순서 관련 테스트 사례에서 Magento 예외가 throw되었습니다.
...
#### 선물 레지스트리 제품이 제대로 표시되지 않습니다.

사용 가능한 설명이 없습니다.

_AC-13797_

#### 선물 등록 제품이 제대로 표시되지 않습니다.
...
#### 선물 레지스트리에서 주문 문제 Magento 2.4.4 Enterprise/Commerce
...
#### 스토어 보기를 변경할 때 [Cloud] 관련 제품 규칙이 작동하지 않음
...
#### 주소 검색을 사용하도록 설정한 경우 [CLOUD] 체크 아웃에 선택한 청구 주소가 유지되지 않습니다.
...
#### 기프트 카드 제품 | 장바구니 병합이 기프트 카드를 병합하는 중입니다.
...
#### 선물 등록 항목을 구입하면 고객은 레지스트리에 없는 항목을 볼 수 있습니다

선물 레지스트리 업데이트에 선물 레지스트리에 속하지 않는 항목이 더 이상 포함되지 않습니다.

_ACP2E-3495_

#### [클라우드] &quot;모두 제거&quot; 확인 팝업이 확인 없이 장바구니 항목을 제거하는 문제
...
#### 선물 등록 항목을 주문하는 동안 배송 주소가 비어 있음
...
#### YouTube nocookie url이 페이지 빌더에서 작동하지 않음
...
#### 범주 페이지의 콘텐츠 스테이징 미리 보기에 제품 위젯이 표시되지 않습니다
...
#### CMS 페이지 계층으로 인해 URL 재작성 문제가 발생할 수 있음
...
#### 카탈로그 가격 규칙이 다이내믹 블록으로 예약되도록 설정된 경우 표시되지 않음
...
#### 고객 세그먼트 > 조건 > 제품 내역* > &quot;제품을 조회함&quot;이 작동하지 않음
...
#### charset 및 데이터 정렬 변경 사항으로 인해 MariaDB 11.4 버전에서 :upgrade 설정이 실패했습니다.
...
#### DDL 작업을 수행하는 동안 mview cron을 실행할 때 기본 테이블 또는 뷰를 찾을 수 없음 오류 발생
...
#### 2.4.8-beta1 EE 보고서가 magento 예외로 인해 생성되지 않음
...
#### OrderAddress에서 selected_options를 쿼리하는 동안 오류 발생
...
#### 가 업그레이드 시와 완전히 다르게 새로 생성된 경우의 테이블 스키마
...
#### Mozilla Firefox 브라우저에서 관리자 주문을 만든 후 항목이 복제됩니다.
...
#### [Braintree] 환불 온라인 저장 트랜잭션을 transactionid-refund로 저장

사용 가능한 설명이 없습니다.

_BUNDLE-3394_

#### [Braintree] + [클라우드] Braintree(신용 카드) 주문에서 요금을 분할할 수 없습니다.

사용 가능한 설명이 없습니다.

_BUNDLE-3421_

#### [Braintree] [Cloud]Braintree SSL 인증서가 6월 30일까지 만료됨
...
#### 실제 기프트 카드 쿼리의 이메일 prop에 대한 서버 오류
...
#### productInterface의 only_x_left_in_stock 속성은 구성 가능한 제품에 대해 정확하지 않습니다.
...
#### GraphQL 오류: 사용자 지정 가능한 옵션 쿼리에 지원되지 않는 &#39;파일&#39; 유형입니다.
...
#### Product attribute > trademark short form ™은 로 반환됩니다™
...
#### CustomerOrder.items_propriable_for_return 목록은 주문 항목과 일치해야 합니다.
...
#### quantity_return_requested 필드 추가
...
#### 주문 가능 조치는 전체 수량의 모든 품목에 대해 반품 생성된 후 반품을 포함하지 않아야 합니다.
...
#### Guest 사용자 확인 url은 orderRef가 누락되어 주문 상태 페이지로 리디렉션됩니다(GuestRMA의 경우).
...
#### GraphQL이 적용된 쿠폰 충돌 시 업데이트된 장바구니 데이터를 반환하지 않음
...
#### 장바구니 항목에서 선물 포장을 제거하는 데 문제가 있습니다.

updateCartItems 돌연변이를 사용하여 장바구니 항목에서 선물 포장을 제거하는 것이 예상대로 작동하지 않는 문제를 해결했습니다. 이제 선물 포장 적용 및 제거 모두 오류 없이 올바르게 작동합니다.

_LYNX-717_

#### 일치하는 등록 고객 기능이 Boilerplate에서 작동하지 않으며 게스트에 대해 trackViewedProduct 돌연변이를 활성화해야 합니다.
...
#### 장바구니 항목에 대한 선물 포장을 검색하는 중 문제 발생
...
#### 방문자에 대한 [CLOUD]장바구니 가격 규칙 고객 세그먼트가 장바구니에 할인을 적용하지 않음
...
#### 관련 제품 규칙의 &quot;일치시킬 제품&quot; 탭에 &quot;Type&quot; 속성이 누락됨
...
#### [CLOUD] Magento 업그레이드로 인해 쿠폰이 대소문자를 구분하게 되었습니다
...
#### [클라우드] 제한된 관리자는 반환 메뉴와 단추를 볼 수 있습니다.
...
#### 첫 번째 장바구니 규칙이 이미 적용된 경우 두 번째 장바구니 가격 규칙은 적용되지 않습니다.
...
#### UPS REST &quot;sandbox&quot; 및 &quot;prod&quot; 설치 지침 업데이트
...
#### 업데이트를 실행하여 원래 추가한 경우 예약된 업데이트 설정이 저장되지 않음
...
#### 일자 및 일자 사이의 장바구니 가격 규칙이 스테이징 업데이트와 동기화되지 않음
...
#### 제품 특별 가격 스테이징된 컨텐츠는 업데이트할 수 없습니다.
...
#### 통합 테스트가 실패했습니다. Magento\NegotiableQuote\Controller\Quote\DownloadTest::testCompanyManagerDownloadWithNQSubPermission

사용 가능한 설명이 없습니다.

_AC-13716_

#### [데이터베이스 비교] 데이터베이스에 조건 없는 Target 규칙에 대한 레코드가 포함된 경우 치명적인 오류 발생
