...
Magento Open Source 2.4.8 코어 코드에서 221개 문제를 해결했습니다. 이 릴리스에 포함된 해결된 문제의 하위 집합은 아래에 설명되어 있습니다.
...
* _AC-9922_: Google Analytics CSP 오류 https://region1.analytics.google.com
   * _참고 사항 수정_: 이제 Google Analytics이 활성화되면 시스템에서 &#39;https://region1.analytics.google.com&#39;에 대한 연결을 올바르게 허용하여 CSP(콘텐츠 보안 정책) 오류가 발생하지 않습니다. 이전에는 Google Analytics을 활성화하고 EU에서 웹 사이트를 보면 &#39;https://region1.analytics.google.com&#39;에 대한 연결 거부로 인해 CSP 콘솔 오류가 발생합니다.
...
* _AC-9712_: 복잡한 `calc` 식을 사용하는 php 및 nodejs 라이브러리(grunt) 간의 컴파일이 적은 차이
   * _참고 사항 수정_: php와 nodejs 라이브러리 간의 컴파일 감소 차이를 수정합니다(grunt).
...
* _ACP2E-2959_: [Cloud] 가져오기는 디렉터리 변수와 함께 사용할 수 없습니다.
   ...
   * _GitHub 코드 기여_: <https://github.com/magento/magento2/commit/3a7c4d17>
* _AC-11651_: LoggerProxy의 __wakeup 메서드에서 읽기 전용 속성을 수정하려는 Magento
   ...
   * _GitHub 코드 기여_: <https://github.com/magento/magento2/commit/c8f87c25>
* _AC-9630_: 필요한 상태/지역이 있는 새 국가가 추가되면 업그레이드 Magento이 general/region/state_required로 재설정됩니다.
...
* _AC-8358_: [문제] 금지된 `@author` 태그 제거
   * _참고 사항 수정_: 이제 시스템에서 특정 모듈에서 금지된 `@author` 태그를 제거하여 코드 품질 및 일관성을 높여 코딩 표준을 준수합니다. 이전에는 일부 모듈에 이 태그가 있으면 설정된 코딩 표준을 위반했습니다.
   * _GitHub 문제_: <https://github.com/magento/magento2/issues/37264>
   * _GitHub 코드 기여_: <https://github.com/magento/magento2/pull/37014>
* _AC-8360_: [문제] 금지된 `@author` 태그 제거
   * _참고 사항 수정_: 이제 시스템에서 특정 모듈에서 금지된 `@author` 태그를 제거하여 전체 코드 품질을 개선하여 코딩 표준을 준수합니다. 이전에는 일부 모듈에 이 태그가 있으면 설정된 코딩 표준을 위반했습니다.
   * _GitHub 문제_: <https://github.com/magento/magento2/issues/37261>
   * _GitHub 코드 기여_: <https://github.com/magento/magento2/pull/37011>
* _AC-8361_: [문제] 금지된 `@author` 태그 제거
   * _참고 사항 수정_: 이제 시스템에서 특정 모듈에서 금지된 `@author` 태그를 제거하여 보다 깔끔하고 표준화된 코드를 만들어 코딩 표준을 준수합니다. 이전에는 일부 모듈에 이 태그가 있으면 설정된 코딩 표준을 위반했습니다.
   * _GitHub 문제_: <https://github.com/magento/magento2/issues/37260>
   * _GitHub 코드 기여_: <https://github.com/magento/magento2/pull/37010>
* _AC-8400_: [문제] 금지된 `@author` 태그 제거
   * _참고 사항 수정_: 이제 시스템에서 특정 모듈에서 금지된 `@author` 태그를 제거하여 전체 코드 품질을 개선하여 코딩 표준을 준수합니다. 이전에는 일부 모듈에 이 태그가 있으면 설정된 코딩 표준을 위반했습니다.
   * _GitHub 문제_: <https://github.com/magento/magento2/issues/37254>
   * _GitHub 코드 기여_: <https://github.com/magento/magento2/pull/37004>
...
* _AC-8714_: Varnish 7.3 - 기본 범주의 하위 범주 링크/옵션이 상점 첫 번째 홈 페이지에 표시되지 않음
   * _참고 사항 수정_: 이제 Storefront 홈 페이지에 기본 범주 하위 범주가 예상대로 표시됩니다. 이전에는 쇼핑객이 URL로만 하위 범주에 액세스할 수 있었습니다.
   * _GitHub 코드 기여_: <https://github.com/magento/magento2/commit/d1c335aa>, &lt;https://github.com/magento/magento2/ (내부, 병합되지 않음)>
...
* _ACP2E-3002_: [CLOUD]은(는) 개발 문서가 표시하는 대로 관리 UI에서 전자 메일 보내기를 비활성화할 수 없습니다.
   ...
   * _GitHub 코드 기여_: <https://github.com/magento/magento2/commit/c8931218>
* _AC-12462_: 댓글 추가 양식을 사용하여 관리자의 중간 사용자 지정 상태를 변경할 수 없습니다.
   * _참고 사항 수정_: 이제 관리 패널의 &quot;댓글 추가&quot; 양식에서 변경된 경우 시스템이 주문 상태를 &quot;픽업 준비&quot;로 올바르게 업데이트하여 주문 진행 상황을 정확하게 추적할 수 있습니다. 기존에는 &#39;댓글 추가&#39; 양식을 통해 &#39;픽업 준비&#39;로 변경을 시도해도 주문 상태가 &#39;전송됨&#39;으로 유지됐다.
   * _GitHub 문제_: <https://github.com/magento/magento2/issues/38917>
   * _GitHub 코드 기여_: <https://github.com/magento/magento2/pull/38923>
...
* _AC-11798_: [문제] 배송 가격(인쇄 pdf에서 차이 표시)
   * _참고 사항 수정_: 이제 시스템에서 세금 구성 설정에 따라 인쇄된 PDF에 배송료를 올바르게 표시하므로 판매 주문 송장 조회 페이지와 인쇄된 송장 간의 일관성이 보장됩니다. 기존에는 인쇄된 PDF에 표시된 출고가가 세금 구성 설정과 관계없이 세금을 제외한 것이었다.
...
* _AC-10658_: [문제] visual.phtml에서 HTML 구문 오류 수정
   * _참고 사항 수정_: 이제 시스템에서 visual.phtml 파일의 시작 태그를 올바르게 닫아 올바른 HTML 구문을 확인합니다. 이전에는 시작 태그가 제대로 닫히지 않아 HTML 구문 오류가 발생했습니다.
...
* _AC-12128_: [Cloud] Prototype.js 보안 취약점 CVE-2020-27511
   * _참고 사항 수정_: 보안 취약성 CVE-2020-27511을 해결해야 합니다
   * _GitHub 코드 기여_: <https://github.com/magento/magento2/commit/de4dfb8e>
