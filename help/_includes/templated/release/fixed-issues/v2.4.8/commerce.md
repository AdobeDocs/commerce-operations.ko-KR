...
* __구성 가능한 제품에 대한 PLP에 정가가 표시되지 않음__
...
* __번들 제품에 대한 고급 가격 책정 저장 문제__
번들 제품 절약 성능 개선.
  _ACP2E-2630 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/b2286ecf)_
* __[온-프레미스] 색인 재지정 프로세스는 카탈로그 가격 규칙을 만들 때 비효율적입니다__
...
* __CSV 가져오기를 통해 날짜 및 시간 유형 제품 특성을 업데이트하는 중__
...
* __요청에 웹 사이트 ID가 잘못된 경우 적절한 오류 메시지가 없습니다__
...
* __카탈로그 프로모션 규칙이 quantity_and_stock_status 특성과 함께 작동하지 않음__
...
* __REST API를 통해 가격을 업데이트하는 동안 제품 엔터티 updated_at 열 값이 업데이트되지 않음__
...
* __제품 가져오기를 통해 고유하지 않은 값을 설정할 수 있습니다__
...
* __단일 스토어 모드가 활성화된 경우 프론트엔드의 제품에서 스토어 특정 데이터를 사용합니다__
...
* __rest API를 사용하여 범주에서 &quot;기본 정렬 기준&quot;을 설정할 수 없습니다__
...
* __[클라우드] 판매자가 위시리스트 수__와(과) 관련된 문제에 직면했습니다.
...
* __번들 제품을 사용할 때 프런트 엔드의 범주 페이지에 빈 슬롯이 표시됩니다__
...
* __[설명] 번들 제품 시퀀스 테이블 문제__
이제 번들 제품이 삭제되거나 번들 제품 옵션이 삭제되면 번들 제품 시퀀스 테이블(sequence_product_bundle_option, sequence_product_bundle_selection)의 레코드가 제거됩니다.
이전에는 번들 제품 시퀀스 테이블의 레코드가 제거되지 않았습니다.
  _ACP2E-2888_
* 다중 웹 사이트 아키텍처의 __[클라우드] 견적 문제__
...
* __번들 제품을 편집하는 동안 dynamic-rows.js:658 발견되지 않은 TypeError: dataRecord.slice__
...
* __[Cloud] 번들 제품의 가격이 주문 확인에서 잘못됨__
...
* store_id=0 __에 대해서만__[ Cloud] URL 업데이트
...
* __async.operations.all이 실행되고 오류가 발생했습니다.__
...
* __[클라우드] 모바일 문제에서 PDP 이미지만 고정할 수 없음__
...
* __옵션 이름이 0__인 LayeredNavigation에 레이블이 없습니다.
...
* __범주 사용 권한이 메모리 부족 브라우저 문제__
카테고리 권한 UI는 기본 UI 구성 요소 및 페이지 매김을 사용하여 많은 양의 권한을 렌더링할 수 있도록 다시 디자인되었습니다. 이전에는 카테고리 권한이 있으면 카테고리에 할당된 많은 권한으로 브라우저가 충돌합니다.
  _ACP2E-3094_
* __[Cloud] 이미지 파일이 New Relic 오류 로그에 없습니다__
...
* __새 제품 RSS 피드가 캐시로 인해 새 제품으로 업데이트되지 않습니다__
...
* __[클라우드] 제품 미디어 갤러리 GQL 응답이 이미지 위치별로 정렬되지 않음__
...
* __[클라우드] 하위 범주 항목이 관리 백엔드의 위젯 편집에 표시되지 않음__
...
* __[클라우드] 실제 모바일 장치에서 두 손가락 확대/축소 및 이동 문제__
...
* __공유 카탈로그에서 제품 할당을 취소하면 위시리스트 제품이 지워지지 않습니다__
...
* __[클라우드] 잘못된 언어로 스톡 경고 전자 메일 번역__
...
* __비활성화된 범주의 이름이 더 이상 범주 트리에서 회색으로 표시되지 않습니다__
...
* __구성 가능한 제품 편집 양식 로드로 인해 시간 초과 및 메모리 소진 발생__
...
* 견본을 사용할 때 __Fotorama가 비디오를 올바르게 로드하지 않고 URL을 통해 옵션을 미리 선택했습니다__
...
* __[클라우드] 특별 가격이 구성 가능한 제품에 표시되지 않음__
...
* 프로세스가 종료되면 __인덱서 임시 테이블이 정리되지 않습니다__
...
* __[클라우드] 콘텐츠 위젯에서 앵커/비앵커 범주가 반전됨__
...
* __주문 가져오기(배송|대변 메모|송장)컬렉션 - 컬렉션을 로드할 수 없습니다__
...
* __[클라우드]후속 작업: 데이터에 변경 사항이 있는지 확인할 때 데이터 비교가 일치하지 않음__
...
* __제품 GraphQL에서 계층 가격이 잘못된 값을 반환함(Storefront와 비교)__
...
* __[클라우드] B2B: GraphQL을 통한 범주 문제__
수정 후 카테고리 graphql 쿼리는 루트 카테고리에 허용 권한이 없는 경우에도 허용 권한이 있는 카테고리를 반환합니다.
  _ACP2E-3385_

### 카탈로그, 가격 책정, 스테이징 및 미리보기

* 동시에 많은 수의 제품을 업데이트할 때 __[Cloud] 특별 가격 API 끝점이 오류를 반환함__
...
* __제품 편집의 범주 선택 트리가 카탈로그->범주__에 설정된 순서와 다릅니다.
...
* __페이지 > 1__일 때 범주의 정식 URL이 잘못되었습니다.
...
* __제품이 범주 및 검색에 표시되지 않지만 직접 링크가 작동 중입니다__
...
* __[클라우드] 특정 범주 페이지의 탄력적인 검색 오류__
...
* 개체를 만드는 동안 __형식 오류가 발생했습니다. Magento\CatalogSearch\Model\Indexer\Fulltext\Interceptor 예외__
...
* Magento 관리자에 저장한 후 __[CLOUD] 제품 문제가 프론트엔드에 표시되지 않습니다__
...
* __선물 등록 항목을 주문하는 동안 배송 주소가 비어 있음__
이전에는 게스트 사용자 선물 레지스트리 항목의 경우 이메일 기능에서 반환되면 빈 주소가 생성되었으므로 주문하기에 적합하지 않았습니다. 이 수정 사항이 적용되면 선물 레지스트리는 로그인한 사용자/게스트 사용자 및 할당된 주소가 있는지 확인합니다.
  _ACP2E-3195_

### 클라우드

* __[클라우드] PHPSESSID가 각 POST 요청을 변경 중입니다__
...
* 최근에 본 위젯의 가격 표시에 대한 __[문제] 문제__
...
* __[문제] acl.xsd 파일의 오타 및 문법 수정__
...
* __Pagebuilder 배너 이미지가 갤러리에 표시되지 않음__
...
* 2.4.5-p8 __(으)로 업데이트한 후__&quot;영역 코드가 설정되지 않음&quot;
...
* __디자인 구성 페이지에서 테마를 변경할 때 &quot;기본값 사용&quot; 메시지를 볼 수 없음__
...
* __[문제] TinyMCE 플러그인과의 이전 버전과의 호환성을 다시 복원합니다(그 후...__).
...
* 제품 페이지의 WYSIWYG 편집기에서 __[문제] 파일 업로드 문제__
...
* __[On-PREM] 동적 블록 문제__
이제 동적 블록 내에서 위젯이 제대로 렌더링되고 있습니다.
  _ACP2E-2392 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/a12063bd)_
* __YouTube nocookie url이 페이지 빌더에서 작동하지 않음__
이제 페이지 빌더를 사용하여 유효성 검사 규칙의 양식 요소 설정에서 youtube no-cookie url을 사용할 수 있습니다. 이전에는 Youtube no-cookie URL이 페이지 빌더에서 작동하지 않았습니다.
  _ACP2E-2606_
* 뉴스레터 템플릿의 문제로 인해 __[클라우드] 프론트엔드가 로드되지 않음__
...
* __ACP2E-2836: [Cloud] 로그에 예외가 있습니다. InvalidArgumentException: vendor/magento/module-rule/Model/ConditionFactory.php__에 클래스가 없습니다.
...
* __단일 저장소 모드로 전환 - 전역 콘텐츠가 더 이상 표시되지 않음__
...
* __링크 및 기타 유용성 오류를 추가하려고 하면 Page Builder가 이미지를 바꿉니다.__
...
* __0바이트 이미지가 디렉터리에 있을 때 이전 미디어 갤러리에서 이미지를 렌더링하지 못했습니다__
...
* __CMS 블록을 편집할 때 페이지 빌더 오류 발생__
...
* __[클라우드] 장바구니 섹션에 체크 아웃 또는 장바구니 편집 단추가 없습니다.__
...
* __범주 페이지의 콘텐츠 스테이징 미리 보기에 제품 위젯이 표시되지 않습니다__
CMS 블록에 연결된 추가 카테고리의 제품 항목이 데이터베이스에 정확하게 기록되도록 하여 문제를 해결했습니다. 이전에는 카테고리 미리 보기 페이지가 요청되면 빈 결과 세트를 반환했습니다.
  _ACP2E-3113_
* __[클라우드] 이미지 업로드 단추가 작동하지 않음__
...
* __imagecreatetruecolor(): 인수 #2($height)는 0보다 커야 합니다. 특정 이미지를 업로드할 수 없습니다.__
...
* Google Maps API와 충돌하는 __Prototype.js Array.from__
...
* __[클라우드] - CMS 슬라이더가 최신 변경 내용을 반영하지 않음__
...
* __페이지 빌더를 사용하여 특정 순서로 CMS 블록을 삽입할 때 CSM 페이지에 오류가 발생합니다__
...
* __[클라우드] 동적 블록이 제대로 작동하지 않음__
이제 로그아웃한 고객 세그먼트가 지워지므로 게스트 세션이 이전에 로그인한 세그먼트를 상속할 수 없습니다
  _ACP2E-3388_
* __큰 콘텐츠에 대한 Pagebuilder의 템플릿 미리 보기 실패__
...
* __TinyMCE 7에 글꼴 크기가 없는 최신 보안 업데이트__
...
* __관리자가 PX가 아닌 PT의 TinyMCE 7 편집기 글꼴 크기를 명확히 하십시오__
...
* __페이지 빌더의 제품 콘텐츠 형식이 올바른 메시지 없이 축소됩니다__
...
* __[페이지 빌더]결과를 차단하기 위한 제품 목록 추가__
...
* __CMS 페이지 계층 구조로 인해 URL 다시 작성 문제가 발생할 수 있음__
이전에는 웹 사이트가 아닌 루트 페이지에 대한 사용자 지정 영구 URL 재작성의 경우 무한으로 리디렉션하고 페이지를 로드하지 않았습니다. 이 수정 사항이 적용된 후 비 웹 사이트 루트 페이지에 대한 사용자 지정 URL 다시 작성은 예상대로 작동하며 리디렉션 루프가 발생하지 않습니다.
  _ACP2E-2870_

### 컨텐츠, 스테이징 및 미리보기

* __카탈로그 가격 규칙이 동적 블록으로 예약되도록 설정되어 있을 때 표시되지 않습니다.__
이제 시스템에서 예약된 카탈로그 가격 규칙과 연관된 동적 콘텐츠를 제품 세부 정보 페이지에 올바르게 표시합니다. 이전에는 카탈로그 가격 규칙이 예약된 경우 다이내믹 콘텐츠를 로드하지 못했습니다.
  _ACP2E-2979_

### 고객/고객

* __프론트 엔드 - 고객 생성 페이지에서 생년월일을 확인하지 못했습니다.__
...
* __고객 세그먼트 > 조건 > 제품 내역* > &quot;제품을 본&quot; 항목이 작동하지 않음__
이제 조건이 충족되면 시스템에서 고객 세그먼트 아래의 &quot;제품을 본 횟수&quot; 조건에 일치하는 등록된 고객을 올바르게 표시합니다. 기존에는 조건이 충족된 경우에도 매칭 등록 고객 수가 0에 머물렀다.
  _AC-13060_
* 국가 드롭다운을 변경할 때 __지역 텍스트 필드가 재설정되지 않음__
...
* __고객을 삭제하면 로그인하고 삭제된 고객에 대한 Storefront의 모든 브라우저 세션 데이터가 정리되지 않습니다__
...
* __[질문]`app/code/Magento/Translation/etc/di.xml`__에서 사용되지 않은 형식 구성
...
* __V1/customers/password 끝점 질문/문제__
...
* __바니시 구성은 모든 마케팅 매개 변수를 제외하지 않습니다__
...
* __카탈로그 검색 색인 프로세스 오류 색인 지정 프로세스__
...
* __created_at, status 및 grand_total 필터를 고객 주문 쿼리에 추가하고 여러 필터 오류를 수정했습니다__
...
* __관련/업셀/크로스셀 블록 및 가격 인덱싱의 쿼리가 임의로 쇄도합니다__
...
* __예외: 경고: ICU 74.1(PHP Intl)로 업그레이드한 이후 Calendar.php에서 배열 오프셋에 액세스하려고 합니다.__
Commerce은 쇼핑객 또는 판매자가 상점 또는 관리자를 방문할 때마다 exception.log에 `main.CRITICAL: Exception: Warning: Trying to access array offset on value of type null in /vendor/magento/framework/View/Element/Html/Calendar.php on line 114 in /vendor/magento/framework/App/ErrorHandler.php:62` 예외를 더 이상 기록하지 않습니다. [GitHub-38214](https://github.com/magento/magento2/issues/38214)
...
* __[문제] 양식에 이름이`method`__인 요소가 포함되어 있는 경우 고객 데이터 관련 문제를 수정합니다
...
* __[문제] \Magento\Framework\Data\Collection::getItemById__에 대한 PHPDocs 수정
...
* __[문제]`setup:di:compile`__ 동안 올바른 환경 설정만 허용
...
* __Magento이 LoggerProxy의 __절전 모드 해제 메서드__에서 읽기 전용 속성을 수정하려고 합니다.
...
* __[문제] AC-2039 AC-1667 업그레이드 TinyMCE 참조__
...
* __ChangelogBatchWalker가 여러 스레드에서 작동하지 않습니다__
...
* __[문제] XML을 통해 사용자 지정 특성을 현재 링크에 전달__
...
* __일부 구성에 대해 2.4.7에서 기본 제공 FPC 캐시가 손상되었습니다__
...
* __[문제] 개발자와 프로덕션 모드 간의 불일치를 처리하는 예외 수정__
...
* __token_list.phtml에서 &#39;PayPal 계정&#39; 번역 바꾸기__
...
* __이전 버전과의 호환성이 Magento\Catalog\Model\ProductRepository 클래스에서 손실되었습니다__
...
* __[문제] [보기] 링크 및 스크립트 태그에서 추가 공간을 제거함__
...
* __개체 관리자는 Magento\Csp\Model\Mode\Data\ModeConfigured에 사용되지 않습니다__
...
* __제품 재고 및 가격 알림에서 부정확한 문서 차단 댓글__
...
* __[문제] 일반 구성 대신 생성된 데이터에 대해 컴파일된 구성을 사용합니다__
...
* __[문제] e1ccdb에서 제거된 extjs에서 파일에 대한 참조를 제거합니다...__
...
* __[문제] 사소한 정리: sprintf의 잘못된 사용이 수정되었습니다. 여기에 자리 표시자 2개만 필요합니다...__
...
* __[문제] Magento 모듈에서 참조되는 잘못된 클래스를 수정합니다.__
...
* __CLI 명령 `dev:di:info`의 영역을 정의하는 기능__
...
* __[문제] 이미지 양식 요소 템플릿에 isMultipleFiles 속성 추가__
...
* __설치:문자 집합 및 데이터 정렬 변경 사항으로 인해 MariaDB 11.4 버전에서 업그레이드가 실패했습니다__
  _AC-13247_
* __[문제] 캐시를 최소화하기 위해 모든 마케팅 가져오기 매개 변수를 제거__
...
* __[문제] [PHPDOC] 잘못된 phpdoc 수정 Magento\Directory\Model\AllowedCountries::getAllowedCountries()__
...
* __[문제] 더 이상 지원되지 않는 PHP 버전에 대한 일부 코드를 제거합니다.__
...
* __[문제] ImageMagick 어댑터가 php8과 호환되도록 합니다(float에서 int로의 암시적 변환)__
...
* __[문제] [PHPDOC] 잘못된 phpdoc 수정 Magento\Framework\App\Config\ScopeConfigInterface__
...
* __Magento\Framework\Filesystem\Driver\Http은(는) 이유 구에 따라 다릅니다. 확인__
...
* __일정 모드별 업데이트 모드에서 Customer Grid 인덱서가 제대로 작동하지 않습니다__
...
* __[문제] 금지된 `@author` 태그 제거__
이제 시스템에서 특정 모듈에서 금지된 `@author` 태그를 제거하여 보다 깨끗하고 표준화된 코드를 만들어 코딩 표준을 준수합니다. 이전에는 `@author` 태그가 일부 모듈에 있었으며, 이는 설정된 코딩 표준에 맞지 않습니다.
...
* __[문제] `Magento_Customer`(2부)에서 금지된 `@author` 태그를 제거합니다.__
...
* __editorconfig 구문의 공백이 [{composer,auth}.json]__에 대한 규칙을 어깁니다.
...
* __[문제] 특정 설치 cli 명령의 출력에 색상을 더 추가합니다__
...
* __필요한 상태/지역이 있는 새 국가가 추가되면 Magento을 업그레이드하면 general/region/state_required가 재설정됩니다.__
...
* __복잡한 `calc` 식을 사용하는 php 및 nodejs 라이브러리(grunt) 간의 컴파일 감소 차이__
...
* 부분 인덱싱을 실행할 때 __&quot;기본 테이블 또는 뷰를 찾을 수 없음&quot; 오류가 발생합니다.__
...
* __MariaDB를 10.5.1 이상으로 업그레이드한 후 문제 발생__
...
* __데이터의 변경 내용을 확인할 때 데이터 비교에서 형식이 일치하지 않습니다__
...
* DDL 작업을 수행하는 동안 mview cron을 실행할 때 __기본 테이블 또는 뷰를 찾을 수 없음 오류 발생__
이제 mview 업데이트가 백그라운드에서 실행되는 동안 시스템이 데이터베이스 업데이트 작업을 올바르게 처리하므로 &#39;기본 테이블 또는 뷰를 찾을 수 없음&#39; 오류가 발생하지 않습니다. 이전에는 mview 업데이트가 동시에 실행되는 경우 일부 데이터베이스 업데이트 작업으로 인해 &#39;기본 테이블 또는 뷰를 찾을 수 없음&#39; 오류가 발생했을 수 있었습니다.
  _ACP2E-3046_
* __외래 키의 경우 db_schema.xml을 통해 열 길이를 수정할 수 없음__
...
* __주문 레코드를 저장할 때 일부 관계 레코드가 DB에 저장됩니다.__
...
* __[클라우드] 관리자의 경우 콘솔에 JavaScript 오류가 많습니다__
...
* __[클라우드] Magento: 큐 메시지가 삭제되었습니다.__
...
* __캐시 태그에서 해당 캐시 키 항목을 사용할 수 없으므로 캐시 정리가 제대로 작동하지 않습니다__
...
* __MAGENTO_DC_INDEXER__USE_APPLICATION_LOCK 값이 무시됩니다.__
...
* __[문제] GraphQL 스키마에 대한 사용자 지정 스칼라 유형 지원이 도입됨__
...
* magento 예외로 인해 __2.4.8-beta1 EE 보고서가 생성되지 않음__
  _AC-13011_

### 프레임워크, UI 프레임워크

* __구성 값이 잠겨 있어도 덮어쓸 수 있음__
이 수정 이전에는 bin/magento config:set 명령을 통해 디자인 구성을 설정할 수 없었고, 잠긴 값은 표시된 양식을 조작하여 변경할 수 있었습니다. cli에서 —lock-env 또는 —lock-conf로 설정된 고정 잠금 값은 더 이상 업데이트할 수 없습니다.
  _ACP2E-3324 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/55615e61)_

### GraphQL

* __Magento_GraphQl은 헤더 값이 유효성 검사를 통과하지 못하더라도 헤더 처리를 실행합니다__
...
* __실제 기프트 카드 옵션에 올바른 정렬 순서가 없습니다__
...
* 스테이징 업데이트를 만들기/편집/이동/삭제할 때 __[GraphQL] 확인자 캐시가 무효화됨__
...
* __콘텐츠 준비 업데이트 시 빠른 캐시가 지워지지 않음__
...
* __계층화된 네이션을 사용하지 않도록 설정하는 중 - Graphql에서 합계를 제거하지 않음__
...
* 가격 필터 {from:&quot;0&quot;}이(가) 포함된 __GraphQL 제품 호출이 결과를 반환하지 않음__
...
* __각 StoreView의 GraphQL API에 반영되지 않은 고객 반환 특성에 대한 번역__
...
* __[Cloud] 노드 따옴표가 있는 getPurchaseOrder에 대한 GraphQL 호출이 끊어졌습니다__
...
* __[Cloud] &quot;모든 스토어 보기&quot;에서 제품을 사용할 수 없는 경우 구성 가능한 제품이 프로덕션 사이트에 표시되지 않음__
...
* __[Cloud] 동일한 단순 제품이 구성 가능한 여러 제품에 할당된 경우 Products graphql에서 오류 발생__
...
* 다중 사이트 설정에서 사용자 인증 및 사이트 간 토큰 액세스에 대한 __[클라우드] 문제__
...
* __GraphQL 장바구니 항목 수V2 페이지 매김이 제대로 작동하지 않습니다__
...
* customerCart를 가져올 때 __[GRAPHQL] 모델 값을 지정해야 함__
...
* __[GraphQl] 위시리스트 항목은 GraphQl을 통해 표시되지만 상점 앞에는 표시되지 않습니다__
...
* __[GraphQL] 콘텐츠와 주체/링크 간 암호 재설정 이메일 불일치__
...
* __[Cloud] 제품 GraphQL 쿼리가 현재 웹 사이트에 할당되지 않은 관련 제품을 반환합니다.__
...
* __GraphQL 헤더에서 잘못된 저장소 ID를 사용하면 치명적인 메모리 오류가 발생합니다__
...
* 2.4.7 __에서 빈 Graphql 응답에 대한__[ Cloud] 500 응답
...
* __비활성화된 제품이 grpahQL 쿼리의 관련, 상향 판매, 교차 판매 항목에 여전히 표시됨__
...
* __[클라우드]: GraphQl 오류 내부 서버 오류 placeOrder 돌연변이__
...
* __동적 가격이 적용된 번들 제품에 대해 discount_percentage가 계산되지 않습니다__
product.price_details의 discount_percentage에 대한 수정 사항이 동적 가격이 활성화되고 할인 쿠폰이 적용된 번들 제품에 대한 올바른 값을 표시하지 않습니다.
  _LYNX-426_
* __번들 제품 중 하나가 품절되었을 때 번들 제품에 &quot;IN_STOCK&quot;이 표시됩니다__
번들 제품 중 하나가 품절되어도 번들 제품에 여전히 &quot;IN_STOCK&quot;이 표시되던 문제를 해결했습니다.
  _LYNX-485_
* __not_available_message 및 only_x_left_in_stock에 동일한 사용 가능한 재고가 표시되지 않습니다__
not_available_message 및 only_x_left_in_stock에서 일관되지 않은 재고 가용성을 표시하는 문제가 해결되었습니다.
  _LYNX-486_
* __original_row_total 필드가 잘못된 값을 반환함__
사용자 지정 옵션을 선택할 때 잘못된 값을 반환하는 original_row_total 필드의 문제를 해결했습니다
  _LYNX-488_
* __구성에 따라 그룹화된 제품 썸네일이 표시됩니다.     .__
구성 설정에 따라 그룹화된 제품 썸네일이 표시되도록 하는 문제가 해결되었습니다
  _LYNX-503_
* __OrderAddress에서 selected_options를 쿼리하는 동안 오류가 발생했습니다__
주소 GraphQL 응답에서 AttributeSelectedOptions를 custom_attributesV2로 업데이트했습니다.
  _LYNX-510_
* __original_item_price에 할인이 포함되지 않음__
할인을 포함하도록 original_item_price 가 업데이트되었습니다.
  _LYNX-512_
* __사용할 수 없는 메시지에 사용 가능한 재고 수량이 표시되지 않습니다__
AddProductsToCart 돌연변이에 대한 오류 메시지와 오류 코드를 &quot;사용할 수 없음&quot; 메시지 구성과 일치하도록 해결했습니다
  _LYNX-530_
* __&quot;OUT_OF_STOCK&quot; 상태가 다중 선택 옵션이 있는 단순 사용자 지정 옵션 제품에서 반환됩니다__
사용자 지정 옵션이 있는 간단한 제품에 대한 stock_status를 수정하도록 인벤토리 패키지의 StockStatusProvider 해결사를 업데이트했습니다.
  _LYNX-532_
* __오류(GQL): cart.itemsV2.items.product.custom_attributesV2가 서버 오류를 반환합니다__
장바구니 쿼리가 사용자 지정 특성 없이 제품을 추가하여 제품의 사용자 지정 특성을 포함할 때 발생하는 서버 오류를 해결했습니다.
  _LYNX-533_
* __orders/date_of_first_order가 항상 null을 반환함__
orders > date_of_first_order가 항상 null을 반환하는 문제가 해결되었습니다.
  _LYNX-536_
* __고객은 부분적으로 배송된 주문을 취소할 수 없습니다__
고객이 부분적으로 배송된 주문을 취소할 수 없도록 제한하기 위해 유효성 검사가 추가되었습니다.
  _LYNX-544_
* __오류 메시지를 기반으로 주문 취소를 위한 오류 코드__
이제 주문 취소를 위한 오류 코드는 특정 오류 메시지를 기반으로 합니다.
  _LYNX-548_
* __쿠키 관련 속성을 비공개에서 보호됨으로 다시 이동__
Magento\Framework\App\PageCache\Version 클래스 생성자 속성 가시성을 private에서 protected로 되돌립니다.
  _LYNX-581_
* __최대 기본 GraphQL 쿼리 복잡성을 1000으로 증가__
기본 최대 GraphQL 쿼리 복잡도를 300개에서 1000개로 늘렸습니다.
  _LYNX-600_
* __GQL - itemsV2 > 원래 행 총계, 가격 범위 가격은 별도의 가격이 있는 파일 옵션이 있는 다운로드 가능한 제품의 경우 $0.00로 반환됩니다.__
별도의 링크 구매 옵션이 활성화된 다운로드 가능한 제품이 itemsV2 > 원래 행 합계에 대해 $0를 반환하던 문제가 해결되었습니다. 별도의 가격이 있는 파일 옵션이 있는 제품에 대해 가격 범위가 $0.00로 반환되었습니다.
  _LYNX-620_
* __업그레이드할 때와 다른 새 테이블을 만들 때 테이블의 스키마__
새 VARCHAR 열을 기존 테이블에 추가하면 새로 설치한 항목과 업그레이드 간의 스키마 차이로 인해 테스트 오류가 발생하는 문제가 해결되었습니다. 이제 modifyColumn() 메서드가 기본 문자 집합과 데이터 정렬을 설정하여 VARCHAR 열을 올바르게 처리합니다.
  _LYNX-711_
* PHP-8.4 버전용 __GraphQl 호환성__
...
* 원본 장바구니와 대상 장바구니에 동일한 번들 항목이 있는 경우 __MergeCart 돌연변이에 예외가 발생합니다__
&#39;-
...
* __[GraphQL Resolver] Customer Resolver 데이터가 가져오기에서 무효화되지 않음__
...
* __여러 매개 변수를 기준으로 한 GraphQL 제품 목록 정렬이 작동하지 않음__
...
* __GraphQL 쿼리가 total_count 10,000개의 제품만 제한되어 있는 제품 목록__
...
* __Magento\GraphQl\App\GraphQlCustomerMutationsTest.php 통합 테스트 실패__
&#39;-
  _ACP2E-3363 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/a4cf5e62)_

### 가져오기/내보내기

* __사용자 지정 옵션 유형: 파일이 제공된 경우 제품 가져오기 시 문제 발생(만들어진 제품에는 사용자 지정 옵션 가격이 포함되지 않고 제공된 첫 번째 파일 유형 확장명만 표시됨)__
...
* __사용자 지정 옵션을 복제하는 제품에 대한 가져오기 추가/업데이트__
...
* __내보낼 때 고객 &quot;created_at&quot; 날짜가 저장 시간대로 변환되지 않음__
...
* __[클라우드] CSV를 사용하여 데이터 가져오기에서 데이터를 확인하는 동안 오류가 발생했습니다__
...
* __[클라우드] CSV 파일의 잘못된 수량이 오류를 표시하지 않음__
...
* __URL 키가 이미 범주에 속해 있는 경우 제품을 가져올 때 생성된 중복된 URL 키 오류 메시지가 올바르지 않습니다__
...
* __제품 내보내기로 인해 4G 메모리 제한이 있는 경우에도 OOM이 발생합니다__
...
* 특성이 있는 맵을 추가하는 동안 __Google google API 키가 작동하지 않음__
...
* __여러 원본과 손상된 SKU가 있는 주문 항목에 대한 배송을 만들 수 없습니다.__
...
* __[테스트] 매장 앞면에 0개의 인벤토리가 표시된 번들 제품__
번들 제품은 추가 재고를 사용하여 추가 웹 사이트에 표시되지 않습니다.
  _ACP2E-1411_
* __[클라우드] 빈 공백이 있는 제품 목록에 심각한 문제__
...
* __MSI가 활성화된 경우 구성 가능한 이미지 추가가 작동하지 않습니다.__
...
* __정리 M2.4.7-p3에서 번들 제품 + MSI 문제__
...
* __SKU가 검색 가능한 특성으로 설정되지 않은 경우 모든 제품이 [is_out_of_stock] = 1로 색인화됨__
...
* __백엔드 주문 개요 화면: 주문 항목 수준에 미납 주문 수량이 표시되지 않음__
...
* __[문제] 잘못된 저장소 ID가 주문 주소 렌더러에서 사용됨__
...
* __[문제] 인쇄 pdf에서 다른 가격을 표시하는 배송 가격__
...
* __구성 가능한 상위 제품이 삭제된 상태로 순서 바꾸기__
...
* __[문제] 수정 잘못된 \Magento\Sales\Model\Order\Email\Container\Template::$id 속성__
...
* __기존 주문 세부 정보에 전화 번호에 대한 변경 내용을 저장할 수 없습니다__
...
* 부분적으로 배송된 주문을 부분 환불하는 경우 __[Cloud] 주문 상태가 완료로 변경됨__
...
* __[CLOUD]은(는) 개발 문서에 표시된 대로 관리자 UI에서 전자 메일 보내기를 비활성화할 수 없습니다__
...
* __[클라우드] 기본 청구 주소만 설정되지 않은 경우 한 스토어에서 관리자 주문을 만들 수 없습니다.__
...
* __관리자는 결제 방법이 없어도 주문할 수 있습니다__
...
* __Mozilla Firefox 브라우저에서 관리자 주문을 만든 후 항목이 중복됨__
&quot;SKU별 제품 추가&quot;를 사용하여 추가된 제품은 관리자에서 주문을 생성할 때 Firefox에서 더 이상 복제되지 않습니다.
  _ACP2E-3518_
...
* __[CLOUD] 교차 거래를 활성화하고 쿠폰 할인을 적용할 때 RESTFUL 주문 API의 잘못된 base_row_total__
...
* __[Braintree] 트랜잭션을 transactionid-refund로 저장하기 위해 온라인 환불__
  _BUNDLE-3394_
* __[Braintree] + [클라우드] Braintree(신용 카드) 주문에서 요금을 분할할 수 없음__
  _BUNDLE-3421_
* __[Braintree] [Cloud]Braintree SSL 인증서가 6월 30일까지 만료__
  _BUNDLE-3422_
* __private_content_version 쿠키가 GQL 쿼리에서 반환됨__
세션 쿠키가 비활성화된 경우에도 GraphQL 쿼리에서 private_content_version 쿠키가 반환되던 문제를 수정했습니다. 예상대로 세션이 비활성화될 때 쿠키는 더 이상 GraphQL 응답에 포함되지 않습니다.
  _LYNX-339_
* __실제 기프트 카드 쿼리의 전자 메일 Prop에 대한 서버 오류__
실제 기프트 카드의 sender_email 및 recipient_email 쿼리에서 서버 오류가 발생하는 문제를 해결했습니다. 이러한 prop은 이제 가상 기프트 카드에 대해 올바르게 반환되며 쿼리 동작은 일관됩니다.
  _LYNX-366_
* CartItemInterface의 __is_available 특성이 구성 가능한 제품에 대해 항상 false를 반환함__
CartItemInterface의 is_available 속성이 재고 구성 가능한 제품에 대해 항상 false를 반환하는 문제가 해결되었습니다. 이제 해당되는 경우 가용성이 true로 올바르게 반영됩니다.
  _LYNX-380_
* 판매 가능한 재고가 제품 수량보다 낮은 경우에도 CartItemInterface의 __is_available 특성이 true를 반환합니다.1&rbrace;
장바구니 항목 수량이 판매 가능한 재고를 초과하는 경우에도 CartItemInterface의 is_available 속성이 true를 잘못 반환하던 문제가 해결되었습니다.__
  _LYNX-382_
* ProductInterface의 __only_x_left_in_stock 특성이 구성 가능한 제품에 대해 정확하지 않습니다__
ProductInterface의 only_x_left_in_stock 속성이 장바구니에서 구성 가능한 제품 변형에 대한 사용 가능한 재고를 정확하게 반영하지 않던 문제를 수정했습니다. 이제 only_x_left_in_stock 값은 실제 변형 재고 수준에 올바르게 대응하므로 정확한 재고 데이터가 장바구니 GQL 쿼리에 반환됩니다.
  _LYNX-395_
* __간단한 제품이 그룹화된 제품 내의 장바구니에 추가되면 자리 표시자 썸네일이 반환됩니다__
...
이렇게 하면 스토어 설정에 따라 그룹화된 제품에 대해 일관된 썸네일 동작이 보장됩니다.
  _LYNX-399_
* __고객의 사용자 지정 옵션 특성이 정수 값으로 작동하지 않음__
반환된 값이 정수일 때 고객의 사용자 지정 옵션 속성이 작동하지 않던 문제를 수정했습니다. 이제 사용자 지정 옵션이 예상대로 정수 값을 올바르게 처리하고 반환합니다.
  _LYNX-400_
* __동적 가격이 적용된 번들 제품에 대한 priceDetails를 가져오는 동안 내부 서버 오류가 발생했습니다.__
GraphQL을 통해 동적 가격책정이 포함된 번들 제품에 대한 price_details를 쿼리하면 내부 서버 오류가 발생하는 문제가 해결되었습니다. 이러한 향상된 기능을 통해 동적 가격책정으로 구성된 번들 제품으로 작업할 때 장바구니 쿼리가 안정적이 됩니다.
  _LYNX-402_
* __only_x_left_in_stock은 구성 가능한 제품에 대해 항상 0을 반환합니다__
...
* 이렇게 하면 장바구니 및 제품 페이지에서 구성 가능한 제품 변형에 대해 재고 수준이 올바르게 표시됩니다.
  _LYNX-403_
* __GraphQL 오류: 사용자 지정 가능한 옵션 쿼리에 지원되지 않는 &#39;파일&#39; 형식입니다__
GraphQL이 장바구니 항목에 &#39;파일&#39; 유형의 사용자 지정 가능한 옵션에 대해 오류를 반환한 문제가 해결되었습니다. 이제 쿼리가 파일 기반 옵션을 포함하여 사용자 지정 가능한 모든 옵션 유형에 대한 세부 정보를 오류 없이 올바르게 반환합니다.
  _LYNX-405_
* __GraphQL 쿼리가 사용자 지정 가능한 제품에 대해 올바른 계산된 정가를 반환하지 않음__
GraphQL에서 사용자 지정 가능한 제품에 대해 올바르게 계산된 정가를 반환하지 않던 문제를 수정했습니다. 이제 쿼리는 기본 가격과 추가 사용자 지정 비용을 모두 반영하여 가격 속성에 사용자 지정 가능한 값(예: $125)이 적용된 계산된 일반 가격을 올바르게 포함합니다.
  _LYNX-411_
* __EstimatedTotals를 통한 AppliedTaxes가 업데이트된 돌연변이로 지속됨__
지역 또는 우편 번호를 업데이트한 후에도 장바구니에서 적용된 세금이 지속되는 EstimatedTotals 돌연변이 문제를 해결했습니다. 돌연변이는 이제 지역 및 우편 번호 값 사이를 변경할 때 적용된 세금을 올바르게 업데이트하여 현재 장바구니 데이터를 기반으로 올바른 세금 규칙만 적용되도록 합니다.
  _LYNX-412_
* 판매 가능한 재고가 제품 수량보다 낮은 경우에도 CartItemInterface의 __is_available 특성이 true를 반환합니다.1&rbrace;
판매 가능한 재고가 요청된 제품 수량보다 낮은 경우에도 CartItemInterface의 is_available 속성이 true를 잘못 반환하던 문제가 해결되었습니다.__ 이제 is_available 필드는 제품의 수량이 사용 가능한 재고를 초과할 때 올바르게 false를 반환합니다.
  _LYNX-420_
* __장바구니에 배송용 쿠폰을 추가할 수 없음 할인__
배송 전용 할인에 쿠폰을 장바구니에 적용할 수 없는 문제를 해결했습니다. 이제 제품 조건 없이 판매 규칙을 사용할 때 쿠폰을 배송 금액에 올바르게 적용하여 예상 할인이 배송 비용에 적용되도록 합니다.
  _LYNX-421_
* __12개의 소수 자릿수와 잘못된 값이 포함된 제품 정가__
여러 세율이 적용되었을 때 product.price_range.maximum_price 및 minimum_price GraphQL 경로의 regular_price 값이 카탈로그 가격과 일치하지 않던 문제를 수정했습니다. 이제 regular_price는 모든 세금 구성에서 카탈로그 가격을 일관되게 반영하므로 장바구니 요약에서 정확한 단가 책정, 총 행 비용 계산 및 할인 확인을 보장할 수 있습니다.
  _LYNX-425_
* __재고 부족 번들 제품을 포함하는 장바구니에 있는 GraphQL 서버 오류__
...
GraphQL 쿼리에서 only_x_left_in_stock 필드가 있는 번들 제품이 포함된 장바구니를 가져오는 경우 내부 서버 오류가 발생하는 문제를 해결했습니다. 이제 GraphQL은 오류 없이 only_x_left_in_stock 필드에 대해 float 또는 null을 올바르게 반환합니다.
  _LYNX-447_
* 장바구니에서 구성 가능한 제품이 부족한 다른 제품을 제거할 때 __GraphQL 오류 발생__
장바구니에 재고량이 부족한 구성 가능한 제품이 포함되어 있는 경우, 장바구니에서 재고 내 제품을 제거하려고 하면 &#39;요청된 수량을 사용할 수 없음&#39; GraphQL 오류가 발생하는 문제를 해결했습니다. 이제 제거 가 오류를 트리거하지 않고 예상대로 작동합니다.
  _LYNX-464_
* __대/소문자를 구분하는 SKU 돌연변이로 인해 제품을 추가할 수 없음__
대/소문자가 다른 SKU를 사용할 때 addProductsToCart 돌연변이가 &#39;PRODUCT_NOT_FOUND&#39; 오류를 반환하는 문제를 해결했습니다. 돌연변이는 이제 SKU를 대소문자 구분 없이 처리하여 카탈로그 서비스 쿼리 및 PDP 비헤이비어와 일관성을 보장합니다.
  _LYNX-469_
* __제품 속성 > 상표 약식 &trade;가 &trade;로 반환됩니다__
GraphQL API의 제품 이름에 대한 문자 인코딩 문제가 해결되었습니다.
  _LYNX-603_
* __updateCustomerEmail 돌연변이 문제__
필수 사용자 지정 특성(계정 생성 후 추가됨)이 없는 고객이 이메일을 업데이트할 수 없는 updateCustomerEmail 돌연변이 문제를 해결했습니다.
  _LYNX-619_
* __pickup_location_code를 사용할 때 돌연변이 setShippingAddressesOnCart에서 오류가 발생합니다__
customer_address_id 또는 주소를 지정하지 않고 pickup_location_code를 사용할 때 setShippingAddressesOnCart 돌연변이가 오류를 반환하는 문제가 해결되었습니다. 돌연변이는 이제 pickup_location_code만으로 배송 주소를 설정할 수 있습니다.
  _LYNX-626_
* __CustomerOrder.items_propriable_for_return 목록은 주문 항목과 일치해야 함__
주문에서 반품 자격과의 불일치 문제 해결:
...
3. 이제 CustomerOrder.items_propriable_for_return에는 아직 반환 프로세스에 있지 않은 항목만 포함됩니다.
   _LYNX-627_
* __quantity_return_requested 필드 추가__
OrderItemInterface에 quantity_return_requested 필드를 추가하여 반품이 제출된 품목의 수량을 식별할 수 있습니다. 이렇게 하면 기존 quantity_returned 필드와 함께 반환 추적이 향상됩니다.
  _LYNX-628_
* __사용 가능한 주문 작업에는 전체 수량의 모든 항목에 대해 반품을 만든 후 RETURN이 포함되지 않아야 합니다__
모든 항목에 대해 전체 반환이 생성된 후 GraphQL customer.orders 쿼리의 available_actions 필드에 RETURN이 잘못 포함되는 문제가 해결되었습니다. 이제 반환 프로세스가 완료되면 RETURN 작업이 제대로 제거됩니다.
  _LYNX-634_
* __Storefront 호환성 - 접두사가 있는 테이블 이름을 가져오도록 논리를 업데이트하고 기타 사소한 개선 사항__
SCP 변경 사항과 관련하여 접두사로 테이블 이름을 검색하는 논리를 업데이트했습니다.
  _LYNX-637_
* setBillingAddressOnCart GQL의 same_as_shipping 필드를 사용할 때 __주소록에 저장이 작동하지 않습니다__
...
번들 제품 중 하나가 품절되어도 번들 제품의 product.stock_status에 여전히 &quot;IN_STOCK&quot;이 표시되던 문제를 해결했습니다.
  _LYNX-681_
* __삭제된 사용자 지정 특성의 값이 고객에 대해 존재하는 경우 고객 쿼리가 내부 서버 오류를 반환합니다.1&rbrace;
삭제된 사용자 지정 특성에 저장된 값이 있을 때 고객 쿼리가 내부 서버 오류를 반환하던 문제를 수정했습니다.__ 이제 존재하지 않는 속성이 요청되면 적절한 오류 메시지가 반환됩니다. 고객 사용자 지정 특성을 삭제하면 필요한 캐시가 무효화됩니다.
  _LYNX-686_
* __반환 및 취소 확인 링크에 대한 작업 매개 변수__
반환 및 취소 확인 이메일 관련 링크에 대한 작업 매개 변수가 추가됨
  _LYNX-687_
* __Guest 사용자 확인 url은 orderRef(GuestRMA의 경우)가 없으므로 주문 상태 페이지로 리디렉션됩니다.__
게스트 RMA 확인 이메일의 링크에 orderRef 매개 변수를 추가했습니다.
  _LYNX-688_
* __Guest 사용자 확인 url이 orderRef가 없으므로 주문 상태 페이지로 리디렉션됩니다.__
게스트 주문 취소 확인 이메일의 링크에 orderRef 매개 변수를 추가했습니다.
  _LYNX-689_
* __RMA를 사용하지 않도록 설정할 때 고객 쿼리와 관련된 문제__
RMA가 전역적으로 비활성화된 경우에도 이전에 생성된 반품에 액세스할 수 있도록 GraphQL 논리를 업데이트했습니다. 고객이 이전 수익을 계속 볼 수 있도록 상점 UX를 개선하기 위해 오류 메시지가 제거되었습니다.
  _LYNX-690_
* __적용된 쿠폰이 충돌할 때 GraphQL에서 업데이트된 장바구니 데이터를 반환하지 않음__
우선순위가 더 높은 충돌하는 쿠폰을 적용하면 업데이트된 장바구니 데이터를 반환하지 않고 오류 메시지가 표시되는 문제가 해결되었습니다. 이제 새로운 쿠폰이 기존 쿠폰을 무효화하면 돌연변이는 유효한 쿠폰이 적용된 장바구니를 올바르게 반환합니다.
  _LYNX-696_
* __placeOrder GQL에서 null을 허용하지 않는 필드 &quot;TaxItem.title&quot;에 대해 null을 반환할 수 없음__
...
전역으로 비활성화할 때 제품 수준에서 선물 메시지가 적용되지 않던 문제를 수정했습니다. 이제 특정 제품에 대해 선물 메시지를 사용할 수 있는 경우 updateCartItems 돌연변이를 사용하여 성공적으로 추가할 수 있으며 올바르게 저장 및 반영됩니다.
  _LYNX-714_
* __장바구니 항목에서 선물 포장을 제거하는 동안 문제 발생__
updateCartItems 돌연변이를 사용하여 장바구니 항목에서 선물 포장을 제거하는 것이 예상대로 작동하지 않는 문제를 해결했습니다. 이제 선물 포장 적용 및 제거 모두 오류 없이 올바르게 작동합니다.
  _LYNX-717_
* __일치하는 등록 고객 기능이 Boilerplate에서 작동하지 않으며 게스트에 대해 trackViewedProduct 돌연변이를 활성화해야 합니다.__
고객 및 게스트에 대한 제품 보기 이벤트를 추적하는 노출된 trackViewedProduct 변형
  _LYNX-751_
* 활성 장바구니 규칙이 적용되지 않는 경우 __cart.rules 쿼리가 빈 배열 대신 오류를 반환합니다__
활성 장바구니 규칙이 적용되지 않을 때 오류 대신 빈 배열을 반환하도록 cart.rules 쿼리를 수정했습니다.
  _LYNX-757_
* __장바구니 항목에 대한 선물 포장을 검색하는 중 문제__
전체적으로 비활성화되었지만 제품 수준에서 활성화된 경우 장바구니 항목에 대한 선물 포장 옵션을 반환하도록 검색 논리를 업데이트했습니다
  _LYNX-758_
* __adobe-commerce/storefront-compatibility 패키지가 설치될 때 OPTIONS 메서드를 사용한 GraphQL 호출이 500개의 응답 코드를 반환합니다__
...
* __[문제] visual.phtml에서 HTML 구문 오류 수정__
...
* __[문제] bin/magento maintenance:status 명령에서 &quot;active&quot;가 &quot;enabled&quot;로 변경되었습니다__
...
* __범주 트리에서 탐색하면 Redis에 오류가 발생합니다. &quot;Redis 세션 초과 동시 연결&quot;__
...
* __dev/css/use_css_critical_path와 결합된 CSP 문제__
...
* __가상 형식을 사용하여 플러그 인을 구성하면 `setup:di:compile` 명령에 인터셉터 메서드를 올바르게 생성할 수 없습니다__
...
* __데이터 수집기에서 파일을 다운로드할 수 없음__
백업을 다운로드하면 파일을 다운로드하는 대신 빈 페이지가 더 이상 표시되지 않습니다.
  _ACP2E-3441_
* __Adobe Commerce 2.4.7-p3 단위 테스트 실패__
릴리스 노트는 필요하지 않습니다.
  _ACP2E-3631 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/982b1c42)_

### 결제/결제 방법, 주문

* __나중에 사용하기 위해 저장된 Papal Payflow Credit Card 세부 정보가 저장된 결제 방법 페이지에 표시되지 않습니다__
...
* __신용 카드(Payflow Link) 결제가 작동하지 않음__
...
* __트랜잭션 보기 화면에서 가져오기 단추를 클릭할 때마다 페이플로우가 새 트랜잭션을 만듭니다__
...
* __캐나다 paypal 판매자 계정에 대한 PDP에 나중에 메시지가 표시되지 않음__
...
* __[문제] 코드 정리 및 새로운 중요한 헤드 블록 추가, 자산 앞으로 중요한 css 이동__
...
* __테마 컴파일이 mysql 호스트에 포트 정보가 포함되어 있으면 중단됩니다__
...
* __Magento CLI에서 Symfony의 CommandLoaderInterface 지원__
...
* __장바구니 규칙에서 제품 특성을 로드할 때 성능 문제 발생__
...
* __판매 규칙 인덱서 실행이 중지됨__
결합된 필터 그룹이 많더라도 시스템은 이제 판매 규칙 인덱서를 성공적으로 완료하여 장바구니 규칙 조건이 예상대로 장바구니에 적용되도록 합니다. 이전에는 결합된 필터 그룹이 많으면 판매 규칙 인덱서가 완료되지 않아 오류 메시지가 표시되고 장바구니 규칙 조건을 적용할 수 없었습니다.
  _ACP2E-2617_

### 가격 책정

* __Magento2.4.6-p4 주문 API 단순 항목에 가격이 없습니다__
...
* __ProductRepository 함수 GetById가 올바른 캐시 키를 만들지 않습니다__
...
* __[문제] [MTF] 추가 AdminClickAddOptionForBundleItemsActionGroup__
...
* __[문제] Magento >= 2.4.7__에서 끊어진 번들 및 다운로드 가능한 제품 페이지 레이아웃을 수정했습니다.
...
* __AlertProcessor - 인수 #2($storeId)는 int 형식이어야 합니다. 문자열을 지정하십시오.__
...
* __[Cloud] addFilterToMap 함수가 특정 열에 대해 작동하지 않습니다__
...
* 범주의 __[클라우드] 제품 - 제품 추가 - 할당 - 모두 선택__
이제 사용자는 토글을 사용하여 제품을 선택하거나 선택 취소할 수 있습니다.
  _ACP2E-3471_

### 프로모션

* 초대장에서 계정을 만들 때 __고객 특성이 표시되지 않음__
초대를 통해 계정을 만드는 동안 고객 특성을 사용할 수 있습니다.
  _ACP2E-2602 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/39d54c2d)_
* __쿠폰당 사용 한도가 설정된 쿠폰 코드가 결제에 대해 릴리스되지 않고 주문 취소가 실패했습니다.__
...
* __[Cloud] 리인덱싱 카탈로그 규칙 제품 인덱서가 SQLSTATE를 throw합니다[HY000]: 일반 오류: 2006 MySQL 서버가 제거되었습니다.__
...
* __[CLOUD]방문자에 대한 장바구니 가격 규칙 고객 세그먼트가 장바구니에 할인을 적용하지 않음__
이제 시스템에서 규칙이 쿠폰을 사용하지 않더라도 방문자 고객 세그먼트에 대해 장바구니 가격 규칙을 올바르게 적용하여 적절한 할인이 장바구니에 적용되도록 합니다. 이전에는 장바구니 가격 규칙이 쿠폰을 사용하지 않는 한 방문자 고객 세그먼트에 대한 장바구니에 할인이 적용되지 않았습니다.
  _ACP2E-2926_
* __관련 제품 규칙의 &quot;일치하는 제품&quot; 탭에 &quot;Type&quot; 특성이 없음__
이제 &quot;Type&quot; 속성을 &quot;Related Product Rules&quot; 모듈의 &quot;Products to Match&quot; 탭에서 필터 옵션으로 사용할 수 있으므로 규칙을 보다 정확하게 정의할 수 있습니다. 이전에는 이 속성이 &quot;일치시킬 제품&quot; 탭에서 누락되어 정확한 일치 기준을 만드는 기능이 제한되었습니다.
  _ACP2E-3024_
* __할인 수량 단계(구매 X) 특성이 있는 판매 규칙으로 인해 다른 규칙이 적용되지 않습니다.__
...
* __장바구니 가격 규칙의 성능 문제 - 고급 판매 규칙 모듈__
AdvancedSalesRule 필터에 대한 누락된 DB 인덱스 추가
  _ACP2E-3331_
* __고정 금액 할인과 &quot;최대 수량 할인이 적용되는&quot; 판매 규칙 문제__
...
* __[CLOUD] Magento 업그레이드로 인해 쿠폰이 대/소문자를 구분하게 됨__
수정하기 전에 쿠폰 코드를 대문자 또는 소문자를 고려하여 구성된 것과 정확히 동일하게 입력해야 했습니다. 이제 대문자 또는 소문자 코드 구성에 관계없이 백엔드에서 쿠폰의 유효성이 검사됩니다.
  _ACP2E-3342_
* __장바구니 규칙 &quot;전체 장바구니에 대한 고정 금액 할인&quot;  액션이 할인을 잘못 적용합니다__
...
* __usage_limit 및 uses_per_customer가 salesrule_coupon 테이블에서 업데이트되지 않음__
...
* __장바구니 가격 규칙이 &quot;다음 이상&quot; 조건을 사용할 때 상위 범주를 고려하지 않습니다.__
...
* __[클라우드] 배송 계산에서 장바구니 규칙을 고려하지 않습니다__
...
* __장바구니 규칙 sku 조건이 송장에 대해 실패했습니다.__
...
* __여러 장바구니 가격 규칙이 할인/특별 가격 제품과 동시에 적용되는 경우 잘못된 할인 값__
...
* __[클라우드] 제한된 관리자는 반환 메뉴와 단추를 볼 수 있습니다__
이제 제한된 관리자 사용자는 RMA 관련 컨트롤(메뉴 및 단추)에 액세스할 수 없습니다.
이전에는 제한된 관리자 사용자가 반환 메뉴와 단추를 볼 수 있었습니다.
  _ACP2E-3330_
* __화면을 새로 고칠 때 반환 화면이 엉망입니다__
사용자는 화면 왜곡 없이 페이지를 새로 고칠 수 있습니다.
  _ACP2E-3443_

### SEO

* __악센트가 있는 URL 다시 쓰기를 추가하면 무한 로드가 발생합니다__
...
* __세 번째 수준 범주에 대한 다중 저장소 잘못된 범주 URL 다시 작성__
...
* __제품 이름 필드의 더블바이트 문자(특수 문자)가 백엔드의 제품 생성을 차단합니다__
...
* __한 저장소 그룹에 여러 저장소가 있는 잘못된 url_rewrite 항목을 만들었습니다__
...
* __첫 번째 장바구니 규칙이 이미 적용된 경우 두 번째 장바구니 가격 규칙이 적용되지 않습니다__
  _AC-13751_

### 검색

* __검색어를 입력하고 다시 시도하세요. storefront 2.4.8-beta1__의 고급 검색 페이지에 오류가 발생했습니다.
...
* __magento/module-open-search는 존재하지 않는 opensearch-php 분기에 따라 다릅니다__
  _AC-13721 - [GitHub 코드 기여](https://github.com/magento/magento2/commit/05dc0bbf)_
* __search_query 테이블의 크기가 클 경우 로드 시간 프런트 엔드에 큰 영향을 미칩니다__
...
* __[문제] HTML으로 재해석된 js.js DOM 텍스트 업데이트__
...
* __ReCaptcha V2가 독일어 체크아웃 시 잘못 표시됨__
...
* __UPS REST &quot;선적은 측정 단위로 KGS/IN 또는 LBS/CM 또는 OZS/CM을 가질 수 없습니다.&quot;__
...
* __devdoc의 UPS REST &quot;sandbox&quot; 및 &quot;prod&quot; 설치 지침 업데이트__
  _AC-12938_
* __[문제] 고객 주소에 대한 변수의 맞춤법 수정__
...
* Jenkins 환경에 자격 증명이 추가되지 않아 __MFTF 테스트 AdminCreatingShippingLabelTest가 실패했습니다.__
mtf 테스트 수정
  _ACP2E-2765 - [GitHub 코드 기여도](https://github.com/magento/magento2/commit/ea79f7dd)_
* __FedEx Track API가 REST 자격 증명으로 작동하지 않음__
...
* __[클라우드] FedEx 협상 요금이 REST에서 반환되지 않음__
...
* __업데이트를 실행하여 원래 추가한 경우 예약된 업데이트 설정이 저장되지 않음__
이제 현재 실행 중인 업데이트에서 제품 속성 값이 수정되면 후속 예약된 업데이트에서 해당 값이 올바르게 지워집니다. 이전에는 제품 속성이 실행 중인 예약된 업데이트에 의해 수정되면 새 예약된 업데이트를 만들 때 이러한 속성 값을 지울 수 없어 사용자가 만든 후 다시 편집해야 했습니다.
  _ACP2E-2901_
* __날짜 및 날짜까지의 장바구니 가격 규칙이 스테이징 업데이트와 동기화되지 않음__
날짜는 장바구니 가격 규칙 스테이징의 업데이트에 따라 저장됩니다.
  _ACP2E-2999_
* __스테이징 미리 보기의 JS 오류__
이제 Developer 툴에서 Js 구문 오류 없이 form-mini-stub.js 파일을 성공적으로 로드합니다.
  _ACP2E-3104_
* __제품 특별 가격 스테이징된 콘텐츠를 업데이트할 수 없음__
이제 가격 업데이트 캠페인이 시작된 후 이 캠페인의 종료 날짜를 편집할 수 있으므로 사용자가 필요한 조정을 할 수 있습니다. 이전에는 활성 캠페인의 종료 날짜를 업데이트하려고 할 때 오류가 발생하여 사용자가 변경할 수 없었습니다.
  _ACP2E-3162_
* __고유한 사용자 지정 범주 특성을 사용할 때 예약된 업데이트를 업데이트할 수 없음__
...
* __[문제] 유지 관리 허용 목록에서 CIDR 범위 사용 허용__
...
* __[문제] 기능/php8.1 생성자 속성 승격 wee graph ql__
...
* __MAGETWO-95118: 세션이 만료된 후 영구 장바구니로 동작 확인__
  _AC-13478 - [GitHub 코드 기여](https://github.com/magento/magento2/commit/7d5e3906)_
* __통합 테스트가 실패했습니다. Magento\NegotiableQuote\Controller\Quote\DownloadTest::testCompanyManagerDownloadWithNQSubPermission__
  _AC-13716_
* __[데이터베이스 비교] 데이터베이스에 조건 없는 Target 규칙에 대한 레코드가 포함된 경우 치명적인 오류__
이전에는 데이터베이스에 조건 없이 대상 규칙에 대한 레코드가 포함된 경우 치명적인 오류가 발생했지만 수정 데이터베이스 비교 도구가 치명적인 오류 없이 성공적으로 통과했습니다.
  _AC-13722_
* __3d 파티 확장에서 사용할 수 있도록 정적 테스트를 수정__
  _AC-13848 - [GitHub 코드 기여](https://github.com/magento/magento2/commit/9e383b4d)_
* 실행 중 또는 로그에 __[내부] 고정장치 적용 오류가 표시되지 않습니다__
&#39;-
...
* __Prototype.js 보안 취약점 수정 CVE-2020-27511__
...
* __Grunt Less가 sourcemap에 pub/ 접두사를 사용함__
...
* __[문제] js 콘솔의 날짜 형식 개선: 12시간에서 24시간으로 전환...__
...
* __[문제] 개발자 모드에서 더 적은 수의 파일에 대해 sourceMap 생성 추가__
...
* __최소 재고 임계값으로 &quot;재고 부족&quot; 정렬에서 일관되지 않은 동작__
...
* __require.js 로드 문제에 대한 오류 보고 개선 제안__
...
* __PHP 8.4 사용 중단 오류로 인해 2.4-develop에서 빌드 오류가 발생했습니다__
  _AC-14004 - [GitHub 코드 기여](https://github.com/magento/magento2/commit/1da9ba6f)_
* __[문제] 프론트엔드에 백엔드 블록 컨텍스트를 로드하지 않음__
...
* __Recaptcha가 활성화된 경우 기프트 카드 잔액을 확인할 때 예외 발생__
...
* __[클라우드] AC 2.4.4-p8의 Effect-drop.js에 혼동 라이브러리 코드가 있습니다.__
