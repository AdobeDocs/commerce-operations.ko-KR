...
* _필드: 상거래 소프트웨어_
* _관련 용어: area, commerce admin_

### 영역

_명사_

영역은 Magento 적용 범위에 대한 추상적인 용어입니다.
...
Adobe Commerce 또는 Magento Open Source 사용자는 사용자 지정 특성을 만들어 기본 특성 집합 또는 사용자 지정 특성 집합에 추가할 수 있습니다.
Admin을 통해 또는 프로그래밍 방식으로 이러한 특성을 만듭니다.
예: 색상, 크기, 무게, 가격, 나이, 성별 등.

사용자 지정 속성은 EAV(Entity-Attribute-Value) 속성 유형입니다.

Google 쇼핑 광고 채널 및 Amazon Sales Channel과 같은 통합의 경우, 상거래 속성을 서드파티 속성에 매핑하여 제품, 디스플레이 광고를 올바르게 표시 및 판매합니다.
...
[동적 블록](https://experienceleague.adobe.com/docs/commerce-admin/content-design/elements/dynamic-blocks/dynamic-blocks.html) 가격 규칙과 같은 논리를 기반으로 콘텐츠를 제공합니다.

Page Builder가 의 상호 작용 및 생성 시 확장됨 [블록](https://experienceleague.adobe.com/docs/commerce-admin/page-builder/add-content/block.html) 및 [동적 블록](https://experienceleague.adobe.com/docs/commerce-admin/page-builder/add-content/dynamic-block.html).
...
제조업체 또는 디자이너의 특정 제품 또는 제품 그룹을 정의하는 고유 ID입니다.
...
대상 [Inventory management](https://experienceleague.adobe.com/docs/commerce-admin/inventory/sources/sources-manage.html) 및 [Order Management](https://omsdocs.magento.com/getting-started/terminology/), 이 스토어는 제품 수량, 배송 주문 및 매장 픽업 지원을 위한 소스입니다.
...
* _분야: 비즈니스, 디자인, 주문, 제품, 프로그래밍_

### 클라우드 변수

_명사_

클라우드 변수는 클라우드 인프라의 Adobe Commerce과 관련된 환경 변수이며 다음을 사용합니다. **`MAGENTO_CLOUD`** 접두사입니다.
...
의 특별한 변형 [차단](https://experienceleague.adobe.com/docs/commerce-admin/content-design/elements/blocks/blocks.html) 관리자에서만 만들 수 있으며 레이아웃 파일을 통해 참조할 수 없습니다.
...
Elasticsearch 코어 계층 확장은 OpenSearch, Cache, MariaDB, Redis와 같은 데이터 스토리지, 캐시 및 서비스를 위한 세 가지 서비스 노드로 구성됩니다.
...
Commerce 애플리케이션을 관리하고 배포하기 위해 설계된 스크립트 및 도구 세트입니다. 이 패키지는 Docker 환경에 배포, Cron 관리, 프로젝트 구성 확인 및 Adobe 패치 적용 등 다양한 클라우드 인프라 프로세스의 Adobe Commerce을 간소화합니다.
...
자세히 알아보기: [엔티티에 확장 속성 추가](https://developer.adobe.com/commerce/php/development/components/add-attributes/)
...
FOB는 출발지 또는 목적지를 기준으로 할 수 있으며, 운임수취인 또는 선불 운임으로 지정될 수 있습니다.
...
사용자가 Storefront와 상호 작용하여 Commerce 서버에서 실행되는 코드를 사용합니다.
...
Commerce 페이지의 구성에서 레이아웃은 계층 구조로 조립된 일련의 블록으로 페이지의 구조를 나타냅니다.
...
[마크업 구성](https://experienceleague.adobe.com/docs/commerce-admin/catalog/products/settings/settings-advanced-custom-options.html), 또는 제품 사용자 정의 가능 옵션을 통한 제품의 Markdown.
...
QA 전문가, PHP 개발자, 시스템 통합자와 같은 상거래 개발자 및 소프트웨어 엔지니어를 위한 테스트 프레임워크를 제공합니다.
개발자와 QA는 웹 애플리케이션과의 사용자 상호 작용을 시도하기 위한 테스트를 작성하고, 기능을 확인하고, 회귀 테스트를 자동화할 수 있습니다.

_용어 속성:_

* _분야: 상거래 소프트웨어, 프로그래밍_
* _관련 용어: cms 블록, 정적 블록, 컨테이너, 레이아웃_
...
[OMS](https://omsdocs.magento.com) 은(는) Adobe의 Order Management System 제공 항목입니다.
...
자세히 알아보기: [기원 차단](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/faq/fastly-origin-cloaking-enablement-faq.html)
...
자세히 알아보기: [페이지 빌더 사용 안내서](https://experienceleague.adobe.com/docs/commerce-admin/page-builder/introduction.html), [페이지 빌더 개발 문서](https://developer.adobe.com/commerce/frontend-core/page-builder/)
...
CSS, 글꼴, 이미지 및 테마에 사용되는 JavaScript와 같은 에셋의 컬렉션입니다.
...
&quot;스토어&quot;의 상거래 범위 수준은 웹 사이트 계층 구조의 두 번째 수준이며, 이는 다음과 같습니다. 웹 사이트 > 스토어 > 스토어 보기.
...
&#39;스토어 뷰&#39;의 커머스 범위 수준은 웹 사이트, 스토어 및 스토어 뷰 계층 구조에서 세 번째 수준을 의미한다.
...
대부분의 블록에는 블록에서 생성된 정적 HTML을 포함하는 PHTML(템플릿) 파일이 하나 이상 있습니다.
...
* 레이아웃 핸들 XML 파일 단순화
* 관리 사용자 인터페이스 요소를 HTML+JavaScript에서 &quot;순수 JavaScript&quot; 사용자 정의 위젯 시스템으로 이동
...
[PWA Studio](https://github.com/magento/pwa-studio) 사용 [위로](https://developer.adobe.com/commerce/pwa-studio/guides/packages/upward/) 개발 중입니다.
...
따라서 응용 프로그램은 UPPER 서버의 HTTP 끝점 동작만 걱정하므로 Progressive Web Application이 모든 기술 스택에 있는 모든 언어의 UPPER 호환 서버의 맨 위에서 실행될 수 있습니다.
...
상거래 행동을 확장하거나 사용자 정의하고 서드파티 확장으로 작동하는 공급업체에서 생성한 코드는 공급업체 번들 확장 프로그램(VBE)으로 간주됩니다.
VBE는 철저하게 테스트되어 지원되는 각 버전의 Magento Open Source 및 Adobe Commerce에 포함됩니다.
VBE에는 모듈, 테마 및 언어 팩이 포함될 수 있습니다.

다음에서 자세히 알아보기 [공급업체 번들 확장 항목](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/modules/upgrade.html).
...
가상 형식은 의 인수 재정의에서만 참조할 수 있습니다. `<type>` 요소는 di.xml 파일 내에 있으며 소스 코드에는 없습니다.
...
A [위젯](https://experienceleague.adobe.com/docs/commerce-admin/content-design/elements/widgets/widgets.html) 는 저장소 페이지의 특정 위치에 블록, 링크 및 동적 콘텐츠를 배치하는 데 사용할 수 있는 준비된 코드 조각입니다.
