...
현재 지원되는 모든 Adobe Commerce 버전의 개발 및 테스트 환경에서 사용되는 Nginx 버전은 안정적인 최신 Nginx 릴리스에 맞춰 1.28로 업데이트됩니다. 이제 PR 수준 테스트가 Nginx 1.28에 대해 실행되어 모든 Adobe Commerce 버전에 대한 모든 호환성과 지원을 확인할 수 있습니다.
...
#### 확장된 Composer 2.x 버전 지원

이전에는 Composer 버전 지원이 2.2.x로 제한되었습니다. Adobe Commerce 2.4.9-beta1은 이 지원을 Composer 2.4.x 이상 릴리스로 확장하여 개발 및 배포 환경에 대한 호환성을 확장합니다.
...
#### 포괄적인 다중 사이트 액세스 제어 강화

이 업데이트는 Adobe Commerce 관리자의 다중 사이트 ACL(액세스 제어 목록)을 포괄적으로 강화하여 다중 사이트 액세스 제어 구성에서 알려진 권한 및 이전에 알려지지 않은 권한 문제를 모두 해결합니다. 특정 웹 사이트 또는 저장소에 대한 액세스 권한이 있는 관리자는 더 이상 다중 사이트 배포에 있는 다른 웹 사이트 또는 저장소에 속하는 데이터를 보거나 수정할 수 없습니다.
