...
다음 [!UICONTROL Summary] 탭 / [!DNL Observation for Adobe Commerce] 는 사이트에서 경험하는 몇 가지 문제를 빠르게 확인하여 사이트 문제의 잠재적 근본 원인을 자동으로 해결하거나 식별하는 데 도움이 되도록 설계되었습니다. 추가 탭은 구성 요소 서비스, 데이터베이스, 인프라 및 프로세스 상태에 대한 심층적인 정보를 제공합니다.

## [!UICONTROL Transaction Overview]

![거래 개요](../../assets/tools/transaction-overview.jpg)

### [거래란 무엇입니까?](https://docs.newrelic.com/docs/apm/transactions/intro-transactions/transactions-new-relic-apm/#:%7E:text=transactions%20are%20reported.-,내용%20is%20a%20transaction%3F,작업%20in%20a%20software%20application.&amp;text=For%20APM%2C%20it%20will%20frequently, when%20the%20response%20is%20sent)

&quot;위치 [!DNL New Relic]트랜잭션은 소프트웨어 애플리케이션에서 작업의 하나의 논리 단위로 정의됩니다. 특히 해당 작업 단위를 구성하는 함수 호출과 메서드 호출을 가리킵니다. 종종 웹 트랜잭션을 참조합니다. 이 트랜잭션은 애플리케이션이 웹 요청을 수신할 때부터 응답이 전송될 때까지 발생하는 활동을 나타냅니다.&quot;

### 거래 유형:

**웹:** 웹 트랜잭션은 HTTP 요청으로 시작됩니다. 대부분의 조직에서 이는 고객 중심의 상호 작용을 나타내므로 모니터링해야 할 가장 중요한 트랜잭션입니다.

**웹이 아닌 항목:** 웹이 아닌 트랜잭션은 웹 요청으로 시작되지 않습니다. 비웹 작업자 프로세스, 백그라운드 프로세스, 스크립트, 메시지 대기열 활동 및 기타 작업을 포함할 수 있습니다.

다음을 살펴보십시오. **[!UICONTROL Transaction Overview]** 위의 프레임에서 평균 APDEX 점수가 .76인 거래는 거의 53,000건이었으며, 이러한 거래의 95%는 2.313초 이내에 발생했습니다. 짧은 기간 동안 APDEX 히트가 있는 경우 더 엄격한 기간이 현재 평균과의 편차를 보일 수 있는 프레임입니다.

## [!UICONTROL 404 page errors frame]

![404 페이지 오류 프레임](../../assets/tools/404-page-errors.jpg)

다음 **[!UICONTROL 404 page errors]** 프레임이 다음을 나열합니다. [URI](https://en.wikipedia.org/wiki/Uniform_Resource_Identifier) 선택한 일정의 404 페이지 오류 수입니다.

## [!UICONTROL % of Storage Free frame]

![스토리지 여유 프레임 비율](../../assets/tools/percent-of-storage-free.jpg)

다음 **[!UICONTROL % of Storage Free]** frame은 클러스터의 모든 노드에 걸쳐 스토리지 마운트에서 사용 가능한 평균 비율을 표시합니다. 예를 들어 3개의 노드 클러스터가 있는 경우 프레임에는 \&lt;mount point=&quot;&quot;>, \&lt;environment name=&quot;&quot;>. 이 프레임은 세 노드에 걸쳐 분산이 있는 경우 기만적일 수 있다. 분산의 예는 다음과 같습니다. `/data/mysql` 마운트 지점 사용 가능(mount point free) 값이 세 노드 클러스터 전체에서 다른 값입니다. 아래 액자가 있습니다 [!UICONTROL MySQL] 노드 이름별로 마운트 지점을 패싯하여 `/data/mysql` 실제로 각 노드에서 스토리지가 사용 안 함

## [!UICONTROL % of system memory that is free frame]

![사용 가능한 프레임 시스템 메모리 비율](../../assets/tools/percent-of-system-memory-that-is-free.jpg)

다음 **사용 가능한 시스템 메모리의 %** 프레임은 각 노드에서 사용 가능한 시스템 메모리의 양을 노드별로 표시합니다.

## [!UICONTROL Swap memory free in bytes]

![사용 가능한 메모리 교환(바이트)](../../assets/tools/swap-memory-free-in-bytes.jpg)

다음 **[!UICONTROL Swap memory free in bytes]** 프레임은 노드에 사용 가능한 SWAP 메모리의 양을 노드별로 표시합니다.

## [!UICONTROL CPU % by host]

![호스트별 CPU 비율](../../assets/tools/cpu-percent-by-host.jpg)

모든 환경 및 노드의 집계가 **[!UICONTROL CPU % by host]** frame을 참조하십시오. 비프로덕션 환경은 선택 해제해야 합니다. 또한 프로덕션 환경의 모든 노드가 없는 모든 인스턴스를 확인합니다. 높은 CPU 사용률에 대한 자세한 정보는 다음을 참조하십시오. [Adobe Commerce에서 New Relic을 사용하여 성능 문제 해결](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/troubleshoot-performance-using-new-relic-on-magento-commerce.html).

## [!UICONTROL Alerts during timeframe]

![일정 동안의 경고](../../assets/tools/alerts-during-timeframe.jpg)

다음 **[!UICONTROL Alerts during timeframe]** 다음을 포함한 모든 경고를 표시합니다. [!UICONTROL Managed Alerts] Adobe Commerce 지원에서 추가함.

## [!UICONTROL CPU Usage]

![사용](../../assets/tools/cpu-usage.jpg)

다음과 같은 경우 **[!UICONTROL CPU Usage]** 프레임이 비어 있으면 의 인프라 응용 프로그램이 [!DNL New Relic] 이(가) 활성화되지 않았습니다. 사이트가 Starter에 있는 경우 이 정보가 표시되지 않습니다. 사이트가 Pro에 있는 경우 [지원 티켓](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/help-center-guide/magento-help-center-user-guide.html) 가지고 있음 [!DNL New Relic Infrastructure] 이(가) 귀하의 사이트에 대해 활성화되었습니다.

## [!UICONTROL Average Response Time]

![평균 응답 시간](../../assets/tools/average-response-time.jpg)

다음 **[!UICONTROL Average Response Time]** 그래프는 트랜잭션(웹 및 기타)에 대한 평균 응답 시간을 보여 줍니다.

## [!UICONTROL Long duration cron_schedule updates]

![장기 cron_schedule 업데이트](../../assets/tools/long-duration-cron-schedule-updates.jpg)

다음 **[!UICONTROL cron_schedule]** 표는 cron jobs의 시작과 끝에 작성됩니다. 긴 기간 cron 작업은 이 테이블 업데이트에 지연 시간을 나타낼 수 있으며, 이는 cron 스택 또는 cron 예약 방식의 문제를 나타낼 수 있습니다.

## [!UICONTROL Response Code]

![응답 코드](../../assets/tools/response-code.jpg)

다음 **[!UICONTROL Response Code]** 프레임은 웹 트래픽과 요청의 응답 코드를 잘 나타냅니다. 다음과 같습니다. [!DNL New Relic's] 거래 데이터가 참조되는 경우 `httpResponseCode` 반환되었습니다.

## [!UICONTROL Web Traffic volume compared with one week ago Magento Managed Alerts Information]

![1주일 전과 비교한 웹 트래픽 양](../../assets/tools/web-traffic-volume-compared.jpg)
...
다음 **[!UICONTROL Deployment Log Entries]** frame에는 배포 및 클라우드 로그 항목의 수가 표시되며, 배포 로그 이름별로 카운트가 표시됩니다.

## [!UICONTROL Deployment State]

![배포 상태](../../assets/tools/deployment-state.jpg)

다음 **[!UICONTROL Deployment State]** 프레임 패싯 배포 로그의 특정 배포 단계. 다음은 로그 및 Facet 이름에서 계산되는 단계의 몇 가지 예입니다.
...
다음 **[!UICONTROL IP Frequency]** 프레임은 의 각 IP에 대한 (&#39;MISS&#39; 및 &#39;PASS&#39;) 상태를 계산합니다. [!DNL Fastly] 로그. 이러한 상태의 웹 요청은 원천 서버에 도달하며 서버에 로드를 추가합니다. 상위 20개 주소를 빈도로 표시합니다. 이 프레임은 웹 사이트에서 IP 공격 또는 과도한 부하를 감지하는 데 사용할 수 있습니다.

## [!UICONTROL IP Response – top 20 URLs in duration]

![ip 응답 - 기간 내 상위 20개 url](../../assets/tools/ip-response-top-20-urls.jpg)

다음 **[!UICONTROL IP Response – top 20 URLs in duration]** frame은 응답에서 가장 긴 지속 시간을 가진 URL을 표시합니다. 응답 지속 시간이 가장 긴 큰 이미지 파일 또는 페이지, API 또는 페이지를 나타낼 수 있습니다.

## [!UICONTROL API Calls by IP]

![ip를 통한 api 호출](../../assets/tools/api-calls-by-ip.jpg)

다음 **[!UICONTROL API Calls by IP]** 프레임은 API 및 API URL에서 요청을 하는 IP 주소에 대해 과도한 트래픽을 식별하는 데 도움이 됩니다.

## [!UICONTROL API Calls by IP, details by URL]

![url별 ip별 api 호출 세부 정보](../../assets/tools/api-calls-by-ip-details-by-url.jpg)

다음 **[!UICONTROL API Calls by IP, details by URL]** frame은 API에 대한 대량 트래픽의 세부 정보와 요청을 수행하는 URL의 세부 정보를 제공합니다.

## [!UICONTROL IP Frequency Rate per minute]

![분당 ip 빈도](../../assets/tools/ip-frequency-rate-per-minute.jpg)

어떤 IP 주소가 다른 프레임에서 가장 많은 요청을 하는지 알 수 없는 경우가 있습니다. 다음 **[!UICONTROL IP Frequency Rate per minute]** frame에는 IP 주소당 분당 속도가 표시됩니다.

## [!UICONTROL Potential Bots]

![잠재적인 봇](../../assets/tools/potential-bots.jpg)

다음 **[!UICONTROL Potential Bots]** frame은 NULL 또는 &#39;%bot%&#39;과(와) 같은 request_user_agent 이름이 있는 요청을 찾습니다. 일반적으로 &#39;%bot%&#39; request_user_agent는 의 정책 설정을 따릅니다 `robots.txt` 파일.

## [!UICONTROL Transaction Errors]

![트랜잭션 오류](../../assets/tools/transaction-errors.jpg)

다음 **[!UICONTROL Transaction Errors]** 프레임에서 트랜잭션 오류 수가 표시됩니다. [!DNL New Relic].

## [!UICONTROL Nginx access by node]

![노드별 nginx 액세스](../../assets/tools/nginx-access-by-node.jpg)

다음 **[!UICONTROL Nginx access by node]** frame은 다음에서 카운트를 봅니다. `access.log` 노드별. 부하가 고르게 분포되어 있는지 확인하는 것이 도움이 된다. 종종 노드가 드롭될 때 표시됩니다. 프레임에는 사이트에 대한 로드도 표시됩니다.

## [!UICONTROL Galera Log]

![게레라나무](../../assets/tools/galera-log.jpg)

[[!DNL Galera]](https://galeracluster.com/library/galera-documentation.pdf) 데이터베이스 클러스터에 사용됩니다. 이 프레임은 의 특정 신호에 중점을 둡니다. [!UICONTROL Galera] 클러스터. 이 신호는 클러스터를 시작 및 종료하는 노드에 중점을 둡니다. 이 동작은 데이터베이스 데이터 무결성을 유지하기 위한 정상적인 동작입니다. 노드는 다음과 같이 동기화된 상태로 유지됩니다. [!UICONTROL Galera] 클러스터 상태가 변경되었습니다.
...
다음 **[!UICONTROL Database traces]** frame은 [sql 추적](https://docs.newrelic.com/docs/apm/transactions/transaction-traces/transaction-traces-database-queries-page/) 엔티티 [!DNL New Relic] 추적의 경로를 반환합니다.

## [!UICONTROL Database mysql-slow.log]

![데이터베이스 mysql-slow.log](../../assets/tools/database-mysql-slow-log.jpg)

다음 **[!UICONTROL Database mysql-slow.log]** frame은 의 항목 수를 계산합니다. [mysql-slow.log](https://dev.mysql.com/doc/refman/5.7/en/slow-query-log.html) 쿼리 요청 유형별. 이 메서드는 mysql-slow.log(느린 쿼리 로그)에 관심 있을 수 있는 시간대를 시각적으로 격리합니다. 인덱스가 없는 테이블의 쿼리나 큰 테이블을 업데이트하는 쿼리는 다른 쿼리를 차단할 수 있습니다.
...
PHP 프로세스가 작동하는 방식은 [구성](https://www.php.net/manual/en/install.fpm.configuration.php). 많은 변수와 옵션이 있는 구성이 복잡합니다. 다음 **[!UICONTROL PHP process states]** frame을 사용하면 PHP 프로세스가 종료되고 다시 시작되는 시기를 이해할 수 있습니다.

### [!UICONTROL PHP errors]

![php 오류](../../assets/tools/php-errors.jpg)

다음 **[!UICONTROL PHP errors]** frame은 선택한 기간 동안 작업자가 발생한 PHP 오류 수를 보여 줍니다. 자세한 내용은 다음을 참조하십시오. [Adobe Commerce 설정](../../installation/prerequisites/php-settings.md).
...
[PHP-FPM](https://php-fpm.org/) 다음 값: [!UICONTROL FastCGI Process Manager] 사용한 사람 [!DNL Nginx]. 시스템 요구 사항에 대해 알아보려면 다음을 참조하십시오. [Adobe Commerce 버전에 매핑된 PHP 버전 요구 사항](../../installation/system-requirements.md). 다음 **[!UICONTROL PHP processes]** frame은 선택한 타임라인에서 특정 시간에 실행되는 PHP 프로세스 수를 보여 줍니다.

## [!UICONTROL Secondary processes]

![보조 프로세스](../../assets/tools/secondary-processes.jpg)

보조 프로세스가 사이트 응답에 영향을 줄 수 있습니다. 다음 **[!UICONTROL Secondary processes]** 프레임은 사이트에 로드를 추가할 수 있는 프로세스를 나타냅니다. 데이터베이스에는 주로 가장 많은 보조 프로세스가 실행되고 있습니다.

## [!UICONTROL Traffic vs Week Ago]

![트래픽 대 주 전](../../assets/tools/traffic-vs-week-ago.jpg)

다음 **[!UICONTROL Traffic vs Week Ago]** frame은 웹 사이트 트래픽(요청)을 [!DNL Fastly] 캐시 상태가 (&#39;누락&#39;, &#39;통과&#39;)인 로그. 이러한 요청은 원천 서버에 로드를 추가합니다. 이 프레임에는 동일한 기간 동안 현재 주와 지난 주의 비교 웹 요청 볼륨이 표시됩니다.

## [!UICONTROL Fastly Cache]

![fastly 캐시](../../assets/tools/fastly-cache.jpg)

다음 **[!UICONTROL Fastly Cache]** 프레임은에서 요청의 캐시 상태에 대한 집계 보기를 표시합니다. [!DNL Fastly] 로그. ERROR 를 선택하면 요청의 오류 비율이 표시됩니다. 일반적으로 원천 서버가 페이지 요청에 충분히 신속하게 응답하지 않으면 증가합니다.

## [!UICONTROL Page Rendering]

![페이지 렌더링](../../assets/tools/page-rendering.jpg)

다음 **[!UICONTROL Page Rendering]** 프레임은 의 페이지 보기 소스에서 현재 주의 평균 페이지 렌더링 기간을 표시합니다. [!DNL New Relic] 같은 기간 동안 이전 주와 비교합니다.

## [!UICONTROL Page loading detail]

![페이지 로드 세부 정보](../../assets/tools/page-loading-detail.png)

다음 **[!UICONTROL Page loading detail]** frame은 페이지 로드 이벤트를 설명합니다. 그것은 이러한 측면들의 의미들을 자세히 설명합니다. 다음은 이 프레임에 대해 실행되는 쿼리입니다.
...
트랜잭션 기간은 초 단위입니다. 거래에 따라 장기 실행 중인 경우 다른 거래에 영향을 줄 수 있습니다. 이름 및 기간에 나열된 트랜잭션은 특정 기간에 대한 것입니다. 간결한 문제 일정이 있는 경우 [!DNL Observation for Adobe Commerce] 해당 좁은 시간대에 대한 날짜/시간 선택기입니다.

## [!UICONTROL Admin Activities]

![관리 활동](../../assets/tools/admin-activities.jpg)

다음 **[!UICONTROL Admin Activities]** 프레임은 관리자 사용자와의 트랜잭션을 식별합니다.

## [!UICONTROL Order transactions (default?)]

![주문 거래 기본값](../../assets/tools/order-transactions-default.jpg)

다음 **[!UICONTROL Order transactions (default?)]** 프레임이 트랜잭션을 찾습니다. `request.headers.host` 트랜잭션. 여기서 name = `WebTransaction/Action/checkout/onepage/success`. 주문 성공 URL이 다른 경우 이 프레임에는 데이터가 없습니다.

## [!UICONTROL Elasticsearch Index information]

![elasticsearch 색인 정보](../../assets/tools/elasticsearch-tab-elasticsearch-index-information-image-1.jpg)
...
다음 **[!UICONTROL Cron view]** 프레임은 cron 로그를 확인하여 시작된 cron 수와 완료된 cron 수 사이의 균형을 유지합니다.
...
다음 **[!UICONTROL cron_schedule table updates]** frame은 데이터 저장소 작업 업데이트에 cron_schedule 테이블이 포함된 최대 기간(초)을 봅니다. SQL 요청 유형에 적용됩니다.

## [!UICONTROL Datastore Operations Tables]

![데이터 저장소 작업 테이블](../../assets/tools/datastore-operations-tables.jpg)

이 **[!UICONTROL Datastore Operations Tables]** frame은 기간, 테이블 이름 및 SQL 요청 유형별로 상위 25개 작업을 표시합니다. 스파이크 위로 마우스를 가져가면 액세스 중인 테이블과 요청 유형에 의한 세부 정보를 볼 수 있습니다.
