...
도구 모음에 있습니다. `tests/` 테스트 유형과 동일한 디렉토리(단위 테스트는에 위치함) `tests/unit` directory). 테스트를 시작하려면 phpunit가 설치되어 있어야 합니다. 현재 디렉터리를 테스트 디렉터리와 실행 phpunit로 변경합니다. For example:
