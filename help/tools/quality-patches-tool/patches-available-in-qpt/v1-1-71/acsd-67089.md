...
# ACSD-67089: `page_size`이(가) 설정되면 `inventory/export-stock-salable-qty` API에서 잘못된 `total_count`을(를) 반환합니다.
...
`page_size`이(가) 설정되면 `inventory/export-stock-salable-qty` API에서 잘못된 `total_count` 값을 반환합니다.
...
`inventory/export-stock-salable-qty` API가 요청에 지정된 `page_size` 값에 관계없이 올바른 `total_count`(*22*)을(를) 반환합니다.

<u>실제 결과</u>:

`page_size`이(가) 실제 합계 *22* 대신 *5*(으)로 설정된 경우 `inventory/export-stock-salable-qty` API에서 *5*&#x200B;의 잘못된 `total_count`을(를) 반환합니다.
