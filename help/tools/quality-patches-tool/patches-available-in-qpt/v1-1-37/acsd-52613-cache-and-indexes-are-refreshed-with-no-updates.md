...
ACSD-52613 패치는 `Inventory_source`까지 [!DNL REST API]개 항목에 대한 업데이트가 이루어지지 않을 때 캐시와 인덱스가 새로 고쳐지는 Adobe Commerce 문제를 해결합니다. 이 패치는 [!DNL Quality Patches Tool (QPT)] 1.1.37이 설치되어 있을 때 사용할 수 있습니다. 패치 ID는 ACSD-52613입니다. 이 문제는 Adobe Commerce 2.4.7에서 해결되었습니다.
...
`Inventory_source`이(가) [!DNL REST API]개 항목을 업데이트하지 않으면 캐시와 인덱스가 새로 고쳐집니다.
