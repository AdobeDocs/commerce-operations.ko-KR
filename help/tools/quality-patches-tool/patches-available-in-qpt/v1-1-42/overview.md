...
1. **ACSD-54626**: GraphQL을 통해 `createPurchaseOrderApprovalRule` 특성이 있는 새 구매 주문 규칙(`NUMBER_OF_SKUS`)을 만들 수 없는 문제를 해결했습니다.
1. **ACSD-53845**: `consumer max_messages` = 0일 때 MySQL 연결 시간 초과 문제를 해결했습니다.
1. **ACSD-54890**: `aggregate_sales_report_bestsellers_data` 디스크의 공간이 부족하여 `/tmp`에서 MySQL 오류가 발생하는 문제를 해결했습니다.
1. **ACSD-55112**: *[!UICONTROL Submit review]* 유효성 검사 없이 [!DNL Google reCAPTCHA v3] 단추를 여러 번 클릭할 수 있는 문제를 해결했습니다.
...
1. **ACSD-55381**: GraphQL을 통해 B2B `configurable_product_option_uid`에서 `configurable_product_option_value_uid` 및 *[!UICONTROL Requisition list]* 필드를 쿼리할 때 내부 서버 오류를 수정합니다.
