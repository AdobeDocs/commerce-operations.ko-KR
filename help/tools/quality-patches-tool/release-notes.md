...
* **ACSD-61622**(Adobe Commerce 및 Magento Open Source >=2.4.6-p1 &lt;2.4.7) - [!DNL FedEx]의 계정별 비율이 응답에 없는 문제를 수정합니다.
* **ACSD-61895**(Adobe Commerce >=2.4.4 &lt;2.5.0) - 루트 범주에 **allow** 권한이 없어도 [!DNL GraphQL] 쿼리가 **allow** 권한이 있는 범주를 반환하는 문제를 해결했습니다.
* **ACSD-62212**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.5.0) - **암호 찾기** 전자 메일 콘텐츠가 스토어 보기의 언어로 번역되지 않는 문제를 해결했습니다.
...
* **ACSD-62689**(Adobe Commerce 및 Magento Open Source >=2.4.7 &lt;2.5.0) - 고객이 *깊이 4* 이후 **[!UICONTROL Related Product Rules and Widgets]**&#x200B;에 범주를 추가할 수 없는 문제를 해결했습니다.
* **ACSD-62708**(Adobe Commerce 및 Magento Open Source >=2.4.4-p11 &lt;2.4.5) || >=2.4.5-p10 &lt;2.4.6-p2 || >=2.4.6-p8 &lt;2.4.7-p1) - 관리자의 [!DNL TinyMCE] 7 편집기 글꼴 크기가 *PX*&#x200B;이 아닌 *PT*&#x200B;을(를) 표시하는 문제를 해결했습니다. 이제 *PT* 대신 *PX*&#x200B;에서 글꼴 크기를 설정할 수도 있습니다.
* **ACSD-62758**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.5.0의 경우) - [!DNL URL]에 선택한 옵션이 포함된 경우 **[!UICONTROL Configurable Product]**&#x200B;의 세부 정보 페이지에서 제품 비디오가 올바르게 렌더링되지 않는 문제를 해결합니다.
* **ACSD-62951**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.5.0) - 항목 및 합계를 포함하지 않고 대변 메모 전자 메일이 전송되는 문제를 해결했습니다.
...
* **ACSD-58383**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.4.8의 경우) - 동시에 실행되는 두 개의 동일한 요청으로 [!DNL REST API]을(를) 통해 환불을 발행하고 중복 대변 메모를 만드는 문제를 해결했습니다.
...
* **ACSD-58685**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.4.8) - 전자 메일 통신을 사용하지 않도록 설정하는 동안 시작된 판매 전자 메일이 전자 메일 통신을 다시 사용하도록 설정한 후에도 계속 전송되는 문제를 수정합니다.
...
* **ACSD-61348**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.4.8) - 위시리스트 항목이 [!DNL GraphQL]을(를) 통해 표시되지만 다중 웹 사이트 환경에 있는 경우 상점 앞에는 표시되지 않는 문제를 해결했습니다.
* **ACSD-61534**(Adobe Commerce 및 Magento Open Source >=2.4.7 &lt;2.4.8의 경우) - `bin/magento config:set` 명령을 사용하여 디자인 구성을 설정할 수 없고 잠긴 값은 양식 조작을 통해 변경할 수 있는 문제가 해결되었습니다. `--lock-env` 또는 `--lock-conf`을(를) 사용하여 [!DNL CLI]에서 설정된 잠긴 값은 업데이트할 수 없습니다.
...
* **ACSD-61200**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.4.8) - 매출의 *[!UICONTROL Total Amount]* 및 *[!UICONTROL Total Amount Actual]* 계산에 *[!UICONTROL Discount Tax Compensation Amount]* 및 *[!UICONTROL Shipping Discount Tax Compensation Amount]*&#x200B;이(가) 누락되어 판매 주문 데이터가 일치하지 않는 문제를 해결했습니다.
...
* **ACSD-61799**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.4.5) - 고정 할인이 있는 여러 장바구니 규칙이 견적에 적용될 때 총 할인이 잘못 계산되는 문제가 수정되었습니다.
* **ACSD-61845**(Adobe Commerce 및 Magento Open Source >=2.4.7-p1 &lt;2.4.8) - *text/html* 수락 헤더로만 요청을 보낼 때 발생하는 오류를 수정합니다.
* **ACSD-62056**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.4.8) - MSI가 설치된 경우 구성 가능한 제품에 대한 이미지 업로드가 실패하는 문제를 해결합니다.
