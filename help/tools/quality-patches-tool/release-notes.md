...
* **ACSD-68759**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4-p2 &lt;2.4.5 || >=2.4.5-p1 &lt;2.4.9) - 아랍어 로케일을 사용할 때 고객 계정을 만들 수 없고 DOB(생일) 특성이 상점 앞에 표시되도록 설정된 문제를 해결했습니다.
...
* **ACSD-69129**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.5 &lt;2.4.7 || >=2.4.8 &lt;2.4.9) - REST API를 통해 보조 웹 사이트에 대한 계층 가격을 업데이트하려고 할 때 기본 기본 웹 사이트를 삭제하고 보조 웹 사이트를 기본값으로 사용하면 오류가 발생하는 문제를 해결했습니다.
...
* **ACSD-68206**(Adobe Commerce 및 Magento Open Source >=2.4.7 &lt;2.4.9) - **[!UICONTROL Rate Limiting]** 확장이 설치된 [!DNL PHP Redis] 기능과 함께 GraphQL Application Server를 사용할 때 발생하는 오류를 수정합니다.
...
* **ACSD-67424**(Adobe Commerce의 경우 B2B >=1.5.0 &lt;1.5.3) - 협상 가능한 견적을 사용할 때 `updated_at` `GET /carts/search` API 응답의 [!DNL REST] 값이 **[!UICONTROL Admin panel]**&#x200B;에 표시된 값과 일치하지 않는 문제를 해결했습니다.
...
* **ACSD-60624**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.4.8) - 페이지 빌더의 **[!UICONTROL Upload Image]**, [!UICONTROL Image] 및 [!UICONTROL Banner] 섹션에 있는 빈 콘텐츠에 대해 [!UICONTROL Slider]가 작동하지 않는 문제를 해결했습니다.
...
* **ACP2E-3977**(Adobe Commerce >=2.4.4 &lt;2.4.9의 경우) - **[!UICONTROL Cap Reward Points Balance At]**&#x200B;이(가) 설정되어 유효성 검사 오류가 발생할 때 [!UICONTROL Rewards Points Balance Redemption Threshold] 필드를 비워 둘 수 없는 문제를 해결했습니다.
...
* **ACSD-66179**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6 || >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.9) - &quot;캡처 안 함&quot; 결제 유형으로 생성된 청구서를 취소하면 404 오류 페이지가 표시되는 문제를 해결합니다.
...
* **ACSD-66049**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.5-p3 &lt;2.4.6 || >=2.4.7 &lt;2.4.9) - ICU 라이브러리 버전으로 인해 영어가 아닌 상점 앞에 잘못된 가격이 표시되는 문제를 수정합니다.
...
* **ACSD-66301**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.6-p9 &lt;2.4.7 || >=2.4.7-p4 &lt;2.4.8) - 제품을 주문에서 카트로 다시 이동하면 수량이 일치하지 않는 문제를 해결했습니다.
...
* **AC-14984**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.6-p10 &lt;2.4.7 || >=2.4.8 &lt;2.4.9) - RabbitMQ SSL 연결에서 *잘못된 프레임 유형 21* 오류를 수정합니다.
...
* **ACSD-64813**(Adobe Commerce >=2.4.4 &lt;2.4.9) - [!DNL B2B] API를 통해 [!DNL REST] 공유 카탈로그에서 범주 할당을 취소하는 데 시간이 너무 오래 걸리거나 큰 카탈로그가 시간이 초과되는 문제를 해결했습니다.
...
* **ACP2E-3838**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4-p9 &lt;2.4.4-p13 || >=2.4.5-p8 &lt;2.4.5-p12 || >=2.4.6-p6 &lt;2.4.6-p10 || >=2.4.7 &lt;2.4.7-p5) - [!DNL Page Builder] CORS 오류로 인해 프로덕션 모드의 관리 패널에 변경 사항이 저장되지 않는 문제가 해결되었습니다.
...
* **ACSD-64112**(Adobe Commerce >=2.4.5 &lt;2.4.8) - `indexer_update_all_views`이(가) 설정되면 `MAGE_INDEXER_THREADS_COUNT` cron 실행이 실패하는 문제가 해결되었습니다.
* 업데이트된 버전: **ACSD-61622**
* 대체된 패치: **ACSD-61553**
...
* **ACSD-48570**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4 &lt;2.4.7) - [!UICONTROL Admin]URL에 스토어 코드 추가&#x200B;**가**&#x200B;활성화&#x200B;*되어 이전에 로그인 페이지 또는 404 페이지가 표시되었을 때* 암호 재설정 링크를 클릭하여 암호 재설정 페이지에 연결할 수 없는 문제를 해결했습니다.
* **ACSD-62118**(Adobe Commerce >=2.4.6 &lt;2.4.8) - 구매 주문 방법을 사용하여 `sales_order_tax_item` 주문을 했을 때 [!DNL B2B] 테이블이 완전히 업데이트되지 않는 문제를 해결했습니다.
...
* **ACSD-59083**(Adobe Commerce >=2.4.4 &lt;2.5.0의 경우) - *업데이트가 동시에 실행되는 경우 일부 데이터베이스 업데이트 작업으로 인해*&#x200B;기본 테이블 또는 보기를 찾을 수 없는 문제가 해결됩니다[!DNL mview].
* **ACSD-61622**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.6-p1 &lt;2.4.7) - [!DNL FedEx]의 계정별 비율이 응답에 없는 문제를 해결합니다.
* **ACSD-61895**(Adobe Commerce >=2.4.4 &lt;2.5.0의 경우) - 루트 범주에 [!DNL GraphQL]allow **권한이 없어도** 쿼리가 **allow** 권한이 있는 범주를 반환하는 문제를 해결했습니다.
...
* **ACSD-62708**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6-p2 || >=2.4.6-p8 &lt;2.4.7-p1) - 관리자의 [!DNL TinyMCE] 7 편집기 글꼴 크기에 *PT*&#x200B;가 표시되고 *PX*&#x200B;가 표시되지 않는 문제가 해결되었습니다. 이제 *PT* 대신 *PX*&#x200B;에서 글꼴 크기를 설정할 수도 있습니다.
* **ACSD-62758**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4 &lt;2.5.0) - **[!UICONTROL Configurable Product]**&#x200B;에 선택한 옵션이 포함된 경우 [!DNL URL]의 세부 정보 페이지에서 제품 비디오가 올바르게 렌더링되지 않는 문제를 해결했습니다.
...
* **ACSD-62979**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.4.7) - [!UICONTROL Store ID] 헤더에서 잘못된 [!DNL GraphQL]을(를) 사용하면 치명적인 메모리 오류가 발생하는 문제를 해결했습니다.
...
* **ACSD-62755**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6 || >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.8) - [!DNL TinyMCE] 7을 사용하려면 글꼴 크기와 글꼴을 편집기 초기화 설정 내에 구체적으로 추가해야 하는 문제가 해결되었습니다.
* **ACSD-62670**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6 || >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.8) - [!DNL CSV] 보고서로 내보내고 [!DNL XML]에서 오류가 반환되는 문제를 수정합니다.[!UICONTROL Products Ordered]
...
* **ACSD-58566**(Adobe Commerce >=2.4.6 &lt;2.4.8) - [!DNL GraphQL] 돌연변이의 `created_at` 필드를 쿼리할 때 `addPurchaseOrderComment`이(가) 내부 서버 오류를 반환하는 문제를 수정합니다.
...
* **ACSD-62485**(Adobe Commerce >=2.4.4 &lt;2.4.6-p8 || >=2.4.7 &lt;2.4.8) - 회사가 만들어질 때 `async.operations.all` 소비자가 작동하지 않는 문제가 해결되었습니다.
...
* **ACSD-60804**(Adobe Commerce >=2.4.4 &lt;2.4.8) - 삭제된 회사에 연결된 고객을 편집하면 *null`getSuperUserId()`의 멤버 함수* 호출에 오류가 발생하는 문제가 해결되었습니다.
* **ACSD-61133**(Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.8) - `sales_clean_quotes` [!DNL cron]이 승인되지 않은 구매 주문에서 견적을 삭제하는 문제를 해결합니다.
* **ACSD-61528**(Adobe Commerce >=2.4.6 &lt;2.4.8) - [!UICONTROL Admin]을(를) 사용하여 [!DNL GraphQL]에서 역할을 검색하면 결과가 반환되지 않는 문제가 해결되었습니다.
* **ACSD-61553**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.5 &lt;2.4.7) - 우선 순위가 다른 여러 할인 및 **[!UICONTROL Cart Price Rule]**&#x200B;이(가) 제품에 적용될 때 **[!UICONTROL Maximum Qty Discount is Applied To]** 할인이 잘못 계산되는 문제가 해결되었습니다.
...
* **ACSD-61366**(Adobe Commerce 및 Magento Open Source >=2.4.7 &lt;2.4.8) - DB 연결에 대해 포트를 지정한 경우 `setup:static-content:deploy --jobs 4`호스트 매개 변수 내에서 포트를 구성해야 함&#x200B;*오류가 발생하여* 명령이 여러 작업 실패로 실행되는 문제를 해결했습니다.
...
* **ACSD-45049**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.4.4-p8 || >=2.4.5 &lt;2.4.6) - 관리자의 웹 사이트 범위에 따라 고객 *[!UICONTROL Is required]* 특성 설정이 제대로 작동하지 않는 문제를 해결했습니다.
...
* **ACSD-54887**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4-p4 &lt;2.4.4-p9 || >=2.4.5-p3 &lt;2.4.5-p8 || >=2.4.6-p1 &lt;2.4.6-p6) - [!UICONTROL Persistent Shopping Cart] 활성화로 고객 세션이 만료된 후 고객 장바구니가 지워지는 문제를 수정합니다.
...
* **ACSD-59514**(Adobe Commerce >=2.4.4 &lt;2.4.7-p2) - 관리 영역에서 [!DNL Page Builder]인 양식에서 *[!DNL Page Builder]이(가) 잠금 해제 없이 5초 동안 렌더링되는 오류를 발생시키는 문제를 해결했습니다.양식을 제출한 후 브라우저 콘솔에서*&#x200B;을(를) 실행하므로 변경 내용을 저장할 수 없습니다.
* **ACSD-60303**(Adobe Commerce >=2.4.4-p9 &lt;2.4.5 || >=2.4.5-p8 &lt;2.4.6 || >=2.4.6-p6 &lt;2.4.8) - HTML 축소가 활성화된 경우 관리자의 주문을 할 수 없는 문제를 해결합니다.
* **ACSD-60441**(Adobe Commerce 및 Magento Open Source 2.4.4-p9 || 2.4.5-p8 || 2.4.6-p6 || 2.4.7-p1) - 백엔드에서 생성된 통합 액세스 토큰을 사용할 때 `V1/customers` [!DNL REST API] 엔드포인트를 통해 고객을 업데이트하는 문제가 해결되었습니다.
* 업데이트된 패치: ACSD-57003
...
* **ACSD-58375**(Adobe Commerce 및 Magento Open Source >=2.4.2 &lt;2.4.7) - 스토어 보기 수준에서 *[!DNL YouTube API Key]* 비디오를 추가할 때 잘못된 [!DNL YouTube] 구성으로 인해 오류가 발생하는 문제를 해결했습니다.
...
* **ACSD-57045**(Adobe Commerce >=2.4.5 &lt;2.4.7) - *[!UICONTROL Website Root]*&#x200B;에서 *[!UICONTROL Hierarchy]* 선택을 취소한 후 URL 재작성으로 인해 무한 페이지 루프가 발생하는 문제를 수정합니다.
* 업데이트된 패치: ACSD-46815, ACSD-47027, ACSD-51683, ACSD-55031, ACSD-51819, ACSD-54965-v2

## v1.1.48 {#v1-1-48}

* **ACSD-55566**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.3 &lt;2.4.7) - 동일한 번들 항목이 있는 소스 및 대상 카트를 병합할 때 `mergeCart` 응답에서 *돌연변이가 &quot;*&#x200B;내부 서버 오류[!DNL GraphQL]&quot;(으)로 인해 실패하는 문제를 해결했습니다.
...
* **ACSD-56741**(Adobe Commerce 및 Magento Open Source >=2.4.6 &lt;2.4.7) - 데이터베이스에 색인 지정 메커니즘 및 *과(와) 관련이 없는 사용자 지정* 트리거가 포함된 경우 `setup:upgrade` 동안 표시되는 &quot;[!DNL MySQL]null[!DNL MView]&quot; 형식의 값에 대한 배열 오프셋에 액세스하려고 함&quot; 오류 메시지를 수정합니다.
...
* **ACSD-57074**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.6 &lt;2.4.7) - *로 시작하는*&#x200B;의 `attrbute_code`예/아니요`price_` 사용자 지정 특성이 인덱싱에서 제대로 작동하지 않는 문제를 해결했습니다. 이러한 특성이 있는 제품은 프런트 엔드에서 사용할 수 없습니다.
...
* **ACSD-57003**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.6 &lt;2.4.7) - 주문이 부분적으로 환불 및 부분적으로 배송될 때 주문 상태가 *[!UICONTROL Complete]*(으)로 변경되지 않고 *[!UICONTROL Processing]*(으)로 변경되는 문제를 해결했습니다.
...
* **ACSD-53845**(Adobe Commerce 및 Magento Open Source >=2.4.0 &lt;2.4.7) - [!DNL MySQL] = 0일 때 `consumer max_messages` 연결 시간 초과 문제를 해결했습니다.
* **ACSD-54890**(Adobe Commerce 및 Magento Open Source >=2.4.0 &lt;2.4.7) - `aggregate_sales_report_bestsellers_data` 디스크의 공간 부족으로 인해 [!DNL MySQL]에서 `/tmp` 오류가 발생하는 문제를 해결했습니다.
...
* **ACSD-55628**(Adobe Commerce의 경우 >=2.4.4-p2 &lt; 2.4.5 || >=2.4.5-p1 &lt; 2.4.6) - 회사 등록 양식에 파일을 업로드하고 상점 첫 화면에서 고객 특성에 대한 파일을 바꾸는 문제가 해결되었습니다.
...
* **ACSD-54040**(Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6) - B2B 모듈이 활성화되어 있을 때 *[!UICONTROL Created]* 필드가 비어 있는 문제가 해결되었습니다.
...
* **ACSD-54989**(Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.7) - *[!UICONTROL Enable Purchase Orders]*&#x200B;이 *[!UICONTROL Yes]*&#x200B;으로 설정되고 *[!UICONTROL Purchase Order]*&#x200B;이 *[!UICONTROL No]*&#x200B;으로 설정된 경우 회사 관리자가 주문을 할 수 없는 문제를 수정합니다.
...
* **ACSD-54660**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4 &lt;2.4.6) - *및*&#x200B;까지 GraphQL에서 고객 주문을 정렬하기 위해 새 입력 특성 `sort_field`sort`sort_direction`을(를) 추가했습니다.
...
* **ACSD-53998**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4-p2 &lt;2.4.5 || >=2.4.5-p1 &lt;2.4.7) - 고객 계정에서 로그아웃한 후 **[!UICONTROL Dynamic Block]**&#x200B;을(를) 기반으로 하는 **[!UICONTROL Customer Segment]**&#x200B;가 제대로 작동하지 않는 문제를 해결했습니다.
* **ACSD-53204**(Adobe Commerce 및 Magento Open Source >=2.4.6 &lt;2.4.7) - 수정 *제품을 저장할 수 없습니다.* 끝점을 사용하여 제품 갤러리에 이미지를 추가하도록 동시에 요청할 때 `rest/V1/products/<sku>/media` 오류가 발생했습니다.
...
* **ACSD-52606**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.0 &lt;2.4.7) - 사용자가 *을(를) 클릭하면*&#x200B;주문이 픽업될 준비가 되지 않았습니다&#x200B;**[!UICONTROL Notify Order is Ready for Pickup]**. 오류 메시지가 표시되는 문제를 수정합니다.
...
* **ACSD-53750**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.4 &lt;2.4.7) - 다중 스레드 *다시 인덱싱 중에*&#x200B;끊어진 파이프 또는 닫힌 연결을 수정합니다`catalog_product_price` 오류.
* **ACSD-49843**(Adobe Commerce 및 Magento Open Source의 경우 >=2.3.7 &lt;2.4.0 || >=2.4.1 &lt;2.4.7) - **[!UICONTROL Payment Action]** = **[!UICONTROL Sale]** 설정이 활성화된 온라인 결제 방법으로 주문 항목이 자동 청구된 후 제품 다운로드에 대한 링크를 사용할 수 없는 문제를 해결했습니다.
...
* **ACSD-50887**(Adobe Commerce 및 Magento Open Source >=2.4.0 &lt;2.4.7) - *[!UICONTROL Use in Search Results Layered Navigation]* 옵션을 *예*(으)로 설정하지 않고 제품 특성 속성 *[!UICONTROL Use in search]*&#x200B;을 *예*(으)로 설정할 수 있는 문제를 해결했습니다.
...
* **ACSD-51666**(Adobe Commerce 및 Magento Open Source >=2.3.7 &lt;2.4.7) - *세션이 만료되었습니다. 다시 로그인하십시오.*: 고객이 로그인을 시도한 후에 발생합니다.
...
* **ACSD-52041**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.4.7) - *[오류] [!DNL Page Builder] 오류 메시지가 5초 동안 잠금 해제 없이 렌더링되는 문제를 해결했습니다.*(으)로 편집한 콘텐츠를 저장할 때 [!DNL Page Builder]이(가) Chrome 브라우저에 표시됩니다.
...
* **ACSD-51857**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.0 &lt;2.4.7) - 큰 sales_order 및 `aggregate_sales_report_bestsellers_data` 데이터베이스 테이블에 대한 `sales_order_item` cron 보고서의 성능이 느린 이유는 기본 데이터 쿼리가 작성된 방식 때문입니다.
...
* **ACSD-51735**(Adobe Commerce 및 Magento Open Source >=2.4.4 &lt;2.4.5) - 제품 재고가 *[!UICONTROL Ordered]* 0 *일 때 주문 항목 상태가*&#x200B;으로 잘못 설정되는 문제가 수정되었습니다.
...
* **ACSD-50345**(Adobe Commerce 및 Magento Open Source의 경우 >=2.4.3 &lt;2.4.4 || >=2.4.4-p1 &lt;2.4.6) - 실패한 지급을 제출한 후 Recaptcha v2가 다시 로드되지 않는 문제를 해결했습니다.
* **ACSD-50817**(Adobe Commerce 및 Magento Open Source의 경우 >=2.3.7 &lt;2.4.7) - Cron 작업 `sales_clean_quotes`을 최적화하여 더 빠르게 실행합니다.
* **ACSD-49392**(Adobe Commerce 및 Magento Open Source의 경우 >=2.3.7 &lt;2.4.0 || >= 2.4.1 &lt;2.4.7) - 번들 제품에 대한 부분 환불 후 주문 상태가 마감됨으로 변경되는 문제를 수정합니다.
...
* **ACSD-49129**(Adobe Commerce 및 Magento Open Source >=2.4.2 &lt;2.4.6) - *[!UICONTROL content]* 제품 미디어 API 응답에서 `rest/V1/products/sku/media` 특성(base64 이미지 코드)이 반환되지 않는 문제를 해결했습니다.
...
* **ACSD-49849**(Adobe Commerce 및 Magento Open Source의 경우 >=2.3.7 &lt;2.4.6) - GraphQL을 통해 [!DNL PayPal]에 주문할 때 고객 이메일이 [!DNL PayPal Express] 이메일로 대체된 문제를 해결했습니다.
...
* **ACSD-48216**(Adobe Commerce 및 Magento Open Source의 경우 >=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7) - [!UICONTROL AUTO_INCREMENT] 작업에서 [!UICONTROL inventory_source_item] 표의 [!UICONTROL UPDATE]이 증가하는 문제를 해결했습니다.
* **ACSD-47908**(Adobe Commerce 및 Magento Open Source >=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7) - 체크아웃 중 배송 단계에서 소스 및 수량을 선택할 때 &quot;0보다 작거나 같은 값이 예상됩니다&quot; 오류를 수정합니다.
...
* **ACSD-47937**(Adobe Commerce 및 Magento Open Source 2.4.4 || >=2.4.5 &lt;2.4.6) - 응용 프로그램 수준 캐싱으로 인해 가격 하락 알림이 항상 전송되지 않는 문제를 해결했습니다.
...
* **MDVA-43718**(*Adobe Commerce 및 Magento Open Source >=2.3.0 &lt;2.4.5*) - 수정 *소비자가 %resources에 액세스할 수 있는 권한이 없습니다.사용자 지정 통합에서 공유 카탈로그에 액세스할 때 표시되는* 오류입니다.
...
* **MDVA-43824**(*Adobe Commerce 및 Magento Open Source >=2.3.6 &lt;=2.3.7-p3 || >=2.4.1 &lt;2.4.5*) - 할인 가격으로 주문 취소 시 오류가 표시되는 문제를 수정합니다.
...
* **MDVA-42046**(*Adobe Commerce 및 Magento Open Source >=2.3.4 &lt;=2.3.5-p2 || >=2.4.0 &lt;2.4.5*) - 제품을 업데이트하는 동안 날짜 입력 필드가 있는 제품 특성에 잘못된 값이 지정되는 문제를 해결했습니다.
...
* **MDVA-39993**(*Adobe Commerce 및 Magento Open Source >=2.3.5 &lt;=2.3.7-p2 || >=2.4.0 &lt;2.4.4*) - API를 통해 수행된 인벤토리 변경이 프론트엔드의 제품 페이지에 반영되지 않는 문제를 해결했습니다.
...
* **MDVA-42326**(*Adobe Commerce 및 Magento Open Source >=2.3.6 &lt;=2.3.7-p2 || >=2.4.1 &lt;2.4.4*) - 영구 장바구니가 활성화된 경우에도 세션 시간 제한 후 고객이 체크아웃 시 오류가 발생하는 문제를 해결했습니다.
...
* **MDVA-39482**(*Adobe Commerce 및 Magento Open Source >=2.3.6 &lt;=2.3.7-p2 || >=2.4.1 &lt;2.4.4*) - 미납주문을 사용할 때 &quot;0&quot; 수량으로 제품을 가져올 경우 제품이 품절되는 문제를 해결했습니다.
...
* **MDVA-29400**(*Adobe Commerce 및 Magento Open Source >=2.3.0 &lt;=2.3.7-p1 || >=2.4.0 &lt;=2.4.0-p1*) - [!DNL PayPal Express Checkout]&#x200B;(으)로 수행된 중복 주문 문제를 해결합니다.
...
* **MDVA-37234**(*Adobe Commerce 및 Magento Open Source >=2.3.5 &lt;2.4.0 || >=2.4.1 &lt;=2.4.2-p2*) - 동일한 SKU에 대해 장바구니에 항목을 여러 번 추가(병렬 요청)하면 동일한 장바구니 ID에 대해 중복 라인 항목이 생성되는 문제가 해결됩니다.
...
* **MDVA-39384**(*Adobe Commerce 및 Magento Open Source >=2.3.1 &lt;=2.3.6 || >=2.4.1 &lt;=2.4.3*) - 회사 사용자에 대한 사용자 지정 고객 특성이 저장되지 않는 문제를 해결했습니다.
...
* **MDVA-34948**(*Adobe Commerce 및 Magento Open Source >=2.3.6 &lt;=2.3.6-p1 || >=2.4.0 &lt;=2.4.0-p1*) - `GET_LOCK`와 같이 오래 실행되는 쿼리와 관련된 문제를 수정합니다.
...
* **MDVA-34680**(*Adobe Commerce >=2.3.6 &lt;=2.3.7 || >=2.4.1 &lt;2.4.3*) - 고객 계정에서 만든 시간이 고객 그리드에서 올바르게 필터링되지 않는 문제를 해결했습니다.
...
* **MDVA-37874**(*Adobe Commerce >=2.3.6 &lt;=2.3.7 || >=2.4.1 &lt;=2.4.2-p1*) - *전체 장바구니에 대한 고정 할인 금액*&#x200B;이 두 개 이상의 옵션이 포함된 번들 제품에 잘못 적용되는 문제가 수정되었습니다.
...
* **MDVA-32694** (*Adobe Commerce >=2.3.0 &lt;2.3.6 || >= 2.4.0 &lt;2.4.2*) - 기본 스토어가 아닌 것과 관련된 경우 관리자가 협상 가능 견적에 제품을 추가할 수 없는 문제를 해결했습니다.
...
* **MDVA-20376**(*Adobe Commerce >=2.3.2 &lt;2.3.4*) - 주문 배치 후 로그인한 고객의 경우 *에* customerId = 1`exception.log` 오류가 없는 엔터티가 있는 문제를 수정합니다.
...
* **MDVA-32449**(*Adobe Commerce >=2.3.0 &lt;2.3.6 || 2.4.0 || >=2.4.1 &lt;2.4.2, B2B 확장 포함*) - 주문 내역이 매우 느리게 로드되거나 전혀 로드되지 않는 문제를 해결했습니다.
...
* **MDVA-28651**(*Adobe Commerce >=2.3.0 &lt;2.3.6 || >2.4.0, B2B 확장 포함*) - REST API를 통해 협상가능한 견적을 로드할 때 성능 문제가 발생하는 문제를 해결했습니다.
...
* **MDVA-29954**(*Adobe Commerce >=2.3.0 &lt;2.3.6 || 2.4.0 || 2.4.2 with B2B extension*) - *새 회사 등록 요청* 및 *회사에 연결되었습니다* 전자 메일이 잘못된 주소에서 전송되는 문제가 수정되었습니다.
* **MDVA-28357** (*Adobe Commerce >=2.3.2 &lt;2.3.6 || >=2.4.0 &lt;2.4.1*) - 표준 분석기를 사용자 지정 분석기로 대체하고 [!DNL ElasticSearch] 인덱스의 SKU 필드에 대한 키워드 토큰라이저로 대체하여 와일드카드 검색 쿼리가 하이픈(&quot;-&quot;)이 포함된 SKU에서 작동하도록 합니다.
...
* **MDVA-25028**(*Adobe Commerce >=2.3.2 &lt;2.3.3 || >=2.3.5 &lt;2.3.6*) - 사기 필터가 트리거될 때 [!DNL PayPal Payflow Pro]을 사용하여 수행한 주문이 사기 행위가 의심되는 상태로 설정되지 않은 경우 문제를 수정합니다.
...
* **MDVA-30841**(*Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) - *이(가) 검색 엔진으로 사용된 경우 부울 유형 제품 특성에 대한* No[!DNL Elasticsearch] 값이 계층 탐색에 포함되지 않은 계층 탐색 문제를 해결했습니다.
...
* **MDVA-28409**(*Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) - 데이터베이스의 만료된 따옴표 수가 많을 때 `sales_clean_quotes`메모리 부족&#x200B;*오류로 인해* cron 작업이 실패하는 문제를 해결했습니다.
...
* **MDVA-28656**(*Adobe Commerce >=2.3.1 &lt;2.3.6 || >=2.4.0 &lt;2.4.2*) - 결제 정보가 필요하지 않은 주문이 체결되고(예: 100% 할인) 주문에 대한 송장이 생성된 경우 주문 상태가 완료가 아닌 *마감됨*&#x200B;으로 변경되는 문제를 해결합니다.
...
* **MDVA-30131**(*Adobe Commerce >=2.3.4 &lt;2.3.6 || 2.4.0*) - *이(가) 검색 엔진으로 사용된 경우 부울 유형 제품 특성에 대한* No[!DNL Elasticsearch] 값이 계층 탐색에 포함되지 않은 계층 탐색 문제를 해결했습니다.
