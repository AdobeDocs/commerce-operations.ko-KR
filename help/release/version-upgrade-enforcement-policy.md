...
**업그레이드 적용 날짜** = 일반 사용 가능 날짜 + 3년 정기 지원 + 최대 1년 연장 지원

확장 지원은 각 릴리스 라인에 대한 일반 지원이 끝날 무렵에 발표됩니다.

## 버전 업그레이드 적용 날짜에 나타나는 결과

게시된 업그레이드 시행 날짜에, Adobe은 영향을 받는 릴리스 버전을 아직 실행 중인 모든 Adobe Commerce on Cloud 환경의 유지 관리를 중단하고 서비스 해제 권한을 예약합니다. 버전 업그레이드 적용 날짜까지의 주요 이정표에 대한 사전 알림을 받게 됩니다. Adobe에서는 데이터를 검색할 수 있도록 환경 비활성화 전에 데이터 내보내기 창을 제공합니다.
