...
Adobe Commerce Optimizer 카테고리 머천다이징은 판매자가 카테고리 페이지에서 제품 주문 방법을 제어하는 데 도움이 됩니다. 판매자는 **검색 규칙 및 기본 제품 목록 규칙**&#x200B;과 함께 머천다이징 규칙 워크플로에서 동작을 [카테고리 규칙](https://experienceleague.adobe.com/ko/docs/commerce/optimizer/merchandising/rules/overview)&#x200B;(으)로 구성합니다. 각 규칙은 **지능적인 등급**(해당되는 경우 동작 신호 및 AI), 선택적 **가격 기반 정렬** 및 고정, 증폭, 묻기와 같은 **수동** 작업을 결합할 수 있으므로, 팀은 외부 도구 없이 검색을 조정하고, 프로모션을 실행하고, 전략에 맞게 범주 페이지를 조정할 수 있습니다.
