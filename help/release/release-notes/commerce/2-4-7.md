...
* **RabbitMQ 3.13 지원**. 이 릴리스는 RabbitMQ 3.13의 최신 버전과 호환됩니다. RabbitMQ 3.11 및 3.12와의 호환성은 2024년 8월 및 2024년 12월까지 각각 지원되지만, Adobe에서는 RabbitMQ 3.13에서만 Adobe Commerce 2.4.7을 사용하는 것이 좋습니다. <!-- AC-9659 -->
...
* **주문당 여러 개의 쿠폰 지원**. 판매자는 이제 새로운 [ 구성 옵션을 사용하여 주문당 적용할 수 있는 최대 쿠폰 수를 ](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/sales#multicoupon-settings)구성&#x200B;**[!UICONTROL Maximum number of coupons per order]**&#x200B;할 수 있습니다. 이 값은 기본적으로 1로 설정됩니다. 이제 [REST](https://developer.adobe.com/commerce/webapi/rest/modules/multicoupon/) 또는 [GraphQL](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/apply-coupons/)을 사용하여 장바구니에 여러 쿠폰을 적용할 수 있습니다.

* UPS에서 [API 보안 모델](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/delivery/shipping-carriers/ups)을(를) 업데이트하기 위해 Commerce UPS XML API 게이트웨이가 새 [Commerce UPS REST API](https://developer.ups.com/oauth-developer-guide)&#x200B;(으)로 마이그레이션되었습니다. (UPS는 모든 API에 대해 OAuth 2.0 보안 모델(전달자 토큰)을 구현하고 있습니다.) 이전의 모든 Commerce UPS XML API는 Adobe Commerce 2.4.7 코드 베이스에서 제거되었습니다. <!-- AC-8918 -->
...
GraphQL Application Server는 Adobe Commerce에만 사용할 수 있습니다. Magento Open Source에서는 사용할 수 없습니다. Pro 프로젝트에서 Adobe Commerce Application Server를 사용하려면 [GraphQL 지원 제출](https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/help-center-guide/magento-help-center-user-guide) 티켓을 제출해야 합니다.

>[!NOTE]
>
>GraphQL Application Server는 현재 [[!DNL Amazon Simple Storage Service (AWS S3)]](https://aws.amazon.com/s3/)과(와) 호환되지 않습니다. 현재 [!DNL AWS S3]원격 저장소[에 대해 ](../../../configuration/remote-storage/cloud-support.md)을(를) 사용하는 클라우드 인프라의 Adobe Commerce 고객은 GraphQL Application Server를 사용할 수 없습니다.<!-- ACPT-1840 -->
...
* **에 대한[!DNL Google Pay]** 3DS 지원 - 토큰화되지 않은 [!DNL Google Pay]개의 카드에 대해 3DS 확인 지원이 포함되었습니다. 자세한 내용은 [Braintree 설명서](https://developer.paypal.com/braintree/docs/guides/3d-secure/advanced-options/javascript/v3#verify-google-pay-card-using-3ds)를 참조하세요.
...
* **을(를) 제외한 모든 [!DNL Braintree] 결제 방법에 대해[!DNL Venmo]** GraphQL 지원—더 많은 구성이 GraphQL API에 노출됩니다. 이는 Headless 애플리케이션에 특히 유용합니다.

* **[!DNL Vaulting]계정 영역의 결제**—로그인한 고객은 이제 고객 계정 영역에서 새 신용/직불 카드 및 [ 계정을 ](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/payments/braintree#step-3-complete-the-advanced-settings)저장/저장[!DNL PayPal]할 수 있습니다. 이전에는 고객이 체크아웃에서 트랜잭션을 완료할 때 나중에 사용하기 위해 결제를 저장할 때만 저장/저장할 수 있었지만, 이제 새 트랜잭션을 만들 필요 없이 새 신용/직불 카드 및 PayPal 계정을 저장할 수 있습니다.
...
* **장바구니 돌연변이 만들기의 개선 사항**. 더 이상 사용되지 않는 `createGuestCart` 돌연변이를 대체하기 위해 `createEmptyCart` 돌연변이가 추가되었습니다. 이전에는 `createEmptyCart`을(를) 사용한 경우 장바구니가 손님 대상인지 또는 로그인한 고객 대상인지 확인할 수 없었습니다. <!-- LYNX-224 -->
...
* **주문 취소 지원**. [cancelOrder](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/) 돌연변이를 사용하면 고객이 주문을 취소하고 해당 식별자와 취소 이유를 전달할 수 있습니다. <!-- LYNX-197 -->
   ...
   * `order_cancellation_enabled`의 새 `order_cancellation_reasons.description` 및 `storeConfig` 응답 필드
...
* `quickorder_active`storeConfig[ 및 ](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/)availableStores[ 쿼리에 ](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/available-stores/) 필드를 추가했습니다. 이 필드는 빠른 주문 기능이 활성화되었는지 여부를 나타냅니다.
...
>[!NOTE]
>
>처음으로 여러 REST 끝점 경로에 `V2` 대신 `V1` 문자열이 포함되어 있습니다. 예: `GET /rest/default/V2/carts/{cartId}/coupons`. Commerce 판매자의 경우 이러한 끝점은 `V1` 버전의 끝점을 대체하기 위한 것입니다. 이러한 끝점은 Adobe Commerce에서만 사용할 수 있습니다.<!-- LYNX-208 -->
...
**추가 끝점**
이 릴리스에서는 REST API `GET` 및 `POST V1/products/attributes` 끝점으로 인한 제한을 해결하는 방법을 제공하는 두 개의 새 REST 끝점을 도입했습니다. 이러한 끝점은 `is_filterable` 옵션의 `Filterable(with results)` 및 `Filterable(no results)` 옵션 모두에 대해 **[!UICONTROL Use in Layered Navigation]** 특성에 동일한 값을 반환합니다. `is_filterable` 특성 속성은 `Boolean` 형식이며 이 속성을 `Filterable(no results)`(으)로 설정할 수 없습니다.
...
* `bin/magento setup:install`이(가) 삭제된 후 `app/etc/config.php`이(가) 정상적으로 완료됩니다. 이전에는 설치 중에 누락된 파일이 다시 생성되지 않았으며 Commerce에서 오류가 발생했습니다. [GitHub-37805](https://github.com/magento/magento2/issues/37805)

<!-- AC-8381 -->

* 테이블 및 연결된 `bin/magento setup:upgrade` 인덱서를 모두 설치하는 새 모듈을 설치할 때 `mview`이(가) 성공적으로 실행되도록 리팩터링되었습니다. [GitHub-37304](https://github.com/magento/magento2/issues/37304)
...
* MySQL 메모리 제한 관련 오류 `bin/magento setup:upgrade` 유형으로 `PHP Fatal error:  Allowed memory size of 4294967296 bytes exhausted (tried to allocate 20480 bytes)` 명령이 더 이상 실패하지 않습니다. 다중 선택 특성 마이그레이션이 `setup:upgrade` 동안 메모리를 적게 사용하도록 최적화되었습니다.
...
* `setup:config:set` 매개 변수를 지정하지 않고 `--lock-db-prefix`을(를) 실행하면 `env.php` 파일에서 현재 값이 더 이상 지워지지 않습니다.
...
* 테이블 및 연결된 `bin/magento setup:upgrade` 인덱서를 모두 설치하는 새 모듈을 설치하면 `mview`이(가) 성공적으로 완료됩니다.
...
* 그리드에서 여러 페이지를 선택하면 이제 관리자 **[!UICONTROL Select]** > **[!UICONTROL Content]**&#x200B;에 **[!UICONTROL Pages]** 드롭다운 메뉴 옵션이 표시됩니다. [GitHub-35386](https://github.com/magento/magento2/issues/35386)
...
* 이제 제품에 `GET V1/shipment/` 배송 유형이 설정되어 있을 때 **[!UICONTROL together]** API 경로를 사용하여 번들 제품으로 배송을 만들 수 있습니다. 번들 제품은 이제 배송 유형 속성을 기반으로 유효성이 검사됩니다.
...
* `categoryList` 구성 설정을 사용하도록 설정하면 **[!UICONTROL Show out-of-stock products]** 쿼리가 모든 번들 옵션을 예상대로 반환합니다. 이전에는 품절 옵션이 쿼리 응답에 포함되지 않았습니다.

<!-- ACP2E-1345 -->

* 카탈로그 가격 범위가 `POST V1/products`(으)로 설정된 경우 `website`(으)로 만들어진 번들 제품이 성공했습니다. 이전에는 무결성 제약 조건 위반이 발생했습니다. [GitHub-35595](https://github.com/magento/magento2/issues/35595)
...
* 이제 쇼퍼가 Chrome 자동 완성을 사용하여 배송 주소를 입력한 다음 배송 방법 블록을 다시 로드하기 전에 **[!UICONTROL My billing and shipping address are the same]**&#x200B;다음&#x200B;**을(를) 클릭하면** 확인란이 기본적으로 선택된 상태로 유지됩니다. [GitHub-33725](https://github.com/magento/magento2/issues/33725)

<!-- ACP2E-2005 -->

* 이제 `applied_rule_ids` 테이블의 `quote_item` 값에 적용된 규칙 ID의 올바른 목록이 포함됩니다. 이전에는 이 값에 마지막으로 적용된 규칙 ID만 포함되었습니다.
...
* 이제 장바구니 가격 규칙은 한 배송 방법과 관련된 장바구니 가격 규칙이 스토어에 대해 구성되어 있고 이 배송 방법이 체크아웃 중에 다른 배송 방법으로 변경될 때 예상대로 적용됩니다. 이전에는 `applied_rule_ids` 테이블에서 `sales_order_item` 값이 변경되지 않았으며 장바구니 가격 규칙이 주문에 적용되지 않았습니다.
...
* 이 필드를 비워 두면 Commerce에서 제품 설명 텍스트를 제품 세부 사항 페이지 **[!UICONTROL Meta Description]** 필드에 더 이상 삽입하지 않습니다. 이전에는 제품 설명에 페이지 빌더 또는 다른 HTML 편집기에서 생성된 HTML 코드가 포함되어 있으면 HTML이 Meta **[!UICONTROL Description]** 필드에 표시되었으며 이 필드를 비워 둘 수 없었습니다.
...
* 제품 이미지 `productDetail` 특성이 비어 있는 것으로 설정된 경우 `alt` 쿼리가 이제 null 대신 제품 이름을 반환합니다.
...
* `manage_stock` 값이 `use_config_manage_stock`인 경우 내보낸 제품에 올바른 `1` 값이 있습니다. 이전에는 이 기본값이 올바르지 않았습니다.
...
* 테마 `product_image_white_borders`의 `view.xml`을(를) 사용하지 않도록 설정한 경우 페이지 로드 중에 제품 세부 정보 페이지의 기본 제품 이미지가 더 이상 아래쪽으로 이동하지 않습니다. 이전에는 Fotorama JS의 잘못된 높이 설정으로 인해 페이지 로드 중에 제품 이미지가 아래쪽으로 가시적으로 이동했습니다.
...
* 웹 API를 통해 제품 특성을 만드는 프로세스에 서버측 유효성 검사가 추가되었습니다. **[!UICONTROL Use in Layered Navigation Catalog]** 필드 값이 **[!UICONTROL Catalog Input Type for Store Owner]**, `Text Field`, `Text Area`, `Text Editor` 또는 `Date`인 경우 `Date and Time` 필드를 사용하도록 설정할 수 없습니다. 이전에는 카테고리 및 검색 결과에서 제품이 생략되었습니다.
...
* 이제 구매자는 동일한 장바구니 가격 규칙으로 생성된 두 개의 자동 생성된 쿠폰 코드를 사용할 수 있습니다. 이제 `Uses per Coupon`에게 선택된 쿠폰 유형(쿠폰 유형 =`Auto`의 경우와 유사)이 있을 때 `Specific Coupon` 필드를 사용할 수 있습니다. 이렇게 하면 쇼핑객이 코드가 다른 경우 여러 쿠폰 코드를 동일한 장바구니 가격에 적용할 수 있습니다.
...
* `phpcodesniffer-composer-installer`의 `composer.json`이(가) v1.0.0으로 업그레이드되었습니다. [GitHub-36913](https://github.com/magento/magento2/issues/36913)
...
* 종속성 삽입 컴파일 중에 오류를 방지하기 위해 `$tagGenerators`의 `CompositeTagGenerator`에 기본 빈 배열을 추가했습니다. 이전에는 `bin/magento setup:upgrade` 및 `Magento_DirectoryGraphQl` 모듈이 비활성화되었을 때 `Magento_StoreGraphQl` 명령을 실행하면 오류가 발생했습니다. [GitHub-38165](https://github.com/magento/magento2/issues/38165)
...
* `Magento_CatalogWidget` 이스케이프 함수를 `$block` 이스케이프 함수로 대체하도록 `$escaper` 모듈을 리팩터링했습니다. [GitHub-37107](https://github.com/magento/magento2/issues/37107)
...
* `reponseBody`에서 `responseBody` 변수의 이름을 `app/code/Magento/CatalogRule/Controller/Adminhtml/Promo/Catalog/NewConditionHtml.php`(으)로 변경했습니다.[GitHub-38093](https://github.com/magento/magento2/issues/38093)
...
* `bin/magento setup:upgrade` 변수 데이터 형식으로 인해 `$schemaPatch` 명령이 더 이상 실패하지 않습니다. 이전에는 문자열 값이 `get_class` 함수에 전달되었으므로 오류가 발생했습니다. [GitHub-37545](https://github.com/magento/magento2/issues/37545)
...
* `@author`, `Magento_user-variable-wee` 및 `Magento_Catalog`, `Magento_Customer`, `Magento_Wishlist` `Magento_Review` 모듈을 포함하여, 그리고 프레임워크 전체에서 금지된 `Tax` 태그가 코드 기반에서 제거되었습니다. [GitHub-37245](https://github.com/magento/magento2/issues/37245), [GitHub-37246](https://github.com/magento/magento2/issues/37246), [GitHub-37249](https://github.com/magento/magento2/issues/37249), [GitHub-37263](https://github.com/magento/magento2/issues/37263)
...
* `bin/magento config:show` 변수를 사용하는 `set` 및 `$_ENV` 명령은 이제 동백 문자나 대문자를 포함하는 웹 사이트 및 스토어 코드 사용을 지원합니다.
...
* 이제 이벤트 관련 세부 정보가 `context` 메서드의 두 번째 특성(`log`)으로 전달됩니다. [GitHub-37879](https://github.com/magento/magento2/issues/37879)
...
* 더 이상 사용되지 않는 `precision` 클래스와의 기능 패리티를 유지하기 위해 통화 형식에 대한 `ZendCurrency` 옵션에 대한 지원을 추가했습니다.
...
* `bin/magento setup:config:set` 파일에 잘못된 데이터베이스 사용자 이름이 포함되어 있으면 `app/etc/env.php` 명령이 더 이상 실패하지 않습니다. [GitHub-37409](https://github.com/magento/magento2/issues/37409)
...
* 이제 잠금 공급자의 인스턴스화가 `\Magento\Framework\Lock\Proxy` 클래스를 거칩니다. 이제 잠금 공급자가 `lock` 파일을 통해 `app/etc/env.php` 설정으로 구성되었습니다.
...
* 이제 `value_id`의 `customer_entity_int` 값이 올바르게 증가합니다. 이러한 특성을 포함하는 엔터티를 업데이트할 때 `insert on duplicate key update` 대신 업데이트 쿼리가 실행됩니다. 따라서 EAV 모델 내에서 사용되는 자동 증가 열이 이제 선형 방식으로 증가합니다. 이전에는 삽입 쿼리 실패로 인해 열 자동 증가 프로세스에서 값을 건너뛰었습니다. [GitHub-28387](https://github.com/magento/magento2/issues/28387)
...
* **[!UICONTROL RSS Feed Top Level Category]**&#x200B;을(를) 사용하도록 설정한 경우 범주의 RSS 피드를 요청하는 동안 오류가 더 이상 발생하지 않습니다. 이전에는 `CRITICAL`을(를) 사용하도록 설정할 때 RSS 피드 범주 페이지를 탐색하면 로그 파일에서 Elasticsearch **[!UICONTROL RSS Feed Top Level Category]** 오류가 발생했습니다.
...
* 저장소 보기를 필터링한 다음 관리자 **[!UICONTROL Save]** > **[!UICONTROL Content]**&#x200B;에서 **[!UICONTROL Hierarchy]**&#x200B;을(를) 클릭하면 CMS 계층 구조 필터가 예상대로 작동합니다. 이전에는 **[!UICONTROL Save]**&#x200B;을(를) 클릭하면 페이지가 새로 고쳐졌지만 컨텍스트가 손실되어 선택한 스토어 보기가 더 이상 표시되지 않았습니다.
...
* `send()` 호출 동안 에뮬레이션이 완료되고 오류가 기록되지 않으면 이제 `getInfoBlockHtml()` 호출 동안 에뮬레이션이 시작됩니다. 이전에는 `system.log` 파일이 다음 오류로 인해 플러시되었습니다. `main.ERROR: Environment emulation nesting is not allowed`. [GitHub-35603](https://github.com/magento/magento2/issues/35603)
...
* X-Magento-Cache-Id 헤더가 GraphQL 돌연변이 응답에서 제거되었습니다. 돌연변이는 상태를 변경하는 작업(예: 장바구니에 항목 추가 또는 주문 배치)이므로 캐시되지 않습니다. 이 헤더를 포함시키는 것은 오해의 소지가 있으며 캐싱이 적용되면 잘못된 비헤이비어가 발생할 수 있습니다. 이 변경 사항은 GraphQL 우수 사례와 Adobe Commerce 아키텍처의 핵심 원칙과 일치합니다.
...
* `categories` 응답 값에 null 값이 포함되어 있으면 `items.redirect_code` 쿼리가 더 이상 오류를 반환하지 않습니다. [GitHub-36675](https://github.com/magento/magento2/issues/36675)
...
* `categoryList` 쿼리가 이제 번들 제품의 하위 제품에 대해 `product` 블록의 채워진 `options` 섹션을 예상대로 반환합니다. 이전에는 번들 제품 하위 제품에 대한 정보가 반환되지 않았습니다.
...
* 이제 `ProductCard`에서 `related_products`, `upsell_products` 및 `crosssell_products`을(를) 쿼리할 때 조각 `ProductInterface`을(를) 사용할 수 있습니다. [GitHub-29769](https://github.com/magento/magento2/issues/29769)
...
* 내보내기 프로세스에서 디코딩이 제외되었습니다. CSV 내보내기 파일 텍스트는 가져오기 후에도 변경되지 않습니다. 따라서 다시 가져온 후에는 설명이 변경되지 않습니다. 이전에는 제품 내보내기 및 가져오기 후에 HTML 태그가 페이지 빌더 편집기에서 제거되었습니다.
...
* 저장 시 업데이트 모드의 카탈로그 검색 및 가격 인덱서는 가져오기 후 더 이상 무효화되지 않습니다. 이전에는 새 제품 및 기존 제품을 가져올 때 Elasticsearch Indexer 문서 버전이 변경되었습니다.
...
* 가격 인덱서 성능 문제가 해결되었습니다. 인덱서 성능이 더 이상 점차 저하되지 않습니다. 인덱서는 임시 테이블을 만드는 방법을 변경하여 인덱서 처리 속도를 높일 수 있도록 리팩터링되었습니다. 이제 시간이 더 많이 걸리는 DELETE 문에서 처리하는 대신 이러한 임시 테이블을 다시 만듭니다.
...
* MySQL 8.x를 준수하도록 리팩터링된 `CREATE TEMPORARY TABLE ... LIKE`(으)로 MySQL `CREATE TEMPORARY TABLE`을(를) 대체했습니다. [GitHub-37926](https://github.com/magento/magento2/issues/37926)
...
* `getId()`에서 멤버 함수 `int`을(를) 호출할 때 발생할 수 있는 오류를 해결했습니다. 이 오류는 고객을 관리자에서 볼 때 가끔 예외를 트리거합니다.
...
* `AUTO_INCREMENT` 작업마다 `inventory_source_item` 테이블의 `UPDATE`이(가) 더 이상 증가하지 않습니다. 이전에는 각 업데이트를 통해 이 테이블의 `AUTO_INCREMENT`이(가) 증가했으며, 결과적으로 `AUTO_INCREMENT`에 새 레코드를 추가할 때 `inventory_source_item table` 값이 범위를 벗어났습니다. 따라서 관리자 사용자가 관리자로부터 새 제품을 만들려고 할 때 다음 오류가 발생했습니다. `Numeric value out of range...`.
...
* 이제 Braintree 결제 정보가 매장별로 저장되며, 쇼핑객에게는 쇼핑하는 매장에 저장된 카드 정보만 표시됩니다. 기존에는 체크아웃 과정에서 모든 매장의 저장된 카드 정보가 표시됐다. [GitHub-34253](https://github.com/magento/magento2/issues/34253)

<!-- BUNDLE-3304 -->

* 이제 카드 결제를 위한 보관함이 비활성화될 때 구매자는 Braintree 결제 방법을 사용하여 여러 주소로 배송되는 주문을 확인할 수 있습니다. [GitHub-37615](https://github.com/magento/magento2/issues/37615)
...
* **[!UICONTROL Enable Vault for Card Payments]**&#x200B;이(가) 활성화된 경우 Adobe Commerce에서 지불한 주문에 대한 두 번째 부분 송장을 만들 때 Braintree에서 더 이상 오류가 발생하지 않습니다. 이전에는 판매자가 두 번째 송장을 만들려고 할 때 Adobe Commerce에서 다음 오류가 발생했습니다. `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.

<!-- ACP2E-1229 -->

* 구매자가 결제 주소를 업데이트할 때 Braintree Vault 결제 방법 옵션이 체크아웃 워크플로우에서 더 이상 자동으로 선택 해제되지 않습니다. 이전에는 구매자가 **[!UICONTROL My billing and shipping address are the same]** 단추를 선택 취소하면 결제 방법이 선택 해제되었습니다.

<!-- BUNDLE-3248 -->

* Adobe Commerce 2.4.5-p2에서 업그레이드 후 Braintree을 통해 PayPal을 사용하여 체크아웃할 때 발생하는 문제를 해결했습니다. [GitHub-37242](https://github.com/magento/magento2/issues/37242)
...
* 쇼핑객이 301 리디렉션을 트리거하는 페이지로 이동할 때 페이지의 Cache-Control 헤더에 `200`이(가) 포함되어 있지 않으면 `404` 또는 `private ` 응답만 캐시됩니다. 이전에는 Cache-Control 헤더에 `private`이(가) 있는 HTTP 상태 200이 캐시되었으며, HTTP 상태 301은 Cache-Control 헤더에 `private`이 포함되어 있지 않은 한 캐시되었습니다. [GitHub-36492](https://github.com/magento/magento2/issues/36492)

<!-- AC-9302 -->

* `Magento\Catalog\Controller\Adminhtml\Product\Initialization\Helper`에서 사용하지 않은 `Magento\Catalog\Model\ProductRepository`을(를) 제거했습니다. 이전에 `ProductRepository`에서 `Adminhtml` 컨트롤러인 이 도우미를 로드했습니다. [GitHub-37864](https://github.com/magento/magento2/issues/37864)
...
* **[!UICONTROL Products in Category]** 탭에서 제품을 범주에 할당하는 프로세스가 최적화되었습니다. 이제 제품이 그리드에 완전히 로드될 때까지 **[!UICONTROL Assign]** 탭의 **[!UICONTROL Remove]** 및 **[!UICONTROL Add Product by SKU]** 단추가 비활성화됩니다. 이전에는 관리자 사용자가 수백만 개의 등록된 제품이 포함된 카탈로그에 SKU별 제품을 추가할 수 없었습니다.
...
* **[!UICONTROL Fixed amount discount for whole cart]** 규칙을 적용할 때 **[!UICONTROL Apply to shipping amount]** 가격 규칙의 효과가 이제 제품과 배송 금액 간에 예상대로 비례적으로 배분됩니다. 이전에는 할인 계산이 올바르지 않았습니다.
...
* 이제 보상 포인트는 대변 메모 생성 후 환불될 수 있습니다. 이전에는 가맹점이 대변 메모를 만든 후 보상 포인트를 환불할 수 없었고 주문 상태는 `Completed`이(가) 아닌 `Closed`으로 유지되었습니다.
...
* 이제 **[!UICONTROL Use in Search Results Layered Navigation]** 설정이 **[!UICONTROL Use in Search]**(으)로 설정된 경우에만 **[!UICONTROL Yes]** 제품 특성 속성이 표시됩니다.
...
* 이제 관리 `Search Weight` > `Storefront Properties` > **[!UICONTROL Stores]**&#x200B;에서 **[!UICONTROL Attributes]** 특성의 **[!UICONTROL Product]** 입력 필드 아래에 있는 정보 텍스트가 가능한 값을 설명합니다. [GitHub-36646](https://github.com/magento/magento2/issues/36675)
...
* 이제 주문을 여러 주소로 배송하는 총 금액이 올바르게 계산됩니다. 이전에는 견적에 두 개 이상의 배송 주소가 포함된 경우 총 수집자가 예상대로 `$quote->getShippingAddress()`까지 배송하는 대신 `$shippingAssignment`에 제공된 주소의 배송 금액을 업데이트했습니다. [GitHub-37283](https://github.com/magento/magento2/issues/37283)
...
* **[!UICONTROL Shipping Policy]**&#x200B;을(를) 사용하고 사용자 지정 배송 정책을 사용할 수 있는 경우 이제 **[!UICONTROL Apply custom Shipping Policy]** 텍스트 상자가 관리 저장소 보기 범위에 표시됩니다. [GitHub-36425](https://github.com/magento/magento2/issues/36425)

<!-- AC-7550 -->

* 이제 `cart` 쿼리가 `selected_shipping_method` 통신사 및 메서드 코드 중 하나와 일치하는 `available_shipping_methods` 값을 반환합니다. 이전에는 선택한 배송 방법 중 하나에 밑줄이 있는 경우 이러한 통신사/방법 코드가 일치하지 않았습니다. [GitHub-36606](https://github.com/magento/magento2/issues/36606)
...
* 처리할 스테이징 업데이트가 없으면 `staging.synchronize_entity_period` 테이블의 `magento_bulk` 소비자 큐가 더 이상 증가하지 않습니다. 이전에는 `cron`이(가) 매분마다 실행되도록 구성되면 `staging.synchronize_entity_period` 소비자와 관련된 새 레코드가 매분마다 이 테이블에 추가되었습니다.
...
* **[!UICONTROL Regular Price]**&#x200B;이(가) 세금을 포함하도록 설정되어 있을 때 **[!UICONTROL Special Price]** 값을 가진 제품에 대해 사용자 지정 옵션을 선택하면 이제 Commerce에 **[!UICONTROL Price Display Settings]** 값에 대한 올바른 값이 표시됩니다.
...
* `Magento\GraphQl\Catalog\ProductSearchTest::testFilterProductsBySingleCategoryId` 쿼리 응답에서 임의의 범주 순서를 처리하기 위해 `products` 테스트를 리팩터링했습니다.
...
* 여러 라인 사용자 정의 가능 옵션이 이제 송장, 선적 및 대변 메모 PDF 문서에서 제대로 렌더링됩니다. 이전에는 여러 줄 텍스트 앞에 예기치 않은 페이지 구분이 있으며 텍스트가 페이지를 초과할 경우 텍스트가 잘렸습니다.
...
* `crossorigin` 요소의 `linkTypes`에 `HEAD` 특성을 추가했습니다.[GitHub-32509](https://github.com/magento/magento2/issues/32509)
...
* `Integration not secure` 및 **[!UICONTROL System]** 필드가 비어 있는 경우 Commerce에서 더 이상 관리자 **[!UICONTROL Extensions]** > **[!UICONTROL Integrations]** > **[!UICONTROL Callback URL]**&#x200B;에 **[!UICONTROL Identity link URL]** 경고를 표시하지 않습니다. [GitHub-36407](https://github.com/magento/magento2/issues/36407)
...
* `POST V1/guest-carts`이(가) 비활성화되어 있으면 **[!UICONTROL Allow Guest Checkout]** 호출에서 더 이상 게스트 주문을 전송하지 않습니다.

<!-- ACP2E-1336 -->

* 이제 `Received`이(가) 활성화되지 않은 경우 `AsyncOrder` 상태의 관리자에서 주문을 열 수 있습니다. 이전에는 주문 상태가 `POST V1/orders`에 의해 변경된 경우 책임자로부터 주문 세부 사항 페이지를 열 수 없었습니다.

<!-- ACP2E-1526 -->

* 이제 Adobe Commerce에서 `content` 제품 미디어 API 응답에 `GET V1/products/sku/media` 특성(base64 이미지 코드)을 성공적으로 반환합니다. `content` 특성은 이전에 응답에 나타나지 않았습니다.
