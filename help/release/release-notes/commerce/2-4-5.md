...
* Adobe Systems Commerce 캐시가 플러시되고 `use_stale_cache` 활성화되면 전체 페이지 캐시가 더 이상 관리에서 비활성화된 것으로 표시되지 않습니다.

<!-- AC-2410-->

* 새로 만들기 Relic 배포 마커는 이제 캐시가 플러시될 때 예상대로 작동합니다. [GitHub-32649](https://github.com/magento/magento2/issues/32649)
...
* 제품이 품절된 경우 `cart` 쿼리가 더 이상 null 응답을 반환하지 않습니다. 오류 메시지가 포함된 새 `errors` 요소가 응답에 도입되었습니다. 이전에는 품절 제품으로 쿼리를 실행했을 때 Adobe Commerce이 응답의 `items` 섹션 아래에 `null` 값을 표시했습니다. [장바구니 쿼리](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/queries/cart/)를 참조하세요.
...
* 구성 가능한 제품에 대한 속성이 있는 `Quantity` 범주 규칙은 이제 스테이징이 구현될 때 올바르게 작동합니다. 이전에는 스테이징 업데이트로 인해 제품의 테이블 값이 다른 경우 `row_id` `entity_id` `catalog_product_entity` Visual Merchandiser **Match 규칙** 기능이 제품을 올바르게 필터링하지 않았습니다.

<!-- ACP2E-13-->

* 이제 관리자는 관리 슬라이드 패널에서 쇼핑객 장바구니의 구성 가능한 제품 옵션을 변경할 수 있습니다. 이전에는 슬라이드 패널이 제대로 작동하지 않았습니다.

<!-- AC-1705-->

* 이제 하위 제품에 대한 변경 사항이 저장될 때 구성 가능한 제품 상위에 대해 페이지 캐시가 예상대로 지워집니다. 이전에는 캐시가 지워지지 않았기 때문에 상점 구성 제품 페이지 에서 변경 사항이 선택되지 않았습니다. [GitHub-34508](https://github.com/magento/magento2/issues/34508)
...
* 이제 카탈로그 가격 범위가 **웹 사이트**&#x200B;에서 **글로벌**(으)로 변경된 후 다중 스토어 배포의 제품 세부 정보 페이지 및 상점 검색에서 제품 가격이 동일합니다. 이전에는 카탈로그 Search 결과 페이지 글로벌 가격을 표시&#x200B;**하고 제품 세부 정보 페이지 웹 사이트** 가격을 표시 **&#x200B;**&#x200B;했습니다. [GitHub-34074](https://github.com/magento/magento2/issues/34074)

<!-- ACP2E-316-->

* 이제 계층화된 탐색은 가격 탐색 단계 계산&#x200B;**이 수동으로** 설정된 **경우**&#x200B;예상대로 가장 높은 가격의 제품을 표시합니다.
...
* 이제 `updateCartCurrency` 함수가 장바구니 개체 내의 개체 대신 문자열을 설정합니다. 이전에는 `updateCartCurrency` 함수가 장바구니 개체 내에서 문자열 대신 개체를 설정했기 때문에 Adobe Commerce에서 `getQuote`을(를) 사용하여 견적을 로드하지 않았습니다. [GitHub-34199](https://github.com/magento/magento2/issues/34199)
...
* 범주 목록을 가져올 때 쿼리가 `categories` 더 이상 예외를 throw하지 않으며, 그 중 하나에는 파일 시스템에서 찾을 수 없는 이미지가 포함되어 있습니다. 이전에는 Adobe Systems Commerce에서 이 예외 `Category image not found`가 발생했습니다. [GitHub-34266](https://github.com/magento/magento2/issues/34266)

<!-- AC-2368-->

* 이제 쿼리가 `products` 예상대로 집계로 반환 `category_uid` 됩니다. [GitHub-32557](https://github.com/magento/magento2/issues/32557)

<!-- AC-2373-->

* 이제 변형은 `updateCartItems` 제품 재고가 최대 재고 금액에 도달했을 때 예상대로 제품을 제거합니다. [GitHub-30220](https://github.com/magento/magento2/issues/30220)
...
* GraphQL 요청 후 Adobe Systems Commerce 세션이 더 이상 종료되지 않습니다. 이전에는 `ClearCustomerSessionAfterRequest` 플러그 인이 쇼핑객을 로그아웃했습니다. [GitHub-34550](https://github.com/magento/magento2/issues/34550)
...
* 이제 끝점이 `GET /V1/returnsAttributeMetadata` 엔터티 기본 속성 집합 ID가 기본 설치 ID와 다를 때 `rma_item` 올바르게 작동합니다. 이전에는 이러한 ID가 다른 경우 이 엔드포인트가 빈 결과를 반환했습니다.

<!-- ACP2E-247-->

* **제품 편집 페이지 에서 RMA를 활성화하는 데 사용되는 Use Default(기본값** 사용) 확인란이 이제 Default Store에 대해 예상대로 작동합니다. 이전에는 제품이 저장되면 바로 확인란이 지워졌습니다.
...
* 이제 엔드포인트를 `POST /V1/salesRules/` 통해 생성된 장바구니 가격 규칙은 상태를 비활성화에서 활성화로 변경한 후에도 기존 쿠폰 코드 값을 유지합니다. [GitHub-35298](https://github.com/magento/magento2/issues/35298)

<!-- AC-2764-->

* 이제 끝점을 `POST /V1/salesRules/` 통해 생성된 장바구니 가격 규칙에 유효한 `from_date` 값과 `to_date` 값이 포함됩니다. [GitHub-35265](https://github.com/magento/magento2/issues/35265)
...
* 이제 위시리스트 페이지에서 품목 수량을 업데이트하면 제품 세부 사항 페이지의 수량이 예상대로 업데이트됩니다. 이제 Adobe Systems Commerce가 제품 URL에서 업데이트된 값을 선택하고 위시리스트 자체에서 제품 세부 사항 페이지 필드를 채웁니다 `qty` .

## 알려진 문제

**문제**: 2.4.5 이전 제품 버전에서 Adobe Systems Commerce 2.4.6 이상으로 업그레이드한 후 단계적 판매 규칙 레이블이 일치하지 않을 수 있습니다. **해결 방법**: 메타데이터 업데이트 단계 전에 업그레이드 중에 ACSD-50625_2.4.5-P1.patch를 적용. 기술 자료 문서에서 업그레이드 &lt; 2.4.5[&#128279;](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/known-issues-patches-attached/salesrule-label-issues-when-upgrade-from-versions-under-2.4.5.html?lang=ko) 시 salesRules 레이블 문제를 참조하십시오.
...
**문제**: Adobe Commerce 2.4.5를 새로 설치(업그레이드하지 않음)한 후 [!DNL RabbitMQ] 인프라 요소(교환, 큐)가 초기화되지 않을 수 있습니다. 이 [!DNL RabbitMQ]의 잘못된 구성은 비동기 작업이 실행되거나 성능 프로필이 생성될 때 치명적인 오류가 발생할 수 있습니다. 클라우드 배포는 영향을 받지 않습니다. **해결 방법**: 온-프레미스에서 Adobe Commerce 2.4.5를 새로 설치하는 판매자는 `bin/magento setup:config:set`을(를) 사용하여 [!DNL RabbitMQ]을(를) 구성하거나 [!DNL RabbitMQ] 구성 옵션을 `setup:install` 명령에 전달하여 Adobe Commerce을 설치한 후 `bin/magento setup:upgrade --keep-generated`을(를) 실행해야 합니다. [[!DNL RabbitMQ]](../../../installation/prerequisites/rabbitmq.md)을(를) 참조하십시오.
