...
* 제품이 품절된 경우 `cart` 쿼리가 더 이상 null 응답을 반환하지 않습니다. 오류 메시지가 포함된 새 `errors` 요소가 응답에 도입되었습니다. 이전에는 품절 제품으로 쿼리를 실행했을 때 Adobe Commerce이 응답의 `items` 섹션 아래에 `null` 값을 표시했습니다. [장바구니 쿼리](https://devdocs.magento.com/guides/v2.4/graphql/queries/cart.html)를 참조하세요.
...
* `int`이(가) 검색 가능한 백엔드 `type` 특성으로 구성된 경우 Elasticsearch 쿼리가 예상대로 작동합니다. 이전에는 Adobe Commerce에서 `Elasticsearch\Common\Exceptions\BadRequest400Exception` 예외를 throw했습니다.
