...
* `Braintree_disabled_partial_capture_for_applepay_googlepay.zip`. 이 패치는 결제 방법으로 Apple Pay 또는 Google Pay through Braintree을 사용할 때 사용자가 부분 송장을 생성할 수 없는 문제를 해결합니다. [Adobe Commerce 2.4.4: 부분 송장을 만들 수 없음](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) 기술 자료 문서를 참조하십시오.
...
* **Adobe Commerce 온-프레미스 배포**: Adobe Commerce 2.4.5가 테스트되었고 Elasticsearch 7.17과 호환되는지 확인했습니다(제한 사항이 있는~7.17.0). Adobe Commerce 온-프레미스를 호스팅하는 가맹점은 Elasticsearch 또는 OpenSearch 1.2를 사용할 수 있습니다. <!-- AC-2425-->
...
* USPS 배송을 위한 기본 게이트웨이 URL 대신 `http`사용하도록 업데이트 `https` 되었습니다.<!-- AC-2426-->

* `Froogaloop` 라이브러리가 Vimeo `Player.js` 라이브러리(2.16.4)로 대체되었습니다.<!-- AC-2527 2262-->

* (NPM) 라이브러리가 `grunt-eslint` 최신 버전으로 업그레이드되었습니다. <!-- AC-2700-->

* `jQuery Storage` 라이브러리가 로 대체`julien-maurel/js-storage`되었습니다.<!-- AC-2627-->

* `php-cs-fixer` 및 `phpcs` 정적 코드 분석 도구는 이제 PHP 8.x와 호환됩니다.<!-- AC-2259-->
...
테이블의 기본 인덱스 수를 3개에서 1개로 줄임으로써 기본 인덱스 성능이 개선되었습니다 `catalog_product_index_price_tmp` . 이 향상된 기능은 공유 카탈로그에서 발생하는 eSKU 곱셈을 줄여 가격 인덱스에서 생성된 레코드 수를 줄였습니다. 색인 생성 시간이 크게 단축되었습니다. <!-- MCP-653 MC-43342-->
...
이제 판매자는 고객이 Adobe Systems Commerce 체크아웃 중에 보증 문서, 구매 계약서 및 사용 약관과 같은 사용자 지정 계약서에 전자적으로 서명하도록 허용할 수 있습니다.
...
* Braintree는 KOUNT 사기 방지 통합을 중단했습니다. Adobe Systems Commerce 코드베이스에서 제거되었습니다.
...
* 결제 작업 과정의 과금 단계에서 팝업검색 주소로 인해 더 이상 DOM 오류가 발생하지 않습니다.

<!-- ACP2E-368-->

* 이제 변형이 `addProductsToCart` 여러 제품에서 올바르게 작동합니다. 이전에는 이 쿼리가 정확한 부분합이 있는 첫 번째 제품을 반환했지만 다른 제품에 대해서는 부분합 0을 반환했습니다.

<!-- ACP2E-245-->

* 이제 SKU 에 의해 추가된 제한된 제품에 대한 권한 예외가 처리됩니다. 이제 쇼핑객에게 상점에서 적절한 메시지가 제공되고 오류 테이블의 수량 필드가 비활성화됩니다. 이전에는 Adobe Commerce에서 다음과 같은 예외가 발생했습니다. `There has been an error processing your request`.
...
* 글로벌 계정 공유를 사용하는 쇼핑객은 게스트 체크아웃이 비활성화된 경우 더 이상 다중 사이트 배포 시 보조 웹 사이트에 다시 로그인할 필요가 없습니다. 이제 쇼핑객 고객이 하위 도메인으로 이동할 때 고객 데이터가 로드됩니다. 쇼핑객 사용자에게 더 이상 다시 로그인하라는 메시지가 표시되지 않으며 이전 장바구니 내용물이 표시됩니다.
...
* `cart` 쿼리는 제품 재고가 없을 때 더 이상 null 응답을 반환하지 않습니다. 오류 메시지가 포함된 새 `errors` 요소가 응답에 도입되었습니다. 이전에는 품절 제품으로 쿼리를 실행했을 때 Adobe Commerce이 응답의 `items` 섹션 아래에 `null` 값을 표시했습니다. [장바구니 쿼리](https://developer.adobe.com/commerce/webapi/graphql/queries/cart.html)를 참조하세요.
...
* 이제 쇼핑객은 허용 국가&#x200B;**필드에서**&#x200B;옵션을 선택하지 않은 경우 장바구니에 제품을 추가할 수 있습니다.
...
* 가맹점은 이제 **환불 배송(포함)이 포함된 대변 메모를 만들 수 있습니다. Tax)** 가 -0.01로 설정되고 이제 이 금액을 0으로 설정할 수 있습니다. 이전에는 이러한 조건에서는 대변 메모를 생성할 수 없었습니다.

### 장바구니 가격 규칙

<!-- ACP2E-680-->

* 이제 속성 범위가 `Parent Only` 장바구니 규칙 조건에서 제대로 사용됩니다.

### 카탈로그

<!-- ACP2E-758-->

* 이제 스토어 보기를 위해 `PUT /V1/products/` 제품 이름을 업데이트할 때 제품 URL 키가 변경되지 않은 상태로 유지됩니다. 이전에는 새 제품 이름을 기반으로 새 URL 키가 생성되고 해당 스토어 보기의 URL 키를 재정의한 제품에 할당되었습니다.
...
* 동적 번들 속성이 이제 질량 속성 업데이트 페이지 에서 올바르게 업데이트됩니다. 이전에는 **다이내믹 SKU** 속성이 대량 업데이트 페이지에서 비활성화되더라도 예&#x200B;**균일 로**&#x200B;설정된 상태로 유지되었습니다.
...
* 구성 가능한 제품에 대한 `Quantity` 특성이 있는 범주 규칙은 이제 스테이징이 구현될 때 올바르게 작동합니다. 이전에는 스테이징 업데이트로 인해 `catalog_product_entity` 테이블의 `row_id` 및 `entity_id`에 대한 값이 다를 때 시각적 머천다이저 **규칙별로 제품 일치** 기능이 제품을 제대로 필터링하지 않았습니다.
...
* 이제 제품 목록이 관리에서 올바로 렌더링됩니다. 이전에는 제품 목록이 렌더링되지 않아 Adobe Systems Commerce에 다음 오류가  `Item (Magento\Catalog\Model\Product\Interceptor) with the same ID "<ID>" already exists`표시되었습니다. [GitHub-33145](https://github.com/magento/magento2/issues/33145)

<!-- AC-1214-->

* 카탈로그 가격 범위가 웹사이트에서&#x200B;**글로벌**&#x200B;로 **변경**&#x200B;된 후 멀티 스토어 배포의 제품 상세 정보 페이지 및 매장 검색에서 제품 가격은 이제 동일합니다. 이전에는 카탈로그 검색 결과 페이지에 **글로벌** 가격이 표시되었으며 제품 세부 정보 페이지에 **웹 사이트** 가격이 표시되었습니다. [GitHub-34074](https://github.com/magento/magento2/issues/34074)
...
* 이제 [범주 페이지 매김&#x200B;**기억]이 활성화된 경우**&#x200B;카테고리 내에 표시되는 페이지 당 제품 제한을 변경할 수 있습니다. 이전에는 쿠키 `form_key` 와 UI `form_key` 가 달라서 Adobe Systems Commerce에 다음 오류가  `Invalid Form Key. Please refresh the page`표시되었습니다.

<!-- ACP2E-807-->

* 이제 EAV 인덱서는 가능한 성능 문제를 방지하기 위해 제품 ID를 유형 `int` 으로 처리합니다.

<!-- AC-2597-->

* 이제 이름에 &#39;.&#39;가 포함된 새 이미지를 추가하면 새 제품 캐시가 성공적으로 생성됩니다.  을 추가한 다음, 제품을 저장하고 이미지 캐시를 정리합니다. [GitHub-32699](https://github.com/magento/magento2/issues/32699)
...
* 판매자는 더 이상 엔티티 편집 페이지 에서 활성 예약된 업데이트를 편집할 수 없습니다.
...
* 이제 쿼리는 `products` Display Out Of Stock **구성 설정을 정확하게 반영**&#x200B;하는 구성 가능한 제품의 가격을 검색합니다. 이전에는 쿼리가 정확한 가격을 반환하지 않았습니다.
...
* 이제 다중 선택 속성이 제품 편집 중에 올바르게 저장됩니다. 이전에는 제품을 저장할 때 Adobe Systems Commerce가 선택한 속성뿐만 아니라 선택되지 않은 속성에 대한 기본 옵션도 저장했습니다.
...
* 이 `bin/magento setup:config:set` 명령은 에서 이미 설정된 캐시 ID 접두어를 `app/etc/env.php`더 이상 무시하지 않습니다.

<!-- ACP2E-517-->

* `bin/magento setup:static-content:deploy -s compact` 이제 명령에 자식 테마의 스타일이 예상대로 포함됩니다. 이전에는 배포 후 상점에 테마 CSS 파일이 없었습니다.

<!-- AC-1999-->

* 요소에 닫는 슬래시가 사용 `void` 되는지 확인하기 위해 새로운 스니프가 추가되었습니다.

<!-- AC-2896-->

* 제품에 새 소스를 할당하고 수량 변경 후 더 이상 Adobe Systems Commerce에서 SQL 오류가 발생하지 않습니다. [GitHub-35262](https://github.com/magento/magento2/issues/35262)

<!-- AC-401-->

* 이제 속성 정렬 순서가 `di.xml` 업데이트 후 파일에 지정된 대로 작동합니다.

<!-- AC-1716-->

* 이 `updateCartCurrency` 함수는 이제 장바구니 개체 내의 개체 대신 string을 설정합니다. 이전에는 함수가 `updateCartCurrency` 장바구니 개체 내에 문자열 대신 개체를 설정했기 때문에 Adobe Systems Commerce에서 `getQuote` 사용하여 따옴표를 로드하지 않았습니다. [GitHub-34199](https://github.com/magento/magento2/issues/34199)

<!-- AC-2428-->

* 사용 중단 알림은 다음`\DateTimeFormatter::formatObject()` 문제로 인해 단위 테스트에서 더 이상 발생하지 않습니다. 이 메서드는 이제 에 대한 `$format`숫자 값에서 예상대로 작동합니다.

<!-- AC-2893-->

* Adobe Systems Commerce에서 더 이상 관리 오류가 표시되지 `preg_replace()` 않습니다. 세 번째 인수 (`$subject`)는 이제 가 아닌 `bool`유형 `array|string` 입니다.
...
* 이제 상점으로 이동할 `/checkout/sidebar/updateItemQty/?item_qty=error` 때 Adobe Systems Commerce에서 500 오류 대신 404 오류를 반환합니다. 이전에는 이 오류가 발생했습니다  `Warning: A non-numeric value encountered in /vendor/magento/module-checkout/Controller/Sidebar/UpdateItemQty.php on line 69`. [GitHub-34380](https://github.com/magento/magento2/issues/34380)

<!-- ACP2E-847-->

* SSL 구성 없이 AMPQ 연결을 구성할 때 Adobe Systems Commerce가 더 이상 오류를 트리거 `trim(): Passing null to parameter #1 ($string) of type string is deprecated` 하지 않습니다.

<!-- AC-2414-->

* 데이터베이스 정의를 `long blob`.(으)로 수정했습니다`longblog`. [GitHub-35108](https://github.com/magento/magento2/issues/35108)

<!-- AC-1526-->

* 이제 작은따옴표가 포함된 녹아웃 텍스트를 번역할 수 있습니다. [GitHub-34319](https://github.com/magento/magento2/issues/34319)

<!-- AC-2505-->

* A `TypeError` 가 `magento2/app/code/Magento/Security/Model/AdminSessionsManager.php` 에서 로 `string`수정 `int` 되었습니다.  [GitHub-34415](https://github.com/magento/magento2/issues/34415)

<!-- AC-2511-->

* `.htpasswd` 이(가) 구성 파일에서 금지된 위치에 `nginx` 추가되었습니다. [GitHub-35150](https://github.com/magento/magento2/issues/35150)

<!-- AC-2013-->

* 에 를 추가하여 `Magento_Ui/js/core/app` `deps` `app/code/Magento/Ui/view/frontend/requirejs-config.js`카테고리 제품 목록 페이지의 로드 시간이 개선되었습니다. [GitHub-34847](https://github.com/magento/magento2/issues/34847)

<!-- AC-2101-->

* 이제 메서드가 `ProductRepository.php:get` 캐시 키를 한 번 반환합니다. 이전에는 두 번 반환되었습니다. [GitHub-34958](https://github.com/magento/magento2/issues/34958)

<!-- AC-2507-->

* cron 작업에 대한 예외 처리기에서 만든 새 예외에 오류 메시지를 추가했습니다. [GitHub-34941](https://github.com/magento/magento2/issues/34941)
...
* 최대 세션 크기를 0(Admin **Store** > 설정&#x200B;****> **Configuration - 고급**)으로 설정해도 더 이상 관리자가 로그아웃되지 않습니다. [GitHub-35312](https://github.com/magento/magento2/issues/35312)
...
* 이제 iOS 디바이스에서 이름, 중간 또는 성에 기울어진 아포스트로피(&#39;)를 사용하여 고객 계정 만들 수 있습니다. 이전에는 직선 아포스트로피만 허용되었으며 iOS 11+ 기본 경사 아포스트로피를 `Name is not valid!` 사용하면 오류가 발생했습니다.
...
* 역할이 제한된 사용자에게는 더 이상 새 모듈에 대한 액세스 권한이 자동으로 부여되지 않습니다.
...
* 판매자는 이제 제품 비교에 계층 가격 특성(`tier_price`)을 추가할 수 있습니다. 이전에는 이 속성에 대해 상점&#x200B;**에서 비교 가능 설정이 활성화된 경우**&#x200B;제품 비교 페이지 작동이 중단되었습니다. [GitHub-35244](https://github.com/magento/magento2/issues/35244)
...
* 불필요한 `csp_whitelist.xml` 파일을 제거했습니다. [GitHub-30607](https://github.com/magento/magento2/issues/30607)

### 기프트 카드

<!-- AC-560-->

* `products` 이제 쿼리는 `gift-card` URL 키가 있는 선물 카드 제품이 포함된 제품 데이터를 올바르게 반환합니다.

### 그래프QL

<!-- AC-2361-->

* 누락된 `price_range` 속성이 GraphQL `BundleItemOption` 유형에 추가되었습니다. [GitHub-35010](https://github.com/magento/magento2/issues/35010)

<!-- AC-2367-->

* 쿼리는 `products` Use in Search Results Layered Navigation( **검색 결과** 에서 사용) 계층화된 탐색 설정을 사용하지 않도록 설정한 경우 더 이상 특성을 집계로 반환하지 않습니다. [GitHub-33318](https://github.com/magento/magento2/issues/33318)

<!-- AC-1760-->

* `price_including_tax` 필드에 필드가 추가되었습니다`CartItemPrices`. [GitHub-29057](https://github.com/magento/magento2/issues/29057)

<!-- AC-2520-->

* 및 `new_to_datefields` in `ProductInterface` 은 `new_from_data` 더 이상 사용되지 않습니다. [GitHub-34783](https://github.com/magento/magento2/issues/34783)
...
* 이제 쿼리는 `urlResolver` 여러 홈 페이지의 동일한 식별자를 가질 때 경로 구분 기호(/)를 올바르게 확인합니다. 이전에는 쿼리가 구분 기호를 확인하지 않고 null을 반환했습니다. [GitHub-33615](https://github.com/magento/magento2/issues/33615)
...
* Adobe Commerce 세션은 GraphQL 요청이 수행된 후 더 이상 종료되지 않습니다. 이전에는 플러그인 이 `ClearCustomerSessionAfterRequest` 쇼핑객 로그아웃을 했습니다. [GitHub-34550](https://github.com/magento/magento2/issues/34550)

<!-- AC-818-->

* 쿼리는 `products` 비활성화된 변형의 영향을 받는 구성 가능 제품에 대한 값을 더 이상 반환 `price_range` 하지 않습니다. [GitHub-33629](https://github.com/magento/magento2/issues/33629)

<!-- ACP2E-634-->

* 이제 Display Out of Stock Products(재고 부족 제품&#x200B;**표시)**&#x200B;구성 설정이 활성화된 경우 쿼리 응답의 `products` 구성 가능한 제품 가격 범위가 올바르게 계산됩니다. 이전에는 비활성화된 옵션이 최소 및 최대 가격 계산에 계정 고려되었습니다.

<!-- ACP2E-531-->

* 이제 쿼리는 `products` 위치별로 정렬할 때 올바르게 필터링된 여러 범주를 반환합니다.
...
* 이제 쿼리는 `categoryList` 쿼리된 스토어가 헤더에 지정된 경우 쿼리된 스토어의 루트 범주를 반영하는 결과를 반환합니다. 이전에는 헤더에 다른 저장소가 지정되었더라도 기본 루트 범주의 범주가 결과에 포함되었습니다.

<!-- AC-1955-->

* 쿼리는 `products` Use in Search Results Layered Navigation( **검색 결과에서 사용) 계층화된 탐색** 설정을 사용하지 않도록 설정한 경우 더 이상 특성을 집계로 반환하지 않습니다. [GitHub-33318](https://github.com/magento/magento2/issues/33318)
...
* `generateCustomerTokenAsAdmin` 돌연변이는 이제 예상대로 고객 토큰을 검색합니다. 이전에는 토큰이 반환되지 않았으며 다음 오류가 반환 `Customer email provided does not exist`되었습니다.

<!-- AC-2371-->

* 이제 사용자 지정 `type` 제품 속성이 정의된 경우 GraphQL 스키마가 유효합니다. 이전에는 제품 유형의 속성을 사용자 지정 `type` 속성으로 덮어썼기 때문에 `type` 스키마가 잘못된 상태였습니다. [GitHub-34929](https://github.com/magento/magento2/issues/34929)

<!-- AC-2797-->

* , `createCustomerV2`, `updateCustomer`또는 `updateCustomerV2` 변형으로 `createCustomer`추가되거나 업데이트된 고객은 이제 활성 뉴스레터 가입으로 추가됩니다. 이전에는 요청에 적절한 입력 매개 변수가 포함되어 있어도 뉴스레터 구독을 취소했습니다. [GitHub-33599](https://github.com/magento/magento2/issues/33599)
...
* 이제 Adobe Commerce은 보고서를 내보낼 때 열을 필터링하기 전에 사용자 지정 보기를 확인합니다. 이전에는 내보낸 보고서가 계정 사용자 지정 보기를 고려하지 않았고 내보낸 열이 올바르지 않았습니다.

<!-- ACP2E-502-->

* 이제 Adobe Systems Commerce에서 파일 이름이 긴 이미지를 성공적으로 가져옵니다. 이전에는 Adobe Systems Commerce에서 이미지를 가져오지 않고 다음 오류가 `Imported resource (image) could not be downloaded from external resource due to timeout or access permissions in row(s):`발생했습니다.
...
* 물음표(?) 포함 번들 제품 이제 QueryBuilder 내부 `populateExistingOptions` 메서드의 개선으로 인해 옵션 제목을 성공적으로 가져올 수 있습니다. 옵션 제목도 올바르게 표시됩니다. 이전에는 초기 가져오기 후 연속적인 가져오기로 인해 동작이 손상되고 옵션이 두 배로 늘어났습니다. 쇼핑객도 장바구니에 제품을 추가할 수 없습니다.
...
* 이제 새 레코드를 삽입하기 전에 테이블의 기존 레코드 `catalog_url_rewrite_product_category` 가 삭제됩니다. 이전에는 다중 스토어 제품을 가져오는 `SQLSTATE[23000]: Integrity constraint violation`동안 다음 오류가 발생했습니다. [GitHub-34210](https://github.com/magento/magento2/issues/34210)
...
* 이제 고객, 고객 주소 및 주문 작업이 관리 작업 보고서에 올바르게 기록됩니다. 이전에는 Adobe Systems Commerce가 구성 설정에 핸들러가 지정되지 않은 경우 `postDispatch` 작업을 기록하지 않았습니다.
...
* 이제 스토어 크레딧 기능이 비활성화된 경우 보상 포인트를 환불할 수 있습니다.

<!-- ACP2E-757-->

* 보상 포인트가 적용될 때 결제 성능이 개선되었습니다. `entity`이(가) null이면 `magento_reward_history` 테이블 쿼리가 필수 항목이 아니므로 데이터베이스 쿼리가 실행되지 않습니다.
...
* **이제 >**&#x200B;구성&#x200B;**>**&#x200B;서비스&#x200B;**저장** 페이지 리소스 탭에서 리소스 액세스가 사용자 지정&#x200B;**으로**&#x200B;설정된 경우 Adobe Systems Commerce Web API 정보가 예상대로 표시됩니다. [GitHub-35506](https://github.com/magento/magento2/issues/35506)

### 검색 {#search-heading}

<!-- ACP2E-645-->

* 계층화된 탐색에서 색상 견본별로 제품을 필터링하면 수정 후 제품에 대한 올바른 이미지가 표시됩니다.

<!-- ACP2E-615-->

* 이제 Elasticsearch 쿼리가 검색 가능한 백엔드 `type` 속성으로 구성된 경우 `int` 예상대로 작동합니다. 이전에는 Adobe Systems Commerce에서 예외가 `Elasticsearch\Common\Exceptions\BadRequest400Exception` 발생했습니다.

<!-- ACP2E-99-->

* 이제 Elasticsearch 쿼리에서 Minimum Terms to Match **파라미터와**&#x200B;함께 검색 동의어를 사용할 수 있습니다. 이전에는 설정에 이 매개 변수를 지정하고 특정 키워드에 대한 검색어를 추가한 경우 검색 결과가 반환되지 않았습니다.
...
* `custom_design_to` 이제 예약된 업데이트가 변경될 때 속성 값이 예상대로 업데이트됩니다. 이전에는 종료 시간이 없는 두 개의 별도 범주 디자인 업데이트가 하나로 처리되었습니다. 첫 번째 업데이트가 실행되는 `custom_design_from` 동안 예약된 업데이트 중 하나가 삭제되면 시간 값이 예약된 다음 업데이트의 시간 값보다 `custom_design_to` 커졌습니다.

<!-- ACP2E-58-->

* 이제 예약된 업데이트가 편집된 후 Forms 양식이 예상대로 데이터로 채워집니다.

<!-- ACP2E-18-->

* `vendor/magento/module-catalog-staging/Setup/Patch/Data/MigrateCatalogProducts.php` 데이터베이스에 사용자 지정 옵션이 있는 제품이 포함되어 있고 속성이 설정된 경우 데이터 패치가 더 이상 실패하지 `special_from_date` 않습니다.

<!-- ACP2E-741-->

* 이제 실행 중인 업데이트에서 변경 내용이 추가될 때 예약된 두 번째 업데이트 중에 변경 내용이 저장됩니다. 이전에는 변경 사항이 저장되지 않아 판매자가 변경 사항을 다시 입력하고 저장해야 했습니다.
...
* 이제 판매자는 로캘에 관계없이 예약된 업데이트에서 제품 가격을 편집할 때 쉼표로 구분된 가격을 사용하도록 제한됩니다. 이전에는 예약된 업데이트에 쉼표가 포함된 가격이 로케일에 따라 1000으로 나누어졌습니다.

<!-- AC-3214-->

* PHP 8.1을 실행하는 배포에서 DHL 국내 배송과 관련된 문제는 국제 배송에만 과세 대상 태그를 요구함으로써 해결되었습니다.

### 세금

<!-- ACP2E-735-->

* 이제 장바구니 제품에 대해 고정 상품세(FPT)가 올바르게 표시됩니다. 이전에는 장바구니에 있는 여러 제품에 **고정 제품 세금(FPT)**&#x200B;이 있고 **FPT에 세금 적용**&#x200B;이 활성화된 경우 모든 FPT가 장바구니의 마지막 제품에 할당되고 다른 제품에 대해 재설정되었습니다.
...
**문제**: 2.4.5 이전 제품 버전에서 Adobe Commerce 2.4.6 이상으로 업그레이드한 후 스테이징된 판매 규칙 레이블이 일치하지 않을 수 있습니다. **해결 방법**: 메타데이터 업데이트 단계 전에 업그레이드 중에 ACSD-50625_2.4.5-P1.patch를 적용합니다. 버전 &lt; 2.4.5](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/known-issues-patches-attached/salesrule-label-issues-when-upgrade-from-versions-under-2.4.5.html) 기술 자료 문서에서 업그레이드할 때 [salesRules 레이블 문제 를 참조하십시오.

이 문제를 해결하기 위한 일반 단계:

1. 필요한 패치를 다운로드합니다. 다음 버전 &lt; 2.4.5](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/known-issues-patches-attached/salesrule-label-issues-when-upgrade-from-versions-under-2.4.5.html) 에서 업그레이드할 [때 salesRules 레이블 문제 기술 자료 문서를 참조하십시오.
2. 업그레이드 프로세스의 [패키지 관리 단계에 설명된 업그레이드 단계를 따릅니다](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/implementation/perform-upgrade.html).
3. 업그레이드 중에 패키지 관리 단계 후 메타데이터 업데이트 단계 전에 이 패치를 적용하십시오. [Adobe이 제공하는 작성기 패치를 적용하는 방법](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/how-to/how-to-apply-a-composer-patch-provided-by-magento.html) 을 참조하십시오.
4. 업그레이드 프로세스를 완료합니다.

**문제**: Braintree이 활성화된 경우 관리자는 관리자로부터 고객에 대한 주문 또는 재주문을 만들 수 없습니다. 관리자가 **순서** 또는 **순서 바꾸기**&#x200B;를 클릭하면 Adobe Commerce에서 순서를 제출하지 않고 `system.log`에 다음 오류가 표시됩니다. `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`.  **해결 방법**: `BUNDLE-3137-composer.patch`을(를) 사용할 수 있습니다. [이 문제에 대한 논의 및 패치에 대한 액세스는 관리자가 Braintree 결제가 활성화된](https://support.magento.com/hc/en-us/articles/8322266352525) 경우 기술 자료 문서를 만들 수 없습니다. 클라우드 판매자는 Cloud Patches for Commerce v1.0.18을 다운로드 해야 합니다. 수정 사항은 Adobe Systems Commerce 2.4.5-p1에도 포함됩니다. <!-- AC-6222-->

**문제**: [!DNL RabbitMQ] Adobe Systems Commerce 2.4.5를 새로 설치(업그레이드 아님) 후 인프라 요소(교환, 큐)가 초기화되지 않을 수 있습니다. 이 잘못된 구성 [!DNL RabbitMQ] 으로 인해 비동기 작업이 실행되거나 성능 프로필이 생성될 때 치명적인 오류가 발생할 수 있습니다. 클라우드 배포는 영향을 받지 않습니다. **해결 방법**: 온-프레미스에서 Adobe Systems Commerce 2.4.5를 새로 설치하는 판매자는 구성 `bin/magento setup:config:set` 옵션을 `setup:install` 명령에 전달 [!DNL RabbitMQ] 하여 Adobe Systems Commerce를 구성 [!DNL RabbitMQ] 하거나 설치한 후 실행 `bin/magento setup:upgrade --keep-generated` 해야 합니다. 를 참조하십시오 [[!DNL RabbitMQ]](../../../installation/prerequisites/rabbitmq.md).
