...
[Adobe Sensei](https://www.adobe.com/sensei.html)에서 제공하는 [실시간 검색](https://experienceleague.adobe.com/en/docs/commerce/live-search/overview)은(는) 인공 지능과 머신 러닝 알고리즘을 사용하여 집계된 방문자 데이터를 심층 분석함으로써 직관적인 검색 경험을 제공합니다. [실시간 검색 릴리스 정보](https://experienceleague.adobe.com/docs/commerce/live-search/release-notes.html)를 참조하세요.
...
* 이제 가져오는 동안 예상대로 고객 주소에서 지역 정보를 삭제할 수 있습니다. 이전에는 빈 지역을 사용하여 고객 주소를 가져올 때 할당된 지역이 변경되지 않았습니다.

<!-- MC-38810-->

* 추가 제품 속성으로 JSON 또는 HTML 컨텐츠 포함 제품이 이제 CSV 파일로 올바르게 내보내집니다. 이전에는 CSV 파일의 잘못된 필드에 겹치는 데이터 문자열이 포함되어 있었습니다.

### 색인

<!-- MC-39718 ENGCOM-8567-->

* 이제 하위 프로세스가 실패할 때 프로세스 관리자가 종료되고 오류가 발생합니다. 이전에는 프로세스 관리자에 전달된 함수 수(예: 인덱서 차원)가 환경 변수 값보다 작으면 항상 프로세스 관리자가 성공적으로 종료되었습니다 `MAGE_INDEXER_THREADS_COUNT` . [GitHub-30964](https://github.com/magento/magento2/issues/30964)
...
* 이제 인덱서 모드가 저장&#x200B;**시 업데이트로**&#x200B;설정된 경우 카탈로그 가격 규칙 인덱서가 예상대로 작동합니다. [GitHub-370](https://github.com/magento/partners-magento2ee/issues/370)

<!-- ENGCOM-8890-->

* 이제 제품을 포함하지 않는 비활성화된 범주를 삭제해도 카탈로그 검색 및 범주 플랫 인덱스 테이블에는 영향을 주지 않습니다. 이전에는 비활성 카테고리를 삭제하면 전체 색인 재지정이 트리거되었습니다. [GitHub-23297](https://github.com/magento/magento2/issues/23297)
...
* 종속성 `pelago/emogrifier` 이 버전 3.1.0에서 5.0.0으로 업데이트되었습니다. 이 업데이트로 인해 이전 버전과 호환되지 않는 변경 사항이 클래스에 도입되었습니다 `Magento\Email\Model\Template\Filter` . 변경된 코드는 이메일 템플릿 렌더링 중에 실행됩니다. BIC 참조를](https://developer.adobe.com/commerce/php/development/backward-incompatible-changes/reference/) 참조하십시오[.

<!-- MC-42212-->

* 새 제품을 저장할 수 없는 문제를 `\Magento\CatalogInventory\Model\Indexer\Stock\CacheCleaner::getCategoryIdsByProductIds` 수정했습니다.
...
* 이제 애플리케이션 앱은 환불된 고객 잔액을 저장할 때 수정된 주문을 예상대로 저장합니다. [GitHub-393](https://github.com/magento/partners-magento2ee/issues/393)
...
* 이제 번들 제품은 제품에 계층 가격이 `Magento\Framework\Api\ExtensibleDataObjectConverter` 할당되고 제품 데이터를 전환 하는 데 사용되는 경우 저장할 수 있습니다. 이전에는 제품이 저장될 때 제품 데이터를 어레이로 전환 하는 데 사용되었을 때 `Magento\Framework\Api\ExtensibleDataObjectConverter` 애플리케이션 프로그램이 제품을 저장하지 않고 다음 오류를 표시했습니다. `Notice: Undefined index: price in app/code/Magento/Catalog/Model/Product/Type/Price.php on line 382.`

<!-- MC-36811-->

* 이제 예약된 가격 업데이트가 이미 쇼핑객 장바구니 속에 있는 제품에 적용됩니다. [GitHub-356](https://github.com/magento/partners-magento2ee/issues/356)

<!-- MC-24693-->

* 이제 계층 가격은 수량 증분이 활성화되고 소수 재고가 1보다 작을 때 예상대로 제품에 적용됩니다. 이전에는 최소 계층 가격 수량 값이 1로 설정되었습니다.
...
* 이제 탐색 화살표 단추(**다음** 및 **이전**)가 상점 제품 비디오에 예상대로 표시됩니다.

<!-- MC-40463-->

* 이제 판매자는 예상대로 제품 페이지 의 동영상&#x200B;**삽입 버튼 버튼을 사용하여** Vimeo 동영상을 추가할 수 있습니다. 이전에는 응용 프로그램에 404 오류가 표시되었습니다. [GitHub-31753](https://github.com/magento/magento2/issues/31753)
...
* 이제 인보이스 전송을 구성할 수 있습니다. 이전에는 송장 전송을 구성할 수 없었고, 생성된 후에는 항상 애플리케이션에서 송장을 전송했습니다. 송장 및 주문 이메일은 모두 한 관찰자의 범위 내에서 전송되었습니다. 이제 별도의 관찰자가 주문 이메일 및 송장 이메일 전송을 제어합니다. [GitHub-27656](https://github.com/magento/magento2/issues/27656)

### 보고서

<!-- MC-40998-->

* 이제 클라우드 인프라 기반 Adobe Systems 상거래 프로젝트에서 모든 시스템 보고서를 보고 다운로드할 수 있습니다. 이전에는 로그, 데이터, 디자인 또는 스토어에 대한 시스템 보고서를 다운로드할 수 없었습니다.
...
* 이제 장바구니에 있는 일부 품목이 무료 배송에 적합할 때 애플리케이션에서 올바른 조정된 배송 가격을 표시합니다. 이전에는 장바구니에 있는 품목의 하위 집합이 무료 배송에 적합할 때 애플리케이션이 배송료를 조정하지 않고 전체 배송료를 쇼핑객에게 표시했습니다.

<!-- MC-41186-->

* 특정 웹사이트로 제한된 관리자는 이제 동일한 웹사이트에서 접수된 주문에 대한 배송을 생성할 수 있습니다. 이전에는 기본 스토어 보기에 대한 권한이 없는 관리자가 관리자가 액세스할 수 있는 스토어 보기에 있는 주문을 배송하려고 할 때 애플리케이션에서 이 예외가 `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`발생했습니다.
...
* 범주 일정 업데이트 미리 보기 중에 모든 제품 ID를 포함하는 대용량 데이터베이스 쿼리가 더 이상 실행되지 않습니다. 대신 카테고리 필터가 제품 컬렉션에 적용되어 카테고리의 스테이징 미리 보기를 위한 임시 테이블을 만듭니다. 이전에는 응용 프로그램이 스테이징 미리 보기를 위해 모든 제품을 임시 테이블에 삽입했기 때문에 성능이 저하되었습니다.

<!-- MC-40714-->

* 2.3.x에서 업그레이드하는 동안 모듈 문제가 `Magento_SalesRuleStaging` 더 이상 오류를 트리거하지 않습니다.
...
* 애플리케이션은 위시리스트에서 제품에 대한 편집 항목&#x200B;**버튼 버튼을 클릭**&#x200B;할 때 더 이상 구성 가능한 제품의 구성 설정을 재설정하지 않습니다. [GitHub-32119](https://github.com/magento/magento2/issues/32119)
