...
* 플러그인이 상위 클래스에 추가될 때 애플리케이션에서 더 이상 오류가 발생하지 않습니다. (이제 `optionsProvider` 매개 변수가 `Assest.php`의 `getContentIdentities` 뒤에 선언되었습니다.) 이전에는 응용 프로그램에서 다음 오류가 발생했습니다. `Error: Cannot instantiate interface Magento\Framework\Data\OptionSourceInterface`. [GitHub-31291](https://github.com/magento/magento2/issues/31291)
