...
Adobe Commerce 2.4.8은 향상된 보안, PHP 8.4 및 MariaDB 11.4와의 호환성, 광범위한 GraphQL API를 개선하여 Edge Delivery에서 제공하는 새로운 [Adobe Commerce Storefront](https://experienceleague.adobe.com/developer/commerce/storefront/)&#x200B;(으)로의 마이그레이션을 가속화하고 500개 이상의 품질 수정 및 개선 사항을 통해 기존 기능을 향상시킵니다. 2.4.8로 업그레이드하면 플랫폼을 안전하게 유지하고 최신 기술 발전에 맞게 조정하며 2028년 4월까지 지원됩니다.

>[!BEGINSHADEBOX]

**입양 권장 사항**

Adobe Commerce 2.4.8로의 업그레이드를 계획할 때는 비즈니스 우선 순위, 현재 버전 및 새로운 [Adobe Commerce as a Cloud Service](https://experienceleague.adobe.com/en/docs/commerce/cloud-service/overview)을(를) 채택할 계획을 고려하십시오.
    ...
    <tbody>
        <tr>
            ...
            <td>이번 릴리스에는 다음과 같은 보안 설정 개선 사항이 포함됩니다.
              <ul>
                <li><strong>Duo Security 2FA</strong>: 이 변경 사항은 최신 SDK(Web SDK v4)를 사용하도록 Adobe Commerce의 <a href="https://experienceleague.adobe.com/en/docs/commerce-admin/systems/security/tfa/security-two-factor-authentication-use#duo-security">Duo Security</a> 2단계 인증(2FA) 구현을 업데이트합니다. 이 업그레이드를 통해 판매자는 Duo Universal Prompt 사용으로 원활하게 전환할 수 있습니다. 판매자는 클라이언트 ID 및 암호를 포함하도록 관리 설정에서 구성을 업데이트해야 합니다.<!-- AC-10982 --></li>
                <li><strong>암호화 키</strong>: <!-- AC-12679 -->유용성을 높이고 이전 제한 사항과 버그를 없애기 위해 암호화 키 관리를 다시 디자인했습니다. 이제 <a href="https://experienceleague.adobe.com/en/docs/commerce-admin/systems/security/encryption-key">변경</a> 키와 <a href="https://developer.adobe.com/commerce/php/development/security/data-encryption/">다시 암호화</a> 특정 시스템 구성, 결제 및 사용자 지정 필드 데이터에 새 CLI 명령을 사용할 수 있습니다. 이 릴리스에서는 관리 UI의 키 변경이 더 이상 지원되지 않습니다. CLI 명령을 사용해야 합니다. 자세한 내용은 <a href="https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/troubleshooting/known-issues-patches-attached/troubleshooting-encryption-key-rotation-cve-2024-34102">암호화 키 순환 문제 해결: CVE-2024-34102</a> 기술 문서를 참조하십시오.</li>
...
<table style="table-layout:auto">
    <tbody>
        ...
        <tr>
            ...
            <td>이 릴리스에는 RabbitMQ 4.x에 대한 지원이 추가되었습니다. RabbitMQ 4로 업그레이드하기 전에 Adobe Commerce Cloud 및 온-프레미스 고객은 <a href="https://www.rabbitmq.com/docs/3.13/migrate-mcq-to-qq">을(를) [!DNL classic mirrored queues]에서 [!DNL quorum queues]</a>(으)로 마이그레이션해야 합니다. 다음과 같은 이유로 이 마이그레이션이 필요합니다.<ul>
...
<table style="table-layout:auto">
    <tbody>
        ...
        <td>
            <ul>
                <li><!--LYNX-387-->정확한 가격 표시 및 할인 계산을 지원하기 위해 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#cartitemprices"><code>CartItemPrices</code></a> 형식에 다음과 같은 새 필드를 추가했습니다.
                <ul>
              ...
              <li><!--LYNX-542-->예외 대신 오류 세부 정보와 함께 성공 응답을 반환하도록 <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/update-items"><code>updateCartItems</code></a> 돌연변이를 업데이트했습니다. 사용자 알림의 명확성을 개선하기 위해 오류 매핑이 개선되었습니다.</li>
              ...
              <li><!--LYNX-451--><code>grand_total_excluding_tax</code> 필드를 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#cartprices"><code>CartPrices</code></a> 유형에 추가하여 명확한 세금 포함 가격을 제공했습니다.</li>
              <li><!--LYNX-377--><a href="https://developer.adobe.com/commerce/webapi/graphql/schema/cart/interfaces/"><code>CartItemInterface</code></a>에 <code>not_available_message</code>을(를) 추가했습니다.</li>
            </ul>
        </td>
       </tr>
        <tr>
           ...
           <td>이번 릴리스에는 다음과 같은 고객 관리 개선 사항이 포함됩니다.
             <ul>
               <li><!--LYNX-391-->확인되지 않은 전자 메일에 특정 메시지를 제공하기 위해 <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/customer/mutations/generate-token"><code>generateCustomerToken</code></a> 돌연변이에서 오류 처리를 개선했습니다. 향상된 사용자 지침 및 오류 해결을 지원합니다.</li>
        ...
        <tr>
           ...
           <td>이번 릴리스에는 다음과 같은 사용자 주문 관리 개선 사항이 포함됩니다.
             <ul>
              ...
              <li><!--LYNX-470--><a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#customerorders"><code>CustomerOrders</code></a> 형식에 새 <code>date_of_first_order</code> 필드를 추가했습니다.</li>
              <li><!--LYNX-468-->사용자 지정 특성을 포함하도록 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-k-p#orderaddress"><code>OrderAddress</code></a> 유형을 확장하여 순서 세부 정보 가시성을 개선했습니다. 주문 확인 페이지에 추가 정보를 표시할 수 있습니다.</li>
              <li><!--LYNX-450-->가상 제품 식별을 지원하는 <code>is_virtual</code> 필드를 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#customerorder"><code>CustomerOrder</code></a> 형식에 추가했습니다. 가상 제품과 실제 제품을 구별하여 주문 처리를 강화합니다.</li>
              <li><!--LYNX-449-->가격에 대한 새 필드가 여러 개 있는 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-k-p#orderiteminterface"><code>OrderItemInterface</code></a>에 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#cartitemprices"><code>CartItemPrices</code></a>과(와) 유사한 <code>OrderItemPrices</code> 형식을 추가했습니다.</li>
              <li><!--LYNX-448-->이메일 일치를 기반으로 게스트 주문을 고객 계정과 병합하는 API 기능이 개선되었습니다. 재방문 고객을 위한 주문 관리를 간소화합니다.</li>
              <li><!-- LYNX-523: -->더 나은 주문 관리를 위해 <code>available_actions</code> 필드를 포함하도록 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#customerorder"><code>CustomerOrder</code></a> 형식을 확장했습니다. 'available_actions' 필드는 해당 주문에서 수행할 수 있는 작업을 나열하는 열거형에 매핑됩니다.</li>
              <li><!-- LYNX-524 --><a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#customerorder"><code>CustomerOrder</code></a> 형식에 <code>customer_info</code> 필드를 추가했습니다. 이 필드에는 고객 이름에 대한 세부 정보가 포함된 <code>OrderCustomerInfo</code> 형식이 필요합니다.</li>
              <li><!--LYNX-519-->자세한 오류 코드를 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#cancelorderoutput"><code>CancelOrderOutput</code></a> 형식에 추가했습니다. 주문 취소 프로세스에 대한 오류 처리 및 사용자 피드백이 개선되었습니다.</li>
              ...
              <li><!--LYNX-642-->상인이 <a href="https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/shopper-tools/reorders-allow#configure-to-allow-customer-reorders">관리자에서 순서 변경 기능을 사용하지 않도록 설정</a>하는 경우 <code>REORDER</code> 작업이 반환되지 않도록 <code>order.availableActions</code> 필드를 업데이트했습니다.</li>
              ...
              <li><!--LYNX-470--><code>created_at</code> 필드를 기반으로 주문 내역 필터를 추가하고 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#customerorders"><code>CustomerOrders</code></a> 유형의 <code>date_of_first_order</code>을(를) 노출했습니다.</li>
              <li><!--LYNX-450--><a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#cart"><code>Cart</code></a> 형식과 유사한 <code>is_virtual</code> 필드를 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#customerorder"><code>CustomerOrder</code></a> 형식에 추가했습니다. 이를 통해 고객은 가상 제품만 포함된 주문에 대한 배송 정보를 조건부로 숨길 수 있습니다.</li>
              ...
              <li><!--LYNX-701-->주문 합계에 적용된 스토어 크레딧 및 보상 포인트를 포함하도록 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#customerorder"><code>CustomerOrder</code></a> 유형을 업데이트했습니다. 이제 주문 페이로드가 매장 크레딧과 보상 포인트 모두에 대한 레이블과 값을 반환하여 전체 비용 요약 세부 정보를 확인할 수 있습니다.</li>
            </ul>
           </td>
        </tr>
        <tr>
        ...
        <td>
            <ul>
              <li><!--LYNX-540-->재고 수준 세부 정보를 제공하기 위해 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-k-p#productinterface"><code>ProductInterface</code></a>에 <code>quantity</code> 필드를 추가했습니다. 관리자 설정에 따라 사용 가능한 재고 또는 null이 표시됩니다.</li>
              <li><!--LYNX-460-->번들 제품에 대한 가격 표시가 수정되어 정확한 가격 및 통화 정보가 보장됩니다.</li>
              <li><!--LYNX-401--><a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-q-s#storeconfig"><code>StoreConfig</code></a> 형식에 <code>grouped_product_image</code> 및 <code>configurable_product_image</code> 필드를 추가했습니다.</li>
            </ul>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <ul>
              <li><!--LYNX-713-->선물 포장(항목 및 주문당) 및 인쇄된 카드에 대해 <code>price_excluding_tax</code> 및 <code>price_including_tax</code> 필드를 모두 포함하도록 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-f-i#giftoptionsprices"><code>GiftOptionsPrices</code></a> 및 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-k-p#ordertotal"><code>OrderTotal</code></a> 형식을 업데이트했습니다. 이러한 향상된 기능을 통해 장바구니 및 주문 쿼리 모두에 대한 스토어 구성 설정을 기반으로 정확한 세금 표시가 가능합니다.</li>
              ...
              <li><!--LYNX-693--><a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-f-i#giftcarditem"><code>GiftCardCartItem</code></a> 형식에 <code>gift_message</code> 및 <code>gift_wrapping</code> 필드를 추가했습니다. 가상 제품에 대한 선물 포장을 삭제하도록 논리를 업데이트했습니다.</li>
    ...
    <tr>
        ...
        <td>
            <ul>
                <li><!--LYNX-632-->반환 특성 관련 쿼리와 돌연변이 사이의 불일치를 해결하기 위해 RMA 항목 입력에 대한 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/mutations#requestreturn"><code>requestReturn</code></a> 돌연변이를 업데이트했습니다.</li>
                <li><!--LYNX-515-->게스트 주문 반품을 지원하도록 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/mutations#requestreturn"><code>requestReturn</code></a> 돌연변이를 업데이트했습니다.</li>
            </ul>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <ul>
                <li><!--LYNX-633-->관리자 구성의 세금 표시 설정과 일치하도록 필드가 있는 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-q-s#storeconfig"><code>StoreConfig</code></a> 유형을 업데이트했습니다.</li>
                <li><!--LYNX-697--><a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-k-p#ordertotal"><code>OrderTotal</code></a> 형식에 <code>subtotal_excl_tax</code> 및 <code>subtotal_incl_tax</code> 필드를 추가했습니다. <code>subtotal</code> 필드를 더 이상 사용되지 않는 것으로 표시했습니다.</li>
            </ul>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <ul>
                <li><!--LYNX-618-->배송 또는 청구 카트 주소의 주소록 식별자를 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-c-e#cartaddressinterface"><code>CartAddressInterface</code></a> 유형에 추가했습니다.</li>
                <li><!--LYNX-617-->배송 및 청구 주소가 동일한지 여부를 나타내는 <code>same_as_billing</code> 필드를 <a href="https://developer.adobe.com/commerce/webapi/reference/graphql/2-4-8/types-q-s#shippingcartaddress"><code>ShippingCartAddress</code></a> 형식에 추가했습니다.</li>
                <li><!--LYNX-458-->사용자 지정 주소 특성을 포함하도록 <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/orders/queries/guest-order"><code>guestOrder</code></a> 및 <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/orders/queries/guest-order-by-token"><code>guestOrderByToken</code></a> 쿼리를 업데이트하여 새 계정의 전체 주소 정보를 확인했습니다.</li>
            </ul>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <ul>
                <li><!--LYNX-522--><a href="https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/recaptcha-v3-config"><code>recaptchaV3Config</code></a> 쿼리에 <code>theme</code> 필드를 추가했습니다. 이 필드에서는 정확한 가격 및 통화 정보를 보장하기 위해 reCaptcha 렌더링에 사용할 테마의 이름을 지정할 수 있습니다.</li>
