...
* 수명 종료에 도달한 Zend 프레임워크(ZF1) 구성 요소가 코드 베이스에서 제거되었습니다. [이전 버전과 호환되지 않는 변경 내용을 강조 표시](https://developer.adobe.com/commerce/php/development/backward-incompatible-changes/highlights/).<!-- AC-1512 -->합니다.
...
* [빠른 체크 아웃 볼트](https://experienceleague.adobe.com/docs/commerce/quick-checkout/overview.html)
...
PWA Studio v.13.0.x는 Adobe Commerce 2.4.6과 호환됩니다. 여기에는 액세스 가능성을 개선하기 위한 여러 개선 사항이 포함되어 있습니다. 버그 수정에 대한 자세한 내용은 [PWA Studio 릴리스](https://github.com/magento/pwa-studio/releases)를 참조하십시오. PWA Studio 버전 및 호환되는 Adobe Systems Commerce 핵심 버전 목록은 버전 호환성](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/)을 참조하십시오[.
...
* 정적 컨텐츠 배포 시 여러 로케일 및 웹 사이트가 있는 표준 또는 압축 배포 중에 더 이상 시간 초과되거나 실패하지 않습니다.

<!-- ACP2E-923-->

* 압축 전략은 정적 컨텐츠 배포`bin/magento set:static-content:deploy -f --strategy compact`()를 실행할 때 더 이상 외부 도메인에 대한 잘못된 참조를 발생시키지 않습니다.

<!-- AC-5994-->

* 이제 실행 `bin/magento setup:di:compile -vvv` 하면(또는 or `-vv` 옵션 사용`-v`) 실패 시 정보 메시지가 반환됩니다. [GitHub-35391](https://github.com/magento/magento2/issues/35391)
...
* 쿠폰 생성 시 쿠폰 코드 설정을 강제 적용할 수 있도록 쿠폰 코드 관리 작업 과정에 유효성 확인 논리가 추가되었습니다. 이전에는 쿠폰 수량&#x200B;**값이 2500보다 클 때**&#x200B;쿠폰 코드 생성 시 이 설정이 무시 **[!UICONTROL Code Length]** 되었습니다.

<!-- ACP2E-1029-->

* 이제 PayPal Express 결제 방법으로 결제하는 동안 보상 쿠폰이 적용되면 보상 포인트가 예상대로 고객의 계정에 추가됩니다.
...
* 이제 쿠폰 코드를 Excel XML로 내보낼 수 있습니다. 이전에는 생성된 파일에 선택한 모든 쿠폰 코드가 포함되지 않았지만 페이지 선택기당 항목에서 선택한 만큼의 항목만 포함되었습니다.[GitHub-36073](https://github.com/magento/magento2/issues/36073)
...
* 이제 AWS S3를 원격 스토리지로 사용하면 전체 내보내기가 제대로 작동합니다. 이전에는 업로드한 파일 크기 때문에 내보내기에 실패했습니다.

<!-- ACP2E-982-->

* 이제 CSV 파일에 재고 데이터 열이 누락된 경우 제품 상태를 올바르게 가져옵니다. 이전에는 제품을 가져온 후 기본 소스가 제품에 자동으로 할당되고, 수량이 0으로 설정되며, CSV 파일의 재고 데이터 열이 누락된 경우 상태가 재고 부족으로 표시되었습니다.

<!-- ACP2E-841-->

* 이제 가져오기에 100개 이상의 레코드가 포함된 경우 고급 가격 책정 값을 가져오면 새 레코드와 업데이트된 레코드에 대한 올바른 개수가 제공됩니다. 이전에는 가져오기 성공 메시지에서 잘못된 개수를 제공했습니다.
...
* `Fixer.io` 이제 가져오기 서비스가 예상대로 작동합니다. 이제 통화 환율을 성공적으로 가져올 수 있습니다. [GitHub-36010](https://github.com/magento/partners-magento2ee/issues/36010)
...
3. 업그레이드 중에 패키지 관리 단계 후 메타데이터 업데이트 단계 전에 이 패치를 적용하십시오. [Adobe에서 제공하는 작성기 패치를 적용하는 방법](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/how-to/how-to-apply-a-composer-patch-provided-by-magento.html)을 참조하십시오.
4. 업그레이드 프로세스를 완료합니다.
