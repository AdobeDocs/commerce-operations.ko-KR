...
>
>이 날짜 이후에 Adobe Commerce 2.4.5는 호환성을 제공하지 않거나>mySQL 8.0 이후 릴리스된 모든 MySQL 버전에 대한 지원 Adobe은>이 Adobe에서 최신 MySQL 주요 버전의 유효성을 검사하거나 지원을 제공합니다.>Commerce 릴리스 라인.
>
>버전 2.4.5를 실행하는 모든 Adobe Commerce 온-프레미스 고객은 강력합니다>데이터베이스 서버를 호환 가능한 MariaDB 버전으로 마이그레이션하는 것이 좋습니다.

## 2.4.5-p17

{{extended-support}}

Adobe Commerce 2.4.5-p17 보안 릴리스는 이전 릴리스 2.4.5에서 식별된 취약점에 대한 보안 버그 수정 사항을 제공합니다.
...
Adobe Commerce 2.4.5는 이제 Valkey 8.1 LTS와 호환되며, 클라우드 인프라의 Adobe Commerce에서 지원되는 장기 지원 캐시 백엔드 옵션을 제공합니다.

#### RabbitMQ 4.2 지원

Adobe Commerce 2.4.5는 이제 2026년 2월로 예정된 RabbitMQ 4.1 지원 종료 날짜를 해결하는 RabbitMQ 4.2와 호환됩니다. Apache ActiveMQ Artemis와의 호환성은 유지되고 ActiveMQ는 이 보안 전용 릴리스 라인의 기본 메시지 큐 서비스로 유지됩니다.

>[!NOTE]
>
>2.4.5용 확장 지원 보안 패치는 Adobe Commerce 고객만 사용할 수 있습니다. 이러한 패치는 Magento Open Source 코드 기반에 사용할 수 없습니다. [확장 지원](https://experienceleague.adobe.com/en/docs/commerce-operations/release/planning/lifecycle-policy#extended-support)을 참조하세요.

## 2.4.5-p16

{{extended-support}}

Adobe Commerce 2.4.5-p16 보안 릴리스는 이전 릴리스 2.4.5에서 식별된 취약점에 대한 보안 버그 수정 사항을 제공합니다.
...
#### Apache ActiveMQ Artemis와의 호환성

RabbitMQ 4와 관련된 지원 종료 위험으로 인해 Adobe Commerce은 메시지 큐 인프라에 대한 장기적인 안정성과 지원을 보장하기 위해 Apache ActiveMQ Artemis로 전략적 마이그레이션을 시작했습니다.

* Adobe Commerce 2.4.5는 이제 Apache ActiveMQ Artemis 최신 버전을 지원합니다.
* 기존 MQ 서비스를 유지하려는 고객을 지원하기 위해 RabbitMQ 4.1과의 호환성을 유지합니다.
* ActiveMQ는 이제 Cloud Native 배포용 AWS ActiveMQ를 포함하여 Adobe Commerce Cloud에서 완전히 지원됩니다.

#### 2.4.5-p16 호환성과 MariaDB 10.11

Adobe Commerce 2.4.5-p16은 MariaDB 10.11과의 호환성을 검증받았으며, MariaDB 10.6을 계속 완벽하게 지원합니다. 현재 2.4.5px를 실행하는 상인은 2.4.5-p16으로 안전하게 업그레이드하고 MariaDB 10.6에서 MariaDB 10.11로 전환할 수 있습니다.

## 2.4.5-p15

{{extended-support}}

Adobe Commerce 2.4.5-p15 보안 릴리스는 이전 릴리스 2.4.5에서 식별된 취약점에 대한 보안 버그 수정 사항을 제공합니다.
