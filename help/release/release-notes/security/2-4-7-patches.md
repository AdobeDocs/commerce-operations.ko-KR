...
Adobe Commerce 2.4.7-p10은 MariaDB 버전 11.8과의 호환성을 제공합니다. MariaDB 11.8에서는 SQL 동작, 기본값, 사용 중단 및 성능 최적화의 변경 사항이 도입되었습니다. 이 업데이트는 플랫폼 안정성 및 향후 준비 상태를 유지하기 위해 잠재적인 문제를 사전에 해결합니다.

#### OpenSearch 3 최신 부 버전 지원

Adobe Commerce 2.4.7은 이제 Adobe Commerce on cloud infrastructure, Cloud Native 및 온프레미스 배포의 최신 부 버전의 OpenSearch 3를 지원합니다. OpenSearch 2와의 호환성은 유지됩니다.

#### Valkey 8.1 LTS 지원

Adobe Commerce 2.4.7은 이제 Valkey 8.1 LTS와 호환되며, 클라우드 인프라의 Adobe Commerce에서 지원되는 장기 지원 캐시 백엔드 옵션을 제공합니다.

#### RabbitMQ 4.2 지원

Adobe Commerce 2.4.7은 이제 2026년 2월로 예정된 RabbitMQ 4.1 지원 종료 날짜를 해결하는 RabbitMQ 4.2와 호환됩니다. Apache ActiveMQ Artemis와의 호환성은 유지되고 ActiveMQ는 이 보안 전용 릴리스 라인의 기본 메시지 큐 서비스로 유지됩니다.
...
Adobe Commerce 2.4.7은 Composer 2.2 LTS와 호환되지만 Composer 2.9.x를 지원하도록 업데이트되었습니다.
