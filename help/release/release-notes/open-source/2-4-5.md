...
* PayPal, 신용 및 나중에 결제 버튼의 미리 보기는 이제 관리자의 체크아웃, 미니 카트, 장바구니 및 제품 페이지에서 사용할 수 있습니다. 미리 보기는 이러한 단추가 상점에서 활성화되고 렌더링될 때 어떻게 표시되는지 보여줍니다.

#### Braintree

* Braintree는 KOUNT 사기 방지 통합을 중단했습니다. Magento Open Source 코드베이스에서 제거되었습니다.
...
* 제품이 품절된 경우 `cart` 쿼리가 더 이상 null 응답을 반환하지 않습니다. 오류 메시지가 포함된 새 `errors` 요소가 응답에 도입되었습니다. 이전에는, 품절 제품으로 쿼리를 실행했을 때 Source 열기 응용 프로그램이 응답의 `items` 섹션 아래에 `null` 값을 표시했습니다. [장바구니 쿼리](https://developer.adobe.com/commerce/webapi/graphql/queries/cart.html)를 참조하세요.
...
* 이제 지속성&#x200B;**활성화가 활성화된 경우**&#x200B;세션 시간 초과 후 이전에 추가된 제품이 미니 장바구니 에 표시됩니다. [GitHub-35183](https://github.com/magento/magento2/issues/35183)

<!-- ACP2E-707-->

* 판매자는 이제  **환불 배송비(포함 Tax)** 가 -0.01로 설정되고 이제 이 금액을 0으로 설정할 수 있습니다. 이전에는 이러한 조건에서는 대변 메모를 생성할 수 없었습니다.
...
* 새 `ConfigurableWishlistItem.configured_variant` 필드가 `ConfigurableWishlistItem.child_sku` 필드를 대체했습니다. 후자의 필드는 고객 위시리스트에 구성되지 않은 구성 가능한 제품이 포함되어 있을 때 내부 오류를 트리거했습니다.

<!-- ACP2E-84-->

* 이제 제품 가시성을 변경하기 위해 대량 속성 업데이트 중에 선택한 매장에 대해서만 URL 재작성이 생성됩니다. 이전에는 대량 속성 업데이트로 인해 잘못된 스토어에 대한 URL 재작성이 생성되었습니다.

<!-- ACP2E-39-->

* `Synchronize widget products with backend storage` 이 설정이 활성화되면 애플리케이션 오픈 소스(Open Source)는 최근 보기 제품 데이터를 `catalog_product_frontend_action` 데이터베이스 테이블에 추가합니다. 여기에는 레코드를 추가할 때 고객 또는 방문자 ID가 포함됩니다. 고객 ID와 방문자 ID가 `recently_viewed_product` null인 경우 응답의 섹션이 비어 있습니다. 따라서 `customer/section/load` Ajax 요청이 전송되면 Source 열기 응용 프로그램은 고객 또는 방문자 ID를 기반으로 최근에 본 제품을 올바르게 필터링할 수 있습니다. 이전에는 빈 고객 또는 방문자 ID를 확인하지 않았기 때문에 응답에 `catalog_product_frontend_action` 데이터베이스 테이블에서 사용할 수 있는 모든 데이터가 포함되었습니다.
...
* Magento Open Source 는 이제 상점의 웹 사이트 설정에 따라 사전 정의 된 EAV 시스템 속성을 올바르게 표시합니다. 이전에는 한 웹 사이트에 대해 활성화되고 다른 웹 사이트에 대해 비활성화된 웹 사이트 수준 고객 속성이 두 웹 사이트 모두에 대해 활성화된 것으로 표시되었습니다.
...
* Magento Open Source 클래스`vendor/magento/module-backend/Model/Menu.php``Error: Call to undefined method ReflectionUnionType::getName()`의 모든 메서드에 대한 플러그인을 만들 때 더 이상 다음 오류가 발생하지 않습니다. [GitHub-35292](https://github.com/magento/magento2/issues/35292)

<!-- AC-1518-->

* 이제 상점에서 이동할 때 `/checkout/sidebar/updateItemQty/?item_qty=error` Magento Open Source 에서 500 오류 대신 404 오류를 반환합니다. 이전에는 이 오류가 발생했습니다  `Warning: A non-numeric value encountered in /vendor/magento/module-checkout/Controller/Sidebar/UpdateItemQty.php on line 69`. [GitHub-34380](https://github.com/magento/magento2/issues/34380)

<!-- ACP2E-847-->

* Magento Open Source SSL 구성 없이 AMPQ 연결을 구성할 때 더 이상 오류가 발생하지 `trim(): Passing null to parameter #1 ($string) of type string is deprecated` 않습니다.
...
* 주소 편집 및 추가 페이지에서 주소 주소의 두 번째 줄에 유효성 검사가 추가되었습니다. 이제 두 번째에 지정된 최소 및 최대 텍스트 길이가 적용됩니다.

<!-- ACP2E-798-->

* 업그레이드 `A technical problem with the server created an error. Try again to continue what you were doing. If the problem persists, try again later`후 관리자에서 카탈로그 규칙 생성 중에 Magento Open Source 에서 더 이상 다음 오류가 발생하지 않습니다.

<!-- AC-2445-->

* 위Magento Open Source 공유&#x200B;**를 클릭**&#x200B;하기 전에 RSS 피드 위시리스트&#x200B;**에 링크하려면 여기를 확인 확인란을 활성화**&#x200B;해도 더 이상 오류가 발생하지 않습니다. [GitHub-34998](https://github.com/magento/magento2/issues/34998)

<!-- AC-2019-->

* 이제 보기 암호&#x200B;**확인란의**&#x200B;제목(고객 로그인, 고객 등록, 고객 편집(변경 암호 섹션) 및 고객 집합 새로 만들기 암호 양식)을 번역할 수 있습니다. [GitHub-34857](https://github.com/magento/magento2/issues/34857)
...
* GraphQL 요청이 수행된 후 Magento Open Source 세션이 더 이상 종료되지 않습니다. 이전에는 플러그인 이 `ClearCustomerSessionAfterRequest` 쇼핑객 로그아웃을 했습니다. [GitHub-34550](https://github.com/magento/magento2/issues/34550)
...
* `setShippingAddressesOnCart` 이제 요청이 지역 ID의 유효성을 성공적으로 검사합니다. 이전에는 지역 코드 대신 지역 ID를 사용할 때 applicationOpen 소스에서 오류가 발생했습니다.
...
* 이제 고객, 고객 주소 및 주문 작업이 관리 작업 보고서에 올바르게 기록됩니다. 이전에는 applicationOpen 소스 구성 설정에 처리기가 `postDispatch` 지정되지 않은 경우 작업을 기록하지 않았습니다.
...
`AdminConfigurableProductChildrenOutOfStockTest`[GitHub-32378](https://github.com/magento/magento2/issues/32378)`AdminCreateStoreViewTest` [GitHub-34631
](https://github.com/magento/magento2/issues/34631)
...
* 이제 Magento Open Source는 PayPal로 결제한 게스트 주문에 올바른 고객 이름을 표시합니다. 이전에는 고객 이름이 게스트로 표시되었습니다.

<!-- AC-2606-->

* createPaypalExpressToken 변형에 대한 해석기가 use_PayPal_credit 입력 필드에 지정된 값을 올바르게 사용하도록 업데이트되었습니다. 이전에는 잘못된 `paypal_credit` 필드를 사용하려고 했습니다. [GitHub-35180](https://github.com/magento/magento2/issues/35180)

### 공연

<!-- ACP2E-52-->

* 구성 가능한 제품의 관련 제품 데이터 캐시에 대한 TTL(만료 날짜)이 추가되어 Redis 캐시 관리가 개선되었습니다. 이전에는 Redis 키 제거 정책이 일시적 제거 정책으로 구성된 경우 TTL 값 누락으로 인해 이러한 캐시가 제거되지 않았습니다.

<!-- ACP2E-464-->

* 새 `Grid Filter Condition Type` 고객/고객 주소 속성은 속성 필터가 데이터베이스의 속성 값과 일치하는 방법을 제어하며, 옵션에는 , `Prefix Match`, 및 `Full Match`가 포함됩니다`Partial Match`.

<!-- AC-1136-->

* 전체 텍스트 인덱서Search 카탈로그가 저장소 루프 외부로 재배치되어 다시 인덱싱이 간소화되었습니다. [GitHub-33984](https://github.com/magento/magento2/issues/33984)

<!-- AC-2508-->

* in 루프 문제가 `array_merge` 수정되었습니다. [GitHub-33929](https://github.com/magento/magento2/issues/33929)
...
* 허용된 배송 방법을 선택하지 않은 경우 UPS 배송 비율을 로드할 때 Magento Open Source에서 더 이상 오류가 발생하지 않습니다. 이전에는 이러한 조건에서 쇼핑객이 결제 작업 과정 에 배송 주소를 입력하면 다른 배송 방법이 표시되지 않았으며 Magento Open Source 상점에 오류가 표시되었습니다. [GitHub-34411](https://github.com/magento/magento2/issues/34411)
...
* PHP 8.1  `exception main.ERROR: /var/www/html/lib/internal/Magento/Framework/DataObject.php:131 strpos(): Passing null to parameter #1 ($haystack) of type string is deprecated`에서 제품 이미지가 설정되지 않은 경우 친구 보내기 기능에 대한 WebAPI 테스트를 실행할 때 다음 예외가 더 이상 발생하지 않습니다. [GitHub-34864](https://github.com/magento/magento2/issues/34864)
...
* 제품 세부 사항 페이지 페이지의 장바구니&#x200B;**에 추가 버튼 버튼에 있는**&#x200B;텍스트는 이제 연결된 로케일에 지정된 언어로 번역된 상태로 유지됩니다. 이제 번역 파일은 영역, 테마 및 로케일에 `translate_strategy=embedded`따라 해당 JavaScript 파일로 변환됩니다. 이전에는 제품이 장바구니에 추가된 후 텍스트가 영어 형식으로 되돌아갔습니다.

<!-- ACP2E-752-->

* 이제 Search 동의어는 할당된 스토어 범위를 따릅니다. 이전에는 특정 스토어에 할당된 동의어를 다른 스토어에서 검색할 수 있었습니다.

<!-- AC-1178-->

* 필리핀어(필리핀) 로케일과 관련된 문제가 해결되었습니다. [GitHub-33996](https://github.com/magento/magento2/issues/33996)

### UI

<!-- ACP2E-557-->

* 카탈로그&#x200B;**>**&#x200B;제품&#x200B;**그리드의**&#x200B;긴 제품 이름이 이제 한 줄로 표시되지 않고 단어 줄바꿈으로 표시됩니다.

<!-- ACP2E-347-->

* 이제 최소 및 최대 생년월일 범위가 올바른 타임스탬프로 저장된 다음 유효한 타임스탬프에서 유효한 날짜 포맷 형식으로 변환됩니다.

<!-- ACP2E-280-->

* 를 `magento.com` 사용할 수 없어도 관리자 로그인 중에 더 이상 성능 문제가 발생하지 않습니다. 릴리스 알림 가져오기 요청 시간 초과가 추가되었습니다.

<!-- ACP2E-88-->

* 이제 관리 주문, 고객 및 제품 그리드 필터의 결과가 크롬 브라우저 에 표시될 때 예상대로 유지됩니다.
...
**문제**: Braintree 가 활성화된 경우 관리자 사용자는 관리자에서 고객에 대한 주문 또는 재주문을 생성할 수 없습니다. 관리 사용자 중 하나가 **주문** 또는 **재정렬**&#x200B;을 클릭하면 Adobe Systems Commerce가 주문을 제출하지 않고  `system.log` 다음 오류가 `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`표시됩니다.  **해결 방법**: `BUNDLE-3137-composer.patch`을(를) 사용할 수 있습니다. 이 문제에 대한 논의와 패치에 대한 액세스는 [Braintree 결제를 사용할 때 관리자가 주문/순서를 만들 수 없습니다](https://support.magento.com/hc/en-us/articles/8322266352525) 기술 자료 문서를 참조하십시오. 수정 사항이 Adobe Commerce 2.4.5-p1에도 포함됩니다. <!-- AC-6222-->
...
다음 표에는 커뮤니티 구성원의 기여가 나와 있습니다. 이 표에는 가져오기 요청에 기여한 커뮤니티 멤버, 외부 가져오기 요청 번호 및 이와 관련된 GitHub 문제 번호(사용 가능한 경우)가 나열되어 있습니다.
...
Composer](../../../installation/composer.md)를 사용하여 [Magento Open Source 2.4.5를 설치할 수 있습니다.
