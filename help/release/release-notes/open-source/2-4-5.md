...
Magento Open Source에서는 플랫폼 품질, 결제 방법, GraphQL 캐싱 성능 및 접근성 개선 사항이 도입되었습니다. 여기에는 통합 Google 모듈에 대한 업데이트가 포함되어 있습니다.
...
* 제품이 품절된 경우 `cart` 쿼리가 더 이상 null 응답을 반환하지 않습니다. 오류 메시지가 포함된 새 `errors` 요소가 응답에 도입되었습니다. 이전에는, 품절 제품으로 쿼리를 실행했을 때 Source 열기 응용 프로그램이 응답의 `null` 섹션 아래에 `items` 값을 표시했습니다. [장바구니 쿼리](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/queries/cart/)를 참조하세요.
...
* 이제 `updateCartCurrency` 함수가 장바구니 개체 내의 개체 대신 문자열을 설정합니다. 이전에는 `getQuote` 함수가 장바구니 개체 내에서 문자열 대신 개체를 설정했기 때문에 applicationOpen Source이 `updateCartCurrency`을(를) 사용하여 견적을 로드하지 않았습니다. [GitHub-34199](https://github.com/magento/magento2/issues/34199)
...
* Magento Open Source은 더 이상 관리자에 `preg_replace()` 오류를 표시하지 않습니다. 세 번째 인수(`$subject`)는 이제 `array|string` 대신 `bool` 형식입니다.
...
* 가맹점은 이제 Braintree을 통해 Apple 페이로 결제한 주문의 일부 환불을 제출할 수 있다. 이는 Magento Open Source 2.4.4에서 알려진 문제입니다.
...
* 이제 CSP가 `banner`(으)로 설정된 경우 `restrict mode` 요소에서 백그라운드에서 Vimeo 비디오를 실행하도록 설정할 수 있습니다. 이전에는 applicationOpen Source에서 JavaScript 오류가 발생했습니다.
