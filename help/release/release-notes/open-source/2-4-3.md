...
이 오류는 PHP 8.x 함수인 `str_contains` 함수를 사용했기 때문에 발생합니다. 응용 프로그램 Open Source 2.4.3은 PHP 8.x를 지원하지 않습니다. 이 핫픽스는 이 함수를 지원되는 PHP 7.x 함수로 대체합니다. [Adobe Commerce 업그레이드 2.4.3, 2.3.7-p1 PHP 치명적 오류 핫픽스](https://support.magento.com/hc/en-us/articles/4408021533069-Adobe-Commerce-upgrade-2-4-3-2-3-7-p1-PHP-Fatal-error-Hotfix) 기술 자료 문서를 참조하십시오.
...
#### 원격 코드 실행(RCE) 및 크로스 사이트 스크립팅(XSS) 취약성을 해결하는 데 도움이 되는 33가지 보안 개선 사항

현재까지 이러한 문제와 관련된 확인된 공격은 발생하지 않았습니다. 그러나 특정 취약성은 잠재적으로 악용되어 고객 정보에 액세스하거나 관리자 세션을 장악할 수 있습니다. 이러한 문제의 대부분은 공격자가 먼저 관리자에 대한 액세스 권한을 얻어야 합니다. 따라서 IP 허용 목록, [이중 인증](https://developer.adobe.com/commerce/testing/functional-testing-framework/two-factor-authentication/), VPN 사용, 가 아닌 `/admin`고유한 위치 사용, 양호한 암호 위생 등을 포함하되 이에 국한되지 않는 관리자를 보호하기 위해 필요한 모든 조치를 취해야 합니다. 이러한 수정된 문제에 대한 자세한 내용은 Adobe Systems 보안 게시판](https://helpx.adobe.com/security/products/magento/apsb21-64.html)을 참조하십시오[.
...
* 더 이상 사용되지 않는 TinyMCE v3 라이브러리가 제거되었습니다. `Magento_Tinymce3Banner` TinyMCE v3.x와 관련된 모듈 및 MFTF 테스트가 Adobe Systems Commerce에서 제거되었습니다.<!-- MC-42199 42170 -->

* Magento Open Source 2.4.3은 테스트를 거쳐 Redis 6.0.12와 호환되는 것으로 확인되었습니다. (버전 2.4.x는 Redis 5.x와 계속 호환됩니다.)

* Laminas 라이브러리 종속성이 PHP 8.x 호환 버전으로 업그레이드되었습니다. `composer.json` 파일에서 일부 중복 종속성이 제거되었습니다. **Magento Open Source 2.4.3은 Laminas 3.4.0**&#x200B;을 사용합니다. <!-- MC-39513-->
...
이 릴리스에는 공유 경로에 대한 GraphQL 적용 범위가 추가됩니다. [라우팅 쿼리](https://developer.adobe.com/commerce/webapi/graphql/queries/route.html) 및 [RoutableInterface](https://developer.adobe.com/commerce/webapi/graphql/interfaces/routable-interface.html)는 제품, 범주 및 CMS 페이지에서 라우팅 요청을 지원합니다. `urlResolver` 쿼리는 더 이상 사용되지 않으며 해당 기능은 `route` 쿼리로 대체되었습니다.
...
* `updateProductsInWishlist` 돌연변이가 이제 위시리스트의 번들 제품에 속하는 항목을 성공적으로 업데이트했습니다. 이전에는 이 돌연변이가 위시리스트 항목을 업데이트하는 대신 항목을 삭제하고 새 항목을 만들어 항목 ID를 변경했습니다.
...
* 이제 CAPTCHA는 쇼핑객이 제공한 데이터의 유효성을 올바르게 검사하며, 이제 쇼핑객이 PayPal Payflow Pro로 결제를 여러 번 시도했지만 실패한 후 CAPTCHA 필드가 예상대로 표시됩니다.

<!-- MC-41281-->

* CAPTCHA 유효성 검사가 더 이상 결제 작업 과정의 결제 페이지 에서 임의로 실패하지 않습니다.

<!-- MC-41001-->

* 이제 실패한 완료 시도 횟수를 초과한 후 애플리케이션 앞에 CAPTCHA 필드가 표시됩니다. 이전에는 애플리케이션 프로그램에서 CAPTCHA 챌린지를 다시 시도하라는 메시지가 표시되더라도 CAPTCHA 필드가 표시되지 않았습니다.

<!-- MC-40359-->

* 이제 CAPTCHA가 체크아웃 페이지 예상대로 작동합니다. 이전에는 쇼핑객이 CAPTCHA 질문에 올바르게 응답한 후 결제 페이지의 로더가 완료되지 않고 애플리케이션에 다음 오류가 `captchaData[formId] is undefined`표시되었습니다. (이 오류는 쇼핑객 고객이 이전에 2.3.5-p1을 실행하는 배포에 액세스한 것과 동일한 브라우저 를 사용하는 경우에만 발생했습니다.)
...
* 게스트가 생성한 모든 유료 결제 거래는 이제 데이터베이스에 저장되고 예상대로 관리자에 표시됩니다. 이전에는 동시 주문의 작은 하위 집합만 데이터베이스에 저장되었으며 데이터베이스 잠금으로 인한 시간 초과로 인해 대부분의 주문이 손실되었습니다. [GitHub-25862](https://github.com/magento/magento2/issues/25862)

<!-- MC-23989-->

* 이제 애플리케이션 앱은 게스트가 미니 장바구니에 제품을 넣을 때 특수 문자가 포함된 인라인 환영 메시지를 올바르게 표시합니다. 이전에는 애플리케이션 프로그램이 제품을 미니 장바구니에 추가하거나 환영 메시지를 표시하지 않았습니다. [GitHub-32250](https://github.com/magento/magento2/issues/32250)

<!-- MC-41165-->

* 이제 스토어 내 배송이 활성화된 경우 체크아웃 작업 과정의 배송 페이지가 성공적으로 로드됩니다. 이전에는 애플리케이션 시 JavaScript 오류가 발생하여 배송 체크아웃 페이지 가 완전히 렌더링되지 않았습니다.
...
* 관리자와 최근 제품 동기화를 활성화한 후 애플리케이션에서 더 이상 JavaScript 오류가 발생하지 않습니다. 이전에는 애플리케이션 프로그램에 이 JavaScript 오류가 `Cannot read property 'status' of undefined`표시되었습니다.
...
* 제한된 권한을 가진 관리자가 관리자의 CMS 페이지에 제품 위젯을 추가할 때 애플리케이션에서 더 이상 오류가 발생하지 않습니다. 이전에는 관리자가 **저장** 단추를 클릭할 때 응용 프로그램에서 이 오류가 발생했습니다. `We are sorry, an error has occurred while generating the content`.

<!-- MC-39134-->

* 이제 제품 세부 사항 페이지가 열리며 JavaScript 달력&#x200B;**사용이 활성화된 경우**&#x200B;이전 주문의 날짜로 채워진 사용자 지정 가능한 날짜 옵션이 표시됩니다. 사용자 지정 날짜 옵션 값 확인자는 이제 값이 현재 구성에 따라 형식화되지 않은 경우 대체 포맷 형식으로 대체됩니다. 이전에는 사용자 지정 날짜 옵션 값이 비어 있었습니다.

<!-- MC-39104-->

* 이제 제품이 품절되었을 때 애플리케이션 프로그램은 장바구니 하나에 하나의 오류만 표시합니다. 이전에는 애플리케이션 시스템에 중복 메시지가 표시되었습니다. [GitHub-27469](https://github.com/magento/magento2/issues/27469)
...
* `indexer_update_all_views`  이제 cron 작업이 이전 실패 후 예상대로 실행됩니다. 실패한 실행은 에서 실패 `cron_schedule schedule`로 표시되며 후속 실행은 자동으로 실패하지 않습니다. 이전에는 테이블이 `cron_schedule` 보류 중인 작업으로 가득 찼고 `indexer_update_all_views` cron 작업이 실행되지 않았습니다. [GitHub-23054](https://github.com/magento/magento2/issues/23054)

<!-- ENGCOM-8571-->

* `cron` 교착 상태는 그룹이 겹치는 대규모 배포에서 잠금을 설정하려고 시도한 결과로 `cron` 더 이상 발생하지 않습니다. [GitHub-8933](https://github.com/magento/magento2/issues/8933)

<!-- ENGCOM-8571-->

* `cron`몇 개의 `cron` 작업만 실행된 후 테이블에서 `cron_schedule` 더 이상 교착 상태가 발생하지 않습니다. [GitHub-22438](https://github.com/magento/magento2/issues/22438)

### 사용자 지정 고객 속성

<!-- MC-41611-->

* **상점 고객 계정 주소록의 상태** 필드는 이제 드롭다운 페이지 요소로 로드되고 유지됩니다. **이제 모든 페이지 요소가 완전히 로드될 때까지 제출** 버튼 비활성화됩니다. 이전에는 애플리케이션이 이 필드를 드롭다운 요소로 렌더링하기 전에 텍스트 상자로 로드했으며 쇼핑객이 텍스트 필드에 값을 입력하고 저장할 수 있었기 때문에 나중에 결제 중에 오류가 발생했습니다.

<!-- MC-40329-->

* 파일을 업로드할 때 Admin Customer address(관리자 고객 주소) 필드에 첨부 파일이 있는 고객 주소 속성을 저장할 때 애플리케이션 에서 더 이상 오류가 발생하지 않습니다. 이 문제는 컨트롤러 작업에서 반환 문이 누락되어 발생했습니다. 이전에는 응용 프로그램에서 다음 오류가 발생했습니다. `Something went wrong while saving the file`.
...
* 이제 애플리케이션 기능은 모든 웹 사이트에서 비동기 이메일 전송을 활성화하지 않은 다중 사이트 배포에서 예상대로 이메일을 전송합니다. 이전에는 하나 이상의 웹 사이트에서 이 설정을 비활성화한 경우 활성화된 경우 어떤 웹 사이트에서도 이메일이 전송되지 않았으며 균일 했습니다. 송장, 발송물 및 대변 메모 이메일에도 비슷한 문제가 있었습니다. 그러나 주문 설명, 송장 설명, 배송 설명 및 대변 메모 설명 이메일은 정상적으로 전송되었습니다. [GitHub-31950](https://github.com/magento/magento2/issues/31950)
...
* 관리자의 부모 클래스는 이제 원래 클래스 및 상속된 인터페이스와 함께 docblock 주석 여부를 확인합니다. 이전에는 상위 클래스가 확인되지 않았기 때문에 이벤트를 트리거한 관리에서 고객 관련 작업을 수행하면 오류가 발생했습니다. 애플리케이션 예외 로그 `report.CRITICAL: Method's return type must be specified using @return annotation.`에 이 오류를 기록했습니다.
...
* 이제 애플리케이션 에서 백업 활성화된 경우 백업&#x200B;**에서 미디어 `bin/magento config:set system/backup/functionality_enabled 1`폴더 제외 설정을 인식**. 이전에는 이 설정에도 불구하고 미디어 폴더 경로 `/magento` 가 double //로 잘못 구성되었기 때문에 백업되었습니다.

<!-- MC-40654 ENGCOM-8881-->

* 이제 애플리케이션이 관리 장바구니 페이지에 대해 예상대로 번역 가능한 모든 문자열을 번역합니다. 이전에는 번역 로드가 너무 늦게 발생하여 컨트롤러 작업 사전 디스패치를 구독한 모든 관찰자를 건너뛰었습니다. [GitHub-31849](https://github.com/magento/magento2/issues/31849)
...
* 전역 Magento Open Source 플러그인(예: `webapi_rest` 및 `graphql` )은 이 영역 파일의 `di.xml`에 등록된 플러그인이 없는 경우 더 이상 새 사용자 지정 영역 유형에 대해 트리거되지 않습니다.
...
* 더 이상 특성의 `media_gallery` 범위 를 변경할 수 없습니다. 이전에는 특성의 `media_gallery` 범위 를 다시 `global`로 변경하면 애플리케이션 프로그램에서 오류가 발생했습니다.

<!-- MC-36818  magento/partners-magento2ee#470-->

* 예약된 업데이트로 추가된 관련 제품은 업데이트 종료 날짜 이후 더 이상 상점 제품 페이지 에 표시되지 않습니다. [GitHub-469](https://github.com/magento/partners-magento2ee/issues/469)
...
* 이제 범주가 데이터베이스 저장 방법을 사용하여 저장될 때 범주 이미지가 디렉터리 `catalog/category` 에서 `catalog/tmp/category` 디렉터리로 예상대로 복사됩니다. 이제 테이블의 이미지 행 `media_storage_file_storage` 에도 올바른 `directory_id`. [GitHub-11995](https://github.com/magento/magento2/issues/11995)

<!-- MC-24768-->

* 이제 판매자는 CMS 계층 구조에 변경사항을 제출한 후 예상대로 무효화된 캐시에 대한 알림을 받습니다. 이전에는 애플리케이션 프로그램이 CMS 계층 구조를 저장하는 동안 캐시를 무효화하지 않았습니다.

<!-- MC-24725-->

* 장바구니의 장바구니 섹션에 대한 중복 AJAX 요청이 줄었습니다. 이전에는 애플리케이션이 장바구니 부분합을 제대로 로드하지 않아 장바구니 다시 로드가 다시 트리거되었습니다.
...
* `nowdoc` 이(가) 스토어 전환기에서 교체 `heredoc` 되었습니다 `Magento_Backend` . [GitHub-32262](https://github.com/magento/magento2/issues/32262)

<!-- ENGCOM-8685-->

* 아르헨티나, 볼리비아, 칠레, 에콰도르, 가이아나, 파라과이, 페루, 수리남, 베네수엘라 지역이 `directory_country_region` 테이블에 추가되었습니다. [GitHub-31169](https://github.com/magento/magento2/issues/31169)
...
* 이제 Royal Mail Click &amp; Drop 통합이 예상대로 작동합니다. 이전에는 애플리케이션 통합을 `Sorry! Something went wrong. Please try again later`활성화하려고 할 때 이 오류가 발생했습니다. [GitHub-28996](https://github.com/magento/magento2/issues/28996)

<!-- ENGCOM-8498-->

* 이제 잘못된 발송물, 대변 메모 또는 송장 ID가 URL에 전달될 때 애플리케이션에 정보 오류 메시지가 표시됩니다. 이전에는 애플리케이션 프로그램에서 치명적인 오류가 발생했습니다. [GitHub-30424](https://github.com/magento/magento2/issues/30424)

<!-- ENGCOM-8632-->

* 정적 파일 위치의 재귀가 제거되었습니다.  이제 애플리케이션에 500 오류 대신 404 페이지가 표시됩니다. 이전에는 기본 NGINX 구성 리드 버그로 인해 무한 재귀가 발생했습니다. [GitHub-31530](https://github.com/magento/magento2/issues/31530)

<!-- ENGCOM-8550-->

* 파일 이름이 `cms_index_noroute.xml` (으 `cms_noroute_index.xml`)로 수정되었습니다. [GitHub-31300](https://github.com/magento/magento2/issues/31300)

<!-- ENGCOM-8516-->

* `.editorconfig` 파일의 자동 서식 `db_schema_whitelist.json` 을 수정하기 위해 파일이 리팩토링되었습니다. [GitHub-31171](https://github.com/magento/magento2/issues/31171)

<!-- ENGCOM-9018-->

* 브라우저 창 크기를 조정해도 더 이상 중복 바인딩 돋보기 이벤트가 트리거되지 않습니다. [GitHub-30788](https://github.com/magento/magento2/issues/30788)
...
* 이제 POST `V1/products/special-price-delete` 요청에서 예상대로 지정된 `store_id`의 가격만 삭제합니다. 이전에는 이 호출로 모든 스토어에서 지정된 SKU에 대한 모든 특별 가격이 제거되었습니다. [GitHub-25907](https://github.com/magento/magento2/issues/25907)
...
* 구성 가능한 제품 재고 상태는 이제 가져오기를 통해 하위 제품 재고 상태가 업데이트될 때 예상대로 자동으로 업데이트됩니다. 이전에는 가져오기를 통해 하위 제품 재고 상태를 업데이트할 때 제품 재고 상태가 자동으로 업데이트되지 않았습니다.
...
* 이제 `tax_class_name` 값이 `None` 또는 `0`인 제품을 가져오는 경우 응용 프로그램에서 제품 세금 클래스를 `None`(으)로 설정합니다. 이전에는 CSV 파일에서 `product tax_class_name`이(가) `None`이면 응용 프로그램에서 새 세금 클래스 `None`을(를) 만들어 기존 세금 클래스를 복제했습니다. CSV 파일에서 `product tax_class_name`이(가) `0`인 경우 응용 프로그램에서 해당 값을 무시했으며 가져온 후 제품 세금 클래스가 변경되지 않았습니다.
...
* 페이지 레이아웃은 더 이상 에서 하드 코딩 `Magento\Widget\Block\Adminhtml\Widget\Instance\Edit\Chooser\Container`되지 않습니다.  결과적으로 `getPageLayouts()` 함수는 이제 다른 모듈에서 선언한 실제 페이지 레이아웃 목록을 예상대로 반환합니다. 이전에는 하드 코딩된 레이아웃만 반환했습니다. [GitHub-31168](https://github.com/magento/magento2/issues/31168)

<!-- ENGCOM-8509-->

* 파일이 `composer.lock` 최신 버전의 코딩 표준으로 업데이트되었습니다. [GitHub-31152](https://github.com/magento/magento2/issues/31152)

<!-- ENGCOM-9015 -->

* 패키지`magento/module-jwt-framework-adapter`에 대한 누락된 종속성을 `web-token/jwt-framework` . [GitHub-32578](https://github.com/magento/magento2/issues/32578)

<!-- ENGCOM-9019 -->

* Lighthouse 메트릭 점수를 개선하기 위해 수동적 수신기가 라이브러리에 `fotorama.js` 추가되었습니다. [GitHub-31140](https://github.com/magento/magento2/issues/31140)
...
* 플러그인이 상위 클래스에 추가될 때 애플리케이션에서 더 이상 오류가 발생하지 않습니다. (이제 `Assest.php`에서 `optionsProvider` 매개 변수가 `getContentIdentities` 뒤에 선언되었습니다.) 이전에는 응용 프로그램에서 다음 오류가 발생했습니다. `Error: Cannot instantiate interface Magento\Framework\Data\OptionSourceInterface`. [GitHub-31291](https://github.com/magento/magento2/issues/31291)
...
* SameSite 쿠키 매개 변수에 대한 연결이 수정되었습니다. 이전에는 잘못된 연결로 `value`, `domain` 및 기타 매개 변수에 `lex` 접미사가 추가되었습니다. [GitHub-26377](https://github.com/magento/magento2/issues/26377), [GitHub-32440](https://github.com/magento/magento2/issues/32440)
...
* `Bundle`, `Catalog`, `CatalogRule`, `CatalogRuleConfigurable`, `CatalogUrlRewrite`, `Downloadable`, `Indexer`, `Paypal` 및 `Sales` 모듈의 실행을 개선하기 위해 `magento indexer:reindex` 및 `cache:flush` 명령과 `AdminReindexAndFlushCache` 작업 그룹이 테스트에서 제거되었습니다. [GitHub-31031](https://github.com/magento/magento2/issues/31031)
...
* 큰 테이블(수 백만 행)에서 실행할 때 `catalog_product_alert` cron 프로세스의 성능이 향상되었습니다. 이전에는 `catalog_product_alert`에서 모든 제품 알림을 로드하여 메모리 부족 예외가 발생했습니다.
...
* 이제 쇼핑객은 성이 공백으로 끝나는 경우 주문 및 반품 페이지 에서 주문을 찾을 수 있습니다.

<!-- MC-41438-->

* 이제 100% 할인을 할당하는 장바구니 가격 규칙이 카탈로그 제품 및 할인세가 적용되고 무료 배송이 가능한 주문에 적용되는 경우 송장이 올바른 총합계로 생성됩니다. 이전에는 주문의 가격은 올렸지만 송장은 그렇지 않았습니다. [GitHub-30853](https://github.com/magento/magento2/issues/30853)
...
* Safari를 사용하는 관리자는 이제 관리자의 주문에 제품을 추가할 수 있습니다. 이전에는 관리자가 **선택한 제품을 주문에 추가** 단추를 클릭하면 응용 프로그램에 회전 하중 아이콘이 표시되고 페이지가 정지되었습니다.
...
* 이제 검색 가능으로 구성될 때 속성이 예상대로 Search 결과에 포함됩니다 `weight` .

<!-- MC-40409-->

* Elasticsearch가 활성화된 빈 범주 페이지를 볼 때 애플리케이션에서 더 이상 오류가 발생하지 않습니다. 대신 예상대로 페이지 렌더링을 수행하고 정보 메시지를 표시합니다. 이전에는 빈 전체 텍스트 인덱스가 카테고리 페이지 예외를 트리거했습니다.
...
* 이제 사용자 지정 Elasticsearch 필드 매퍼를 `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver`에 추가할 수 있습니다.

<!-- ENGCOM-9029 MC-24026-->

* 이제 개발자는 Elasticsearch 매핑을 변경할 수 있습니다. 이전에는 기본 매핑에서 Elasticsearch 서버로 전송된 동적 템플릿이 하드 코딩되어 개발자가 사용자 지정 모듈과 연결된 추가 데이터를 Elasticsearch에 색인화할 수 없었습니다.

<!-- ENGCOM-8464 -->

* 검색 제안 기능이 비활성화되면 검색 필드(양식 미니검색)가 예상대로 작동합니다.
...
* 특정 웹 사이트로 제한된 관리자는 이제 동일한 웹 사이트에 배치된 주문에 대해 배송을 만들 수 있습니다. 이전에는 기본 스토어 보기에 대한 권한이 없는 관리자가 스토어 보기에 배치된 주문을 배송하려고 할 때 관리 사용자가 액세스할 수 있는 경우 응용 프로그램에서 이 예외를 throw했습니다. `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`.
...
* 쇼핑객이 **배송 방법 및 요금 가져오기**&#x200B;를 클릭하고 관리자로부터 주문을 만들 때 배송 방법을 선택할 때 응용 프로그램에서 더 이상 **댓글 추가** 확인란의 선택을 취소하지 않습니다.
...
* 이제 구매자가 여러 주소로 배송 페이지에서 수량을 변경하고 브라우저 뒤로 단추를 클릭한 후에도 제품 수량은 예상대로 변경되지 않습니다.
...
* 판매자가 **배송 레이블 만들기** 확인란이 활성화되어 있고 제품 이름에 유니코드 문자가 포함되어 있는 경우 DHL을 사용하여 주문을 배송하려고 할 때 응용 프로그램에서 더 이상 오류가 발생하지 않습니다. 이전에는 레이블 생성을 요청할 때 응용 프로그램에서 이 오류를 표시했습니다. `The response is in wrong format`.

<!-- ENGCOM-8952 -->

* 한 웹 사이트에서 `United States`을(를) 사용하고 다른 웹 사이트에서 `Disable all countries`을(를) 기본 범위로 사용하는 다중 사이트 배포에서 관리자의 주문을 정상적으로 수행할 수 있습니다. 이전에는 응용 프로그램에서 주문을 하지 않았으며 다음 오류가 표시되었습니다. `Please check the shipping address information. "regionId" is required. Enter and try again`.
...
* `lib/internal/Magento/Framework/App/Action/Plugin`에서 `\Magento\Framework\App\ActionInterface`에 대한 플러그인이 제거되어 다른 모듈에서 한 모듈의 동작을 사용자 지정하는 데 플러그인을 사용해야 한다는 지침을 따릅니다. [GitHub-28050](https://github.com/magento/magento2/issues/28050)

<!-- MC-24772-->

* 웹 사이트를 만드는 동안 `Store View`을(를) 사용하지 않도록 설정한 경우 `Website`을(를) 기본적으로 선택하려고 하면 응용 프로그램에서 이 메시지를 표시합니다. `Please enable your Store View before using this Web Site as Default`. 이전에는 웹 사이트가 손상되어 응용 프로그램에 경고가 표시되지 않았습니다.
...
* `customer/create_account/vat_frontend_visibility`을(를) 사용하도록 설정하면 응용 프로그램에서 고객 등록 페이지에 **VAT 번호** 필드를 표시합니다.
...
* 통합 테스트에서 `CacheCleaner::cleanAll();`을(를) 제거했습니다.

<!-- ENGCOM-8898-->

* 이 시나리오에 대한 테스트가 추가되었습니다. 관리자는 고객이 큐에 있는 뉴스레터를 구독할 때 고객 계정을 편집할 수 있습니다.

<!-- ENGCOM-8484-->

* `Catalog`, `CatalogUrlRewrite` 및 `LoginAsCustomer` 모듈의 실행 시간을 개선하기 위해 테스트에서 `cache:flush` 명령을 제거했습니다.
...
* 이제 애플리케이션은 표준 Magento Open Source 테마를 사용하는 상점 첫 페이지에 페이지 요소를 일관되게 표시합니다. 이전에는 일부 스타일이 빈 테마에 적용되지 않았으므로 이 테마를 사용하는 페이지에 일부 페이지 요소가 표시되지 않았습니다. (예를 들어, 내 주문 페이지 검색 필드에 돋보기 아이콘이 없습니다.)

### 번역 및 로케일

<!-- MC-23553-->

* 인라인 변환이 구성되면 문자열에 추가된 대괄호는 더 이상 이스케이프되지 않습니다. 이전에는 `escapeHtmlAttr` 메서드가 대괄호를 HTML 엔터티 코드로 변환했습니다.
...
* 저장된 결제 방법(**내 계정** > **저장된 결제 방법** )을 관리하는 데 사용되는 템플릿의 텍스트 문자열을 변환할 수 있습니다.

<!-- MC-30798 ENGCOM-8717-->

* 이제 관리자에서 보낸 주문 전자 메일은 관리자 계정과 연결된 로케일이 아닌 스토어 로케일을 사용합니다.
...
* 이제 PayPal Express Checkout을 사용하여 체크아웃하는 동안 표시되는 주문 리뷰 페이지가 정상적으로 로드됩니다. 이전에는 템플릿에 사용되지 않은 **게재 방법 업데이트** 단추가 포함되어 있었습니다. 이 단추는 페이지 렌더링 중에 스크립트에 의해 일부만 숨겨져 있었습니다.

<!-- MC-37941-->

* 이제 제품 페이지의 고객 리뷰 섹션에 있는 평가 별과 리뷰 텍스트의 간격은 긴 평가 이름이 있을 때 적절히 조정됩니다.
...
* 이제 애플리케이션에서 고객 > 로그인&#x200B;**페이지의**&#x200B;로그인함 영역에 필터를 표시합니다`DateRange`. **** 이전에는 애플리케이션 에 필터가 `Text` 표시되었습니다. [GitHub-30328](https://github.com/magento/magento2/issues/30328)

<!-- ENGCOM-9009 -->

* 관리 페이지에서 렌더링 문제를 줄이기 위해 JavaScript 템플릿 파일에서 제거되고 별도의 파일로 이동되었습니다. 이전에는 HTML 축소가 활성화된 배포에서 관리 페이지가 제대로 렌더링되지 않았습니다. 애플리케이션 프로그램에 다음 오류가 `An error has happened during application run. See exception log for details`표시되었습니다. [GitHub-32454](https://github.com/magento/magento2/issues/32454)

<!-- ENGCOM-9044 -->

* 이제 위젯은 `dropdownDialog` 필요한 드래그 및 크기 조정 가능한 `jquery-ui` 청크만 로드합니다. 이전에는 불필요한 코드를 대량으로 로드하여 로드 시간이 늘어나고 성능이 저하되었습니다. [GitHub-32810](https://github.com/magento/magento2/issues/32810)
...
* 이제 `PUT /V1/products/:sku` REST 끝점이 예상대로 제품 URL 다시 쓰기를 다시 생성합니다. 이전에는 엔드포인트가 제품 `url_key` 값을 다시 생성했지만 URL 다시 쓰기는 생성하지 않았습니다. [GitHub-30316](https://github.com/magento/magento2/issues/30316)

### 사용자

<!-- MC-40777-->

* 이제 유효한 새 사용자 역할(관리 시스템 > 권한 > 사용자 역할&#x200B;**)을 제외하고 전체 `Catalog` 트리를 제외하고 `Edit Product Design` 저장할 수 있습니다(**&#x200B;카탈로그&#x200B;**>**&#x200B;재고&#x200B;**>**&#x200B;제품&#x200B;**> 편집**&#x200B;제품 디자인&#x200B;**).********** 이전에는 애플리케이션 프로그램이 이 역할의 사용자가 수행한 제품 변경 사항을 저장하지 않고 다음 오류를 `Not allowed to edit the product's design attributes`표시했습니다. [GitHub-31973](https://github.com/magento/magento2/issues/31973)

<!-- MC-38442-->

* 이제 애플리케이션에서는 새 관리자 사용자의 이메일을 생성할 때 올바른 사용자 지정 이메일 템플릿 사용합니다. 이전에는 응용 프로그램에서 관리자 이름과 성을 생략한 이전 기본 템플릿을 사용했습니다.
...
* 이제 Catalog API는 새 값을 추가하고 이전 값을 제거하여 제품의 사용자 지정 옵션 값을 올바르게 업데이트합니다. 이전에는 API가 이전 값을 삭제하지 않았습니다.

<!-- MC-34431-->

* `/V1/guest-carts/:cartId/billing-address` 이제 POST는 주소 ID를 문자열이 아닌 정수로 반환합니다.
...
* 이제 여러 개의 위시리스트가 있는 고객에 대한 위시리스트의 총 제품 수가 위시리스트의 항목 수와 일치합니다. 기존에는 품절 상품이 전체 상품 수치에 포함됐다.
...
**문제**: _SKU에서 장바구니에 추가한 제품_&#x200B;에 대해 **주문에 추가** 단추가 작동하지 않습니다.  SKU별 주문에 제품을 추가한 후 **주문에 추가** 단추를 클릭하면 이 오류 메시지가 표시됩니다. `An error has happened during application run`. 제품이 장바구니에 추가되지 않습니다. **해결 방법**: 제품 추가 기능을 사용하십시오. <!-- AC-345-->
...
Composer](../../../installation/composer.md)를 사용하여 [Magento Open Source 2.4.3을 설치할 수 있습니다.
