...
* Laminas 라이브러리 종속성이 PHP 8.x 호환 버전으로 업그레이드되었습니다. 일부 중복 종속성이 `composer.json` 파일. **Magento Open Source 2.4.3은 Laminas 3.4.0을 사용합니다.**. <!-- MC-39513-->
...
* 의 성능  `catalog_product_alert` 큰 테이블(수백만 행)에서 실행할 때의 cron 프로세스가 개선되었습니다. 이전에는 `catalog_product_alert` 메모리 부족 예외를 일으키는 모든 제품 경고를 로드했습니다.
...
* Safari를 사용하는 관리자는 이제 관리자의 주문에 제품을 추가할 수 있습니다. 이전에는 관리자가 **주문에 선택한 제품 추가** 단추, 응용 프로그램에 회전하는 로드 아이콘이 표시되고 페이지가 정지되었습니다.
...
* 이제 사용자 정의 Elasticsearch 필드 매퍼를 추가할 수 있습니다. `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver`.
...
* 특정 웹 사이트로 제한된 관리자는 이제 동일한 웹 사이트에 배치된 주문에 대해 배송을 만들 수 있습니다. 이전에는, 기본 스토어 보기에 대한 권한이 부족한 관리 사용자가 관리자 사용자가 액세스할 수 있는 스토어 보기에 배치된 주문을 전달하려고 할 때 애플리케이션에서 이 예외가 발생했습니다. `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`.
...
* 응용 프로그램에서 더 이상 **주석 추가** 쇼핑객이 클릭할 때의 확인란 **배송 방법 및 요금 가져오기** 관리자로부터 주문을 만들 때 배송 방법을 선택합니다.
...
* 판매자가 DHL을 사용하여 주문을 배송하려고 할 때 애플리케이션에서 더 이상 오류가 발생하지 않습니다. **배송 레이블 만들기** 확인란이 활성화되고 제품 이름에 유니코드 문자가 포함되어 있습니다. 이전에는 레이블 생성을 요청할 때 애플리케이션에서 이 오류를 표시했습니다. `The response is in wrong format`.

<!-- ENGCOM-8952 -->

* 다중 사이트 배포에서 관리자 주문을 성공적으로 수행할 수 있습니다. `United States` 은(는) 하나의 웹 사이트에서 활성화되며,`Disable all countries` 는 다른 웹 사이트에서 기본 범위로 활성화되어 있습니다. 이전에는 애플리케이션에서 주문을 하지 않고 다음 오류를 표시했습니다. `Please check the shipping address information. "regionId" is required. Enter and try again`.
...
* 이제 응용 프로그램에 **VAT 번호** 다음의 경우 고객 등록 페이지의 필드 `customer/create_account/vat_frontend_visibility` 이(가) 활성화되었습니다.
...
* 제거됨 `CacheCleaner::cleanAll();` 통합 테스트에서.
...
* 을(를) 제거함 `cache:flush` 의 실행 시간을 개선하기 위한 테스트의 명령 `Catalog`, `CatalogUrlRewrite`, 및 `LoginAsCustomer` 모듈.
...
* 인라인 변환이 구성되면 문자열에 추가된 대괄호는 더 이상 이스케이프되지 않습니다. 이전에는 `escapeHtmlAttr` 메서드는 중괄호를 HTML 엔티티 코드로 변환했습니다.
...
* 저장된 결제 방법을 관리하는 데 사용되는 템플릿의 텍스트 문자열(**내 계정**  >  **저장된 결제 방법** )는 이제 번역할 수 있습니다.
...
* 이제 PayPal Express Checkout을 사용하여 체크아웃하는 동안 표시되는 주문 리뷰 페이지가 정상적으로 로드됩니다. 이전에는 템플릿에 사용되지 않은 항목이 포함되어 있었습니다 **게재 방법 업데이트** 단추를 클릭합니다. 이 단추는 페이지 렌더링 중에 스크립트에 의해서만 부분적으로 숨겨졌습니다.
...
다음을 사용하여 Magento Open Source 2.4.3을 설치할 수 있습니다. [작성기](../../../installation/composer.md).
