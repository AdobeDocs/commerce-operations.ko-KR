...
이 릴리스에는 공유 경로에 대한 GraphQL 적용 범위가 추가됩니다. [라우팅 쿼리](https://devdocs.magento.com/guides/v2.4/graphql/queries/route.html) 및 [RoutableInterface](https://devdocs.magento.com/guides/v2.4/graphql/interfaces/routable-interface.html)는 제품, 범주 및 CMS 페이지에서 라우팅 요청을 지원합니다. `urlResolver` 쿼리는 더 이상 사용되지 않으며 해당 기능은 `route` 쿼리로 대체되었습니다.
