...
badgePaas: label="Adobe Commerce on Cloud 전용" type="Informative" url="https://experienceleague.adobe.com/ko/docs/commerce/user-guides/product-solutions" tooltip="클라우드 프로젝트의 Adobe Commerce에만 적용됩니다."
hide: true
...
Adobe 기업 보안 정책을 사용하려면 Adobe Commerce on Cloud에 대한 모든 Adobe 호스팅 환경이 안전하고 호환되는 소프트웨어에서 실행되어야 합니다.

1. 모든 타사 소프트웨어 종속성(PHP, MariaDB, Elasticsearch/OpenSearch, Redis, RabbitMQ)의 지원되는 버전

1. Adobe Commerce on Cloud(버전 2.4.8, 2.4.9 또는 최신 버전)

전자 상거래 환경의 보안 위험을 완화하기 위한 것입니다. [테이블 1](#determine-your-required-actions)의 기한까지 이러한 요구 사항을 충족하지 않는 환경에서는 인바운드 트래픽이 일시 중단되어 상점 오프라인이 됩니다. 이 공지를 시행일을 준수하는 보안 및 규정 준수 요건으로 간주하시기 바랍니다.
...
아래 표에서 Adobe Commerce on Cloud 버전을 찾아 필요한 내용을 확인하십시오.
...
| 2.4.8 또는 2.4.9 | 소프트웨어에 따라 2026년 10월 30일 또는 2027년 5월 31일까지 필요한 작업입니다. | 지금은 필요한 작업이 없습니다. |

## 조치를 취할 필요가 없는 사람

이 통지는 다음 경우에 적용되지 않습니다.

* [!DNL Adobe Commerce as a Cloud Service]을(를) 사용하는 고객
* 모든 환경에서 지원되는 소프트웨어 종속성이 있는 Adobe Commerce on Cloud 버전 2.4.8 또는 2.4.9를 사용하는 고객

### 현재 버전 확인

Cloud 환경의 각 Adobe Commerce에서 실행 중인 버전을 확인하려면 전자 상거래 관리자의 도움이 필요합니다.

#### 1단계: Adobe Commerce on Cloud 버전 확인
...
>[!WARNING]
>
>[필수 작업 및 기한 테이블](#determine-your-required-actions)에 설명된 기한까지 환경이 요구 사항을 충족하지 않으면 Adobe은 영향을 받는 환경에 대한 인바운드 트래픽을 일시 중단해야 합니다. 전자 상거래 상점 앞이 오프라인으로 전환되어 쇼핑객에게 서비스를 제공하지 않습니다.

## 각 작업에 필요한 사항

### 작업 1: 타사 소프트웨어 종속성 업그레이드

소프트웨어에 따라 아래 표에 나와 있는 타임라인을 기준으로 지원되지 않는 모든 소프트웨어 종속성을 업그레이드해야 합니다. [클라우드 콘솔](https://console.adobecommerce.com/)에서 환경을 볼 수 있습니다. 각 환경에서 실행 중인 종속성 버전을 확인하려면 [소프트웨어 종속성 버전 확인](#check-software-dependency-versions)을 참조하세요. 소프트웨어 종속성 업그레이드는 Cloud의 모든 Adobe Commerce 버전 2.4.4부터 2.4.9까지 적용됩니다.
...
* **시작:** 업그레이드해야 하는 환경과 관련된 종속성을 나열하는 지원 티켓을 엽니다. 우리 팀이 작업 일정을 잡을 수 있도록 시행 날짜 최소 30일 전에 티켓을 여십시오.
...
### 작업 2: 지원되는 Adobe Commerce 버전으로 업그레이드

Adobe Commerce on Cloud 버전을 업그레이드해야 하는 경우 두 가지 옵션이 있습니다.

1. [지원되는 클라우드 기반 Adobe Commerce 버전으로 업그레이드](#upgrade-to-adobe-commerce-on-cloud-version-249)
1. [Adobe Commerce as a Cloud Service(SaaS 플랫폼)로 마이그레이션](#migrate-to-adobe-commerce-as-a-cloud-service)

현재 버전에 대한 적용 날짜는 선택한 옵션에 관계없이 적용됩니다.

**표 3: Cloud 버전에서 지원되는 Adobe Commerce으로 업그레이드하는 지침 및 기한**
...
요구 사항에 맞는 옵션을 결정하려면 Cloud의 Adobe Commerce 버전 2.4.9와 Adobe Commerce as a Cloud Service을 비교한 다음 표를 참조하십시오.

**표 4: Cloud의 Adobe Commerce과 Adobe Commerce as a Cloud Service 비교**
...
[필요한 작업을 결정](#determine-your-required-actions)의 적용 날짜까지 환경이 이러한 요구 사항을 충족하지 않으면 Adobe에서 적절한 조치를 취합니다. 여기에는 영향을 받는 인프라에 대한 트래픽 일시 중지가 포함되며, 결과적으로 전자 상거래 상점 오프라인이 됩니다.
...
### Cloud의 Adobe Commerce 버전 2.4.9로 업그레이드
...
Adobe은 Adobe Commerce as a Cloud Service으로 마이그레이션하는 데 드는 비용과 시간을 줄이는 도구를 제공합니다. 이 도구는 마이그레이션에만 적용됩니다. Adobe Commerce on Cloud에서는 버전 업그레이드에 사용되지 않습니다. 마이그레이션 경로 및 단계를 포함한 전체 마이그레이션 가이드는 [마이그레이션 개요](https://experienceleague.adobe.com/ko/docs/commerce/cloud-service/migration/overview)를 참조하십시오.
...
* **Storefront 및 확장 마이그레이션:** Adobe의 [AI 지원 마이그레이션 및 개발자 도구](https://developer.adobe.com/commerce/extensibility/developer-agent/)&#x200B;([!DNL Adobe Developer App Builder] 및 [!DNL Commerce Storefront powered by Edge Delivery Services] 포함)은(는) Storefront 현대화 및 확장 플랫폼 재구성을 가속화하는 데 도움이 됩니다.
