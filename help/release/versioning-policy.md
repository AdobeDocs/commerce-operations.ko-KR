...
핫픽스는 영향력이 큰 보안 또는 품질 수정 사항(예: 제로데이 취약점에 대한 수정 사항)이 포함되어 있는 패치로, 많은 판매자에게 영향을 줍니다. Adobe은 중요한 보안 또는 품질 문제가 영향을 줄 때 지원되는 Adobe Commerce 버전에 대한 핫픽스를 필요에 따라 릴리스합니다. 핫픽스는 [품질 패치 도구](../tools/quality-patches-tool/usage.md)를 통해 제공됩니다. 이러한 수정 사항은 다음에 계획된 패치 릴리스에 포함되어 있습니다.
...
개별 패치에는 특정 문제에 대한 영향이 적은 품질 수정 사항이 포함되어 있습니다. 이러한 수정 사항은 지원되는 Adobe Commerce 부 버전에 적용됩니다. Adobe은 [소프트웨어 수명 주기 정책](lifecycle-policy.md)에 따라 Adobe Commerce에 필요한 개별 패치를 릴리스합니다. 자세한 내용은 [Adobe Commerce 소프트웨어 수명 주기 정책(PDF)](https://www.adobe.com/content/dam/cc/en/legal/terms/enterprise/pdfs/Adobe-Commerce-Software-Lifecycle-Policy.pdf)을 참조하세요. 이 도구는 [품질 패치 도구](../tools/quality-patches-tool/usage.md)를 통해 제공됩니다.
