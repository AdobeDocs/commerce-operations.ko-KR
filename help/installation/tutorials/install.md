...
다음 표에서는 설치 명령 등의 설치 옵션 이름 및 값에 대해 설명합니다. 다음을 참조하십시오 [샘플 localhost 설치](#sample-localhost-installations).
...
Adobe Commerce 버전 2.2.8 이상에서는 설치 중 또는 설치 후에 관리자 사용자를 만들 수 있습니다. 설치 중에 사용자를 만드는 경우 모든 관리자 자격 증명 변수가 필요합니다. 다음을 참조하십시오 [샘플 localhost 설치](#sample-localhost-installations).
...
| `--language` | 관리자 및 상점 첫 화면에서 사용할 언어 코드. (아직 그렇게 하지 않은 경우 다음을 입력하여 언어 코드 목록을 볼 수 있습니다. `magento info:language:list` bin 디렉터리에서 가져옵니다.) | 아니요 |
| `--currency` | 상점 첫 화면에서 사용할 기본 통화. (아직 통화하지 않은 경우 다음을 입력하여 통화 목록을 볼 수 있습니다. `magento info:currency:list` bin 디렉터리에서 가져옵니다.) | 아니요 |
| `--timezone` | 관리자 및 상점 첫 화면에서 사용할 기본 시간대. 아직 시간대를 입력하지 않은 경우 다음을 입력하여 시간대 목록을 볼 수 있습니다 `magento info:timezone:list` bin 디렉터리에서 가져옵니다.) | 아니요 |
...
>[!TIP]
>
>설치하는 동안 원격 스토리지 서비스를 활성화하려면 다음을 참조하십시오. [원격 스토리지 구성](../../configuration/remote-storage/remote-storage.md) 다음에서 _구성 안내서_.

**검색 엔진 구성 옵션:**

| 이름 | 값 | 필수? |
|--- |--- |--- |
| `--search-engine` | 검색 엔진의 버전입니다. 가능한 값은 다음과 같습니다 `elasticsearch7`, `elasticsearch6`, 및 `elasticsearch5`. 기본값은 입니다 `elasticsearch7`. OpenSearch를 검색 엔진으로 설치한 경우 값을 지정합니다 `elasticsearch7`. Elasticsearch 5는 더 이상 사용되지 않으므로 권장되지 않습니다. | 아니요 |
| `--elasticsearch-host` | 검색 엔진이 실행 중인 호스트 이름 또는 IP 주소입니다. 기본값은 입니다 `localhost`. | 아니요 |
| `--elasticsearch-port` | 수신 HTTP 요청의 포트입니다. 기본값은 입니다 `9200`. | 아니요 |
| `--elasticsearch-index-prefix` | 검색 색인을 식별하는 접두사입니다. 기본값은 입니다 `magento2`. | 아니요 |
| `--elasticsearch-timeout` | 시스템 제한 시간(초)입니다. 기본값은 입니다 `15`. | 아니요 |
| `--elasticsearch-enable-auth` | 검색 엔진 서버에서 인증을 활성화합니다. 기본값은 입니다 `false`. | 아니요 |
