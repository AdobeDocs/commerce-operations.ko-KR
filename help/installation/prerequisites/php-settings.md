...
>[!NOTE]
>
>다음을 참조하십시오 [시스템 요구 사항](../system-requirements.md) 지원되는 버전의 PHP용.

## PHP가 설치되었는지 확인

대부분의 Linux 버전에는 기본적으로 PHP가 설치되어 있습니다. 이 항목에서는 사용자가 이미 PHP를 설치했다고 가정합니다. PHP가 이미 설치되어 있는지 확인하려면 명령줄에 다음을 입력합니다.
...
Adobe Commerce 및 Magento Open Source 2.4는 PHP 7.3과 호환되지만, 우리는 PHP 7.4로 테스트하고 사용을 권장합니다.

PHP가 설치되지 않았거나 버전을 업그레이드해야 하는 경우 특정 Linux 버전에 대한 지침에 따라 설치하십시오.
CentOS에서, [추가 단계가 필요할 수 있습니다](https://wiki.centos.org/HowTos/php7).
...
1. PHP 설치에 사용되는 것과 동일한 워크플로를 사용하여 누락된 모듈을 추가합니다. 예를 들어 `yum` php를 설치하려면 다음과 같이 PHP 7.4 모듈을 추가할 수 있습니다.
...
>[!NOTE]
>
>설치 및 업그레이드 중 문제가 발생하지 않도록 PHP 명령줄 구성과 PHP 웹 서버 플러그인 구성 모두에 동일한 PHP 설정을 적용하는 것이 좋습니다. 자세한 내용은 다음 섹션을 참조하십시오.
...
>[!NOTE]
>
>하나밖에 없다면 `php.ini` 파일에서 해당 파일을 변경합니다. 두 개 있으면 `php.ini` 파일, 변경 *모두* 파일. 이렇게 하지 않으면 예기치 않은 성능이 발생할 수 있습니다.
