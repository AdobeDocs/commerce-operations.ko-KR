---
title: 전체 사전 요구 사항
description: 이러한 전제 조건 단계를 완료하여 Adobe Commerce 프로젝트를 업그레이드할 수 있도록 준비하십시오.
...
Adobe Commerce을 실행하는 데 필요한 사항을 이해하는 것이 중요합니다. 먼저 을(를) 검토해야 합니다 [시스템 요구 사항](../../installation/system-requirements.md) 로 업그레이드하려는 버전의 경우
...
다음 [시스템 요구 사항](../../installation/system-requirements.md) Adobe Commerce 릴리스에서 테스트한 타사 소프트웨어 버전을 정확하게 설명합니다.
...
Adobe Commerce을 사용하려면 소프트웨어를 사용하려면 Elasticsearch 또는 OpenSearch가 설치되어 있어야 합니다.
...
* 을(를) 입력합니다. `bin/magento config:show catalog/search/engine` 명령. 명령은 값을 반환합니다 `mysql`, `elasticsearch` (Elasticsearch 2이 구성되었음을 나타냅니다), `elasticsearch5`, `elasticsearch6`, `elasticsearch7`또는 사용자 지정 값으로, 타사 검색 엔진을 설치했음을 나타냅니다. 2.4.6 이전 버전의 경우 `elasticsearch7` Elasticsearch 7 또는 OpenSearch 엔진의 값입니다. 버전 2.4.6 이상의 경우 `opensearch` OpenSearch 엔진의 값.
...
2.4.0으로 업그레이드하기 전에 Elasticsearch 7.6 이상 또는 OpenSearch 1.2를 설치 및 구성해야 합니다. Adobe은 더 이상 Elasticsearch 2.x, 5.x 및 6.x를 지원하지 않습니다. [검색 엔진 구성](../../configuration/search/configure-search-engine.md) 에서 _구성 안내서_ Elasticsearch을 지원되는 버전으로 업그레이드한 후 수행해야 하는 작업에 대해 설명합니다.
...
OpenSearch는 Elasticsearch의 라이센스 변경 후 Elasticsearch 7.10.2의 오픈 소스 포크입니다. 다음 Adobe Commerce 릴리스에서는 OpenSearch에 대한 지원이 도입되었습니다.

* 2.4.6(OpenSearch에는 별도의 모듈 및 설정이 있음)
* 2.4.5
* 2.4.4
* 2.4.3-p2
* 2.3.7-p3

다음을 수행할 수 있습니다 [Elasticsearch에서 OpenSearch로 마이그레이션](opensearch-migration.md) 위에 나열된 Adobe Commerce 버전으로 업그레이드하는 경우에만 해당됩니다.
...
Elasticsearch 8.x에 대한 지원이 Adobe Commerce 2.4.6에서 도입되었습니다. 다음 지침은 Elasticsearch을 7.x에서 8.x로 업그레이드하는 예를 보여줍니다.

1. Elasticsearch 7.x 서버를 8.x로 업그레이드하고 가 작동 중인지 확인합니다. 자세한 내용은 [Elasticsearch 설명서](https://www.elastic.co/guide/en/elasticsearch/reference/current/install-elasticsearch.html).

1. 를 활성화합니다 `id_field_data` 필드에 다음 구성을 추가하여 `elasticsearch.yml` Elasticsearch 8.x 서비스를 다시 시작하고 파일을 작성합니다.
   ...
   >[!INFO]
   >
   >Adobe Commerce 2.4.6은 Elasticsearch 8.x를 지원하기 위해 `indices.id_field_data` 속성은 기본적으로 지정되며 은 `_id` 의 필드 `docvalue_fields` 속성을 사용합니다.

1. Adobe Commerce 프로젝트의 루트 디렉토리에서 작성기 종속성을 업데이트하여 을(를) 제거합니다. `Magento_Elasticsearch7` 모듈 및 설치 `Magento_Elasticsearch8` 모듈.
...
1. [Elasticsearch 구성](../../configuration/search/configure-search-engine.md#configure-your-search-engine-from-the-admin) 에서 [!DNL Admin].
...
실수로 서버의 Elasticsearch 버전을 업그레이드하거나 다른 이유로 다운그레이드해야 한다고 결정하는 경우 Adobe Commerce 프로젝트 종속성도 업데이트해야 합니다. 예를 들어 Elasticsearch 8.x에서 7.x로 다운그레이드하려면

1. Elasticsearch 8.x 서버를 7.x로 다운그레이드하고 가 작동 중인지 확인합니다. 자세한 내용은 [Elasticsearch 설명서](https://www.elastic.co/guide/en/elasticsearch/reference/current/install-elasticsearch.html).

1. Adobe Commerce 프로젝트의 루트 디렉토리에서 작성기 종속성을 업데이트하여 을(를) 제거합니다. `Magento_Elasticsearch8` 모듈 및 해당 작성기 종속성 및 설치 `Magento_Elasticsearch7` 모듈.
...
확장이 Adobe Commerce 릴리스와 완전히 호환되는지 여부를 확인하려면 검색 엔진 공급업체에 문의하는 것이 좋습니다.
...
UNIX 작업 스케줄러 `cron` 는 일상적인 Adobe Commerce 작업에 매우 중요합니다. 색인 재지정, 뉴스레터, 이메일 및 사이트 맵과 같은 일정을 잡습니다. 몇 가지 기능을 사용하려면 파일 시스템 소유자로 실행 중인 하나 이상의 크론 작업이 필요합니다.
...
보안상의 이유로 Adobe Commerce에서는 파일 시스템에 대한 특정 권한이 필요합니다. 권한은 다음과 다릅니다 _[소유권](../../upgrade/prepare/prerequisites.md#verify-file-system-permissions)_. 소유권은 파일 시스템에서 작업을 수행할 수 있는 사용자를 결정합니다. 권한은 사용자가 수행할 수 있는 작업을 결정합니다.
...
Adobe Commerce에서 정적 파일 자산을 의 하위 디렉토리에 배포하므로 `pub`또한 여기에서 권한 및 소유권을 확인하는 것이 좋습니다.
