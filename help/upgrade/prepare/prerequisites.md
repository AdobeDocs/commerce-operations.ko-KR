...
사용자 환경에서 모든 시스템 요구 사항 및 종속성을 업데이트했는지 확인합니다. 대상 PHP 버전 및 [필수 PHP 설정](https://www.php.net/manual/en/appendices.php)에 대한 [PHP 마이그레이션 부록](../../installation/prerequisites/php-settings.md#php-settings)을 검토하십시오.
...
* [Elasticsearch 설치](https://www.elastic.co/docs/deploy-manage/deploy/self-managed/installing-elasticsearch)
...
프로덕션에 배포하기 전에 데이터 백업, 잠재적인 마이그레이션 문제 감지 및 업그레이드 테스트에 대한 전체 지침은 [Elasticsearch 업그레이드](https://www.elastic.co/docs/deploy-manage/upgrade/deployment-or-cluster)를 참조하십시오. 현재 버전의 Elasticsearch에 따라 전체 클러스터를 다시 시작해야 할 수도 있고 필요하지 않을 수도 있습니다.

Elasticsearch을 사용하려면 JDK(Java Development Kit) 1.8 이상이 필요합니다. 설치된 JDK 버전을 확인하려면 [JDK(Java Software Development Kit) 설치](../../installation/prerequisites/search-engine/overview.md#install-the-java-software-development-kit)를 참조하십시오.
...
OpenSearch를 사용하려면 JDK 1.8 이상이 필요합니다. 설치된 JDK 버전을 확인하려면 [JDK(Java Software Development Kit) 설치](../../installation/prerequisites/search-engine/overview.md#install-the-java-software-development-kit)를 참조하십시오.
...
>[!NOTE]
>
>이 단계는 Adobe Commerce 2.4.6 및 2.4.7에만 적용됩니다. Adobe Commerce 2.4.8 이상 버전은 더 이상 Elasticsearch을 지원하지 않습니다. 대신 OpenSearch를 사용하십시오.

1. Elasticsearch 7.x 서버를 8.x로 업그레이드하고 가 실행 중인지 확인합니다. [Elasticsearch 설명서](https://www.elastic.co/docs/deploy-manage/deploy/self-managed/installing-elasticsearch)를 참조하세요.
...
1. Elasticsearch 8.x 서버를 7.x로 다운그레이드하고 가 실행 중인지 확인합니다. [Elasticsearch 설명서](https://www.elastic.co/docs/deploy-manage/deploy/self-managed/installing-elasticsearch)를 참조하세요.
...
Adobe에서는 열린 파일 [ulimit](https://www.gnu.org/software/bash/manual/html_node/Bash-Builtins.html#index-ulimit)을(를) `65536` 이상의 값으로 설정하는 것이 좋지만 필요한 경우 더 큰 값을 사용할 수 있습니다. 명령줄에 대한 ulimit을 설정하거나 사용자의 셸에 대한 영구 설정으로 지정할 수 있습니다.
