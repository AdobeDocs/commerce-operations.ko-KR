...
- 특정 범위(즉, 이 설정은 지정된 스토어 보기 또는 웹 사이트에만 영향을 줌)
  ...
  범위에 대한 자세한 내용은 다음을 참조하십시오.
   ...
   - [범위에 대한 Commerce 사용 안내서 항목](https://docs.magento.com/user-guide/configuration/scope.html)
...
1. `mysql>` 프롬프트에서 다음 명령을 표시된 순서대로 입력하십시오.
...
>[!WARNING]
>
>- `$_ENV` 배열에서 설정한 값을 사용하려면 `php.ini` 파일에서 `variables_order = "EGPCS"`(Environment, Get, Post, Cookie 및 Server)을 설정해야 합니다. 자세한 내용은 [PHP 설명서](https://www.php.net/manual/en/ini.core.php)를 참조하세요.
>
