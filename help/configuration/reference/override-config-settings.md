...
환경 변수를 사용하여 _any_ 구성 설정의 값을 재정의할 수 있습니다. 그러나 [배포 일반 개요](../deployment/overview.md)에 설명된 대로 공유 구성 파일 `config.php`과(와) 시스템별 구성 파일 `env.php`을(를) 사용하여 일관된 설정을 유지하는 것이 좋습니다.
