...
| `validate-data` | 문자(a-z 또는 A-Z), 숫자(0-9) 또는 밑줄(\_)만 사용할 수 있습니다.<br>첫 번째 문자는 문자여야 합니다.<br>(식 `/^[A-Za-z]+[A-Za-z0-9_]+$/`과(와) 일치해야 함)<br>유효성 검사 실패 메시지: &quot;이 필드에는 문자(a-z 또는 A-Z), 숫자(0-9) 또는 밑줄(\_)만 사용하십시오. 첫 번째 문자는 문자여야 합니다.&quot; |
