...
의 책임 `config.xml` 이전 버전의 Commerce에서 사용된 구성 파일은 이제 여러 모듈 디렉터리에 있는 여러 파일로 나뉩니다. 상업의 여러 구성 파일은 모듈이 특정 구성 유형을 요청하는 경우에만 요청 시 로드됩니다.

이러한 파일을 사용할 수 있습니다. _구성 유형_- 모듈 동작의 특정 측면을 사용자정의할 수 있습니다.

여러 모듈이 동일한 구성 유형에 영향을 주는 구성 파일(예: 이벤트)을 선언할 수 있으며, 이러한 여러 구성 파일은 병합됩니다.

다음은 이 주제에서 사용되는 일반적인 용어입니다.

- **구성 개체**- 구성 유형 정의 및 유효성 검사를 담당하는 Commerce 라이브러리 또는 클래스입니다. 예를 들어 의 구성 객체 `config.xml` 은(는) [Magento\Framework\App\Config](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/App/Config.php).

- **구성 단계**- 단계가 다음과 같이 정의됩니다. _기본_, _글로벌_, 및 _영역_. 각 단계는 구성 유형이 로드되고 이름이 같은 구성 유형과 병합되는 시기를 결정합니다. 예를 들어, `module.xml` 파일이 다른 파일과 병합됨 `module.xml` 파일.

- **구성 범위**- 구성 단계에 대한 보완을 위해 범위가 구성 유형 모델을 정의합니다. 예를 들어, `adminhtml` 는 스테이지에서 다른 모듈과 함께 로드되는 영역 범위입니다. `adminhtml` 구성. 자세한 내용은 [모듈 및 영역](https://developer.adobe.com/commerce/php/architecture/modules/areas/).
...
Commerce는 구성 파일을 다음 순서로 로드합니다(모든 경로는 Commerce 설치 디렉토리에 상대적입니다).

- 기본 구성 ([app/etc/di.xml](https://github.com/magento/magento2/blob/2.4/app/etc/di.xml)). 이 파일은 Commerce를 부트스트랩하는 데 사용됩니다.
- 모듈의 전역 구성(`<your component base dir>/<vendorname>/<component-type>-<component-name>/etc/*.xml`). 모든 모듈에서 특정 구성 파일을 수집하여 함께 병합합니다.
- 모듈의 영역별 구성(`<your component base dir>/<vendorname>/<component-type>-<component-name>/etc/<area>/*.xml`). 모든 모듈에서 구성 파일을 수집하여 전역 구성에 병합합니다. 일부 영역별 구성은 전역 구성을 재정의하거나 확장할 수 있습니다.

위치

- `<your component base dir>` 는 구성 요소가 있는 기본 디렉토리입니다. 일반적인 값은 다음과 같습니다 `app/code` 또는 `vendor` Commerce 설치 디렉토리 관련 정보.
- `<vendorname>` 는 구성 요소의 공급업체 이름입니다(예: Commerce의 공급업체 이름). `magento`.
...
- `<component-name>`: 다음에서 정의한 구성 요소 이름 [composer.json](https://github.com/magento/magento2/blob/2.4/composer.json).

### 구성 파일 병합

구성 파일의 노드는 다음에 정의된 특수 속성이 있는 정규화된 XPath를 기반으로 병합됩니다. `$idAttributes` 배열이 식별자로 선언되었습니다. 이 식별자는 동일한 상위 노드 아래에 중첩된 모든 노드에 대해 고유해야 합니다.
...
>[!INFO]
>
>다음을 사용할 수 있습니다. [\Magento\Framework\Config\Reader\Filesystem](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/Reader/Filesystem.php) 디버깅 및 뒤의 논리 이해를 위한 클래스 [구성 파일 로더](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/Reader/Filesystem.php#L125) 및 [구성 병합](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/Reader/Filesystem.php#L144) 프로세스.
...
다음 표는 각 구성 유형과 이 구성 유형과 관련된 Commerce 구성 객체를 보여 줍니다.
...
| `communication.xml` | [메시지 대기열 시스템의 측면을 정의합니다.](https://developer.adobe.com/commerce/php/development/components/message-queues/configuration/#communicationxml) | 글로벌 | [\Magento\WebapiAsync\Code\Generator\Config\RemoteServiceReader\Communication](https://github.com/magento/magento2/blob/2.4/app/code/Magento/WebapiAsync/Code/Generator/Config/RemoteServiceReader/Communication.php) |
...
| `db_schema.xml` | [선언적 스키마](https://developer.adobe.com/commerce/php/development/components/declarative-schema/configuration/) | 글로벌 | [Magento\Framework\Setup\Declaration\Schema](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Setup/Declaration/Schema/SchemaConfig.php) |
...
| `extension_attributes.xml` | [확장 속성](https://developer.adobe.com/commerce/php/development/components/attributes/#extension-attributes) | 글로벌 | [\Magento\Framework\Api\ExtensionAttribute\Config](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Api/ExtensionAttribute/Config.php) |
...
| `persistent.xml` | [Magento 지속](https://developer.adobe.com/commerce/php/module-reference/module-persistent/) 구성 파일 | 글로벌 | [\Magento\Persistent\Helper\Data](https://github.com/magento/magento2/blob/2.4/app/code/Magento/Persistent/Helper/Data.php) |
...
| `webapi.xml` | [웹 API 구성](https://developer.adobe.com/commerce/php/development/components/web-api/services/) | 글로벌 | [\Magento\Webapi\Model\Config](https://github.com/magento/magento2/blob/2.4/app/code/Magento/Webapi/Model/Config.php) |
...
아래의 인터페이스를 사용하여 구성 파일과 상호 작용할 수 있습니다. [Magento\프레임워크\구성](https://github.com/magento/magento2/tree/2.4/lib/internal/Magento/Framework/Config).

다음과 같은 경우 이러한 인터페이스를 사용할 수 있습니다. [구성 유형 만들기](../reference/config-create-types.md#create-configuration-types).

`Magento\Framework\Config` 는 다음 인터페이스를 제공합니다.

- [Framework\Config\ConverterInterface](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/ConverterInterface.php): XML을 구성의 메모리 내 배열 표현으로 변환합니다.
- [프레임워크\구성\데이터 인터페이스](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/DataInterface.php): 지정된 범위에서 구성 데이터를 검색합니다.
...
- [Framework\Config\ReaderInterface](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/ReaderInterface.php): 저장소에서 구성 데이터를 읽고 읽을 저장소를 선택합니다.

즉, 파일 시스템, 데이터베이스, 기타 저장소는 병합 규칙에 따라 구성 파일을 병합하고 유효성 검사 스키마와 함께 구성 파일의 유효성을 검사합니다.

- [Framework\Config\SchemaLocatorInterface](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/SchemaLocatorInterface.php): XSD 스키마를 찾습니다.
...
- [프레임워크\구성\유효성 검사 상태 인터페이스](https://github.com/magento/magento2/blob/2.4/lib/internal/Magento/Framework/Config/ValidationStateInterface.php): 유효성 검사 상태를 검색합니다.
