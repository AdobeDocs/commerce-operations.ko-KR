...
| session-save-valkey-persistent-id | persistent_identifier | 영구 연결을 사용하는 고유한 문자열입니다(예: sess-db0).<br>[phpredis 및 php-fpm의 알려진 문제](https://github.com/phpredis/phpredis/issues/70). |  |
| session-save-valkey-db | 데이터베이스 | 데이터 손실을 방지하는 데 권장되는 고유한 Valkey 데이터베이스 번호입니다.<br><br>**중요**: 둘 이상의 캐싱 유형에 Valkey를 사용하는 경우 데이터베이스 번호가 달라야 합니다. 기본 캐싱 데이터베이스 번호를 `0`에, 페이지 캐싱 데이터베이스 번호를 `1`에, 세션 저장소 데이터베이스 번호를 `2`에 할당하는 것이 좋습니다. | 0 |
