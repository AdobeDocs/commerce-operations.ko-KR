...
| `cache-backend-redis-compress-data` | compress_data | 압축을 비활성화하려면 `0`(으)로 설정하십시오. | `1` |
| `cache-backend-redis-compression-lib` | compression_lib | 사용할 압축 라이브러리입니다. 지원되는 값은 `snappy`, `lzf`, `l4z`, `zstd` 및 `gzip`입니다. 자동으로 결정하려면 비워 둡니다. | |
...
| `page-cache-redis-compress-data` | compress_data | 전체 페이지 캐시를 압축하려면 `1`(으)로 설정하십시오. 압축을 비활성화하려면 `0`을(를) 사용하십시오. | `0` |
| `page-cache-redis-compression-lib` | compression_lib | 사용할 압축 라이브러리입니다. 지원되는 값은 `snappy`, `lzf`, `l4z`, `zstd` 및 `gzip`입니다. 자동으로 결정하려면 비워 둡니다. | |
...
Debian 또는 Ubuntu의 경우 `apt`을(를) 사용합니다.
