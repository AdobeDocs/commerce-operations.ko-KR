...
| session-save-redis-persistent-id | persistent_identifier | 영구 연결을 사용하는 고유한 문자열입니다(예: sess-db0).<br>[phpredis 및 php-fpm의 알려진 문제](https://github.com/phpredis/phpredis/issues/70). |  |
| session-save-redis-db | 데이터베이스 | 데이터 손실을 방지하는 데 권장되는 고유한 Redis 데이터베이스 번호입니다.<br><br>**중요**: 둘 이상의 캐싱 유형에 Redis를 사용하는 경우 데이터베이스 번호가 달라야 합니다. 기본 캐싱 데이터베이스 번호는 0으로, 페이지 캐싱 데이터베이스 번호는 1로, 세션 저장소 데이터베이스 번호는 2로 지정하는 것이 좋습니다. | 0 |
| session-save-redis-compression-threshold | compression_threshold | 압축을 비활성화하려면 0으로 설정하십시오(`suhosin.session.encrypt = On`일 때 권장됨).<br>[64KB를 초과하는 문자열의 알려진 문제](https://github.com/colinmollenhour/Cm_Cache_Backend_Redis/issues/18). | 2048 |
