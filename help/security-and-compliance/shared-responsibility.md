...
이 공유 모델을 통해 판매자는 운영 책임과 비용을 최소화하면서 비즈니스 요구 사항을 충족하는 유연하고 사용자 지정이 가능하며 확장 가능한 솔루션을 설계하고 구현할 수 있습니다.
...
* 보안 사고 대응 및 대응
* 적극적으로 지원되는 버전에서 모든 타사 종속성, 플랫폼 서비스 및 Adobe Commerce 서비스 확장을 유지 관리합니다. Adobe은 지원되지 않는 종속성 버전을 실행하는 배포에 대한 보안 지원 또는 지원을 제공하지 않습니다. 지원되는 버전은 [시스템 요구 사항](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/system-requirements.html) 및 [제품 가용성 매트릭스](https://experienceleague.adobe.com/en/docs/commerce-operations/release/product-availability)를 참조하십시오.
...
Adobe은 클라우드 인프라 환경 및 핵심 솔루션 코드의 Adobe Commerce에 대한 보안 및 가용성을 담당합니다. Adobe은 또한 다음을 포함하여 Adobe Commerce on cloud infrastructure 솔루션의 보안을 유지하는 활동을 수행합니다.
...
* 판매자 지원 및 지원 액세스 제어 관리(예: Experience League 지원)
...
Adobe은 Adobe Commerce 솔루션에 사용되는 인프라 및 서비스에 대한 PCI 인증을 유지 관리합니다. 판매자는 사용자 지정 코드, 시스템 및 네트워크 프로세스, 조직의 준수에 대한 책임을 집니다.

또한 Adobe은 해당 SLA에서 합의한 대로 판매자의 인프라를 사용할 수 있도록 보장합니다.

## 판매자 책임

판매자는 Adobe Commerce on cloud infrastructure 솔루션의 맞춤화된 인스턴스에 대한 다음 보안 모범 사례를 담당합니다.
...
* 각 공급업체 또는 Adobe에서 적극적으로 지원하는 버전에서 모든 플랫폼 서비스, 타사 종속성 및 Adobe Commerce 서비스 확장을 유지 관리합니다. 여기에는 다음이 포함됩니다.

   * 데이터베이스, 캐시, 검색, PHP 런타임 및 웹 서버와 같은 인프라 서비스
   * Adobe Commerce 서비스 확장
   * 모든 타사 확장 및 사용자 정의 통합

  Adobe은 지원되지 않는 버전을 실행하는 배포를 지원하지 않습니다. 지원되는 버전은 [시스템 요구 사항](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/system-requirements.html) 및 [제품 가용성 매트릭스](https://experienceleague.adobe.com/en/docs/commerce-operations/release/product-availability)를 참조하십시오.

## 클라우드 서비스 공급자 책임

Adobe은 클라우드 서비스 공급업체를 통해 클라우드 인프라에서 Adobe Commerce용 클라우드 서버 인프라를 호스팅합니다. 이러한 공급업체는 방화벽 시스템 및 침입 탐지 시스템(ID)을 통해 라우팅, 스위칭 및 경계 네트워크 보안을 포함한 네트워크 보안을 담당합니다. 클라우드 서비스 제공업체는 Adobe Commerce on cloud infrastructure 솔루션을 호스팅하는 데이터 센터의 물리적 및 환경 보안에 대한 책임도 집니다.
...
Adobe Commerce on cloud infrastructure 솔루션은 CDN 공급자를 사용하여 페이지 로드 시간을 단축하고, 콘텐츠를 캐시하며, 오래된 콘텐츠를 즉시 제거합니다. 이러한 공급자는 CDN에 영향을 주는 보안 문제와 CDN WAF 규칙을 정의하고 유지 관리하는 책임도 집니다.
...
<tbody>
  ...
  <tr>
    <td>적극적으로 지원되는 버전(인프라 서비스, SaaS Commerce 추가 기능 및 타사 확장 포함)에서 모든 플랫폼 서비스, 타사 종속성 및 Commerce 서비스 확장 유지</td>
    <td>I</td>
    <td>R</td>
    <td></td>
    <td></td>
...
<tfoot>
  <tr>
    <td colspan="5">
      <p><sup>1</sup> 클라우드 인프라 저장소의 Adobe Commerce이 주 리포지토리로 사용되는 경우에만 해당됩니다. 다른 외부 리포지토리를 사용하는 것은 판매자의 유일한 책임입니다.</p>
      <p><sup>2</sup> Adobe은 CDN 공급자 문제에 대한 레벨 1 지원을 제공합니다.</p>
      <p><sup>3</sup> 판매자는 응용 프로그램에 대해 구성하는 모든 Nginx 컨트롤을 담당합니다.</p>
      <p><sup>4</sup> PCI의 경우 침투 테스트 요구 사항이 Adobe과 판매자 간에 공유됩니다.</p>
    </td>
  </tr>
</tfoot>
</table>
...
| 판매자는 지원되는 버전에서 PHP를 유지해야 합니다. 지원되지 않는 버전에 대한 배포는 Adobe 지원에 적합하지 않으며 패치되지 않은 보안 취약점이 포함될 수 있습니다. |     | R |
...
| 기본 데이터베이스 설정 <br><br>의 유지 관리 진행 중(핵심 테이블 인덱싱 및 최적화, 기본 시스템 관리 설정 최적화) | R |   |
...
| 판매자는 지원되는 버전에서 MariaDB를 유지 관리해야 합니다. 지원되지 않는 버전에 대한 배포는 Adobe 지원에 적합하지 않으며 패치되지 않은 보안 취약점이 포함될 수 있습니다. |     | R |
...
| --- | --- | --- |
| CDN 가용성 및 품질 | R |   |
| Fastly 서비스 구성(확장 또는 API를 통해) |     | R |
| Fastly 확장 품질 | R |   |
| Fastly 통합 VCL 스니펫 품질(Fastly 확장과 함께 번들로 제공) | R |   |
| 페이지 캐시 최적화 |     | R |
...
| 판매자는 지원되는 버전에서 Redis를 유지 관리해야 합니다. 지원되지 않는 버전에 대한 배포는 Adobe 지원에 적합하지 않으며 패치되지 않은 보안 취약점이 포함될 수 있습니다. |     | R |
...
| --- | --- | --- |
| OpenSearch 가용성 | R |   |
| 기본 OpenSearch 설정 구성 | R |   |
| 설치된 Adobe Commerce 버전과 호환되는 OpenSearch 버전을 설치하려면 서비스 요청을 제출하십시오. |   | R |
| 판매자는 지원되는 버전에서 OpenSearch를 유지해야 합니다. 지원되지 않는 버전의 배포는 Adobe 지원에 적합하지 _않습니다_. 패치되지 않은 보안 취약점이 포함될 수 있습니다. |     | R |
...
| 한도에 대한 판매자의 SendGrid 사용 모니터링 | R |   |
| 보내는 트랜잭션 이메일에만 서비스를 사용합니다. 이 서비스는 마케팅 이메일 전송을 지원하지 않습니다. |     | R |
...
| 제품 추천 서비스 가용성 | R |   |
| 제품 추천 모듈 업그레이드 |   | R |
| 제품 권장 사항의 최신 릴리스 버전만 지원됩니다. 판매자는 릴리스 후 즉시 업그레이드해야 합니다. |     | R |
...
| 라이브 검색 모듈 업그레이드 |   | R |
| 라이브 검색의 최신 릴리스 버전만 지원됩니다. 판매자는 릴리스 후 즉시 업그레이드해야 합니다. |     | R |

{style="table-layout:auto"}

#### Storefront 이벤트 데이터 품질

Storefront 이벤트 컬렉션은 제품 추천 및 라이브 검색 출력의 품질을 높입니다. 책임은 상점 구현에 따라 다릅니다.
...
| 핵심 PWA 구현 | R |   |
| 사용자 지정 PWA 구현 |  | R |
| 핵심 Edge Delivery Services 구현(Commerce Boilerplate) | R |   |
| 사용자 지정 Edge Delivery Services 구현 |   | R |
...
| 결제 서비스 가용성 | R |   |
| 결제 모듈 업그레이드 |   | R |
| 결제 서비스의 최신 버전만 지원됩니다. 판매자는 릴리스 후 즉시 업그레이드해야 합니다. |     | R |
...
| 전용 SSL 인증서 만료 | R |   |
| SSL 인증서 프로비저닝 | R |   |
| EV 또는 기타 특정 SSL 인증서 구매 및 유지 관리(제공된 기본값 제외) 및 Adobe 제공 |     | R |
